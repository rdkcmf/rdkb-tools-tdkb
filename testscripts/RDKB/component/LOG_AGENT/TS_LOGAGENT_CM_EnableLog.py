...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_LOGAGENT_CM_EnableLog</name>
  <primitive_test_id/>
  <primitive_test_name>LogAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Enable logging of CM module and check if the logs are getting updated in CMlog.txt.0</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LOGAGENT_1</test_case_id>
    <test_objective>Enable logging of CM module and check if the logs are getting updated in CMlog.txt.0</test_objective>
...
Device.LogAgent.X_RDKCENTRAL-COM_CM_LoggerEnable : True</input_parameters>
    <automation_approch>1. Load LogAgent module
2. Get and save the values of log enable status
3. Set true to CM log enable status
...
8. Unload module </automation_approch>
    <except_output>The logs should get updated in CMlog.txt.0</except_output>
    <priority>High</priority>
    <test_stub_interface>LOG_AGENT</test_stub_interface>
    <test_script>TS_LOGAGENT_CM_EnableLog</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LOGAGENT_CM_EnableLog');
sysobj.configureTestCase(ip,port,'TS_LOGAGENT_CM_EnableLog');
