...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_LOGAGENT_WiFi_DisableLog</name>
  <primitive_test_id/>
  <primitive_test_name>LogAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Disable logging of WiFi module and check if the logs are getting updated in WiFilog.txt.0</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LOGAGENT_8</test_case_id>
    <test_objective>Disable logging of WiFi module and check if the logs are getting updated in WiFilog.txt.0</test_objective>
...
Device.LogAgent.X_RDKCENTRAL-COM_WiFi_LoggerEnable : False</input_parameters>
    <automation_approch>1. Load LogAgent module
2. Get and save the values of log enable status
3. Set false to WiFi log enable status
4. Check if the CcspWiFiSsp process is up and running in device
5. When the process is up, get and save the timestamp of WiFilog.txt.0
6. Get the values of WiFi related parameters and check if the timestamp in log file is changed or not.
7. Revert the values
8. Unload module</automation_approch>
    <except_output>The logs should not get updated in WiFilog.txt.0</except_output>
    <priority>High</priority>
    <test_stub_interface>LOG_AGENT</test_stub_interface>
    <test_script>TS_LOGAGENT_WiFi_DisableLog</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LOGAGENT_WiFi_DisableLog');
sysobj.configureTestCase(ip,port,'TS_LOGAGENT_WiFi_DisableLog');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysloadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
        WiFiLogStatus = tdkTestObj.getResultDetails();
        ...
        if expectedresult in actualresult:
            ...
            tdkTestObj.addParameter("paramList","Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable|true|bool|Device.LogAgent.X_RDKCENTRAL-COM_WiFi_LoggerEnable|false|bool");
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3: Disable logging of WiFi module"
                print "EXPECTED RESULT 3: Should disable WiFi logging"
		...
		RPCCmd = "sh %s/tdk_utility.sh parseConfigFile RPC_CMD" %TDK_PATH;
                ...
                #Get the values of WiFi parameters
                tdkTestObj = obj.createTestStep('TDKB_TR181Stub_Get');
		    ...
		    if lastLineofLog:
                ...
                tdkTestObj = obj.createTestStep("TDKB_TR181Stub_SetMultiple");
                tdkTestObj.addParameter("paramList","Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable|%s|bool|Device.LogAgent.X_RDKCENTRAL-COM_WiFi_LoggerEnable|%s|bool" %(LogStatus,WiFiLogStatus));
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
                if expectedresult in actualresult:
