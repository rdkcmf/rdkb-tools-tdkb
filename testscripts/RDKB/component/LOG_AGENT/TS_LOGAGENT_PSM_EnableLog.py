...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_LOGAGENT_PSM_EnableLog</name>
  <primitive_test_id/>
  <primitive_test_name>LogAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Enable logging of PSM module and check if the logs are getting updated in PSMlog.txt.0</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LOGAGENT_7</test_case_id>
    <test_objective>Enable logging of PSM module and check if the logs are getting updated in PSMlog.txt.0</test_objective>
...
Device.LogAgent.X_RDKCENTRAL-COM_PSM_LoggerEnable : False</input_parameters>
    <automation_approch>1. Load LogAgent module
2. Get and save the values of log enable status
3. Set True to PSM log enable status
4. Check if the PsmSsp process is up and running in device
5. When the process is up, get and save the timestamp of PSMlog.txt.0
6. Get the values of PSM related parameters and check if the timestamp in log file is changed or not.
7. Revert the values
8. Unload module</automation_approch>
    <except_output>The logs should get updated in PSMlog.txt.0</except_output>
    <priority>High</priority>
    <test_stub_interface>LOG_AGENT</test_stub_interface>
    <test_script>TS_LOGAGENT_PSM_EnableLog</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LOGAGENT_PSM_EnableLog');
pamobj.configureTestCase(ip,port,'TS_LOGAGENT_PSM_EnableLog');
sysobj.configureTestCase(ip,port,'TS_LOGAGENT_PSM_EnableLog');

#Get the result of connection with test component and DUT
loadmodulestatus=obj.getLoadModuleResult();
pamloadmodulestatus=obj.getLoadModuleResult();
sysloadmodulestatus=sysobj.getLoadModuleResult();

if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysloadmodulestatus.upper() and "SUCCESS" in pamloadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
        PsmLogStatus = tdkTestObj.getResultDetails();
        ...
        if expectedresult in actualresult:
            ...
            tdkTestObj.addParameter("paramList","Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable|true|bool|Device.LogAgent.X_RDKCENTRAL-COM_PSM_LoggerEnable|true|bool");
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3: Enable logging of PSM module"
                print "EXPECTED RESULT 3: Should enable PSM logging"
		...
		tdkTestObj = pamobj.createTestStep('pam_Setparams');
			...
			tdkTestObj.setResultStatus("FAILURE");
                ...
                tdkTestObj = obj.createTestStep("TDKB_TR181Stub_SetMultiple");
                tdkTestObj.addParameter("paramList","Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable|%s|bool|Device.LogAgent.X_RDKCENTRAL-COM_PSM_LoggerEnable|%s|bool" %(LogStatus,PsmLogStatus));
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
                if expectedresult in actualresult:
