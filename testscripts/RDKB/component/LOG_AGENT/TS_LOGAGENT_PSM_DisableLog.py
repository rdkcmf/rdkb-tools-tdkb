...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_LOGAGENT_PSM_DisableLog</name>
  <primitive_test_id/>
  <primitive_test_name>LogAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Disable logging of PSM module and check if the logs are getting updated in PSMlog.txt.0</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LOGAGENT_6</test_case_id>
    <test_objective>Disable logging of PSM module and check if the logs are getting updated in PSMlog.txt.0</test_objective>
...
Device.LogAgent.X_RDKCENTRAL-COM_PSM_LoggerEnable : True</input_parameters>
    <automation_approch>1. Load LogAgent module
2. Get and save the values of log enable status
3. Set false to PSM log enable status
...
8. Unload module</automation_approch>
    <except_output>The logs should not get updated in PSMlog.txt.0</except_output>
    <priority>High</priority>
    <test_stub_interface>LOG_AGENT</test_stub_interface>
    <test_script>TS_LOGAGENT_PSM_DisableLog</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LOGAGENT_PSM_DisableLog');
pamobj.configureTestCase(ip,port,'TS_LOGAGENT_PSM_DisableLog');
sysobj.configureTestCase(ip,port,'TS_LOGAGENT_PSM_DisableLog');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysloadmodulestatus.upper() and "SUCCESS" in pamloadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            tdkTestObj.addParameter("paramList","Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable|true|bool|Device.LogAgent.X_RDKCENTRAL-COM_PSM_LoggerEnable|false|bool");
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
