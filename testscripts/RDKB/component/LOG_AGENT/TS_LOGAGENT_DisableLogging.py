...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_LOGAGENT_DisableLogging</name>
  <primitive_test_id/>
  <primitive_test_name>LogAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To disable the logging feature in device and check if the logging is stopped or not.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LOGAGENT_10</test_case_id>
    <test_objective>To disable the logging feature in device and check if the logging is stopped or not.</test_objective>
    ...
    <input_parameters>Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable : False
...
3. Set false to log enable status
...
8. Unload module</automation_approch>
    <except_output>Once the logging is Disabled, no logs should update in any log file.</except_output>
    <priority>High</priority>
    <test_stub_interface>LOG_AGENT</test_stub_interface>
    <test_script>TS_LOGAGENT_DisableLogging</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LOGAGENT_DisableLogging');
sysobj.configureTestCase(ip,port,'TS_LOGAGENT_DisableLogging');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysloadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
	    ...
	    tdkTestObj.executeTestCase(expectedresult);
            ...
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 2: Disable logging"
                print "EXPECTED RESULT 2: Should disable logging"
		...
		tdkTestObj = obj.createTestStep('TDKB_TR181Stub_Set');
                tdkTestObj.addParameter("ParamName","Device.LogAgent.X_RDKCENTRAL-COM_LoggerEnable");
                tdkTestObj.addParameter("ParamValue",LogStatus);
                tdkTestObj.addParameter("Type","bool");
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
		...
		tdkTestObj.setResultStatus("FAILURE");
                print "TEST STEP 3: Disable logging"
                print "EXPECTED RESULT 3: Should disable logging"
