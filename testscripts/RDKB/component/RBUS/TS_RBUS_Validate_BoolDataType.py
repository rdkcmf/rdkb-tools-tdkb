...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_RBUS_Validate_BoolDataType</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Get the value of  Bool data using Device.SampleProvider.AllTypes.BoolData and set a new value of bool data type and check if the value is getting set properly while the rbusSampleProvider App is running</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_RBUS_74</test_case_id>
    <test_objective>Get the value of  Bool data using Device.SampleProvider.AllTypes.BoolData and set a new value of bool data type and check if the value is getting set properly while the rbusSampleProvider App is running</test_objective>
...
3. Enable RBUS and ensure rbus_session_mgr is running</pre_requisite>
    ...
    <input_parameters>set_value :0 or 1</input_parameters>
...
4. When the App is running, fetch the value of Device.SampleProvider.AllTypes.BoolData and store it.
5. Set the set_value of Bool type and check if the set operation is success.
6. Fetch the value of Device.SampleProvider.AllTypes.BoolData and confirm whether the set operation is success.
7. Revert to initial Bool data value
...
9. Revert to the initial RBUS enable state if revert flag is set.</automation_approch>
    <expected_output>Set operation of Device.SampleProvider.AllTypes.BoolData should be success and the get operation should reflect the set value when rbusSampleProvider App is running.</expected_output>
    <priority>High</priority>
    <test_stub_interface>RBUS</test_stub_interface>
    <test_script>TS_RBUS_Validate_BoolDataType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_Validate_BoolDataType');
tr181obj.configureTestCase(ip,port,'TS_RBUS_Validate_BoolDataType');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            #Get the data type of Bool data
            cmd = "rbuscli get Device.SampleProvider.AllTypes.BoolData | grep -i value";
            print "Command : %s" %cmd;
            tdkTestObj.addParameter("command",cmd);
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails().strip();
            print "\nTEST STEP 3: Get the Bool Data type value of Device.SampleProvider.AllTypes.BoolData";
            print "EXPECTED RESULT 3: Should get the bool data type value of Device.SampleProvider.AllTypes.BoolData";
            ...
            if expectedresult in actualresult and "Value" in details:
                ...
                #Set value to Device.SampleProvider.AllTypes.BoolData
                if initial_value == "0":
                    set_value = "1";
                else:
                     set_value = "0";
                cmd = "rbuscli set Device.SampleProvider.AllTypes.BoolData bool " + set_value + " | grep -i succeeded";
                print "Command : %s" %cmd;
                tdkTestObj.addParameter("command",cmd);
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails().strip();
                print "\nTEST STEP 4: Set the Bool Data type value of Device.SampleProvider.AllTypes.BoolData to %s" %set_value;
                print "EXPECTED RESULT 4: Should set the bool data type value of Device.SampleProvider.AllTypes.BoolData successfully";
                ...
                if expectedresult in actualresult and "succeeded" in details:
                    ...
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails().strip();
                    print "\nTEST STEP 5: Get the Bool Data type value of Device.SampleProvider.AllTypes.BoolData";
                    print "EXPECTED RESULT 5: Should get the bool data type value of Device.SampleProvider.AllTypes.BoolData";
