...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_RBUS_EnableandDisableRtmessage_NOTICELogLevel</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if setting rtmessage to NOTICE log level is success and the required log lines are seen in the rtrouted.log file. The resetting of log level from NOTICE level should also be success and the corresponding log lines should be present in rtrouted.log file.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_RBUS_87</test_case_id>
    <test_objective>To check if setting rtmessage to NOTICE log level is success and the required log lines are seen in the rtrouted.log file. The resetting of log level from NOTICE level should also be success and the corresponding log lines should be present in rtrouted.log file.</test_objective>
...
4. Check the initial line count of Set NOTICE loglevel log lines and Reset NOTICE loglevel log lines in rtrouted.log and store it
5. Set the rtmessage log level to NOTICE using the command "rdklogctrl rtrouted LOG.RDK.RTMESSAGE NOTICE"
6. Wait for 2 minutes and then check for the presence of set NOTICE log level log lines are verify if it is incremented by 1 after the set operation.
7. Disable/Reset the NOTICE log level using the command "rdklogctrl rtrouted LOG.RDK.RTMESSAGE ~NOTICE"
8. Wait for 2 minutes and then check if the disable NOTICE log level line is incremented by 1 after the disable operation.
9. Revert to initial state if required
10. Unload the modules</automation_approch>
    <expected_output>The rtmessage NOTICE log level should be enabled and disabled successfully and the corresponding log lines should be present in rtrouted.log file.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_EnableandDisableRtmessage_NOTICELogLevel</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_EnableandDisableRtmessage_NOTICELogLevel');
tr181obj.configureTestCase(ip,port,'TS_RBUS_EnableandDisableRtmessage_NOTICELogLevel');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult and details == "File exist":
            ...
            print "\nGet the current number of log lines of \"rdk_dyn_log_validateComponentName(): Set NOTICE loglevel for the component LOG.RDK.RTMESSAGE of the process rtrouted\"";
            step = 3;
            search_string_loglevelSet = "rdk_dyn_log_validateComponentName(): Set NOTICE loglevel for the component LOG.RDK.RTMESSAGE of the process rtrouted"
            ...
            print "\nGet the current number of log lines of \"rdk_dyn_log_validateComponentName(): Set !NOTICE loglevel for the component LOG.RDK.RTMESSAGE of the process rtrouted\"";
            step = 4;
            search_string_loglevelReset = "rdk_dyn_log_validateComponentName(): Set !NOTICE loglevel for the component LOG.RDK.RTMESSAGE of the process rtrouted"
            ...
            cmd = "rdklogctrl rtrouted LOG.RDK.RTMESSAGE NOTICE";
            print "\nCommand : %s" %cmd;
            tdkTestObj.addParameter("command",cmd);
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails().strip();
            print "TEST STEP 5: Set the rtmessage log level to NOTICE";
            print "EXPECTED RESULT 5: Should successfully set the rtmessage log level to NOTICE";
            ...
            if expectedresult in actualresult and "Sent message to update log level of LOG.RDK.RTMESSAGE for rtrouted process" in details:
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 5: The rtmessage log level is set to NOTICE successfully; Details : %s" %details;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";
                ...
                print "\nGet the final number of log lines of \"rdk_dyn_log_validateComponentName(): Set NOTICE loglevel for the component LOG.RDK.RTMESSAGE of the process rtrouted\"";
                step = 6;
                ...
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails().strip();
                print "TEST STEP 8: Reset the rtmessage log level from NOTICE";
                print "EXPECTED RESULT 8: Should successfully Disable the rtmessage log level from NOTICE";

                if expectedresult in actualresult and "Sent message to update log level of LOG.RDK.RTMESSAGE for rtrouted process" in details:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "ACTUAL RESULT 8: The rtmessage log level NOTICE is disabled successfully; Details : %s" %details;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    ...
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 8: The rtmessage log level NOTICE is not disabled successfully; Details : %s" %details;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else :
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT 5: The rtmessage log level is not set to NOTICE successfully; Details : %s" %details;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            tdkTestObj.setResultStatus("FAILURE");
