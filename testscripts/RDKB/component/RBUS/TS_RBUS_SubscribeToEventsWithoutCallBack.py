...
<xml>
  ...
  <name>TS_RBUS_SubscribeToEventsWithoutCallBack</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Validate the RBUS API rbus_subscribeToEvent by calling the API without callback method</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_50</test_case_id>
    <test_objective>To Validate the RBUS API rbus_subscribeToEvent by calling the API without callback method</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state of DUT
2. TDK Agent should be in running state or invoke it through StartTdk.sh script
3. The DUT should be in RBUS mode</pre_requisite>
    <api_or_interface_used>rbus_subscribeToEvent
rbus_unsubscribeFromEvent</api_or_interface_used>
...
5. Register the Event in the RBUS using rbus_registerEvent API, return status should be success
6. Subscribe to the registered event using rbus_subscribeToEvent API with no call back option.
7. The return status should be failure since callback method is NULL
8. Unsubscribe from the event using rbus_unsubscribeFromEvent API, return status should be success
9. Unregister the object in the RBUS using rbus_unregisterObj API, return status should be success
10.Close the rbus broker connection using rbus_closeBrokerConnection API
11.The rbus_closeBrokerConnection should be success
12.Unload the rbus module</automation_approch>
    <expected_output>rbus_subscribeToEvent API should fail if call method is not defined</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_SubscribeToEventsWithoutCallBack</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_RBUS_SubscribeToEventsWithoutCallBack');
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                print "\n*************Start of Subscribing Event*****************************"
                tdkTestObj = obj.createTestStep('RBUS_RegisterOperation');
                tdkTestObj.addParameter("operation","rbus_subscribeToEvent");
                tdkTestObj.addParameter("objectName","tdkb_server1.obj1");
                tdkTestObj.addParameter("methodName","NullCallBack"); #NullCallBack will Call rbus_subscribeToEvent without callback function, EventName was added in stub directly
                expectedresult="FAILURE";
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
                print "RBUS Subscribe To Event Detail is ",details

                if expectedresult in actualresult:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "TEST STEP 4: Subscribe the Event with RBUS ";
                    print "EXPECTED RESULT 4: rbus_subscribeToEvent should Fail";
                    print "ACTUAL RESULT 4: rbus_subscribeToEvent was Failed as expected";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS"

                    tdkTestObj = obj.createTestStep('RBUS_RegisterOperation');
                    tdkTestObj.addParameter("operation","rbus_unsubscribeFromEvent");
                    tdkTestObj.addParameter("objectName","tdkb_server1.obj1");
                    tdkTestObj.addParameter("methodName","Event1"); # methodName will be converted as Event name in stub
                    expectedresult="SUCCESS";
                    tdkTestObj.executeTestCase(expectedresult);
                    ...
                    if expectedresult in actualresult:
                        #Set the result status of execution
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 5: Subscribe the Event with RBUS ";
                        print "EXPECTED RESULT 5: rbus_unsubscribeFromEvent should be success";
                        print "ACTUAL RESULT 5: rbus_unsubscribeFromEvent was success";
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS"
                    else:
                        #Set the result status of execution
                    ...
                    print "TEST STEP 4: Subscribe the Event with RBUS ";
                    print "EXPECTED RESULT 4: rbus_subscribeToEvent should Fail";
                    print "ACTUAL RESULT 4: rbus_subscribeToEvent was NOT Failed ";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE"
                print "*************End of Subscribing Event*****************************\n"
