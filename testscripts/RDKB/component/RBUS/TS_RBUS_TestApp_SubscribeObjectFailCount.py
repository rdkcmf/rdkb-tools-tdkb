...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_RBUS_TestApp_SubscribeObjectFailCount</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the RBUS functionalities against  Subscribe object using Test App execution</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_RBUS_73</test_case_id>
    <test_objective>To check the RBUS functionalities against  Subscribe object using Test App execution</test_objective>
...
6. Grep through the consumer log file (/tmp/rbusTestConsumer1.log) to get the PASS and FAIL count on Subscribe object
...
9. Unload the modules</automation_approch>
    <expected_output>The Fail count value of Subscribe object from consumer app result should be zero</expected_output>
    <priority>High</priority>
    <test_stub_interface>RBUS</test_stub_interface>
    <test_script>TS_RBUS_TestApp_SubscribeObjectFailCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_TestApp_SubscribeObjectFailCount');
tr181obj.configureTestCase(ip,port,'TS_RBUS_TestApp_SubscribeObjectFailCount');
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                print "Test Name : Subscribe Fail count Result is:",details
                if expectedresult in actualresult and details != "":
                    print "TEST STEP 4: Get the Subscribe Test Result from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "EXPECTED RESULT 4: Should get Subscribe Test Result value from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "ACTUAL RESULT 4: Successfully got Subscribe Test Result value from consumer file (/tmp/rbusTestConsumer1.log)"
                    ...
                    if int(failcount) == 0:
                        print "TEST STEP 5: Get the Fail count value for Test name Subscribe"
                        print "EXPECTED RESULT 5: Fail count value for Test name Subscribe should be zero"
                        print "[ACTUAL RESULT 5: Failcount value for Test name Subscribe is Zero"
                    ...
                    print "TEST STEP 4: Get the Subscribe Test Result from consumer log (/tmp/rbusTestConsumer1.log)"
                    print "EXPECTED RESULT 4: Should get Subscribe Test Result value from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "ACTUAL RESULT 4: Failed to get Subscribe Test Result value from consumer file (/tmp/rbusTestConsumer1.log)"
