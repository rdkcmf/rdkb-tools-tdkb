...
<xml>
  ...
  <name>TS_RBUS_Validate_ByteDataType</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Get the value of Byte data using Device.SampleProvider.AllTypes.ByteData and set a new value of  Byte data type and check if the value is getting set properly while the rbusSampleProvider App is running.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_76</test_case_id>
    <test_objective>Get the value of Byte data using Device.SampleProvider.AllTypes.ByteData and set a new value of  Byte data type and check if the value is getting set properly while the rbusSampleProvider App is running.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state of DUT
2. TDK Agent should be in running state or invoke it through StartTdk.sh script
3.Enable RBUS and ensure rbus_session_mgr is running</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>set_value :'e'</input_parameters>
...
4. When the App is running, fetch the value of Device.SampleProvider.AllTypes.ByteData and store it.
5. Set the set_value of byte type and check if the set operation is success.
6. Fetch the value of Device.SampleProvider.AllTypes.ByteData and confirm whether the set operation is success.
7. Revert to initial byte data value
8. Wait 60s for the rbusSampleProvider App to complete
9. Revert to the initial RBUS enable state if revert flag is set.</automation_approch>
    <expected_output>Set operation of Device.SampleProvider.AllTypes.ByteData should be success and the get operation should reflect the set value when rbusSampleProvider App is running.</expected_output>
    <priority>High</priority>
    <test_stub_interface>RBUS</test_stub_interface>
    <test_script>TS_RBUS_Validate_ByteDataType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_Validate_ByteDataType');
tr181obj.configureTestCase(ip,port,'TS_RBUS_Validate_ByteDataType');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            print "[TEST EXECUTION RESULT] : SUCCESS";
            #Get the data type of Byte data
            cmd = "rbuscli get Device.SampleProvider.AllTypes.ByteData | grep -i value";
            print "Command : %s" %cmd;
            tdkTestObj.addParameter("command",cmd);
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails().strip();
            print "\nTEST STEP 3: Get the Byte Data type value of Device.SampleProvider.AllTypes.ByteData";
            print "EXPECTED RESULT 3: Should get the byte data type value of Device.SampleProvider.AllTypes.ByteData";
            if expectedresult in actualresult and "Value" in details:
                ...
                print "[TEST EXECUTION RESULT] : SUCCESS";
                #Set value to Device.SampleProvider.AllTypes.ByteData
                set_value = "e";
                asciiVal = ord(set_value);
                hexVal = hex(asciiVal).split('0x')[1];
                print "ASCII value of set value is  :",asciiVal;
                print "hexadecimal value of the ascii  is  :",hexVal;
                cmd = "rbuscli set Device.SampleProvider.AllTypes.ByteData byte " + set_value + " | grep -i succeeded";
                print "Command : %s" %cmd;
                tdkTestObj.addParameter("command",cmd);
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails().strip();
                print "\nTEST STEP 4: Set the Byte Data type value of Device.SampleProvider.AllTypes.ByteData to %s" %set_value;
                print "EXPECTED RESULT 4: Should set the byte data type value of Device.SampleProvider.AllTypes.ByteData successfully";
                if expectedresult in actualresult and "succeeded" in details:
                    ...
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails().strip();
                    print "\nTEST STEP 5: Get the Byte Data type value of Device.SampleProvider.AllTypes.ByteData";
                    print "EXPECTED RESULT 5: Should get the byte data type value of Device.SampleProvider.AllTypes.ByteData";
                    if expectedresult in actualresult and "Value" in details:
                        ...
                        print "Set Value : %s, its Ascii to hex converted value is : %s" %(set_value,hexVal);
