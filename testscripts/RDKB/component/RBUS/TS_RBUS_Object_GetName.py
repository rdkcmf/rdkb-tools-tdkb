...
<xml>
  ...
  <name>TS_RBUS_Object_GetName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Validate the RBUS 2.0 API rbusObject_GetName</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_63</test_case_id>
    <test_objective>To Validate the RBUS 2.0 API rbusObject_GetName</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state of DUT
2. TDK Agent should be in running state or invoke it through StartTdk.sh script
3. The DUT should be in RBUS mode</pre_requisite>
    <api_or_interface_used>rbusObject_GetName</api_or_interface_used>
...
3. Get the Object name using rbusObject_GetName API and return status should be success
4. Compare the Get value with the initiated value, the values should be matching
5. Release the RBUS Object using rbusObject_Release API and return status should be success
6. Close the RBUS Connection using rbus_close API
</automation_approch>
    <expected_output>Should be able to get the  RBUS Object Name using rbusObject_GetName API</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_Object_GetName</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_RBUS_Object_GetName');
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if name == obj_name:
                    print "\n Name Retrieved from rbusObject_GetName(%s) function is Matching with initial value(%s)\n" %(name,obj_name)
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print "\n Name Retrieved from rbusObject_GetName(%s) function is NOT Matching with initial value (%s)\n" %(name,obj_name)
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 4: Initiate rbusObject_Release function";
            print "EXPECTED RESULT 4: rbusObject_Release should be success";
            print "ACTUAL RESULT 4: rbusObject_Release  was Success";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult ;
        else:
            #Set the result status of execution
            ...
            print "TEST STEP 4: Initiate rbusObject_Release function";
            print "EXPECTED RESULT 4: rbusObject_Release should be success";
            print "ACTUAL RESULT 4: rbusObject_Release was Failed";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult ;
