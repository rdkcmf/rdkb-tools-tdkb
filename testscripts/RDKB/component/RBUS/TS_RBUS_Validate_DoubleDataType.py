...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_RBUS_Validate_DoubleDataType</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Get the value of double data using Device.SampleProvider.AllTypes.DoubleData and set a new value of double data type and check if the value is getting set properly while the rbusSampleProvider App is running.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_RBUS_58</test_case_id>
    <test_objective>Get the value of double data using Device.SampleProvider.AllTypes.DoubleData and set a new value of double data type and check if the value is getting set properly while the rbusSampleProvider App is running.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state of DUT
2. TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>set_value : 5.141592653589793
</input_parameters>
...
4. When the App is running, fetch the value of Device.SampleProvider.AllTypes.DoubleData and store it.
5. Set the set_value of single type and check if the set operation is success.
6. Fetch the value of Device.SampleProvider.AllTypes.DoubleData and confirm whether the set operation is success.
7. Revert to initial single data value
...
9. Revert to the initial RBUS enable state if revert flag is set.</automation_approch>
    <expected_output>Set operation of Device.SampleProvider.AllTypes.DoubleData should be success and the get operation should reflect the set value when rbusSampleProvider App is running.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_DoubleDataType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_Validate_DoubleDataType');
tr181obj.configureTestCase(ip,port,'TS_RBUS_Validate_DoubleDataType');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            #Get the data type of Double data
            cmd = "rbuscli get Device.SampleProvider.AllTypes.DoubleData | grep -i value";
            print "Command : %s" %cmd;
            tdkTestObj.addParameter("command",cmd);
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails().strip();
            print "\nTEST STEP 3: Get the Double Data type value of Device.SampleProvider.AllTypes.DoubleData";
            print "EXPECTED RESULT 3: Should get the double data type value of Device.SampleProvider.AllTypes.DoubleData";
            ...
            if expectedresult in actualresult and "Value" in details:
                ...
                #Set value to Device.SampleProvider.AllTypes.DoubleData
                set_value = "5.141592653589793";
                cmd = "rbuscli set Device.SampleProvider.AllTypes.DoubleData double " + set_value + " | grep -i succeeded";
                print "Command : %s" %cmd;
                tdkTestObj.addParameter("command",cmd);
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails().strip();
                print "\nTEST STEP 4: Set the Double Data type value of Device.SampleProvider.AllTypes.DoubleData to %s" %set_value;
                print "EXPECTED RESULT 4: Should set the double data type value of Device.SampleProvider.AllTypes.DoubleData successfully";
                ...
                if expectedresult in actualresult and "succeeded" in details:
                    ...
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails().strip();
                    print "\nTEST STEP 5: Get the Double Data type value of Device.SampleProvider.AllTypes.DoubleData";
                    print "EXPECTED RESULT 5: Should get the double data type value of Device.SampleProvider.AllTypes.DoubleData";
