...
<xml>
  ...
  <name>TS_RBUS_GetParameterValue</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>RBUS_Get</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Validate the RBUS2.0 API rbus_get with the given parameter</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_23</test_case_id>
    <test_objective>To Validate the RBUS2.0 API rbus_get with the given parameter</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.DUT should be in RBUS mode</pre_requisite>
    <api_or_interface_used>rbus_get</api_or_interface_used>
    <input_parameters>N/A</input_parameters>
    <automation_approch>1. Load the rbus module
2. Open the rbus connection using rbus_open RBUS API
3. Invoke the RBUS API rbus_get with the parameter name and the return status should be success
4. Close the rbus connection using rbus_close RBUS API
5. Unload the module</automation_approch>
    <expected_output>RBUS2.0 API rbus_get return status should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_GetParameterValue</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_RBUS_GetParameterValue');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj = obj.createTestStep('RBUS_Get');
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: Validate RBUS_Get function";
            print "EXPECTED RESULT 2: RBUS_Get should be success";
            print "ACTUAL RESULT 2: RBUS_Get  was Success";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult ;
        else:
            #Set the result status of execution
            ...
            print "TEST STEP 2: Validate RBUS_Get function";
            print "EXPECTED RESULT 2: RBUS_Get should be success";
            print "ACTUAL RESULT 2: RBUS_Get was Failed";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult ;
