...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_RBUS_Validate_CharDataType</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Get the value of Char data using Device.SampleProvider.AllTypes.CharData and set a new value of Char data type and check if the value is getting set properly while the rbusSampleProvider App is running</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_RBUS_75</test_case_id>
    <test_objective>Get the value of Char data using Device.SampleProvider.AllTypes.CharData and set a new value of Char data type and check if the value is getting set properly while the rbusSampleProvider App is running</test_objective>
...
3. Enable RBUS and ensure rbus_session_mgr is running</pre_requisite>
    ...
    <input_parameters>set_value : 'A'</input_parameters>
...
4. When the App is running, fetch the value of Device.SampleProvider.AllTypes.CharData and store it.
5. Set the set_value of char type and check if the set operation is success.
6. Fetch the value of Device.SampleProvider.AllTypes.CharData and confirm whether the set operation is success.
7. Revert to initial char data value
...
9. Revert to the initial RBUS enable state if revert flag is set.</automation_approch>
    <expected_output>Set operation of Device.SampleProvider.AllTypes.CharData should be success and the get operation should reflect the set value when rbusSampleProvider App is running.</expected_output>
    <priority>High</priority>
    <test_stub_interface>RBUS</test_stub_interface>
    <test_script>TS_RBUS_Validate_CharDataType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_Validate_CharDataType');
tr181obj.configureTestCase(ip,port,'TS_RBUS_Validate_CharDataType');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            print "[TEST EXECUTION RESULT] : SUCCESS";
            #Get the data type of Char data
            cmd = "rbuscli get Device.SampleProvider.AllTypes.CharData | grep -i value";
            print "Command : %s" %cmd;
            tdkTestObj.addParameter("command",cmd);
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails().strip();
            print "\nTEST STEP 3: Get the Char Data type value of Device.SampleProvider.AllTypes.CharData";
            print "EXPECTED RESULT 3: Should get the char data type value of Device.SampleProvider.AllTypes.CharData";
            if expectedresult in actualresult and "Value" in details:
                ...
                print "[TEST EXECUTION RESULT] : SUCCESS";
                #Set value to Device.SampleProvider.AllTypes.CharData
                set_value = "A";
                cmd = "rbuscli set Device.SampleProvider.AllTypes.CharData char " + set_value + " | grep -i succeeded";
                print "Command : %s" %cmd;
                tdkTestObj.addParameter("command",cmd);
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails().strip();
                print "\nTEST STEP 4: Set the Char Data type value of Device.SampleProvider.AllTypes.CharData to %s" %set_value;
                print "EXPECTED RESULT 4: Should set the char data type value of Device.SampleProvider.AllTypes.CharData successfully";
                if expectedresult in actualresult and "succeeded" in details:
                    ...
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails().strip();
                    print "\nTEST STEP 5: Get the Char Data type value of Device.SampleProvider.AllTypes.CharData";
                    print "EXPECTED RESULT 5: Should get the char data type value of Device.SampleProvider.AllTypes.CharData";
                    if expectedresult in actualresult and "Value" in details:
