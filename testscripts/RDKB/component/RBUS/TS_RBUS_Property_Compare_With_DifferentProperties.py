...
<xml>
  ...
  <name>TS_RBUS_Property_Compare_With_DifferentProperties</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Validate the RBUS 2.0 API rbusProperty_Compare by passing two different RBUS Property values</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To Validate the RBUS 2.0 API rbusProperty_Compare by passing two different RBUS Property values</test_objective>
...
2. Initiate the two RBUS properties with different property names using rbusProperty_Init API
3. Compare the RBUS properties using rbusProperty_Compare API , the return value should be failure since properties are different
4. Release the two RBUS properties using rbusProperty_Release and return status should be success
5. Close the RBUS connection using rbus_close API
</automation_approch>
    <expected_output>Should get Failure on comparing two different RBUS Property values</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_Property_Compare_With_DifferentProperties</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_RBUS_Property_Compare_With_DifferentProperties');
    ...
    if expectedresult in actualresult:
        ...
        for count in range (1,3):
            if count == 1:
                ...
                prop_name = "Device.rbusPropertyTest2"

            print "Initiaize the Property prop%d with Property name %s" %(count,prop_name)
        ...
        if init_done == 1:
            ...
            if expectedresult in actualresult:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 4: Initiate rbusProperty_Compare function ";
                print "EXPECTED RESULT 4: rbusProperty_Compare should Fail since Properties are assigned different values";
                print "ACTUAL RESULT 4: rbusProperty_Compare function Failed as expected";
                #Get the result of execution
                ...
                print "TEST STEP 4: Initiate rbusProperty_Compare function ";
                print "EXPECTED RESULT 4: rbusProperty_Compare should Fail since Properties are assigned different values";
                print "ACTUAL RESULT 4: rbusProperty_Compare function NOT Failed";
                #Get the result of execution
