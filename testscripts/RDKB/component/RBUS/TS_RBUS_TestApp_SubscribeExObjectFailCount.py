...
<xml>
  ...
  <name>TS_RBUS_TestApp_SubscribeExObjectFailCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check the RBUS functionalities against SubscribeEx object using Test App execution</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_15</test_case_id>
    <test_objective>To check the RBUS functionalities against SubscribeEx object using Test App execution</test_objective>
...
6. Grep through the consumer log file (/tmp/rbusTestConsumer1.log) to get the PASS and FAIL count on SubscribeEx object
...
9. Unload the modules</automation_approch>
    <expected_output>The Fail count value of SubscribeEx object from consumer app result should be zero
</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_TestApp_SubscribeExObjectFailCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_SampleApp_SubscribeExObjectFailCount');
tr181obj.configureTestCase(ip,port,'TS_RBUS_SampleApp_SubscribeExObjectFailCount');
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                cmd = "grep -w \"# SubscribeEx\" /tmp/rbusTestConsumer1.log";
                ...
                print "Test Name : SubscribeEx Fail count Result is:",details

                if expectedresult in actualresult and details != "":
                    print "TEST STEP 4: Get the SubscribeEx Test Result from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "EXPECTED RESULT 4: Should get SubscribeEx Test Result value from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "ACTUAL RESULT 4: Successfully got SubscribeEx Test Result value from consumer file (/tmp/rbusTestConsumer1.log)"
                    ...
                    if int(failcount) == 0:
                        print "TEST STEP 5: Get the Fail count value for Test name SubscribeEx"
                        print "EXPECTED RESULT 5: Fail count value for Test name SubscribeEx should be zero"
                        print "[ACTUAL RESULT 5: Failcount value for Test name SubscribeEx is Zero"
                    ...
                    print "TEST STEP 4: Get the SubscribeEx Test Result from consumer log (/tmp/rbusTestConsumer1.log)"
                    print "EXPECTED RESULT 4: Should get SubscribeEx Test Result value from consumer log file (/tmp/rbusTestConsumer1.log)"
