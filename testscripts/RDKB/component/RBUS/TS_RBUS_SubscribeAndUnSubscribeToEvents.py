...
<xml>
  ...
  <name>TS_RBUS_SubscribeAndUnSubscribeToEvents</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Validate the RBUS APIs rbus_subscribeToEvent and rbus_unsubscribeFromEvent by subscribing and unsubscribing Events in the RBUS</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_49</test_case_id>
    <test_objective>To Validate the RBUS APIs rbus_subscribeToEvent and rbus_unsubscribeFromEvent by subscribing and unsubscribing Events in the RBUS</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state of DUT
2. TDK Agent should be in running state or invoke it through StartTdk.sh script
3. The DUT should be in RBUS mode</pre_requisite>
    <api_or_interface_used>rbus_subscribeToEvent rbus_unsubscribeFromEvent</api_or_interface_used>
...
6. Subscribe to the registered event using rbus_subscribeToEvent API, return status should be success
7. Unsubscribe from the event using rbus_unsubscribeFromEvent API, return status should be success
8. Unregister the object in the RBUS using rbus_unregisterObj API, return status should be success
...
11.Unload the rbus module</automation_approch>
    <expected_output>Should be able to subscribe and unsubscribe to the events in the RBUS</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_SubscribeAndUnSubscribeToEvents</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_RBUS_SubscribeAndUnSubscribeToEvents');
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    ...
                    print "EXPECTED RESULT 4: rbus_subscribeToEvent should be success";
                    print "ACTUAL RESULT 4: rbus_subscribeToEvent was success";
                    #Get the result of execution
