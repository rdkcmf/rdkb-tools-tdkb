...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether RBus Enabled log is present or not in CR log file</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_12</test_case_id>
    <test_objective>To check whether RBus Enabled log is present or not in CR log file</test_objective>
...
4. Check CRLog file empty or not, if empty reboot the device once to populate the logs in CRLog file
5. Check whether "RBUS Enabled" log is present in CRLog file or not. Success if present else failure
6. Execute post process of RBUS and it should be success
7. Unload the Modules</automation_approch>
    <expected_output>RBus enabled log should be present in CRLog file</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_CheckCRLogs</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_CheckCRLogs');
tr181obj.configureTestCase(ip,port,'TS_RBUS_CheckCRLogs');
