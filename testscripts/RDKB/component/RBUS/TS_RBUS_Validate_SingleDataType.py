...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>TS_RBUS_Validate_SingleDataType</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Get the value of single data using Device.SampleProvider.AllTypes.SingleData and set a new value of single data type and check if the value is getting set properly while the rbusSampleProvider App is running.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Get the value of single data using Device.SampleProvider.AllTypes.SingleData and set a new value of single data type and check if the value is getting set properly while the rbusSampleProvider App is running.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state of DUT
2. TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>set_value : 10.234567
</input_parameters>
...
4. When the App is running, fetch the value of Device.SampleProvider.AllTypes.SingleData and store it.
5. Set the set_value of single type and check if the set operation is success.
6. Fetch the value of Device.SampleProvider.AllTypes.SingleData and confirm whether the set operation is success.
...
9. Revert to the initial RBUS enable state if revert flag is set.</automation_approch>
    <expected_output>Set operation of Device.SampleProvider.AllTypes.SingleData should be success and the get operation should reflect the set value when rbusSampleProvider App is running.</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_SingleDataType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_Validate_SingleDataType');
tr181obj.configureTestCase(ip,port,'TS_RBUS_Validate_SingleDataType');
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult and "Value" in details:
                ...
                #Set value to Device.SampleProvider.AllTypes.SingleData
                set_value = "10.234567";
