...
<xml>
  ...
  <name>TS_RBUS_SetInt32ParamValue</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Set the DML parameter (integer) using rbus_set and rbusValue_SetInt32 RBUS APIs</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_29</test_case_id>
    <test_objective>To Set the DML parameter (integer) using rbus_set and rbusValue_SetInt32 RBUS APIs</test_objective>
...
4. Invoke the RBUS API rbusValue_SetInt32 with the parameter name and parametervalue (Different from initial value)
5. Set operation should be success
6. Get the parameter value using rbusValue_GetInt32 and check value was set properly
7. Revert the parameter value to its initial value using rbusValue_SetInt32 API
8. Close the rbus connection using rbus_close RBUS API
9. Unload the module</automation_approch>
    <expected_output>Set operation should be success using rbusValue_SeInt32 API</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_SetInt32ParamValue</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_RBUS_SetInt32ParamValue');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult and initial_Value != "":
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: Get the value of parameter using rbusValue_GetInt32";
            ...
            if expectedresult in actualresult and details != "":
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3: Set the value for parameter using rbusValue_SetInt32";
                ...
                if expectedresult in actualresult and new_Value != "":
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "TEST STEP 4: Get the value of parameter using rbusValue_GetInt32";
                    ...
                    if int(new_Value) == int(new_value_to_set):
                ...
                print "Value to be set for Revert operation is: ",initial_Value
