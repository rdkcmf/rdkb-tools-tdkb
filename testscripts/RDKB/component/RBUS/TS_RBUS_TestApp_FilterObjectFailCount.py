...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_RBUS_TestApp_FilterObjectFailCount</name>
  <primitive_test_id/>
  <primitive_test_name>RBUS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the RBUS functionalities against Filter object using Test App execution.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_RBUS_71</test_case_id>
    <test_objective>To check the RBUS functionalities against Filter object using Test App execution.</test_objective>
...
6. Grep through the consumer log file (/tmp/rbusTestConsumer1.log) to get the PASS and FAIL count on Filter object
...
9. Unload the modules</automation_approch>
    <expected_output>The Fail count value of Filter object from consumer app result should be zero</expected_output>
    <priority>High</priority>
    <test_stub_interface>RBUS</test_stub_interface>
    <test_script>TS_RBUS_TestApp_FilterObjectFailCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_RBUS_TestApp_FilterObjectFailCount');
tr181obj.configureTestCase(ip,port,'TS_RBUS_TestApp_FilterObjectFailCount');
    ...
    if rbus_set == 1:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                sleep(300);
                cmd = "tail -2 /tmp/rbusTestConsumer1.log | head -1";
                ...
                print "Test Name : Filter Fail count Result is:",details
                if expectedresult in actualresult and details != "":
                    print "TEST STEP 4: Get the Filter Test Result from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "EXPECTED RESULT 4: Should get Filter Test Result value from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "ACTUAL RESULT 4: Successfully got Filter Test Result value from consumer file (/tmp/rbusTestConsumer1.log)"
                    ...
                    if int(failcount) == 0:
                        print "TEST STEP 5: Get the Fail count value for Test name Filter"
                        print "EXPECTED RESULT 5: Fail count value for Test name Filter should be zero"
                        print "[ACTUAL RESULT 5: Failcount value for Test name Filter is Zero"
                    ...
                    print "TEST STEP 4: Get the Filter Test Result from consumer log (/tmp/rbusTestConsumer1.log)"
                    print "EXPECTED RESULT 4: Should get Filter Test Result value from consumer log file (/tmp/rbusTestConsumer1.log)"
                    print "ACTUAL RESULT 4: Failed to get Filter Test Result value from consumer file (/tmp/rbusTestConsumer1.log)"
