...
<xml>
  ...
  <name>TS_RBUS_OpenAndCloseBrokerConnection</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the RBUS APIs rbus_openBrokerConnection and rbus_closeBrokerConnection</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_RBUS_32</test_case_id>
    <test_objective>To validate the RBUS APIs rbus_openBrokerConnection and rbus_closeBrokerConnection</test_objective>
...
4. Close the rbus broker connection using rbus_closeBrokerConnection API
5. The rbus_closeBrokerConnection should be success
6. Unload the rbus module</automation_approch>
    <expected_output>Should be able to open the RBUS broker connection using rbus_openBrokerConnection and close the broker connection using rbus_closeBrokerConnection</expected_output>
    <priority>High</priority>
    <test_stub_interface>rbus</test_stub_interface>
    <test_script>TS_RBUS_OpenAndCloseBrokerConnection</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_RBUS_OpenAndCloseBrokerConnection');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #Set the result status of execution
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: Close the RBUS Broker connection";
            print "EXPECTED RESULT 2: rbus_closeBrokerConnection should be success";
            print "ACTUAL RESULT 2: rbus_closeBrokerConnection was success";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult ;
        else:
            #Set the result status of execution
            ...
            print "TEST STEP 2: Close the RBUS Broker connection";
            print "EXPECTED RESULT 2: rbus_closeBrokerConnection should be success";
            print "ACTUAL RESULT 2: rbus_closeBrokerConnection was Failed";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult ;
    else:
        #Set the result status of execution
