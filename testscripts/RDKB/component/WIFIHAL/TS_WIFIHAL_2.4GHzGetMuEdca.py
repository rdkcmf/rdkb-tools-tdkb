...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzGetMuEdca</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Invoke the HAL API wifi_getMuEdca() for 2.4G radio and check if the Arbitration Inter-Frame Space Number, Lower bound Contention Window, Upper bound Contention Window and Timer values retrieved are valid integers for each of the access categories - Background, Best effort, Video and Voice.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2. Invoke the HAL API wifi_getMuEdca() for 2.4G radio for each of the access categories : Background, Best effort, Video and Voice.
...
4. Unload module</automation_approch>
    <expected_output>The HAL API wifi_getMuEdca() should be invoked successfully for 2.4G radio and the Arbitration Inter-Frame Space Number, Lower bound Contention Window, Upper bound Contention Window and Timer values retrieved should be valid integers for each of the access categories - Background, Best effort, Video and Voice.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzGetMuEdca</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetMuEdca');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        for category in range(0, 4):
            ...
            print "\nTEST STEP %d: Invoke the HAL API wifi_getMuEdca() to retrieve the 2.4G MU-EDCA details for %s category" %(step, accessCategory[category]);
