...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_6GHzSetFTMobilityDomainID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTMobilityDomainID</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the FT Mobility Domain ID by invoking the HAL API wifi_setFTMobilityDomainID() and validate the SET using the GET API wifi_getFTMobilityDomainID() for 6G private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_728</test_case_id>
    <test_objective>Set the FT Mobility Domain ID by invoking the HAL API wifi_setFTMobilityDomainID() and validate the SET using the GET API wifi_getFTMobilityDomainID() for 6G private AP.</test_objective>
...
apIndex : 6G Private AP index
radioIndex : 6G radio index
mobilityDomain : setvalue(dynamically passed)</input_parameters>
    <automation_approch>1. Load the modules
2. Invoke the HAL API wifi_getFTMobilityDomainID() for 6G Private AP and check if the API invocation is success and the FT Mobility Domain ID retrieved is a pair of Hexadecimal values.
3. Combine the Hex value and obtain the equivalent decimal value for FT Mobility Domain ID and store the initial value.
4. Invoke the HAL API wifi_setFTMobilityDomainID() for 6G private AP with a dynamically selected Mobility Domain ID. Check if the SET API returns success.
...
8. Unload the modules
    ...
    <expected_output>Should successfully set the FT Mobility Domain ID by invoking the HAL API wifi_setFTMobilityDomainID() and the FT Mobility Domain ID set should reflect in the GET API wifi_getFTMobilityDomainID() for 6G Private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzSetFTMobilityDomainID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetFTMobilityDomainID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetFTMobilityDomainID');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if idx == -1:
        ...
        if apIndex == -1:
            ...
            tdkTestObj = obj.createTestStep("WIFIHAL_GetOrSetFTMobilityDomainID");
            tdkTestObj, actualresult, details = getMobilityDomain(tdkTestObj, apIndex);

            print "\nTEST STEP 2: Invoke the HAL API wifi_getFTMobilityDomainID() to retrieve the Mobility Domain ID for 6G private AP";
            print "EXPECTED RESULT 2: Should get the Mobility Domain ID using the HAL API successfully";
            ...
            if expectedresult in actualresult :
                ...
                print "\nTEST STEP 3 : Check if the FT Mobility Domain ID is retrieved as two Hex Values";
                print "EXPECTED RESULT 3 : FT Mobility Domain ID should be retrieved as two Hex Values";
                ...
                if "0x" in hex_value0 and "0x" in hex_value1:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "ACTUAL RESULT 3: FT Mobility Domain ID is retrieved as two Hex values";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    ...
                    print "\nTEST STEP 4: Invoke the HAL API wifi_setFTMobilityDomainID() to set the Mobility Domain ID for 6G private AP";
                    print "EXPECTED RESULT 4: Should set the Mobility Domain ID to %d using the HAL API successfully" %setValue;
                    ...
                    if expectedresult in actualresult :
                        ...
                        print "\nTEST STEP 5: Invoke the HAL API wifi_getFTMobilityDomainID() to retrieve the Mobility Domain ID Set operation";
                        print "EXPECTED RESULT 5: Should get the Mobility Domain ID using the HAL API successfully";
                        ...
                        if expectedresult in actualresult :
                            ...
                            #Check if proper Hex values are retrieved and convert them to integer value
                            print "\nTEST STEP 6 : Check if the FT Mobility Domain ID is retrieved as two Hex Values after Set operation";
                            print "EXPECTED RESULT 6 : FT Mobility Domain ID should be retrieved as two Hex Values";
                            ...
                            if "0x" in hex_value0 and "0x" in hex_value1:
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "ACTUAL RESULT 6: FT Mobility Domain ID is retrieved as two Hex values";
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : SUCCESS";

                                #Covert to corresponding Integer values
                                final_FTMDID = HextoIntConversion(hex_value0_final, hex_value1_final);
                                print "\nTEST STEP 7 : Check if the SET value and GET value of FT Mobility Domain ID match";
                                print "EXPECTED RESULT 7 :Tthe SET value and GET value of FT Mobility Domain ID should match";
                                ...
                                if setValue == final_FTMDID:
                                    #Set the result status of execution
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print "ACTUAL RESULT 7: The SET value and GET value of FT Mobility Domain ID match";
                                    #Get the result of execution
                                    print "[TEST EXECUTION RESULT] : SUCCESS";
                                    ...
                                    print "\nTEST STEP 8: Invoke the HAL API wifi_setFTMobilityDomainID() to revert to initial value";
                                    print "EXPECTED RESULT 8: Should set the Mobility Domain ID to %d using the HAL API successfully" %initial_FTMDID;
                                    ...
                                    #Set the result status of execution
                                    tdkTestObj.setResultStatus("FAILURE");
                                    print "ACTUAL RESULT 7: The SET value and GET value of FT Mobility Domain ID does not match";
                                    #Get the result of execution
                                    print "[TEST EXECUTION RESULT] : FAILURE";
                            else:
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("FAILURE");
                                print "ACTUAL RESULT 6: FT Mobility Domain ID is not retrieved as two Hex values";
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : FAILURE";
                        else :
                            #Set the result status of execution
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 3: FT Mobility Domain ID is not retrieved as two Hex values";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                #Set the result status of execution
