...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_6GHzSetBandSteeringPhyRateThreshold</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamIntValue</primitive_test_name>
  <primitive_test_version>5</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set and get the band steering physical modulation rate threshold parameters for 6GHz with the HAL APIs wifi_setBandSteeringPhyRateThreshold and wifi_getBandSteeringPhyRateThreshold.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_629</test_case_id>
    <test_objective>To set and get the band steering physical modulation rate threshold parameters for 6GHz with the HAL APIs wifi_setBandSteeringPhyRateThreshold and wifi_getBandSteeringPhyRateThreshold.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_setBandSteeringPhyRateThreshold
wifi_getBandSteeringPhyRateThreshold
wifi_getBandSteeringCapability</api_or_interface_used>
    <input_parameters>methodname : getBandSteeringCapability
methodname : getBandSteeringPhyRateThreshold
methodname : getBandSteeringCapability
setValue : valid random value
radioIndex : index corresponding to 6G radio
</input_parameters>
...
4. Using WIFIHAL_GetOrSetParamIntValue invoke wifi_setBandSteeringPhyRateThreshold and set a valid value
...
8. Unload wifihal module</automation_approch>
    <expected_output>Setting the Band Steering Physical Rate Threshold using the HAL API wifi_setBandSteeringPhyRateThreshold() should be successful and the get value using wifi_getBandSteeringPhyRateThreshold() should reflect the value set if the device is Band Steering capable for 6GHz. </expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzSetBandSteeringPhyRateThreshold</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetBandSteeringPhyRateThreshold');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult:
            ...
            if "Enabled" in enable:
                print "\nStep 2 : Get initial value of Physical Rate Threshold";
                ...
                if expectedresult in actualresult:
                    ...
                    print "\nStep 3 : Set the Physical Rate Threshold to : %d" %setValue;
                    ...
                    if expectedresult in actualresult:
                        ...
                        radioIndex = idx;
                        primitive = 'WIFIHAL_GetOrSetParamIntValue'
                        print "\nStep 4 : Get the current value of Physical Rate Threshold";
                        tdkTestObj, actualresult, details = ExecuteWIFIHalCallMethod(obj, primitive, radioIndex, 0, getMethod)

                        if expectedresult in actualresult:
                            tdkTestObj.setResultStatus("SUCCESS");
                            finalGetValue = details.split(":")[1].strip()
                            print "\nStep 5 : Validating the set operation for Physical Rate Threshold";
                            print "TEST STEP: Comparing set and get values of Physical Rate Threshold"
                        ...
                        print "\nStep 6 : Reverting to initial Physical Rate Threshold value";
                        tdkTestObj, actualresult, details = ExecuteWIFIHalCallMethod(obj, primitive, radioIndex, setValue, setMethod)
