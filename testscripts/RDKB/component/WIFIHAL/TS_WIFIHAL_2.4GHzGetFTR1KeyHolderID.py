...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_2.4GHzGetFTR1KeyHolderID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTR1KeyHolderID</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getFTR1KeyHolderID() for 2.4G private AP to retrieve the Fast Transition R1 Key Holder ID and cross check with the R1 Key Holder ID value stored in nvram</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_770</test_case_id>
    <test_objective>Invoke the HAL API wifi_getFTR1KeyHolderID() for 2.4G private AP to retrieve the Fast Transition R1 Key Holder ID and cross check with the R1 Key Holder ID value stored in nvram</test_objective>
...
2. Invoke the HAL API wifi_getFTR1KeyHolderID() to get the FTR1 Key Holder ID for 2.4G private AP.
3. Convert the Hex values received to equivalent string representation.
4. Get the command to fetch the FTR1 Key Holder ID for 2.4G private AP from the platform property file.
...
7. Unload the modules.</automation_approch>
    <expected_output>Invocation of the HAL API wifi_getFTR1KeyHolderID() for 2.4G private AP should be success and the FTR1 Key Holder ID retrieved should be the same as the value stored in nvram.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzGetFTR1KeyHolderID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetFTR1KeyHolderID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetFTR1KeyHolderID');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getFTR1KeyHolderID() to retrieve the FTR1 Key Holder ID for 2.4G private AP";
        ...
        if expectedresult in actualresult and "FTR1 Key Holder ID Details -" in details:
            ...
            if num_hex_values >= 1:
                ...
                cmd= "sh %s/tdk_utility.sh parseConfigFile FTR1_KEY_HOLDER_ID_2G" %TDK_PATH;
                print cmd;
                ...
                print "\nTEST STEP 3: Get the command to cross check the FTR1 Key Holder ID from property file";
                print "EXPECTED RESULT 3: Should get the FTR1 Key Holder ID check command from property file"

                if expectedresult in actualresult and check_cmd!= "":
                    print "ACTUAL RESULT 3: FTR1 Key Holder ID check command retrieved from property file";
                    print "TEST EXECUTION RESULT :SUCCESS";
                    tdkTestObj.setResultStatus("SUCCESS");
                    ...
                    print "EXPECTED RESULT 4: Should successfully execute the FTR1 Key Holder ID check command"
                    ...
                    if expectedresult in actualresult :
                        ...
                        print "ACTUAL RESULT 4: FTR1 Key Holder ID : ", fbt_key_id ;
                        print "TEST EXECUTION RESULT :SUCCESS";
                        tdkTestObj.setResultStatus("SUCCESS");

                        #check if the FTR1 Key Holder ID value from the HAL API is same as the value retrieved using the command
                        print "\nTEST STEP 5 : Validate the FTR1 Key Holder ID value retrieved using the HAL API";
                        print "EXPECTED RESULT 5 : The FTR1 Key Holder ID value from the HAL API should be the same as the value retrieved using the command";
                        ...
                        print "ACTUAL RESULT 4: FTR1 Key Holder ID :", details ;
                        print "TEST EXECUTION RESULT :FAILURE";
                        tdkTestObj.setResultStatus("FAILURE");
                else:
                    print "ACTUAL RESULT 3: FTR1 Key Holder ID check command from property file :", check_cmd ;
                    print "TEST EXECUTION RESULT :FAILURE";
                    tdkTestObj.setResultStatus("FAILURE");
            else:
                #Set the result status of execution
