...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_6GHzPushApFastTransitionConfig</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_PushApFastTransitionConfig</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_pushApFastTransitionConfig() for 6G private AP and set the BSS Transition Support, Mobility Domain ID and FT Over DS Activated enable state to new values and cross check if the values set using the Push API is reflected in wifi_getBSSTransitionActivated(), wifi_getFTMobilityDomainID() and wifi_getFTOverDSActivated() HAL APIs.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_803</test_case_id>
    <test_objective>Invoke the HAL API wifi_pushApFastTransitionConfig() for 6G private AP and set the BSS Transition Support, Mobility Domain ID and FT Over DS Activated enable state to new values and cross check if the values set using the Push API is reflected in wifi_getBSSTransitionActivated(), wifi_getFTMobilityDomainID() and wifi_getFTOverDSActivated() HAL APIs.</test_objective>
...
8. Unload the module.</automation_approch>
    <expected_output>The HAL API wifi_pushApFastTransitionConfig() for 6G private AP to set the BSS Transition Support, Mobility Domain ID and FT Over DS Activated enable state to new values should be invoked successfully and the values set using the Push API should be reflected in wifi_getBSSTransitionActivated(), wifi_getFTMobilityDomainID() and wifi_getFTOverDSActivated() HAL APIs.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzPushApFastTransitionConfig</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzPushApFastTransitionConfig');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzPushApFastTransitionConfig');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if idx == -1:
        ...
        if apIndex == -1:
            ...
            actualresult, details, tdkTestObj = getApFTValues(obj, primitive, apIndex, api, step);
