...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzIsRadioIEEE80211hEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check IEEE80211h is enabled for 2.4GHz Radio using wifi_getRadioIEEE80211hEnabled HAL API</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_64</test_case_id>
    <test_objective>To check IEEE80211h is enabled for 2.4GHz Radio using wifi_getRadioIEEE80211hEnabled HAL API</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XB3. XB6, Emulator, Rpi</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getRadioIEEE80211hEnabled()</api_or_interface_used>
    <input_parameters>methodName   :   getRadioIEEE80211hEnabled
methodName   :   setRadioIEEE80211hEnabled
radioIndex   :   0</input_parameters>
    <automation_approch>1.Configure the Function info in Test Manager GUI  which needs to be tested  
(WIFIHAL_GetOrSetParamBoolValue  - func name - "If not exists already"
 WIFIHAL - module name
 Necessary I/P args as Mentioned in Input)
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_2.4GHzIsRadioIEEE80211hEnabled.py)
3.Execute the generated Script(TS_WIFIHAL_2.4GHzIsRadioIEEE80211hEnabled.py) using execution page of  Test Manager GUI
...
wifi_getRadioIEEE80211hEnabled() and wifi_setRadioIEEE80211hEnabled()
...
1:wifi_getRadioIEEE80211hEnabled log from DUT should be available in Agent Console LogCheckPoint 
...
3:Test Manager GUI will publish the result as SUCCESS in Execution page"""</except_output>
    ...
    <test_script>TS_WIFIHAL_2.4GHzIsRadioIEEE80211hEnabled</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzIsRadioIEEE80211hEnabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult :
        ...
        if "Enabled" in enable:
            print "IEEE80211h is Enabled in Radio 2.4Ghz"
            oldEnable = 1
            newEnable = 0
        else:
            print "IEEE80211h is Disabled in Radio 2.4Ghz"
            oldEnable = 0
            newEnable = 1

        setMethod = "setRadioIEEE80211hEnabled"
        ...
        if expectedresult in actualresult :
            ...
            if expectedresult in actualresult and enable not in details.split(":")[1].strip():
                print "getRadioIEEE80211hEnabled Success, verified along with setRadioIEEE80211hEnabled() api"
                ...
                print "getRadioIEEE80211hEnabled() failed after set function"
		tdkTestObj.setResultStatus("FAILURE");
        else:
	    print "setRadioIEEE80211hEnabled() failed"
            tdkTestObj.setResultStatus("FAILURE");
    else:
	print "getRadioIEEE80211hEnabled() failed"
        tdkTestObj.setResultStatus("FAILURE");
