...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzKickApAssociatedDevice</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_518</test_case_id>
...
4. Get the MAC of AssociatedDevice.1 using Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress
...
8. If no: of associated device is greater than 0, then check if the current value of Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress is not equal to the kicked MAC
9. Unload wifihal module</automation_approch>
    ...
    <test_script>TS_WIFIHAL_5GHzKickApAssociatedDevice</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzKickApAssociatedDevice');
wifiobj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzKickApAssociatedDevice');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in wifiloadmodulestatus.upper():
	...
	if expectedresult in actualresult:
		...
		print "TEST EXECUTION RESULT 1: SUCCESS"
		    ...
		    print "Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress :",macAddress;
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in actualresult:
                             ...
                             if int(ApNumDevices) > 0:
                                 print "TEST STEP 4: Check if Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress is no longer ",macAddress
                                 print "EXPECTED RESULT 4: Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress should not be equal to ",macAddress
                                 tdkTestObj = wifiobj.createTestStep("WIFIAgent_Get");
                                 tdkTestObj.addParameter("paramName","Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress");
                                 expectedresult="SUCCESS";
                                 tdkTestObj.executeTestCase(expectedresult);
                                 ...
                                 if expectedresult in actualresult:
                                     ...
                                     print "Device.WiFi.AccessPoint.2.AssociatedDevice.x.MACAddress :",newMacAddress;
                                     if newMacAddress != macAddress:
                                         print "ACTUAL RESULT 4: Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress is not equal to kicked MAC ",macAddress
                                         print "[TEST EXECUTION RESULT] 4: SUCCESS";
                                         tdkTestObj.setResultStatus("SUCCESS");
                                     else:
                                         print "ACTUAL RESULT 4: Device.WiFi.AccessPoint.2.AssociatedDevice.1.MACAddress is equal to kicked MAC",macAddress
