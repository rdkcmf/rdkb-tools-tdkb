...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzIsRadioIGMPSnoopingEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check IGMPSnooping Enable status of Radio 2.4GHZ using wifi_getRadioIGMPSnoopingEnable HAL API</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_65</test_case_id>
    <test_objective>To check IGMPSnooping Enable status of Radio 2.4GHZ using wifi_getRadioIGMPSnoopingEnable HAL API</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XB3. XB6</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getRadioIGMPSnoopingEnable()</api_or_interface_used>
    <input_parameters>methodName   :   getRadioIGMPSnoopingEnable
methodName   :   setRadioIGMPSnoopingEnable
radioIndex   :   0</input_parameters>
    <automation_approch>1.Configure the Function info in Test Manager GUI  which needs to be tested  
(WIFIHAL_GetOrSetParamBoolValue  - func name - "If not exists already"
 WIFIHAL - module name
 Necessary I/P args as Mentioned in Input)
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_2.4GHzIsRadioIGMPSnoopingEnabled.py)
3.Execute the generated Script(TS_WIFIHAL_2.4GHzIsRadioIGMPSnoopingEnabled.py) using execution page of  Test Manager GUI
...
wifi_getRadioIGMPSnoopingEnable() and wifi_setRadioIGMPSnoopingEnable()
...
1:wifi_getRadioIGMPSnoopingEnable log from DUT should be available in Agent Console LogCheckPoint 
...
3:Test Manager GUI will publish the result as SUCCESS in Execution page"""</except_output>
    ...
    <test_script>TS_WIFIHAL_2.4GHzIsRadioIGMPSnoopingEnabled</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzIsRadioIGMPSnoopingEnabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    radioIndex = 0
    getMethod = "getRadioIGMPSnoopingEnable"
    ...
    if expectedresult in actualresult :
        ...
        if "Enabled" in enable:
            print "IGMP Snooping is Enabled in Radio 2.4Ghz"
            oldEnable = 1
            newEnable = 0
        else:
            print "IGMP Snooping is Disabled in Radio 2.4Ghz"
            oldEnable = 0
            newEnable = 1

        setMethod = "setRadioIGMPSnoopingEnable"
        ...
        if expectedresult in actualresult :
            ...
            if expectedresult in actualresult and enable not in details.split(":")[1].strip():
                print "getRadioIGMPSnoopingEnable Success, verified along with setRadioIGMPSnoopingEnable() api"
                ...
                print "getRadioIGMPSnoopingEnable() failed after set function"
		tdkTestObj.setResultStatus("FAILURE");
        else:
	    print "setRadioIGMPSnoopingEnable() failed"
            tdkTestObj.setResultStatus("FAILURE");
    else:
	print "getRadioIGMPSnoopingEnable() failed"
        tdkTestObj.setResultStatus("FAILURE");
