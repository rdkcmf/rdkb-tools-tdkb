...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzSetRadioChannelMode_gOnly</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetRadioStandard</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the applicable radio modes which supports gOnly for 2.4GHZ using wifi_setRadioChannelMode() and validate using wifi_getRadioStandard</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_400</test_case_id>
    <test_objective>Set the applicable radio modes which supports gOnly for 2.4GHZ using wifi_setRadioChannelMode() and validate using wifi_getRadioStandard</test_objective>
...
2.Get the list of 2.4G Radio modes which supports gOnly puremode from tdk_platfrom properties file
...
5.Unload the wifihal and sysutil modules.</automation_approch>
    ...
    <test_script>TS_WIFIHAL_2.4GHzSetRadioChannelMode_gOnly</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetRadioChannelMode_gOnly');
obj1.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetRadioChannelMode_gOnly');
...
if "SUCCESS" in (loadmodulestatus.upper() and  sysutilmodulestatus.upper()):
	...
	radioIndex = idx
        ...
        cmd = "sh %s/tdk_utility.sh parseConfigFile CHANNEL_MODES_2_G_ONLY" %TDK_PATH;
        print cmd;
        expectedresult="SUCCESS";
        ...
        if details != "" and ( expectedresult in  actualresult):
            ...
            print "TEST STEP 1: Get the value of CHANNEL_MODES_2_G_ONLY from tdk_platform properties file";
            print "EXPECTED RESULT 1: Should Get the value of CHANNEL_MODES_2_G_ONLY from platform properties file";
            print "ACTUAL RESULT 1: CHANNEL_MODES_2_G_ONLY values from tdk_platform properties file is : ", modeList;
            ...
            tdkTestObj1.setResultStatus("SUCCESS");
            print "TEST STEP 2: Get the value of CHANNEL_MODES_2_G_ONLY from tdk_platform properties file";
            print "EXPECTED RESULT 2: Should Get the value of CHANNEL_MODES_2_G_ONLY from platform properties file";
            print "ACTUAL RESULT 2:Failed to get CHANNEL_MODES_2_G_ONLY values from tdk_platform properties file";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : FAILURE"
