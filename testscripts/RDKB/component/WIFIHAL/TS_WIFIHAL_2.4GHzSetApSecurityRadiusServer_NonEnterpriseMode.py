...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzSetApSecurityRadiusServer_NonEnterpriseMode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Set Security Radius Server details with non-enterprise modes and check whether the return status is failure</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TS_WIFIHAL_529</test_case_id>
    <test_objective>Set Security Radius Server details with non-enterprise modes and check whether the return status is failure</test_objective>
...
5. For each Non Enterprise mode, try to set the Security Radius Server details by invoking wifi_setApSecurityRadiusServer(). Check if the Set operation returns Failure as expected.
6. Revert back to initial Security mode
7. Unload Wifihal module.</automation_approch>
    <expected_output>Security Radius Server details set with non-enterprise modes should return failure</expected_output>
    <priority>High</priority>
    <test_stub_interface>Wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzSetApSecurityRadiusServer_NonEnterpriseMode</test_script>
    <skipped>No</skipped>
...
def setRadiusServer(idx, setMode) :
    ...
    print "TEST STEP : Should not set the ApSecurityRadiusServer details in the Non Enterprise mode %s"%setMode
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetApSecurityRadiusServer_NonEnterpriseMode');
