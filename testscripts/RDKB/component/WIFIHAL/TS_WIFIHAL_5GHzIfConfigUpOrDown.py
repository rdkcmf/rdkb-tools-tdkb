...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the Ap status for 5GHz and invoke wifi_ifConfigDown() if the status is 'Up' or invoke wifi_ifConfigUp() if the status is 'Disable' and validate the same using wifi_getApStatus() api.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_262</test_case_id>
    <test_objective>To get the Ap status for 5GHz and invoke wifi_ifConfigDown() if the status is 'Up' or invoke wifi_ifConfigUp() if the status is 'Disable' and validate the same using wifi_getApStatus() api.</test_objective>
...
5.Unload the module.</automation_approch>
    <except_output>The Ap status should change to 'Up', if we invoke wifi_ifConfigUp() api and Ap status should change to 'Disable', if we invoke wifi_ifConfigDown() api for 5GHz</except_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzIfConfigUporDown</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzIfConfigUporDown');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
	    ...
	    tdkTestObj, actualresult, details =  GetApStatus(idx);
