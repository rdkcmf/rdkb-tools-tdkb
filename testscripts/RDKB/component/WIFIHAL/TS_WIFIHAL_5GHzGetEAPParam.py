...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzGetEAPParam</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetEAPParam</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getEAP_Param() for 5G private AP index and retrieve the values for EAPOL Key Timeout and Retries, EAP Identity Request Timeout and Retries, EAP Request Timeout and Retries.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_794</test_case_id>
    <test_objective>Invoke the HAL API wifi_getEAP_Param() for 5G private AP index and retrieve the values for EAPOL Key Timeout and Retries, EAP Identity Request Timeout and Retries, EAP Request Timeout and Retries.</test_objective>
...
2. Invoke the HAL API wifi_getEAP_Param() for 5G private AP.
...
4. Unload the module</automation_approch>
    <expected_output>The the HAL API wifi_getEAP_Param() for 5G private AP index should be invoked successfully and the values for EAPOL Key Timeout and Retries, EAP Identity Request Timeout and Retries, EAP Request Timeout and Retries should be retrieved as valid integer values.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetEAPParam</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetEAPParam');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getEAP_Param() to retrieve the 5G EAP Param values";
