...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzSetFTOverDSActivated</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_setFTOverDSActivated() to toggle the enable state of FTOverDSActivation and cross check the enable SET with enable GET using wifi_getFTOverDSActivated() for 2.4G private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_747</test_case_id>
    <test_objective>Invoke the HAL API wifi_setFTOverDSActivated() to toggle the enable state of FTOverDSActivation and cross check the enable SET with enable GET using wifi_getFTOverDSActivated() for 2.4G private AP.</test_objective>
...
param : newEnable/oldEnable(dynamically assigned)
</input_parameters>
...
6. Unload the modules.</automation_approch>
    <expected_output>FTOverDSactivated enable state must be toggled successfully using the HAL API wifi_setFTOverDSActivated() for 2.4G private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzSetFTOverDSActivated</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetFTOverDSActivated');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getFTOverDSActivated() for 2.4G Private AP";
        ...
        if expectedresult in actualresult:
            ...
            print "\nTEST STEP 2: Toggle the enabled state using the HAL API wifi_setFTOverDSActivated() for 2.4G Private AP";
