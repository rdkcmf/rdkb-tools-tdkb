...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_2.4GHzGetApInterworkingServiceCapability</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>To invoke the HAL API wifi_getApInterworkingServiceCapability  and check if the Interworking Service Capability is enabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_547</test_case_id>
    <test_objective>To invoke the HAL API wifi_getApInterworkingServiceCapability  and check if the Interworking Service Capability is enabled.</test_objective>
...
2. Invoke the HAL API wifi_getApInterworkingServiceCapability(). If it is enabled return Success else return Failure.
...
5. Unload the module</automation_approch>
    ...
    <test_script>TS_WIFIHAL_2.4GHzGetApInterworkingServiceCapability</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetApInterworkingServiceCapability');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "TEST STEP 1: Invoke the wifi_getApInterworkingServiceCapability api";
