...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_WIFIHAL_2.4GHzPushApFastTransitionConfig</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_PushApFastTransitionConfig</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_pushApFastTransitionConfig() for 2.4G private AP and set the BSS Transition Support, Mobility Domain ID and FT Over DS Activated enable state to new values and cross check if the values set using the Push API is reflected in wifi_getBSSTransitionActivated(), wifi_getApMobilityDomainID() and wifi_getFTOverDSActivated() HAL APIs.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_785</test_case_id>
    <test_objective>Invoke the HAL API wifi_pushApFastTransitionConfig() for 2.4G private AP and set the BSS Transition Support, Mobility Domain ID and FT Over DS Activated enable state to new values and cross check if the values set using the Push API is reflected in wifi_getBSSTransitionActivated(), wifi_getFTMobilityDomainID() and wifi_getFTOverDSActivated() HAL APIs.</test_objective>
...
8. Unload the module.</automation_approch>
    <expected_output>The HAL API wifi_pushApFastTransitionConfig() for 2.4G private AP to set the BSS Transition Support, Mobility Domain ID and FT Over DS Activated enable state to new values should be invoked successfully and the values set using the Push API should be reflected in wifi_getBSSTransitionActivated(), wifi_getFTMobilityDomainID() and wifi_getFTOverDSActivated() HAL APIs.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzPushApFastTransitionConfig</test_script>
    <skipped>No</skipped>
...
def getApFTValues(obj, primitive, idx, api, step):
    ...
    print "\nTEST STEP %d : Invoke the HAL API wifi_%s and get the FT parameter value for 2.4G private AP" %(step, api);
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPushApFastTransitionConfig');
            ...
            if expectedresult in actualresult and details != "":
                ...
                if "0x" in hex_value0 and "0x" in hex_value1:
                    ...
                    if expectedresult in actualresult and details != "":
                        ...
                        print "\nTEST STEP %d: Invoke the HAL API wifi_PushApFastTransitionConfig() for 2.4G private AP" %step;
                        ...
                        if expectedresult in actualresult and details != "":
                            ...
                            print "\nTEST STEP %d: Invoke the HAL API wifi_PushApFastTransitionConfig() for 2.4G private AP and revert to initial state" %step;
                            print "EXPECTED RESULT %d: The HAL API should be invoked successfully" %step;
