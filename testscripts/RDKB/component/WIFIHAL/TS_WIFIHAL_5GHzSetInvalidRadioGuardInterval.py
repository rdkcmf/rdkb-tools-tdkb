...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzSetInvalidRadioGuardInterval</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamStringValue</primitive_test_name>
  <primitive_test_version>8</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set radio guard interval to a value that is not in possible guard interval list and check whether it is set for 5GHz</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_222</test_case_id>
    <test_objective>To set radio guard interval to a value that is not in possible guard interval list and check whether it is set for 5GHz</test_objective>
...
5. Verify whether the set operation is success
6. Revert back to the initial GuardInterval is set has done
7. Unload wifihal module</automation_approch>
    <except_output>Set operation should not be happen</except_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzSetInvalidRadioGuardInterval</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetInvalidRadioGuardInterval');
