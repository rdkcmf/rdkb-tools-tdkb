...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_5GHzSetFTR0KeyHolderID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTR0KeyHolderID</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the FTR0 Key Holder ID by invoking the HAL API wifi_setFTR0KeyHolderID() and validate the SET using the GET API wifi_getFTR0KeyHolderID() for 5G private AP</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_754</test_case_id>
    <test_objective>Set the FTR0 Key Holder ID by invoking the HAL API wifi_setFTR0KeyHolderID() and validate the SET using the GET API wifi_getFTR0KeyHolderID() for 5G private AP</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_setFTR0KeyHolderID()
wifi_getFTR0KeyHolderID()</api_or_interface_used>
    <input_parameters>methodName : getFTR0KeyHolderID
methodName : setFTR0KeyHolderID
apIndex : 5G private AP index
radioIndex : 5G radio index
KeyHolderID : string value assigned dynamically</input_parameters>
    <automation_approch>1. Load the modules
2. Get the initial FTR0 Key Holder ID using the HAL API wifi_getFTR0KeyHolderID() for 5G private AP.
...
5. Invoke the HAL API wifi_setFTR0KeyHolderID() and pass the newly generated Key Holder ID for 5G private AP and check if the SET operation returns success.
6. Invoke the HAL API wifi_getFTR0KeyHolderID() and retrieve the current Key Holder ID after the set operation.
...
9. Unload the modules.</automation_approch>
    <expected_output>Should be able to set the FTR0 Key Holder ID by invoking the HAL API wifi_setFTR0KeyHolderID() and the SET value should be validated successfully using the GET API wifi_getFTR0KeyHolderID() for 5G private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzSetFTR0KeyHolderID</test_script>
    <skipped>No</skipped>
...
def setKeyHolder(tdkTestObj, apIndex, radioIndex, setValue):
    expectedresult = "SUCCESS";
    tdkTestObj.addParameter("methodName","setFTR0KeyHolderID");
    tdkTestObj.addParameter("apIndex",apIndex);
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetFTR0KeyHolderID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetFTR0KeyHolderID');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getFTR0KeyHolderID() to retrieve the FTR0 Key Holder ID for 5G private AP";
        ...
        if expectedresult in actualresult :
            ...
            if num_hex_values >= 1:
                ...
                print "\nTEST STEP 3: Invoke the HAL API wifi_setFTR0KeyHolderID() to set the FTR0 Key Holder ID for 5G private AP";
                print "EXPECTED RESULT 3: Should set the FTR0 Key Holder ID to %s using the HAL API successfully" %setValue;
                ...
                if expectedresult in actualresult :
                    ...
                    print "\nTEST STEP 4: Invoke the HAL API wifi_getFTR0KeyHolderID() to retrieve the FTR0 Key Holder ID Set operation";
                    print "EXPECTED RESULT 4: Should get the FTR0 Key Holder ID using the HAL API successfully";
                    ...
                    if expectedresult in actualresult :
                        ...
                        #Check if proper Hex values are retrieved and convert them to corresponding string value
                        print "\nTEST STEP 5 : Check if the FTR0 Key Holder ID is retrieved as one or more Hex Values after set operation";
                        print "EXPECTED RESULT 5 : FTR0 Key Holder ID should be retrieved as one or more Hex Values after set operation";
                        ...
                        if num_hex_values >= 1:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT 5: FTR0 Key Holder ID is retrieved as the Hex values : ", hex_values_final;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                            ...
                            print "\nTEST STEP 6 : Check if the SET value and GET value of FTR0 Key Holder ID match";
                            print "EXPECTED RESULT 6 :Tthe SET value and GET value of FTR0 Key Holder ID should match";
                            ...
                            if setValue == final_KeyHolder:
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "ACTUAL RESULT 6: The SET value and GET value of FTR0 Key Holder ID match";
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : SUCCESS";
                                ...
                                print "\nTEST STEP 7: Invoke the HAL API wifi_setFTR0KeyHolderID() to revert to initial value";
                            ...
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("FAILURE");
                            print "ACTUAL RESULT 5: FTR0 Key Holder ID is not retrieved as two Hex values";
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : FAILURE";
                    else :
                        #Set the result status of execution
