...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_6GHzSetFTR1KeyHolderID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTR1KeyHolderID</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the FTR1 Key Holder ID by invoking the HAL API wifi_setFTR1KeyHolderID() and validate the SET using the GET API wifi_getFTR1KeyHolderID() for 6G private AP</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_775</test_case_id>
    <test_objective>Set the FTR1 Key Holder ID by invoking the HAL API wifi_setFTR1KeyHolderID() and validate the SET using the GET API wifi_getFTR1KeyHolderID() for 6G private AP</test_objective>
...
2. Get the initial FTR1 Key Holder ID using the HAL API wifi_getFTR1KeyHolderID() for 6G private AP.
...
5. Invoke the HAL API wifi_setFTR1KeyHolderID() and pass the newly generated Key Holder ID for 6G private API and check if the SET operation returns success.
...
9. Unload the modules.</automation_approch>
    <expected_output>Should be able to set the FTR1 Key Holder ID by invoking the HAL API wifi_setFTR1KeyHolderID() and the SET value should be validated successfully using the GET API wifi_getFTR1KeyHolderID() for 6G private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzSetFTR1KeyHolderID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetFTR1KeyHolderID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetFTR1KeyHolderID');
        ...
        if apIndex == -1:
            ...
            if expectedresult in actualresult :
                ...
                if num_hex_values >= 1:
                    ...
                    print "\nTEST STEP 4: Invoke the HAL API wifi_setFTR1KeyHolderID() to set the FTR1 Key Holder ID for 5G private AP";
                    print "EXPECTED RESULT 4: Should set the FTR1 Key Holder ID to %s using the HAL API successfully" %setValue;
                    ...
                    if expectedresult in actualresult :
                        ...
                        tdkTestObj, actualresult, details = getKeyHolder(tdkTestObj, apIndex);
                        print "\nTEST STEP 5: Invoke the HAL API wifi_getFTR1KeyHolderID() to retrieve the FTR1 Key Holder ID Set operation";
                        print "EXPECTED RESULT 5: Should get the FTR1 Key Holder ID using the HAL API successfully";
                        ...
                        if expectedresult in actualresult :
                            ...
                            #Check if proper Hex values are retrieved and convert them to corresponding string value
                            print "\nTEST STEP 6 : Check if the FTR1 Key Holder ID is retrieved as one or more Hex Values after set operation";
                            print "EXPECTED RESULT 6 : FTR1 Key Holder ID should be retrieved as one or more Hex Values after set operation";
                            details = details.split("Key Holder ID");
                            ...
                            if num_hex_values >= 1:
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "ACTUAL RESULT 6: FTR1 Key Holder ID is retrieved as the Hex values : ", hex_values_final;
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : SUCCESS";

                                #Covert to corresponding unicode string
                                final_KeyHolder = HextoUnicodeConversion(num_hex_values, hex_values_final);
                                print "\nTEST STEP 7 : Check if the SET value and GET value of FTR1 Key Holder ID match";
                                print "EXPECTED RESULT 7 :Tthe SET value and GET value of FTR1 Key Holder ID should match";
                                ...
                                if setValue == final_KeyHolder:
                                    #Set the result status of execution
                                    tdkTestObj.setResultStatus("SUCCESS");
                                    print "ACTUAL RESULT 7: The SET value and GET value of FTR1 Key Holder ID match";
                                    #Get the result of execution
                                    print "[TEST EXECUTION RESULT] : SUCCESS";
                                    ...
                                    print "\nTEST STEP 8: Invoke the HAL API wifi_setFTR1KeyHolderID() to revert to initial value";
                                ...
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("FAILURE");
                                print "ACTUAL RESULT 6: FTR1 Key Holder ID is not retrieved as one or more Hex values";
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : FAILURE";
                        else :
                            #Set the result status of execution
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 3: FTR1 Key Holder ID is not retrieved as two Hex values";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                #Set the result status of execution
