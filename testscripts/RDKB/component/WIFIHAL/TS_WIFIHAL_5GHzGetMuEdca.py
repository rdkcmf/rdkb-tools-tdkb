...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_5GHzGetMuEdca</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetMuEdca</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getMuEdca() for 5G radio and check if the Arbitration Inter-Frame Space Number, Lower bound Contention Window, Upper bound Contention Window and Timer values retrieved are valid integers for each of the access categories - Background, Best effort, Video and Voice.</synopsis>
  <groups_id/>
...
2. Invoke the HAL API wifi_getMuEdca() for 5G radio for each of the access categories : Background, Best effort, Video and Voice.
...
4. Unload module</automation_approch>
    <expected_output>The HAL API wifi_getMuEdca() should be invoked successfully for 5G radio and the Arbitration Inter-Frame Space Number, Lower bound Contention Window, Upper bound Contention Window and Timer values retrieved should be valid integers for each of the access categories - Background, Best effort, Video and Voice.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetMuEdca</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetMuEdca');
