...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzGetApAssociatedDeviceDiagnosticResult</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetApAssociatedDeviceDiagnosticResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the Ap Associated Device Diagnostic Result for 5GHz.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_296</test_case_id>
    <test_objective>To get the Ap Associated Device Diagnostic Result for 5GHz</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, Emulator, RPI</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh scripti
3. Connect a 5G client to the DUT</pre_requisite>
...
3.Check if the STA details retrieved for each of the STAs are valid
4.Unload the module.</automation_approch>
    ...
    <test_script>TS_WIFIHAL_5GHzGetApAssociatedDeviceDiagnosticResult</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetApAssociatedDeviceDiagnosticResult');
