...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_2.4GHzPushRadioChannel2</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_PushRadioChannel2</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This HAL API is used to change the channel to destination channel, with destination bandwidth for 2.4GHz.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_303</test_case_id>
    <test_objective>This HAL API is used to change the channel to destination channel, with destination bandwidth for 2.4GHz.</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>channel = 1
channel_width_MHz = 40
csa_beacon_count=25
radioIndex = 0</input_parameters>
    <automation_approch>1.Load the module
2.Get the value of radio channel of 2.4GHZ and save
3. Change the channel using wifi_pushRadioChannel2 () and validate
4. Revert the channel value
5.Unload the module.</automation_approch>
    <except_output>The Channel should change to set value for 2.4GHZ</except_output>
    <priority>High</priority>
    <test_stub_interface>wfihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzPushRadioChannel2</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPushRadioChannel2');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        print "TEST STEP 1: Get the current radio channel for 2.4GHz";
        print "EXPECTED RESULT 1: Should get the current channel for 2.4GHz";
	...
	tdkTestObj = obj.createTestStep("WIFIHAL_GetOrSetParamStringValue");
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: Get the current bandwidth for 2.4GHz";
            print "EXPECTED RESULT 2: Should get the bandwidth for 2.4GHz";
            print "ACTUAL RESULT 2: %s" %details;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : SUCCESS";
	    currBandwidth = details.split(":")[1];
	    currBandwidth = currBandwidth.split("MHz")[0].strip();
            if int(currChannel) == 1:
                newChannel = 6;
            else:
                newChannel = 1;
            ...
            tdkTestObj.addParameter("channel_width_MHz",40);
	        ...
	        time.sleep(10);
                ...
                if expectedresult in actualresult:
                    ...
                    if int(newValue) == int(newChannel):
                        ...
                        print "EXPECTED RESULT 3: Should get the current channel for 2.4GHz";
        ...
        #Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE");
        print "TEST STEP 1: Get the SSID Enable status for 2.4GHz";
        print "EXPECTED RESULT 1: Should get SSID enable status for 2.4GHz";
        print "ACTUAL RESULT 1: %s" %details;
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : FAILURE";
