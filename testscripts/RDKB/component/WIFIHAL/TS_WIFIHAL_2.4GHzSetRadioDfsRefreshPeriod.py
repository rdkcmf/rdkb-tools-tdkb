...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_2.4GHzSetRadioDfsRefreshPeriod</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamULongValue</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the dfs refresh period if the automatic channel selection is enabled and see if the channel is changed after refresh period for 2.4GHz</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_252</test_case_id>
    <test_objective>To set the dfs refresh period if the automatic channel selection is enabled and see if the channel is changed after refresh period for 2.4GHz</test_objective>
...
4. RadioChannel after the refresh period should be different from previous radio channel</except_output>
    ...
    <test_script>TS_WIFIHAL_2.4GHzSetRadioDfsRefreshPeriod</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzRadioDfsRefreshPeriod');

def setandgetDfsChannelRefreshPeriod(idx):
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
		...
		if expectedresult in actualresult and "Enabled" in enable:
		    ...
		    setandgetDfsChannelRefreshPeriod(radioIndex);
