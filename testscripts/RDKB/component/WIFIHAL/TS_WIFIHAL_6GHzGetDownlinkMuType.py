...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_6GHzGetDownlinkMuType</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamIntValue</primitive_test_name>
  <primitive_test_version>5</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke wifi_getDownlinkMuType() and check if it returns a value from the expected list for 6GHz radio [WIFI_DL_MU_TYPE_NONE, WIFI_DL_MU_TYPE_HE, WIFI_DL_MU_TYPE_MIMO, WIFI_DL_MU_TYPE_HE_MIMO].</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_640</test_case_id>
    <test_objective>Invoke wifi_getDownlinkMuType() and check if it returns a value from the expected list for 6GHz radio [WIFI_DL_MU_TYPE_NONE, WIFI_DL_MU_TYPE_HE, WIFI_DL_MU_TYPE_MIMO, WIFI_DL_MU_TYPE_HE_MIMO].</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getDownlinkMuType()</api_or_interface_used>
    <input_parameters>methodname : getDownlinkMuType
radioIndex : index corresponding to 6G radio</input_parameters>
    <automation_approch>1. Load wifihal module
2. Invoke wifi_getDownlinkMuType() for 6GHz and check if its return was success
...
4. Unload wifihal module</automation_approch>
    <expected_output>The HAL API wifi_getDownlinkMuType() should successfully retrieve the Downlink MuType value within the expected values range for 6GHz radio.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzGetDownlinkMuType</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetDownlinkMuType');
