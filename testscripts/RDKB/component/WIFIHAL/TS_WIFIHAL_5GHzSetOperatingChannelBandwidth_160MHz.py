...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzSetOperatingChannelBandwidth_160MHz</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamStringValue</primitive_test_name>
  <primitive_test_version>8</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the operating channel bandwidth to 160MHz</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TS_WIFIHAL_528</test_case_id>
    <test_objective>To set the operating channel bandwidth to 160MHz for 5GHz and verify the set value by getting it</test_objective>
...
3.Invoke "WIFIHAL_GetOrSetParamStringValue" to set the current operating channel bandwidth to 160MHz
...
8.Unload wifihal module</automation_approch>
    <expected_output>Should successfully set the operating channel bandwidth to 160MHz and successfully get it for 5GHz radio.</expected_output>
    <priority>High</priority>
    <test_stub_interface>Wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzSetOperatingChannelBandwidth_160MHz</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetOperatingChannelBandwidth_160MHz');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult :
            ...
            if expectedresult in actualresult :
                ...
                #Check if operating channel bandwidth is set properly
                tdkTestObj = obj.createTestStep("WIFIHAL_GetOrSetParamStringValue");
                #Giving the method name to invoke the api for getting current operating channel bandwidth, wifi_getRadioOperatingChannelBandwidth()
                tdkTestObj.addParameter("methodName","getChannelBandwidth");
                tdkTestObj.addParameter("radioIndex",idx);
                ...
                details = tdkTestObj.getResultDetails();
                if expectedresult in actualresult :
                    print "TEST STEP 3: Get radio operating channel bandwidth after the set and compare"
                    print "EXPECTED RESULT: Should successfully get the radio operating channel bandwidth"
                    print "ACTUAL RESULT: getChannelBandwidth : %s"%details;
                    print "TEST EXECUTION RESULT :SUCCESS"
                    tdkTestObjTemp.setResultStatus("SUCCESS");
                    ...
                    if CheckBW == setBW :
                        ...
                        print "TEST STEP 4: Get radio operating channel bandwidth after the set operation and compare"
                        print "EXPECTED RESULT: The operating channel bandwidth after the set and get should be same"
                        print "ACTUAL RESULT: getChannelBandwidth : %s"%details;
                        print "TEST EXECUTION RESULT :FAILURE"
                        tdkTestObjTemp.setResultStatus("FAILURE");
