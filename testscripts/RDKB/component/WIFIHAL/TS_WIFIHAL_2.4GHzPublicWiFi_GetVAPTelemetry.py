...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzPublicWiFi_GetVAPTelemetry</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetVAPTelemetry</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getVAPTelemetry() to retrieve the Tx Overflow value for 2.4GHz Public WiFi and cross check the value with the TR181 parameter Device.WiFi.AccessPoint.{i}.X_COMCAST-COM_TXOverflow.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_588</test_case_id>
    <test_objective>Invoke the HAL API wifi_getVAPTelemetry() to retrieve the Tx Overflow value for 2.4GHz Public WiFi and cross check the value with the TR181 parameter Device.WiFi.AccessPoint.{i}.X_COMCAST-COM_TXOverflow.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getVAPTelemetry
...
2. Retrieve the AP index of 2.4GHz Public WiFi from the platform property file.
...
4. Query the TR181 parameter Device.WiFi.AccessPoint.{i}.X_COMCAST-COM_TXOverflow and get the Tx Overflow value.
...
6. Unload the modules</automation_approch>
    <expected_output>The Tx Overflow value retrieved via the HAL API wifi_getVAPTelemetry and the TR181 parameter Device.WiFi.AccessPoint.9.X_COMCAST-COM_TXOverflow should be equal for 2.4GHz Public WiFi.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzPublicWiFi_GetVAPTelemetry</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_GetVAPTelemetry');
obj1.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_GetVAPTelemetry');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_GetVAPTelemetry');
