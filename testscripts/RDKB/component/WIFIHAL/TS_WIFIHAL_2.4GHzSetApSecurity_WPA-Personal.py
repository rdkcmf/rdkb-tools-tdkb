...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_761</test_case_id>
    <test_objective>Invoke the HAL API wifi_setApSecurity() to set the security parameters when the security mode is set to "WPA-Personal" and check if the SET operation of the mfp, encryption mode, key type and key passphrase is success and is getting reflected when the GET  API wifi_getApSecurity() is invoked for 2.4GHz private AP.</test_objective>
...
6. Unload the modules</automation_approch>
    <expected_output>The HAL API wifi_setApSecurity() should successfully set the security parameters such as mfp, encryption type, key type and key passphrase to valid values when the security mode is set to WPA-Personal and the SET values should be reflected by the GET API wifi_getApSecurity() for 2.4G Private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzSetApSecurity_WPA-Personal</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetApSecurity_WPA-Personal');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult and "AP Security details" in details:
            ...
            if status == "SUCCESS":
                ...
                print "\nTEST STEP 3 : Invoke the HAL API wifi_setApSecurity() for 2.4G private AP with Security Mode : %s, MFP : %s, Encryption Mode : %s, Key Type : %s and Key : %s" %(set_values[0], set_values[1], set_values[2], set_values[3], set_values[4]);
