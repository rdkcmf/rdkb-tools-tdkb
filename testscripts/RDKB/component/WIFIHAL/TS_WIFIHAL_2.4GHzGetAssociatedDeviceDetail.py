...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzGetAssociatedDeviceDetail</name>
  ...
  <test_cases>
    <test_case_id>TS_WIFIHAL_289</test_case_id>
    <test_objective>To give the details of single associated device of specific device Index for 2.4GHz wifi.</test_objective>
...
2. Get the number of devices connected to 2.4Ghz
3.Get the details of associated devices of  each device Index using wifi_getAssociatedDeviceDetail api
4.Crosscheck the mac address from TR181 para with mac address from api result
5.Unload the module.</automation_approch>
    <expected_output>Should get the details of single associated device of specific device Index for 2.4GHz</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzGetAssociatedDeviceDetail</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetAssociatedDeviceDetail');
wifiobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetAssociatedDeviceDetail');
...
if "SUCCESS" in loadmodulestatus.upper() and sysloadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        #check the number of associated devices to 2.4G
	    ...
	    if  devCount :
			...
			#Get the result of execution
               		print "[TEST EXECUTION RESULT] : SUCCESS";
			#check the MAC address of associated devices to 2.4G
        		tdkTestObj = wifiobj.createTestStep("WIFIAgent_Get");
        		tdkTestObj.addParameter("paramName","Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress" %devIndex);
        		expectedresult="SUCCESS";
       			tdkTestObj.executeTestCase(expectedresult);
       			actualresult = tdkTestObj.getResult();
       			details = tdkTestObj.getResultDetails();
	  		    ...
	  		    print "Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress :" %devIndex,macAddress;
			    print "Mac address from api : ",mac;
			    if mac == macAddress.lower() :
			        tdkTestObj.setResultStatus("SUCCESS");
				print "TEST STEP 3 :  check the MAC from wifi_getAssociatedDeviceDetail api result with Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress"%devIndex
				print "EXPECTED RESULT 3 : Should successfully matches the MAC from wifi_getAssociatedDeviceDetail api result with Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress " %devIndex
			        print "ACTUAL RESULT 3 : Successfully matches MAC from wifi_getAssociatedDeviceDetail api result with Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress" %devIndex;
		                #Get the result of execution
                		print "[TEST EXECUTION RESULT] : SUCCESS";
			    else :
				...
				print "TEST STEP 3 :  check the MAC from wifi_getAssociatedDeviceDetail api result with Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress" %devIndex
			        print "EXPECTED RESULT 3 : Should successfully matches the MAC from wifi_getAssociatedDeviceDetail api result with Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress " %devIndex
			        print "ACTUAL RESULT 3 : Failed to match MAC from wifi_getAssociatedDeviceDetail api result with Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress" %devIndex;
				#Get the result of execution
                		print "[TEST EXECUTION RESULT] : FAILURE";
		    	else:
		            tdkTestObj.setResultStatus("FAILURE");
             		    print "Not able to  retrieve Device.WiFi.AccessPoint.1.AssociatedDevice.%d.MACAddress" %devIndex
                            #Get the result of execution
                	    print "[TEST EXECUTION RESULT] : FAILURE";
		    else:
                        tdkTestObj.setResultStatus("FAILURE");
