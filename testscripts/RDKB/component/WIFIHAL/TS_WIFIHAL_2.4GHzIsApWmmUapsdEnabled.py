...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzIsApWmmUapsdEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check the Accesspoint U-APSD(Unscheduled Automatic Power Save Delivery) enable status using wifi_getApWmmUapsdEnable HAL API</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_56</test_case_id>
    <test_objective>Check the Accesspoint U-APSD(Unscheduled Automatic Power Save Delivery) enable status using wifi_getApWmmUapsdEnable HAL API</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XB3. XB6</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getApWmmUapsdEnable()</api_or_interface_used>
    <input_parameters>methodName   :   getApWmmUapsdEnable
...
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_2.4GHzIsApWmmUapsdEnabled.py)
3.Execute the generated Script(TS_WIFIHAL_2.4GHzIsApWmmUapsdEnabled.py) using execution page of  Test Manager GUI
...
wifi_getApWmmUapsdEnable() and wifi_setApWmmUapsdEnable()
...
8.Test Manager will publish the result in GUI as SUCCESS/FAILURE based on the response from wifihalstub</automation_approch>
    <except_output>" 
CheckPoint 
1:wifi_getApWmmUapsdEnable log from DUT should be available in Agent Console LogCheckPoint 
...
3:Test Manager GUI will publish the result as SUCCESS in Execution page"""</except_output>
    ...
    <test_script>TS_WIFIHAL_2.4GHzIsApWmmUapsdEnabled</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzIsApWmmUapsdEnabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    apIndex = 0
    getMethod = "getApUAPSDCapability"
    ...
    if expectedresult in actualresult:
	    ...
	    if expectedresult in actualresult :
        	...
        	if "Enabled" in enable:
	            print "Automatic Power Save Delivery for AP is Enabled"
        	    oldEnable = 1
	            newEnable = 0
	        else:
        	    print "Automatic Power Save Delivery for AP is Disabled"
		    oldEnable = 0
        	    newEnable = 1

	        setMethod = "setApWmmUapsdEnable"
        	...
        	if expectedresult in actualresult :
	            ...
	            if expectedresult in actualresult and enable not in details.split(":")[1].strip():
        	        print "getApWmmUapsdEnable Success, verified along with setApWmmUapsdEnable() api"
        	        ...
        	        print "getApWmmUapsdEnable() failed after set function"
			tdkTestObj.setResultStatus("FAILURE");
	        else:
		    print "setApWmmUapsdEnable() failed"
	            tdkTestObj.setResultStatus("FAILURE");
	    else:
		print "getApWmmUapsdEnable() failed"
	        tdkTestObj.setResultStatus("FAILURE");
        elif ApUAPSDCapability == "Disabled":
            print "wifi_getApUAPSDCapability function called successfully and %s"%details
            tdkTestObj.setResultStatus("SUCCESS");
            print "ApUAPSDCapability received: %s"%ApUAPSDCapability;
            print "ApUAPSDCapability is disabled"
    else:
        print "wifi_getApUAPSDCapability function call failed";
        tdkTestObj.setResultStatus("FAILURE");
