...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzPushApEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To push Ap Enable using wifi_pushApEnable() HAL API and validate the same using wifi_getApEnable() HAL API for 2.4GHz</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_319</test_case_id>
    <test_objective>To push Ap Enable using wifi_pushApEnable() HAL API and validate the same using wifi_getApEnable() HAL API for 2.4GHz</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getApEnable()
wifi_pushApEnable()</api_or_interface_used>
    <input_parameters>methodName : getApEnable
methodName : pushApEnable
radioIndex : 0</input_parameters>
    <automation_approch>1. Load the wifihal module.
2. Using getApEnable() get and save current Ap enable state
3.Toggle Ap enable state using pushApEnable()
4. Verify whether the set was success by getting the AP status using getApEnable()
5. Revert back to the initial Ap enable state using pushApEnable()
6. Unload wifihal module</automation_approch>
    <except_output>To successfully push the Ap Enable value using wifi_pushApEnable() HAL API for 2.4GHz.</except_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzPushApEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPushApEnable');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
		...
		print "**************************************************";
		print "TEST STEP 1: Get the initial Ap Enable status for 2.4GHz";
		...
		if expectedresult in actualresult :
		    ...
		    tdkTestObj.setResultStatus("SUCCESS");
		    print "**************************************************";
		    print "TEST STEP 2: Push Ap Enable using wifi_pushApEnable for 2.4GHz";
			...
			tdkTestObj.setResultStatus("SUCCESS");
			print "**************************************************";
			print "TEST STEP 3: Get the Ap Enable status after push operation for 2.4GHz";
			...
			if enable not in details.split(":")[1].strip():
			    #Set the result status of execution
			     tdkTestObj.setResultStatus("SUCCESS");
			     print "**************************************************";
			     print "TEST STEP 4: Check Ap Enable status is equal to the push value for 2.4GHz";
