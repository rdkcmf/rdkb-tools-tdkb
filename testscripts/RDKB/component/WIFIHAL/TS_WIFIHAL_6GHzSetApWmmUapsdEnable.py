...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_6GHzSetApWmmUapsdEnable</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getApUAPSDCapability() to retrieve the ApUAPSDCapability. If the ApUAPSDCapability is enabled toggle ApWmmUapsdEnable using the API wifi_setApWmmUapsdEnable() and confirm if the value is set properly using wifi_getApWmmUapsdEnable() for 6GHz radio.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_646</test_case_id>
    <test_objective>Invoke the HAL API wifi_getApUAPSDCapability() to retrieve the ApUAPSDCapability. If the ApUAPSDCapability is enabled toggle ApWmmUapsdEnable using the API wifi_setApWmmUapsdEnable() and confirm if the value is set properly using wifi_getApWmmUapsdEnable() for 6GHz radio.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getApUAPSDCapability()
wifi_getApWmmUapsdEnable()
wifi_setApWmmUapsdEnable()</api_or_interface_used>
...
8. Unload module</automation_approch>
    <expected_output>If ApUAPSDCapability is Enabled then the HAL API  wifi_setApWmmUapsdEnable() should successfully toggle the value retrieved via  wifi_getApWmmUapsdEnable() for 6GHz.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzSetApWmmUapsdEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetApWmmUapsdEnable');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetApWmmUapsdEnable');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if apIndex == -1:
        ...
        print "\nTEST STEP 2: Check if ApUAPSDCapability is enabled by invoking the wifi_getApUAPSDCapability api";
        ...
        if expectedresult in actualresult and details != "":
            ...
            if ApUAPSDCapability == "Enabled":
                ...
                print "\nTEST STEP 3: Invoke the wifi_getApWmmUapsdEnable api";
                ...
                if expectedresult in actualresult and details != "":
                    ...
                    print "\nTEST STEP 4: Toggle the enabled state using wifi_setApWmmUapsdEnable api";
                    ...
                    if expectedresult in actualresult and details != "":
                        ...
                        print "\nTEST STEP 5: Invoke  wifi_getApWmmUapsdEnable to verify toggling done by wifi_setApWmmUapsdEnable api";
                        ...
                        if expectedresult in actualresult and details != "":
                            ...
                            if enable == newStatus :
                                print "\nTEST STEP 6 : Verify if ApWmmUapsdEnable set value and get value are same"
                                ...
                                print "\nTEST STEP 7: Revert the enabled state to %s using wifi_setApWmmUapsdEnable api" %initial_enable;
