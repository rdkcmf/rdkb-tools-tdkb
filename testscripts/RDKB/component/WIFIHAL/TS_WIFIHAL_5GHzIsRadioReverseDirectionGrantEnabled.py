...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_5GHzIsRadioReverseDirectionGrantEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check Reverse Direction Grant enable status using  wifi_getRadioReverseDirectionGrantEnable HAL API</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_101</test_case_id>
    <test_objective>Check Reverse Direction Grant enable status using  wifi_getRadioReverseDirectionGrantEnable HAL API</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XB3. XB6, Emulator, Rpi</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getRadioReverseDirectionGrantEnable()</api_or_interface_used>
    <input_parameters>methodName	  :    getRadioReverseDirectionGrantEnable
methodName	  :    setRadioReverseDirectionGrantEnable
radioIndex        :    1</input_parameters>
    <automation_approch>1.Configure the Function info in Test Manager GUI  which needs to be tested  
(WIFIHAL_GetOrSetParamBoolValue  - func name - "If not exists already"
 WIFIHAL - module name
 Necessary I/P args as Mentioned in Input)
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_5GHzIsRadioReverseDirectionGrantEnabled.py)
3.Execute the generated Script(TS_WIFIHAL_5GHzIsRadioReverseDirectionGrantEnabled.py) using execution page of  Test Manager GUI
...
wifi_getRadioReverseDirectionGrantEnable() and wifi_setRadioReverseDirectionGrantEnable()
...
8.Test Manager will publish the result in GUI as SUCCESS/FAILURE based on the response from wifihalstub</automation_approch>
    <except_output>" 
CheckPoint 
1:wifi_getRadioReverseDirectionGrantEnable log from DUT should be available in Agent Console LogCheckPoint 
...
3:Test Manager GUI will publish the result as SUCCESS in Execution page"""</except_output>
    ...
    <test_script>TS_WIFIHAL_5GHzIsRadioReverseDirectionGrantEnabled</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzIsRadioReverseDirectionGrantEnabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult :
        ...
        if "Enabled" in enable:
            print "Reverse Direction Grant is Enabled for Radio 5GHz"
            oldEnable = 1
            newEnable = 0
        else:
            print "Reverse Direction Grant is Disabled for Radio 5GHz"
            oldEnable = 0
            newEnable = 1

        setMethod = "setRadioReverseDirectionGrantEnable"
        ...
        if expectedresult in actualresult :
            ...
            if expectedresult in actualresult and enable not in details.split(":")[1].strip():
                print "getRadioReverseDirectionGrantEnable Success, verified along with setRadioReverseDirectionGrantEnable() api"
                ...
                print "getRadioReverseDirectionGrantEnable() failed after set function"
		tdkTestObj.setResultStatus("FAILURE");
        else:
	    print "setRadioReverseDirectionGrantEnable() failed"
            tdkTestObj.setResultStatus("FAILURE");
    else:
