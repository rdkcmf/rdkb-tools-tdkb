...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_6GHzSetApSecurity_WPA3-Personal-Transition</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_SetApSecurity</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_setApSecurity() to set the security parameters when the security mode is set to "WPA3-Personal-Transition" and check if the SET operation of the mfp, encryption mode, key type, key passphrase and the WPA3 transition toggle is success and is getting reflected when the GET  API wifi_getApSecurity() is invoked for 6G private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_766</test_case_id>
    <test_objective>Invoke the HAL API wifi_setApSecurity() to set the security parameters when the security mode is set to "WPA3-Personal-Transition" and check if the SET operation of the mfp, encryption mode, key type, key passphrase and the WPA3 transition toggle is success and is getting reflected when the GET  API wifi_getApSecurity() is invoked for 6G private AP.</test_objective>
...
key : security key
wpa3_transition_disable : {"0" : "Enabled, "1": "Disabled"}</input_parameters>
...
3. Invoke the HAL API wifi_setApSecurity() to set security parameter values in the security mode : "WPA3-Personal-Transition". Set mfp : 2, encryption mode : AESEncryption, key type : PSK_SAE, key : password4321test and WPA3 transition disable : 0 or 1(toggling). Check if the GET API returns success.
4. Invoke the HAL API wifi_getApSecurity() to check if the security mode, mfp, encryption mode, key type, key and WPA3 transition disable values are the same as the values set.
5. Restore to initial state
6. Unload the modules</automation_approch>
    <expected_output>The HAL API wifi_setApSecurity() should successfully set the security parameters such as mfp, encryption type, key type, key value and WPA3 transition disable to valid values when the security mode is set to WPA3-Personal-Transition and the SET values should be reflected by the GET API wifi_getApSecurity() for 6G Private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzSetApSecurity_WPA3-Personal-Transition</test_script>
    <skipped>No</skipped>
...
def setAPSecurity(obj, apIndex, setvalues, wpa3_transition):
    expectedresult = "SUCCESS";
    ...
    if setvalues[0] not in [1,2,64,128,256,2048]:
        ...
        #If security mode is WPA3-Personal-Transition, set the WPA3-Transition enable/disable value as well
        if setvalues[0] == 1024:
            tdkTestObj.addParameter("wpa3_transition_disable", wpa3_transition);
...
def getAPSecurityDetails(details):
    ...
    #WPA3-Transition disable state : 0(enable) and 1(disable)
    ...
    if "Invalid" not in security_mode and mfp.isdigit() and "Invalid" not in encryption_method and wpa3_transition != "" and rekey_interval.isdigit() and strict_rekey != "" and eapol_key_timeout.isdigit() and eapol_key_retries.isdigit() and eap_identity_timeout.isdigit() and eap_identity_retries.isdigit() and eap_timeout.isdigit() and eap_retries.isdigit() and pmksa_cashing != "" and "Invalid" not in security_key_type and "Invalid" not in key:
        ...
        status = "FAILED";
    return status, values, map_to_enum_values, wpa3_transition;
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetApSecurity_WPA3-Personal-Transition');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzSetApSecurity_WPA3-Personal-Transition');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 2: Invoke the HAL API wifi_getApSecurity() for 6G private AP";
        print "EXPECTED RESULT 2: Should successfully invoke wifi_getApSecurity()";

        if expectedresult in actualresult and "AP Security details" in details:
            print "ACTUAL RESULT 2: wifi_getApSecurity() invoked successfully";
            ...
            print "\nTEST STEP 3: Get the Access Point Security details and check if the initial values are valid";
            print "EXPECTED RESULT 3: Should get the Access Point Security details successfully and the initial values should be valid";

            if status == "SUCCESS":
                print "ACTUAL RESULT 3: The Access Point Security details are retrieved and all initial values are valid";
                print "TEST EXECUTION RESULT 3: SUCCESS";
                tdkTestObj.setResultStatus("SUCCESS");

                #Set AP Security
                #New Security Mode = WPA3-Personal-Transition, which has the decimal value 1024 corresponding to the Hex value 0x00000400
                new_security_mode = 1024;
                #New MFP = 2
                new_mfp = 2;
                #New Encruption Method = AESEncryption, which has the value 2
                new_encryption_method = 2;
                #New Key Type = PSK_SAE, which has the value 3
                new_key_type = 3;
                #New Key = "password4321test";
                new_key = "password4321test";

                #Toggle the WPA3-Transition enable/disable state : 0(enable), 1(disable)
                if "Enabled" in wpa3_transition_initial:
                    old_wpa3_status = 0;
                    new_wpa3_status = 1;
                    WPA3_Transition = "Disabled";
                else:
                    old_wpa3_status = 1;
                    new_wpa3_status = 0;
                    WPA3_Transition = "Enabled";

                set_values = ["WPA3-Personal-Transition", "2", "AESEncryption", "PSK_SAE", "password4321test", WPA3_Transition];
                ...
                print "\nTEST STEP 4 : Invoke the HAL API wifi_setApSecurity() for 6G private AP with Security Mode : %s, MFP : %s, Encryption Mode : %s, Key Type : %s Key : %s and WPA3-Transition to : %s" %(set_values[0], set_values[1], set_values[2], set_values[3], set_values[4], set_values[5]);
                print "EXPECTED RESULT 4 : The HAL API should be invoked successfully";

                if expectedresult in actualresult:
                    print "ACTUAL RESULT 4: The SET API returned success; Details : %s" %details;
                    print "TEST EXECUTION RESULT 4: SUCCESS";
                    tdkTestObj.setResultStatus("SUCCESS");

                    #Cross check the SET with GET
                    actualresult, details, tdkTestObj = getAPSecurity(obj, idx)
                    print "\nTEST STEP 5: Invoke the HAL API wifi_getApSecurity() for 6G private AP after the SET operation";
                    print "EXPECTED RESULT 5: Should successfully invoke wifi_getApSecurity()";

                    if expectedresult in actualresult and "AP Security details" in details:
                        print "ACTUAL RESULT 5: wifi_getApSecurity() invoked successfully";
                        ...
                        print "\nTEST STEP 6: Get the Access Point Security details and check if the retrieved values are valid";
                        print "EXPECTED RESULT 6: Should get the Access Point Security details successfully and the retrieved values should be valid";

                        if status == "SUCCESS":
                            print "ACTUAL RESULT 6: The Access Point Security details are retrieved and all retrieved values are valid";
                        ...
                        print "ACTUAL RESULT 5: wifi_getApSecurity() not invoked successfully";
                    ...
                    print "\nTEST STEP 7 : Revert to initial AP Security state";
                    ...
                    print "ACTUAL RESULT 4: The SET API returned failure; Details : %s" %details;
                    print "TEST EXECUTION RESULT 4: FAILURE";
                    tdkTestObj.setResultStatus("FAILURE");
            else:
                print "ACTUAL RESULT 3: All Access Point Security details are not valid";
                print "TEST EXECUTION RESULT 3: FAILURE";
                tdkTestObj.setResultStatus("FAILURE");
        else:
            print "ACTUAL RESULT 2: wifi_getApSecurity() is not invoked successfully";
