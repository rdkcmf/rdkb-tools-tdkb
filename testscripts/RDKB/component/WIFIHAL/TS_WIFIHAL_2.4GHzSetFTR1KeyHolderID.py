...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_2.4GHzSetFTR1KeyHolderID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTR1KeyHolderID</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the FTR1 Key Holder ID by invoking the HAL API wifi_setFTR1KeyHolderID() and validate the SET using the GET API wifi_getFTR1KeyHolderID() for 2.4G private AP</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_773</test_case_id>
    <test_objective>Set the FTR1 Key Holder ID by invoking the HAL API wifi_setFTR1KeyHolderID() and validate the SET using the GET API wifi_getFTR1KeyHolderID() for 2.4G private AP</test_objective>
...
2. Get the initial FTR1 Key Holder ID using the HAL API wifi_getFTR1KeyHolderID() for 2.4G private AP.
...
5. Invoke the HAL API wifi_setFTR1KeyHolderID() and pass the newly generated Key Holder ID for 2.4G private API and check if the SET operation returns success.
...
9. Unload the modules.</automation_approch>
    <expected_output>Should be able to set the FTR1 Key Holder ID by invoking the HAL API wifi_setFTR1KeyHolderID() and the SET value should be validated successfully using the GET API wifi_getFTR1KeyHolderID() for 2.4G private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzSetFTR1KeyHolderID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetFTR1KeyHolderID');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult :
            ...
            if num_hex_values >= 1:
                ...
                print "\nTEST STEP 3: Invoke the HAL API wifi_setFTR1KeyHolderID() to set the FTR1 Key Holder ID for 2.4G private AP";
