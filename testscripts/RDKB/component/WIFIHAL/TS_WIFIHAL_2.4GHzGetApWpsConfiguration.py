...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>TS_WIFIHAL_2.4GHzGetApWpsConfiguration</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetApWpsConfiguration</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getApWpsConfiguration() for 2.4G private AP and check if the WPS enabled is retrieved successfully and WPS PIN and WPS Methods are valid if WPS mode is enabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_776</test_case_id>
    <test_objective>Invoke the HAL API wifi_getApWpsConfiguration() for 2.4G private AP and check if the WPS enabled is retrieved successfully and WPS PIN and WPS Methods are valid if WPS mode is enabled.</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>apIndex : 2.4G private access point index</input_parameters>
    <automation_approch>1. Load the module
2. Invoke the HAL API wifi_getApWpsConfiguration() for 2.4G private access point.
...
6. Unload the module</automation_approch>
    <expected_output>The HAL API wifi_getApWpsConfiguration() should be invoked successfully for 2.4G private AP and the WPS PIN and WPS Methods should be retrieved if WPS Mode is enabled and not retrieved when it is disabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzGetApWpsConfiguration</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetApWpsConfiguration');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getApWpsConfiguration() for 2.4G private AP";
        ...
        if expectedresult in actualresult and "getApWpsConfiguration invoked successfully" in details:
            ...
            print "\nTEST STEP 2: Check the enable status of WPS Enable";
            print "EXPECTED RESULT 2 : Should retrieve the enable status of WPS successfully";
            wps_mode = details.split("WPS Mode : ")[1].split(",")[0];

            if "Enabled" in wps_mode or "Disabled" in wps_mode:
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 2: WPS Enable Mode = %s" %wps_mode;
                print "[TEST EXECUTION RESULT] : SUCCESS";

                #If WPS mode is enabled, get the WPS PIN and WPS Methods
                if "Enabled" in wps_mode:
                    print "\nTEST STEP 3: Get the WPS PIN";
                    print "EXPECTED RESULT 3 : Should retrieve the WPS PIN successfully";
                    wps_pin = details.split("WPS device PIN: ")[1].split(",")[0];

                    if wps_pin != "" and wps_pin.isdigit():
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "ACTUAL RESULT 3: WPS PIN = %s" %wps_pin;
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        ...
                        if len(wps_methods) > 0:
                            ...
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT 4: WPS Methods = ", wps_methods;
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                        ...
                        tdkTestObj.setResultStatus("FAILURE");
                        print "ACTUAL RESULT 3: WPS PIN = %s; Value retrieved is invalid" %wps_pin;
                        print "[TEST EXECUTION RESULT] : FAILURE";
                else :
                    tdkTestObj.setResultStatus("SUCCESS");
                ...
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT 2: WPS Enable Mode = %s; Value retrieved is invalid" %wps_mode;
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            #Set the result status of execution
            tdkTestObj.setResultStatus("FAILURE");
