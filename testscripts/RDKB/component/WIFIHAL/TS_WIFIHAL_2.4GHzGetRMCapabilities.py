...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzGetRMCapabilities</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetRMCapabilities</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getRMCapabilities() with 2.4GHz connected client MAC address and check if the RM Capabilities values retrieved are valid and non-empty.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_759</test_case_id>
    <test_objective>Invoke the HAL API wifi_getRMCapabilities() with 2.4GHz connected client MAC address and check if the RM Capabilities values retrieved are valid and non-empty.</test_objective>
...
wifi_getApAssociatedDeviceDiagnosticResult3()</api_or_interface_used>
    <input_parameters>apIndex : 2.4G private AP
peer : MAC address of 2.4G connected client</input_parameters>
    <automation_approch>1. Load the modules
2. Invoke the HAL API wifi_getApAssociatedDeviceDiagnosticResult3() to get the MAC address of the 2.4G connected client.
...
5. Unload the modules</automation_approch>
    <expected_output>The HAL API wifi_getRMCapabilities() with 2.4GHz connected client MAC address should be invoked successfully and the RM Capabilities values retrieved should be valid and non-empty.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzGetRMCapabilities</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetRMCapabilities');
