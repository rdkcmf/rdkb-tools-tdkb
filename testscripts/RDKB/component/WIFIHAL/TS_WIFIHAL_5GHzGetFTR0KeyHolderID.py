...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WIFIHAL_5GHzGetFTR0KeyHolderID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTR0KeyHolderID</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getFTR0KeyHolderID() for 5G private AP to retrieve the Fast Transition R0 Key Holder ID and cross check with the R0 Key Holder ID value stored in nvram</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_751</test_case_id>
    <test_objective>Invoke the HAL API wifi_getFTR0KeyHolderID() for 5G private AP to retrieve the Fast Transition R0 Key Holder ID and cross check with the R0 Key Holder ID value stored in nvram</test_objective>
...
2. Invoke the HAL API wifi_getFTR0KeyHolderID() to get the FTR0 Key Holder ID for 5G private AP.
3. Convert the Hex values received to equivalent string representation.
4. Get the command to fetch the FTR0 Key Holder ID for 5G private AP from the platform property file.
...
7. Unload the modules.</automation_approch>
    <expected_output>Invocation of the HAL API wifi_getFTR0KeyHolderID() for 5G private AP should be success and the FTR0 Key Holder ID retrieved should be the same as the value stored in nvram.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetFTR0KeyHolderID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetFTR0KeyHolderID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetFTR0KeyHolderID');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult and "FTR0 Key Holder ID Details -" in details:
            ...
            if num_hex_values >= 1:
                ...
                cmd= "sh %s/tdk_utility.sh parseConfigFile FTR0_KEY_HOLDER_ID_5G" %TDK_PATH;
                print cmd;
