...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_2.4GHzIsRadioDCSEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if dynamic channel selection(DCS) enabled for Radio 2.4GHz using wifi_getRadioDCSEnable()</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_60</test_case_id>
    <test_objective>Check if dynamic channel selection(DCS) enabled for Radio 2.4GHz using wifi_getRadioDCSEnable()</test_objective>
...
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_2.4GHzIsRadioDCSEnabled.py)
3.Execute the generated Script(TS_WIFIHAL_2.4GHzIsRadioDCSEnabled.py) using execution page of  Test Manager GUI
...
8.Test Manager will publish the result in GUI as SUCCESS/FAILURE based on the response from wifihalstub</automation_approch>
    ...
    <test_script>TS_WIFIHAL_2.4GHzIsRadioDCSEnabled</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzIsRadioDCSEnabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult :
        ...
        if "Enabled" in enable:
            print "DCS is Enabled for Radio 2.4GHZ"
            oldEnable = 1
            newEnable = 0
        else:
            print "DCS is Disabled for Radio 2.4GHZ"
            oldEnable = 0
            newEnable = 1
