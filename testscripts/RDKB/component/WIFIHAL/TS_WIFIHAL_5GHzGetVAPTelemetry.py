...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_5GHzGetVAPTelemetry</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetVAPTelemetry</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getVAPTelemetry() to fetch the Tx Overflow and compare the value retrieved with the TR-181 parameter Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Invoke the HAL API wifi_getVAPTelemetry() to fetch the Tx Overflow and compare the value retrieved with the TR-181 parameter Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow.</test_objective>
...
paramName : Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow</input_parameters>
...
3. Get the TR-181 parameter Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow. The value should be fetched successfully.
...
5. Unload the modules.</automation_approch>
    <expected_output>Invocation of the HAL API wifi_getVAPTelemetry() should be successful and the Tx Overflow value retrieved should be equal to the value of the TR-181 parameter Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetVAPTelemetry</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetVAPTelemetry');
obj1.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetVAPTelemetry');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult:
            ...
            if details != "":
                ...
                print "TEST STEP 3: Get the TR181 value of Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow";
                print "EXPECTED RESULT 3: Should get the TR181 value of Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow";
                tdkTestObj = obj1.createTestStep('WIFIAgent_Get');
                tdkTestObj.addParameter("paramName","Device.WiFi.AccessPoint.2.X_COMCAST-COM_TXOverflow");
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
