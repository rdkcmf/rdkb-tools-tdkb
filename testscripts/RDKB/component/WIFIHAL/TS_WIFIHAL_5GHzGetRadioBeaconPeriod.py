...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the default Beacon Period (Time interval between transmitting beacons) for 5GHz radio using wifi_getRadioBeaconPeriod HAL API and validate the same</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>To get the default Beacon Period value (Time interval between transmitting beacons) for 5GHz radio using wifi_getRadioBeaconPeriod HAL API and validate the same</test_objective>
...
8.The value received form WIFIHAL_GetOrSetParamUIntValue and the one received from platform properties value should be same.
...
pamobj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetRadioBeaconPeriod');
