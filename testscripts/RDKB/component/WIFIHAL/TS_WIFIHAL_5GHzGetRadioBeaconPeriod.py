...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_5GHzGetRadioBeaconPeriod</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamUIntValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the Beacon Period (Time interval between transmitting beacons) for 5GHz radio using wifi_getRadioBeaconPeriod HAL API and validate the same</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_80</test_case_id>
    <test_objective>To get the Beacon Period value (Time interval between transmitting beacons) for 5GHz radio using wifi_getRadioBeaconPeriod HAL API and validate the same</test_objective>
...
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_5GHzGetRadioBeaconPeriod.py)
3.Execute the generated Script(TS_WIFIHAL_5GHzGetRadioBeaconPeriod.py) using execution page of  Test Manager GUI
...
6.WIFIHAL_GetOrSetParamUIntValue function will call Ccsp Base Function named "ssp_WIFIHALGetOrSetParamUIntValue", that inturn will call WIFIHAL Library Function wifi_getRadioBeaconPeriod() function
7.The value received form WIFIHAL_GetOrSetParamUIntValue and the one received from platform properties value should be same.
8.Response(s)(printf) from TDK Component,Ccsp Library function and wifihalstub would be logged in Agent Console log based on the debug info redirected to agent console
9.wifihalstub will validate the available result (from agent console log and Pointer to instance as updated) with expected result
10.Test Manager will publish the result in GUI as SUCCESS/FAILURE based on the response from wifihalstub</automation_approch>
...
3:Test Manager GUI will publish the result as SUCCESS in Execution page"""</except_output>
    ...
    <test_script>TS_WIFIHAL_5GHzGetRadioBeaconPeriod</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetRadioBeaconPeriod');
obj1.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetRadioBeaconPeriod');
