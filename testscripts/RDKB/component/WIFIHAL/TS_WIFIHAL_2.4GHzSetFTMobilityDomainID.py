...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>17</version>
  <name>TS_WIFIHAL_2.4GHzSetFTMobilityDomainID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTMobilityDomainID</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the FT Mobility Domain ID by invoking the HAL API wifi_setFTMobilityDomainID() and validate the SET using the GET API wifi_getFTMobilityDomainID() for 2.4G private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_726</test_case_id>
    <test_objective>Set the FT Mobility Domain ID by invoking the HAL API wifi_setFTMobilityDomainID() and validate the SET using the GET API wifi_getFTMobilityDomainID() for 2.4G private AP.</test_objective>
...
2. Invoke the HAL API wifi_getFTMobilityDomainID() for 2.4G Private AP and check if the API invocation is success and the FT Mobility Domain ID retrieved is a pair of Hexadecimal values.
3. Combine the Hex value and obtain the equivalent decimal value for FT Mobility Domain ID and store the initial value.
4. Invoke the HAL API wifi_setFTMobilityDomainID() for 2.4G private AP with a dynamically selected Mobility Domain ID. Check if the SET API returns success.
...
8. Unload the modules
    ...
    <expected_output>Should successfully set the FT Mobility Domain ID by invoking the HAL API wifi_setFTMobilityDomainID() and the FT Mobility Domain ID set should reflect in the GET API wifi_getFTMobilityDomainID() for 2.4G Private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzSetFTMobilityDomainID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetFTMobilityDomainID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetFTMobilityDomainID');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult :
            ...
            if "0x" in hex_value0 and "0x" in hex_value1:
                ...
                print "\nTEST STEP 3: Invoke the HAL API wifi_setFTMobilityDomainID() to set the Mobility Domain ID for 2.4G private AP";
