...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzSetSteeringClient</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>WIFIHAL_SteeringClientSet</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Add steering client config using wifi_steering_clientSet()</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_293</test_case_id>
    <test_objective>Add steering client config using wifi_steering_clientSet()</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1. Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_steering_clientSet()</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. Load wifihal module
2. Invoke wifi_steering_clientSet() to add a steering client config for 2.4G.
3. Do a get of steering client config using the platform specific logic specified in tdk_platform_utility.sh and check if added config is present
4. Revert the added client config using wifi_steering_clientRemove()
5. Unload wifihal module</automation_approch>
    <expected_output>Should successfully add steering client config using wifi_steering_clientSet()</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzSetSteeringClient</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetSteeringClient');
sysObj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetSteeringClient');
wifiObj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetSteeringClient');
