...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_2.4GHzPublicWiFi_SetApMacAddressControlMode_WhiteListFilter</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamIntValue</primitive_test_name>
  <primitive_test_version>5</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke wifi_setApMacAddressControlMode HAL API to set the filter control mode to WhiteList for 2.4GHz Public WiFi and check if the set value is getting reflected to the get API wifi_getApMacAddressControlMode.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Invoke wifi_setApMacAddressControlMode HAL API to set the filter control mode to WhiteList for 2.4GHz Public WiFi and check if the set value is getting reflected to the get API wifi_getApMacAddressControlMode.</test_objective>
...
7. Unload the modules.</automation_approch>
    ...
    <test_script>TS_WIFIHAL_2.4GHzPublicWiFi_SetApMacAddressControlMode_WhiteListFilter</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_SetApMacAddressControlMode_WhiteListFilter');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_SetApMacAddressControlMode_WhiteListFilter');
    ...
    if expectedresult in actualresult and details != "":
        ...
        if expectedresult in actualresult and details != "":
            ...
            if initMode == 0 or initMode == 1 or initMode == 2:
                ...
                setMode = 1;
                print "\nTEST STEP 3: Set to Whitelist filter mode by invoking the API wifi_setApMacAddressControlMode for 2.4GHz Public WiFi";
