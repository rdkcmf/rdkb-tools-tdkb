...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_5GHzGetFTR1KeyHolderID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTR1KeyHolderID</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getFTR1KeyHolderID() for 5G private AP to retrieve the Fast Transition R1 Key Holder ID and cross check with the R1 Key Holder ID value stored in nvram</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_771</test_case_id>
    <test_objective>Invoke the HAL API wifi_getFTR1KeyHolderID() for 5G private AP to retrieve the Fast Transition R1 Key Holder ID and cross check with the R1 Key Holder ID value stored in nvram</test_objective>
...
2. Invoke the HAL API wifi_getFTR1KeyHolderID() to get the FTR1 Key Holder ID for 5G private AP.
3. Convert the Hex values received to equivalent string representation.
4. Get the command to fetch the FTR1 Key Holder ID for 5G private AP from the platform property file.
...
7. Unload the modules.</automation_approch>
    <expected_output>Invocation of the HAL API wifi_getFTR1KeyHolderID() for 5G private AP should be success and the FTR1 Key Holder ID retrieved should be the same as the value stored in nvram.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetFTR1KeyHolderID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetFTR1KeyHolderID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetFTR1KeyHolderID');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult and "FTR1 Key Holder ID Details -" in details:
            ...
            if num_hex_values >= 1:
                ...
                cmd= "sh %s/tdk_utility.sh parseConfigFile FTR1_KEY_HOLDER_ID_5G" %TDK_PATH;
                print cmd;
