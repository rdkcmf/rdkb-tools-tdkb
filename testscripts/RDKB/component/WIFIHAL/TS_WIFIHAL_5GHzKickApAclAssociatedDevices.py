...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIHAL_5GHzKickApAclAssociatedDevices</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to remove all the existing connection from the associated device by using wifi_kickApAclAssociatedDevices () api by checking whether the return status is SUCCESS for 5GHz.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_266</test_case_id>
    <test_objective>Test to remove all the existing connection from the associated device by using wifi_kickApAclAssociatedDevices () api by checking whether the return status is SUCCESS for 5 GHz.The API is intended to be validated in the rdklogs/logs/wifilog.txt.0 , but since the logging was not found in the devices we are validating only the return status of the API</test_objective>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzKickApAclAssociatedDevices');
