...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_6GHzGetApVlanID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamIntValue</primitive_test_name>
  <primitive_test_version>5</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getApVlanID() to retrieve the 6G private access point VLAN ID and check if the value returned is non-empty and valid.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_679</test_case_id>
    <test_objective>Invoke the HAL API wifi_getApVlanID() to retrieve the 6G private access point VLAN ID and check if the value returned is non-empty and valid.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getApVlanID()</api_or_interface_used>
    <input_parameters>methodname : getApVlanID
radioIndex : 6G private AP index fetched from platform property file</input_parameters>
    <automation_approch>1. Load the modules
2. Invoke the HAL API wifi_getApVlanID() for 6G private AP index and check if the value is non-empty and a valid integral value.
3. Unload the module</automation_approch>
    <expected_output>The invocation of the HAL API wifi_getApVlanID() to retrieve the 6G private access point VLAN ID should be success and the value returned should be non-empty and valid.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzGetApVlanID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetApVlanID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetApVlanID');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if apIndex == -1:
        ...
        tdkTestObj.addParameter("methodName","getApVlanID");
