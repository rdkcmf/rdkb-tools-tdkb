...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WIFIHAL_6GHzGetFTMobilityDomainID</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetFTMobilityDomainID</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getFTMobilityDomainID() for 6G private AP to retrieve the Fast Transition Mobility Domain ID and cross check with the fbt_mdid value stored in nvram.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_725</test_case_id>
    <test_objective>Invoke the HAL API wifi_getFTMobilityDomainID() for 6G private AP to retrieve the Fast Transition Mobility Domain ID and cross check with the fbt_mdid value stored in nvram.</test_objective>
...
7. Unload modules.</automation_approch>
    <expected_output>The HAL API wifi_getFTMobilityDomainID() for 6G private AP to retrieve the Fast Transition Mobility Domain ID should be invoked successfully and should match with the fbt_mdid value stored in nvram.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzGetFTMobilityDomainID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetFTMobilityDomainID');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetFTMobilityDomainID');
    ...
    if apIndex == -1:
        ...
        if expectedresult in actualresult and details != "":
            ...
            if "0x" in hex_value0 and "0x" in hex_value1:
                ...
                cmd= "sh %s/tdk_utility.sh parseConfigFile FT_MOBILITY_DOMAIN_ID_6G" %TDK_PATH;
                print cmd;
                ...
                print "\nTEST STEP 4: Get the command to cross check the FT Mobility Domain ID from property file";
                print "EXPECTED RESULT 4: Should get the FT Mobility Domain ID check command from property file"

                if expectedresult in actualresult and check_cmd!= "":
                    print "ACTUAL RESULT 4: FT Mobility Domain ID check command from property file :", check_cmd ;
                    print "TEST EXECUTION RESULT :SUCCESS";
                    tdkTestObj.setResultStatus("SUCCESS");
                    ...
                    print "\nTEST STEP 5: Execute the command in the DUT to obtian the Mobility Domain ID";
                    print "EXPECTED RESULT 5: Should successfully execute the FT Mobility Domain ID check command"

                    if expectedresult in actualresult and "mdid=" in details:
                        print "ACTUAL RESULT 5: FT Mobility Domain ID :", details ;
                        print "TEST EXECUTION RESULT :SUCCESS";
                        tdkTestObj.setResultStatus("SUCCESS");
                        ...
                        print "\nTEST STEP 6 : Validate the FT Mobility Domain ID value retrieved using the HAL API";
                        print "EXPECTED RESULT 6 : the FT Mobility Domain ID value from the HAL API should be the same as the value retrieved using the command";  
                        ...
                        if fbt_mdid_value.isdigit() :
                            ...
                            tdkTestObj.setResultStatus("FAILURE");
                            print "ACTUAL RESULT 6 : FT Mobility Domain ID is not of the expected type";
                            tdkTestObj.setResultStatus("FAILURE");
                    else:
                        print "ACTUAL RESULT 5: FT Mobility Domain ID :", details ;
                        print "TEST EXECUTION RESULT :FAILURE";
