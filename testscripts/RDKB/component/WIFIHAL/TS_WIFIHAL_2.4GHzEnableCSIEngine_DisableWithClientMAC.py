...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzEnableCSIEngine_DisableWithClientMAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Invoke the HAL API wifi_enableCSIEngine() to enable the CSI monitoring for the connected client and then disable CSI monitoring. In both cases check if the frame info is retrieved as expected.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Invoke the HAL API wifi_enableCSIEngine() to enable the CSI monitoring for the connected client and then disable CSI monitoring. In both cases check if the frame info is retrieved as expected</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.Connect a wifi client to 2.4GHz radio</pre_requisite>
...
5. Invoke WIFIHAL_EnableCSIEngine to disable the CSI monitoring of the client device. The API should return success.
...
7. Unload the wifihal and sysutil module.</automation_approch>
    <expected_output>Invocation of the HAP API wifi_enableCSIEngine() to enable the CSI monitoring for the connected client and then to disable CSI monitoring should be success. In both cases the frame info should be retrieved as expected.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzEnableCSIEngine_DisableWithClientMAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzEnableCSIEngine_DisableWithClientMAC');
obj1.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzEnableCSIEngine_DisableClientMAC');
