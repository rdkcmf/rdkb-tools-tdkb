...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_2.4GHzPublicWiFi_SetApWpsButtonPush</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_setApWpsButtonPush() for 2.4GHz Public WiFi after ensuring that WPS is enabled for the AP by invoking wifi_setApWpsEnable() and cross checking with wifi_getApWpsEnable().</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_598</test_case_id>
    <test_objective>Invoke the HAL API wifi_setApWpsButtonPush() for 2.4GHz Public WiFi after ensuring that WPS is enabled for the AP by invoking wifi_setApWpsEnable() and cross checking with wifi_getApWpsEnable().</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_setApWpsButtonPush
wifi_getApWpsEnable
wifi_setApWpsEnable</api_or_interface_used>
    <input_parameters>methodName : getApWpsEnable
methodName : setApWpsEnable
methodName : setApWpsButtonPush
...
5. After cross checking the WPS Enable state with the get API, invoke the HAL API wifi_setApWpsButtonPush. The API should return success.
6. Revert the WPS Enable state if required
7. Unload the modules.</automation_approch>
    <expected_output>The HAL API wifi_setApWpsButtonPush() should be invoked successfully after enabling WPS using wifi_setApWpsEnable() for 2.4GHz Public WiFi.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzPublicWiFi_SetApWpsButtonPush</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_SetApWpsButtonPush');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzPublicWiFi_SetApWpsButtonPush');
...
def ButtonPushWPS(idx, step):
    print "\n*******************Invoking AP WPS Button Push**********************";
    status = 1;
    tdkTestObj = obj.createTestStep('WIFIHAL_GetOrSetParamStringValue');
    tdkTestObj.addParameter("radioIndex", idx);
    tdkTestObj.addParameter("methodName", "setApWpsButtonPush");
    expectedresult="SUCCESS";

    print "\nTEST STEP %d: Call the function wifi_setApWpsButtonPush() for 2.4 GHz Public WiFi" %step;
    ...
    if expectedresult in actualresult and details != "":
        ...
        if expectedresult in actualresult and details != "":
            ...
            if "Enabled" in ApWPSEnable:
                ...
                status = ButtonPushWPS(apIndex, step);

                if status == 0:
                    print "ButtonPushWPS operation success";
                    tdkTestObj.setResultStatus("SUCCESS");
                else:
                    print "ButtonPushWPS operation failed";
                    tdkTestObj.setResultStatus("FAILURE");
            else:
