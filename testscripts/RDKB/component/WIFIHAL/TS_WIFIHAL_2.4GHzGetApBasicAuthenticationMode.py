...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzGetApBasicAuthenticationMode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Check Authentication mode set in device for 2.4GHz radio using wifi_getApBasicAuthenticationMode HAL API and validate the same</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_34</test_case_id>
    <test_objective>To Check Authentication mode set in device for 2.4GHz radio using wifi_getApBasicAuthenticationMode HAL API and validate the same</test_objective>
...
2.Python Script will be generated/overrided automatically by Test Manager with provided arguments in configure page (TS_WIFIHAL_2.4GHzGetApBasicAuthenticationMode.py)
3.Execute the generated Script(TS_WIFIHAL_2.4GHzGetApBasicAuthenticationMode.py) using execution page of  Test Manager GUI
...
5.WIFIHAL_GetOrSetParamStringValue function will call Ccsp Base Function named "ssp_WIFIHALGetOrSetParamStringValue", that inturn will call WIFIHAL Library Function wifi_getApBasicAuthenticationMode() function
...
1:wifi_getApBasicAuthenticationMode log from DUT should be available in Agent Console LogCheckPoint
...
3:Test Manager GUI will publish the result as SUCCESS in Execution page</except_output>
    ...
    <test_script>TS_WIFIHAL_2.4GHzGetApBasicAuthenticationMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetApBasicAuthenticationMode');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetApBasicAuthenticationMode');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysloadmodulestatus.upper():
    ...
    if supportedAuthModes and expectedresult in actualresult:
        ...
        if expectedresult in actualresult :
            ...
            print "**************************************************";
            print "TEST STEP 2: Get the Ap Basic Authentication mode for 2.4GHz";
            print "EXPECTED RESULT 2: Should successfully get the Ap Basic Authentication mode for 2.4GHz";
            print "ACTUAL RESULT 2: Successfully got the Ap Basic Authentication mode as %s for 2.4GHz"%receivedMode;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : SUCCESS";
            print "**************************************************";
	    if receivedMode in supportedModes:
            ...
            print "TEST STEP 2: Get the Ap Basic Authentication mode for 2.4GHz";
            print "EXPECTED RESULT 2: Should successfully get the Ap Basic Authentication mode for 2.4GHz";
            print "ACTUAL RESULT 2: Failed to get the Ap Basic Authentication mode for 2.4GHz";
            print details;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : FAILURE";
            print "**************************************************";
