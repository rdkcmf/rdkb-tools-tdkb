...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_765</test_case_id>
    <test_objective>Invoke the HAL API wifi_setApSecurity() to set the security parameters when the security mode is set to "WPA3-Personal-Transition" and check if the SET operation of the mfp, encryption mode, key type, key passphrase and the WPA3 transition toggle is success and is getting reflected when the GET  API wifi_getApSecurity() is invoked for 5G private AP.</test_objective>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetApSecurity_WPA3-Personal-Transition');
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult and "AP Security details" in details:
            ...
            if status == "SUCCESS":
                ...
                print "\nTEST STEP 3 : Invoke the HAL API wifi_setApSecurity() for 5G private AP with Security Mode : %s, MFP : %s, Encryption Mode : %s, Key Type : %s Key : %s and WPA3-Transition to : %s" %(set_values[0], set_values[1], set_values[2], set_values[3], set_values[4], set_values[5]);
