...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHZGetApAssociatedDeviceDiagnosticResult2_NoClient</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if wifi_getApAssociatedDeviceDiagnosticResult2() is returning success and empty buffer when there are no associated devices</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Check if wifi_getApAssociatedDeviceDiagnosticResult2() is returning success and empty buffer for 2.4GHZ when there are no associated devices</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3. 2.4GHZ WiFi client should not be connected to the DUT</pre_requisite>
...
4. Unload WIFI hal module</automation_approch>
    ...
    <test_script>TS_WIFIHAL_2.4GHZGetApAssociatedDeviceDiagnosticResult2_NoClient</test_script>
    <skipped>No</skipped>
