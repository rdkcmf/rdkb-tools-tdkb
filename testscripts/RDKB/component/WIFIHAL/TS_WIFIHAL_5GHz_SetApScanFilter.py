...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate the WiFi HAL API wifi_setApScanFilter for 5GHz AccessPoint</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_364</test_case_id>
    <test_objective>To validate the WiFi HAL API wifi_SetApScanFilter for 5GHz AP</test_objective>
...
mode - scan filter mode (0 or 1 or 2)
</input_parameters>
...
4.Verify the set
5.Revert back to the original ScanFilter mode
6.Unload the module</automation_approch>
    <expected_output>The APScanFilter should be set with all three possible modes
</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHz_SetApScanFilter</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHz_SetApScanFilter');
sysObj.configureTestCase(ip,port,'TS_WIFIHAL_5GHz_SetApScanFilter');

def get_gscanfilter(tdkTestObj):
    query = "sh %s/tdk_platform_utility.sh getAP1ScanFilter | grep gscanfilter | cut -f2 -d ':' " %TDK_PATH
