...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_6GHzGetRadioTransmitPower</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamULongValue</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getRadioTransmitPower() to get the current radio transmit power for 6Ghz radio and check if the transmit power is a valid value.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_675</test_case_id>
    <test_objective>Invoke the HAL API wifi_getRadioTransmitPower() to get the current radio transmit power for 6Ghz radio and check if the transmit power is a valid value.</test_objective>
...
2. Invoke the HAL API wifi_getRadioTransmitPower() and verify if the Transmit Power value received is a valid value.
3. Unload the module</automation_approch>
    <expected_output>The HAL API wifi_getRadioTransmitPower() should be invoked successfully for 6G radio.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzGetRadioTransmitPower</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetRadioTransmitPower');
