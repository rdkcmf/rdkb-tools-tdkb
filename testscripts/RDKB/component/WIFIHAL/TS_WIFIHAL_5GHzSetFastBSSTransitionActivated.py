...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzSetFastBSSTransitionActivated</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamBoolValue</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_setFastBSSTransitionActivated() to toggle the Fast BSS Transition Activated enable status and verify the SET with wifi_getBSSTransitionActivated() HAL API for 5G private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_800</test_case_id>
    <test_objective>Invoke the HAL API wifi_setFastBSSTransitionActivated() to toggle the Fast BSS Transition Activated enable status and verify the SET with wifi_getBSSTransitionActivated() HAL API for 5G private AP.</test_objective>
...
wifi_setFastBSSTransitionActivated()</api_or_interface_used>
    <input_parameters>radioIndex : 5G Private AP index
param : enable/disable(1/0)</input_parameters>
    <automation_approch>1. Load the module
2. Invoke the HAL API wifi_getBSSTransitionActivated() for 5G private AP to retrieve the initial FBT enable status.
...
6. Unload module</automation_approch>
    <expected_output>The HAL API wifi_setFastBSSTransitionActivated() should successfully toggle the FBT enable status for 5G private AP and the SET should reflect in the GET API wifi_getBSSTransitionActivated().</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzSetFastBSSTransitionActivated</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetFastBSSTransitionActivated');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getBSSTransitionActivated() for 5G Private AP";
        ...
        if expectedresult in actualresult:
            ...
            print "\nTEST STEP 2: Toggle the enabled state using the HAL API wifi_setFastBSSTransitionActivated() for 5G Private AP";
