...
<xml>
  ...
  <name>TS_WIFIHAL_5GHzSetRadioObssCoexistenceEnable</name>
  ...
  <test_cases>
    <test_case_id>TS_WIFIHAL_292</test_case_id>
    <test_objective>To validate the functionality of
 wifi_setRadioObssCoexistenceEnable api for  5GHz</test_objective>
...
5. Unload the module</automation_approch>
    ...
    <test_script>TS_WIFIHAL_5HzSetRadioObssCoexistenceEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetRadioObssCoexistenceEnable');
sysObj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetRadioObssCoexistenceEnable');
...
def get_Radio1ObssCoexistenceEnable(tdkTestObj):
    query="sh %s/tdk_platform_utility.sh getRadio1ObssCoexistenceEnable" %TDK_PATH
    print "query:%s" %query
...
if "SUCCESS" in loadmodulestatus.upper() and sysutilloadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        expectedresult="SUCCESS";
        defObssCoexistenceEnable,actualresult = get_Radio1ObssCoexistenceEnable(tdkTestObj);
