...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <status>FREE</status>
  <synopsis>To get the Radio interface name for 2.4GHz and check whether it is "wifi0"</synopsis>
  <groups_id/>
...
2. Invoke "WIFIHAL_GetOrSetParamStringValue" to get the interface name of 2.4GHz
3.Check if the value returned is valid or not
4. If not, return failure
5.Unload wifihal module</automation_approch>
    <except_output>The interface name of 2.4GHz should be "wifi0"</except_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_2.4GHzGetRadioIfName</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetRadioIfName');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetRadioIfName');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysloadmodulestatus.upper():
    ...
    print "Get the radio interface name for 2.4Ghz"
    getRadioIf = "sh %s/tdk_utility.sh parseConfigFile RADIO_IF_2G" %TDK_PATH;
