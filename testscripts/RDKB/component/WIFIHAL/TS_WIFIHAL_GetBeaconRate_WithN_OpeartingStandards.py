...
<xml>
  ...
  <name>TS_WIFIHAL_GetBeaconRate_WithN_OpeartingStandards</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set the operating standard to n and verify beacon rate is 6mbps.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_707</test_case_id>
    <test_objective>To set the operating standard to n and verify beacon rate is 6mbps</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it Through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>wifi_getApBeaconRate</api_or_interface_used>
...
3.Set the operating standards to n
4.Get the ap becaon rate
5.the value is expected to be 6mbps
6.Revert the set value
7.Unload the module</automation_approch>
    <expected_output>The ap becaon rate should be received as 6mbps when the operating standards as n</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_GetBeaconRate_WithNOpeartingStandatds</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_GetBeaconRate_WithN_OpeartingStandards');
obj1.configureTestCase(ip,port,'TS_WIFIHAL_GetBeaconRate_WithN_OpeartingStandards');
...
if "SUCCESS" in (loadmodulestatus.upper() and  sysutilmodulestatus.upper()):
    ...
    if idx == -1:
        ...
        if expectedresult in actualresult and details!="":
	        ...
	        #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
	        print "TEST STEP 2: Set the WiFi Operating standards to n";
	        print "EXPECTED RESULT 2: Should set the WiFi Operating standards to n";
	        ...
	        if expectedresult in actualresult and "6" in details:
	            ...
	            print "EXPECTED RESULT 3: Should get the current ap beacon rate as 6Mbsps for n operating mode";
