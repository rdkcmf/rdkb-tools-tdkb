...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzGetApAssociatedClientDiagnosticResult</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetApAssociatedClientDiagnosticResult</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getApAssociatedClientDiagnosticResult() with the connected client MAC address and check if the client diagnostic results are retrieved successfully for 5G Private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_738</test_case_id>
    <test_objective>Invoke the HAL API wifi_getApAssociatedClientDiagnosticResult() with the connected client MAC address and check if the client diagnostic results are retrieved successfully for 5G Private AP.</test_objective>
...
mac_addr : MAC address of the client connected to 5G Private AP.</input_parameters>
    <automation_approch>1. Load the modules
2. Invoke the HAL API wifi_getApAssociatedDeviceDiagnosticResult3() to retrieve the MAC Address of an associated client of 5G private AP.
...
4. Unload the modules</automation_approch>
    <expected_output>The HAL API wifi_getApAssociatedClientDiagnosticResult() with the connected client MAC address should be invoked successfully and the client diagnostic results should be retrieved for 5G Private AP.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetApAssociatedClientDiagnosticResult</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetApAssociatedClientDiagnosticResult');
