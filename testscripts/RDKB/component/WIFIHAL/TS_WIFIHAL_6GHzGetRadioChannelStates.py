...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_6GHzGetRadioChannelStates</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetRadioChannels</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getRadioChannels() for 6G radio and check if a valid channel state is retrieved for each of the possible channels.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_798</test_case_id>
    <test_objective>Invoke the HAL API wifi_getRadioChannels() for 6G radio and check if a valid channel state is retrieved for each of the possible channels.</test_objective>
...
2. Invoke the HAL API wifi_getRadioPossibleChannels() for 6G radio to retrieve the possible radio channels.
3. Invoke the HAL API wifi_getRadioChannels() for 6G radio with the number of possible radio channels as input.
...
5. Unload the module</automation_approch>
    <expected_output>The HAL API wifi_getRadioChannels() should be invoked successfully for 6G radio and the states of all possible radio channels for 6G radio should be a valid value from {'1' : "CHAN_STATE_AVAILABLE", '2' : "CHAN_STATE_DFS_NOP_FINISHED", '3' : "CHAN_STATE_DFS_NOP_START", '4' : "CHAN_STATE_DFS_CAC_START", '5' : "CHAN_STATE_DFS_CAC_COMPLETED"}.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_6GHzGetRadioChannelStates</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_6GHzGetRadioChannelStates');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1 : Invoke the HAL API wifi_getRadioPossibleChannels() to get the possible radio channels for 6G radio";
