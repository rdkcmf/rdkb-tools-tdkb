...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzGetAPCapabilities</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetAPCapabilities</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getAPCapabilities() to retrieve the AP Capabilities such as RTS threshold support, Security modes supported, Onboarding methods supported, WMM support, UAPSD support, Interworking service support and BSS transition support for 5G Private AP.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_745</test_case_id>
    <test_objective>Invoke the HAL API wifi_getAPCapabilities() to retrieve the AP Capabilities such as RTS threshold support, Security modes supported, Onboarding methods supported, WMM support, UAPSD support, Interworking service support and BSS transition support for 5G Private AP.</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>apIndex : 5G private AP index</input_parameters>
    <automation_approch>1. Load the module
2. Invoke the HAL API wifi_getAPCapabilities() for 5G private AP and check if the API invocation is success.
...
4. Unload the module</automation_approch>
    <expected_output>Invocation of  the HAL API wifi_getAPCapabilities() to retrieve the AP Capabilities such as RTS threshold support, Security modes supported, Onboarding methods supported, WMM support, UAPSD support, Interworking service support and BSS transition support should be success for 5G Private AP and values retrieved should be non-empty.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzGetAPCapabilities</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetAPCapabilities');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getAPCapabilities() to retrieve the 5G Private AP Capabilities";
