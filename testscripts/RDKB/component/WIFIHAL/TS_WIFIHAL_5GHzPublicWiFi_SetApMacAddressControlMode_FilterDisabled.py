...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzPublicWiFi_SetApMacAddressControlMode_FilterDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamUIntValue</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke wifi_setApMacAddressControlMode HAL API to set the filter control mode to Disabled for 5GHz Public WiFi and check if the set value is getting reflected to the get API wifi_getApMacAddressControlMode.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_595</test_case_id>
    <test_objective>Invoke wifi_setApMacAddressControlMode HAL API to set the filter control mode to Disabled for 5GHz Public WiFi and check if the set value is getting reflected to the get API wifi_getApMacAddressControlMode.</test_objective>
...
4. Invoke the function WIFIHAL_GetOrSetParamIntValue which in turn invokes the HAL API wifi_setApMacAddressControlMode() to set the filter control mode to 0 - Filter Disabled.
...
7. Unload the modules.</automation_approch>
    <expected_output>The Disabled filter control mode set using the HAL API wifi_setApMacAddressControlMode for 5GHz Public WiFi should be successful and the value should get reflected when the get API wifi_getApMacAddressControlMode is invoked.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzPublicWiFi_SetApMacAddressControlMode_FilterDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzPublicWiFi_SetApMacAddressControlMode_FilterDisabled');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzPublicWiFi_SetApMacAddressControlMode_FilterDisabled');
    ...
    if expectedresult in actualresult and details != "":
        ...
        if expectedresult in actualresult and details != "":
            ...
            if initMode == 0 or initMode == 1 or initMode == 2:
                ...
                setMode = 0;
                print "\nTEST STEP 3: Set to Disabled filter mode by invoking the API wifi_setApMacAddressControlMode for 5GHz Public WiFi";
