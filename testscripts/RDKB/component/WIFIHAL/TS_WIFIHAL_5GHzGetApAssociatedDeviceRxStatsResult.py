...
<xml>
  ...
  <name>TS_WIFIHAL_5GHzGetApAssociatedDeviceRxStatsResult</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Validate the WiFi HAL API wifi_getApAssociatedDeviceRxStatsResult for 5GHz AP</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_365</test_case_id>
    <test_objective>To validate the WiFi HAL API wifi_getApAssociatedDeviceRxStatsResult for 5GHz AP</test_objective>
...
3.WiFI Client should be connected</pre_requisite>
    ...
    <input_parameters>radioIndex - Radio Index Value
MAC - MAC Address of Associated Device</input_parameters>
...
8. Unload wifihal module </automation_approch>
    ...
    <test_script>TS_WIFIHAL_5GHzGetApAssociatedDeviceRxStatsResult</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzGetApAssociatedDeviceRxStatsResult');
...
if "SUCCESS" in loadmodulestatus.upper():
	...
	if expectedresult in initial_result:
	    ...
	    if initial_stat =="Disabled":
   	        #Enable the RadioStat value which is pre_requisite to get the ApAssoiatedDeviceRxStatsResult
	        value_to_set = 1;
