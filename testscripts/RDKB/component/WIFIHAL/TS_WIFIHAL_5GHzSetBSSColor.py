...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WIFIHAL_5GHzSetBSSColor</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_SetBSSColor</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_setBSSColor() to set a BSS Color in the acceptable range of 1-63 and check if the value set is reflected in the GET API wifi_getBSSColor() for 5G radio.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_783</test_case_id>
    <test_objective>Invoke the HAL API wifi_setBSSColor() to set a BSS Color in the acceptable range of 1-63 and check if the value set is reflected in the GET API wifi_getBSSColor() for 5G radio.</test_objective>
...
6. Unload the module</automation_approch>
    <expected_output>The HAL API wifi_setBSSColor() should be successfully invoked to set BSS Color in the acceptable range of 1-63 and value set should be reflected in the GET API wifi_getBSSColor() for 5G radio.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifihal</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzSetBSSColor</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetBSSColor');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getBSSColor() for 5G radio";
        ...
        if expectedresult in actualresult:
            ...
            if bssColor.isdigit() and 1 <= int(bssColor) and int(bssColor) <= 63:
                ...
                if expectedresult in actualresult:
                    ...
                    print "\nTEST STEP 4: Invoke the HAL API wifi_getBSSColor() for 5G radio after the SET operation";
