...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_764</test_case_id>
    <test_objective>Invoke the HAL API wifi_setApSecurity() to set the security parameters when the security mode is set to "WPA3-Personal-Transition" and check if the SET operation of the mfp, encryption mode, key type, key passphrase and the WPA3 transition toggle is success and is getting reflected when the GET  API wifi_getApSecurity() is invoked for 2.4G private AP.</test_objective>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetApSecurity_WPA3-Personal-Transition');
