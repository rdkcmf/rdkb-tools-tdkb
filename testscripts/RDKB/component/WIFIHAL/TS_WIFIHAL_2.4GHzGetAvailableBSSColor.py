...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIHAL_2.4GHzGetAvailableBSSColor</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetAvailableBSSColor</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Invoke the HAL API wifi_getAvailableBSSColor for 2.4G radio and check if the color list is non-empty if number of colors returned is greater than 0 or empty if the colors returned is equal to 0.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_741</test_case_id>
    <test_objective>Invoke the HAL API wifi_getAvailableBSSColor for 2.4G radio and check if the color list is non-empty if number of colors returned is greater than 0 or empty if the colors returned is equal to 0.</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>radioIndex : radio index of 2.4G
maxNumberColors : 63</input_parameters>
    <automation_approch>1. Load the module
2. Invoke the HAL API wifi_getAvailableBSSColor() for 2.4G radio and check if the API returns success.
...
5. Unload the module.</automation_approch>
    ...
    <test_script>TS_WIFIHAL_2.4GHzGetAvailableBSSColor</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzGetAvailableBSSColor');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if idx == -1:
        ...
        print "\nTEST STEP 1: Invoke the HAL API wifi_getAvailableBSSColor() to retrieve the Color List for 2.4G radio";
