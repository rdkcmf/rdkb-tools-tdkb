...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIHAL_5GHzSetZeroRadioAutoChannelRefreshPeriod</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIHAL_GetOrSetParamULongValue</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the auto channel refresh period  to zero if the automatic channel selection is enabled for 5GHz</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_460</test_case_id>
    <test_objective>This test case is to set the auto channel refresh period  to zero if the automatic channel selection is enabled for 5GHz</test_objective>
...
4. If supported, invoke "WIFIHAL_GetOrSetParamBoolValue" to get if auto channel selection is enabled (getAutoChannelEnable)
5. If not enabled, enable it using setAutoChannelEnable
6. Check if AutoChannelRefreshPeriod is supported, if yes get AutoChannelRefreshPeriod ,getRadioChannel,
7. Set AutoChannelRefreshPeriod to zero and get it , if both are same return SUCCESS
8. Sleep for the previously set refresh period.
9. Get the radio possible channels using wifi_getRadioPossibleChannels()
10. getRadioChannel and check if the channel returned is from the list of possible channels. If yes, return SUCCESS.
11. If not, return failure
12. Unload wifihal module</automation_approch>
    <expected_output>The api call is expected to be success when  AutoChannelRefreshPeriod is set to zero  with AutoChannelEnable supported and enabled</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIHAL</test_stub_interface>
    <test_script>TS_WIFIHAL_5GHzSetZeroRadioAutoChannelRefreshPeriod</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_5GHzSetZeroRadioAutoChannelRefreshPeriod');
...
def getRadioPossibleChannels(idx) :
        ...
        if expectedresult in actualresult:
            ...
            #Set the result status of execution^M
