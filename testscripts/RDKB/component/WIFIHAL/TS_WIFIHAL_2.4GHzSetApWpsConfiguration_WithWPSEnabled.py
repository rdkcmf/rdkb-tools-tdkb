...
<xml>
  ...
  <name>TS_WIFIHAL_2.4GHzSetApWpsConfiguration_WithWPSEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Invoke the HAL API wifi_setApWpsConfiguration() and set the WPS Mode to enabled, WPS PIN to a valid value and WPS Methods to Display and PushButton and cross check if the WPS configuration SET is reflected in the GET API wifi_getApWpsConfiguration() for 2.4G private AP.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIHAL_779</test_case_id>
    <test_objective>Invoke the HAL API wifi_setApWpsConfiguration() and set the WPS Mode to enabled, WPS PIN to a valid value and WPS Methods to Display and PushButton and cross check if the WPS configuration SET is reflected in the GET API wifi_getApWpsConfiguration() for 2.4G private AP.</test_objective>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetApWpsConfiguration_WithWPSEnabled');
sysobj.configureTestCase(ip,port,'TS_WIFIHAL_2.4GHzSetApWpsConfiguration_WithWPSEnabled');
        ...
        if expectedresult in actualresult and "getApWpsConfiguration invoked successfully" in details:
            ...
            if status == "SUCCESS":
                ...
                if expectedresult in actualresult and new_pin.isdigit():
                    ...
                    print "\nTEST STEP 4 : Invoke the HAL API wifi_setApWpsConfiguration() for 2.4G private AP with WPS Mode : %s, WPS PIN : %s and WPS Methods : %s(After combining Display-0x0008 and PushButton-0x0080 methods)" %(setvalues[0], setvalues[1], setvalues[2]);
                    ...
                    if expectedresult in actualresult:
                        ...
                        actualresult, details, tdkTestObj = getApWpsConfiguration(obj, idx);
                        print "\nTEST STEP 5: Invoke the HAL API wifi_getApWpsConfiguration() for 2.4G private AP after the SET operation";
