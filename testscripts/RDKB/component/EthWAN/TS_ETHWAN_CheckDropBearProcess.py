...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_ETHWAN_CheckDropBearProcess</name>
  <primitive_test_id/>
  <primitive_test_name>EthWAN_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Dropbear process must be running in ethwan so that ssh to device works</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_ETHWAN_10</test_case_id>
    <test_objective>Dropbear process must be running in ethwan so that ssh to device works</test_objective>
...
3. If ethwan is enabled, check if Dropbear process is running or not
4. Unload module</automation_approch>
    <except_output>Drop bear process should be running in ethwan mode</except_output>
    <priority>High</priority>
    <test_stub_interface>ETHWAN</test_stub_interface>
    <test_script>TS_ETHWAN_CheckDropBearProcess</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ETHWAN_CheckDropBearProcess');
obj1.configureTestCase(ip,port,'TS_ETHWAN_CheckDropBearProcess');
