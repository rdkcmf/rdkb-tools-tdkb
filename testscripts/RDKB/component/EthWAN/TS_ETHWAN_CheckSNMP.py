...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_ETHWAN_CheckSNMP</name>
  <primitive_test_id/>
  <primitive_test_name>EthWAN_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check whether SNMP support is available in ETHWAN Mode.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_ETHWAN_05</test_case_id>
    <test_objective>Check whether SNMP support is available in ETHWAN Mode.</test_objective>
...
3. If ethwan is enabled, get erouter0  ip using snmp to check if snmp is working or not.
4. Unload module</automation_approch>
    <except_output>SNMP should  not be working in ethwan mode</except_output>
    <priority>High</priority>
    <test_stub_interface>ETHWAN</test_stub_interface>
    <test_script>TS_ETHWAN_CheckSNMP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ETHWAN_CheckSNMP');
obj1.configureTestCase(ip,port,'TS_ETHWAN_CheckSNMP');
