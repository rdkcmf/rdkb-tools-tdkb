...
<xml>
  ...
  <test_cases>
    ...
    <test_objective>Using webpa get the bridge standard and check if its from the list {802.1D-2004 ,802.1Q-2005, 802.1Q-2011}</test_objective>
...
parseWebpaResponse</api_or_interface_used>
    <input_parameters>Device.Bridging.Bridge.1.Standard</input_parameters>
    <automation_approch>1. Load sysutil module
2. Configure WEBPA server to send get request for Device.Bridging.Bridge.1.Standard
...
5. Unload sysutil module</automation_approch>
    <except_output>Get operation status should be SUCCESS and standard returned should be from the list {802.1D-2004 ,802.1Q-2005, 802.1Q-2011}</except_output>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBPA_GetBridgeStandard');
