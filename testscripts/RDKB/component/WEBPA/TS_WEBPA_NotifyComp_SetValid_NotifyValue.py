...
<xml>
  ...
  <name>TS_WEBPA_NotifyComp_SetValid_NotifyValue</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify that set-attributes request with notify value 0 and 1 should return "Success" message.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WEBPA_29</test_case_id>
    <test_objective>To verify that set-attributes request with notify value 0 and 1 should return "Success" message.</test_objective>
    ...
    <input_parameters>webpaQuery
parseWebpaResponse</input_parameters>
    <automation_approch>1. Load sysutil module
2. Configure WEBPA server to set-attributes request with notify value 0  for Device.NotifyComponent.X_RDKCENTRAL-COM_Connected-Client
3. Parse the WEBPA response and get the response
4.Check if it returns success message for notify value 0
5. Configure WEBPA server to set-attributes request with notify value 1  for Device.NotifyComponent.X_RDKCENTRAL-COM_Connected-Client
6. Parse the WEBPA response and get the response
7.Check if it returns success message for notify value 1
8. Unload sysutil module</automation_approch>
    <except_output>set-attributes request with notify value 0 and 1 should return "Success" message.</except_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_WEBPA_NotifyComp_SetValid_NotifyValue</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBPA_NotifyComp_SetValid_NotifyValue');
