...
<xml>
  ...
  <name>TS_WEBPA_2.4GHzSetBandSteeringPhyRateThreshold</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Using webpa, set the BandSteering PhyRateThreshold for 2.4GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WEBPA_35</test_case_id>
    <test_objective>Using webpa, set the BandSteering PhyRateThreshold for 2.4GHZ</test_objective>
...
parseWebpaResponse</api_or_interface_used>
    <input_parameters>Device.WiFi.X_RDKCENTRAL-COM_BandSteering.BandSetting.1.PhyRateThreshold</input_parameters>
...
3. Parse the WEBPA response and save the 2.4GHZ bandsteering PhyRate Threshold
4.  Set a value as 40 using webpa set query
5. Get the new PhyRateThreshold for 2.4GHZ and compare that with the set value, both should be same
6. Revert the PhyRateThreshold
7. Unload sysutil module</automation_approch>
    <expected_output>PhyRateThreshold set operation for 2.4GHZ should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_WEBPA_2.4GHzSetBandSteeringPhyRateThreshold</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBPA_2.4GHzSetBandSteeringPhyRateThreshold');
