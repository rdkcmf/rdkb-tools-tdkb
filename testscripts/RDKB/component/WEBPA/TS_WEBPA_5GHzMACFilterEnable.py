...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WEBPA_5GHzMACFilterEnable</name>
  <primitive_test_id/>
  <primitive_test_name>WEBPA_Donothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To Check the enable and disable feature  of mac filter enable for 5GHz</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBPA_31</test_case_id>
    <test_objective>Using WEBPA get and set the state of  Mac Filter for 5GHz</test_objective>
...
webpaPreRequisite</api_or_interface_used>
    <input_parameters>Device.WiFi.AccessPoint.10002.X_CISCO_COM_MACFilter.Enable</input_parameters>
    <automation_approch>1. Load sysutil module
2. Configure WEBPA server to send get request for Device.WiFi.AccessPoint.10002.X_CISCO_COM_MACFilter.Enable.
...
7. Using sysutil ExecuteCmd command set the current state.
...
10. Unload sysutil module</automation_approch>
    ...
    <test_script>TS_WEBPA_5GHzMACFilterEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBPA_5GHzMACFilterEnable');
...
if "SUCCESS" in result.upper() :
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        print "TEST STEP 1: Get and save the state of Mac Filter for 5GHz"
        queryParam = {"name":"Device.WiFi.AccessPoint.10002.X_CISCO_COM_MACFilter.Enable"}
        ...
        if "SUCCESS" in parsedResponse[0] and parsedResponse[1] != "":
              ...
              print "Mac Filter for 5 GHz's State: ",OrgValue;
              #toggling by using set
              print "TEST STEP 2: Toggling the value"
              if parsedResponse[1] == "false":
                 flag="true"
                 queryParam = {"name":"Device.WiFi.AccessPoint.10002.X_CISCO_COM_MACFilter.Enable","value":flag,"dataType":3}
              ...
              if "SUCCESS" in getResponse[0] and getResponse[1] != "" and getResponse[1]== flag:
                 ...
                 print "Mac Filter  for 5GHz 's  State: ",Value;
              else:
              ...
              queryParam = {"name":"Device.WiFi.AccessPoint.10002.X_CISCO_COM_MACFilter.Enable","value":OrgValue,"dataType":3}
              queryResponse = webpaQuery(obj, queryParam,"set")
              setResponse = parseWebpaResponse(queryResponse, 1,"set")
              tdkTestObj.executeTestCase("SUCCESS");
