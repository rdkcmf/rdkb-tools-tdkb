...
<xml>
  ...
  <name>TS_WEBPA_GetDeviceDescription</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Use webpa to get the device info description of the gateway</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WEBPA_3</test_case_id>
    <test_objective>Use webpa to get the device info description of the gateway</test_objective>
...
parseWebpaResponse</api_or_interface_used>
    <input_parameters>Device.DeviceInfo.Description</input_parameters>
    <automation_approch>1. Load sysutil module
2. Configure WEBPA server to send get request for Device.DeviceInfo.Description
...
5. Unload sysutil module</automation_approch>
    <except_output>WEBPA response status should be SUCCESS</except_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_WEBPA_GetDeviceDescription</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBPA_GetDeviceDescription');
...
if "SUCCESS" in result.upper() :
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        queryParam = {"name":"Device.DeviceInfo.Description"}
