...
<xml>
  ...
  <name>TS_WEBPA_EPON_GetDPoE_MAC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Using webpa get the device MACAddress from EPON parameters</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WEBPA_8</test_case_id>
    <test_objective>Using webpa get the device MACAddress from EPON parameters</test_objective>
...
parseWebpaResponse</api_or_interface_used>
    <input_parameters>Device.DPoE.Mac_address</input_parameters>
    <automation_approch>1. Load sysutil module
2. Configure WEBPA server to send get request for Device.DPoE.Mac_address
...
5. Unload sysutil module</automation_approch>
    ...
    <test_script>TS_WEBPA_EPON_GetDPoE_MAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBPA_EPON_GetDPoE_MAC');
...
if "SUCCESS" in result.upper() :
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        queryParam = {"name":"Device.DPoE.Mac_address"}
