...
<xml>
  ...
  <name>TS_TAD_IPPingTest_InvalidHostName</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set host name of IPPing to invalid and check whether diagnostic state is set to Error_CannotResolveHostName</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TAD_57</test_case_id>
    <test_objective>To set host name of IPPing to invalid and check whether diagnostic state is set to Error_CannotResolveHostName</test_objective>
...
2. From script invoke TADstub_Set with type as string and a host name as www.invalidurl.c
3. Invoke TADstub_Get() to check for the  values from Device.IP.Diagnostics.IPPing.DiagnosticsState
4. check whether status is Error_CannotResolveHostName
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_IPPingTest_InvalidHostName');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        print "TEST STEP 1: Set ping host URL"
        print "EXPECTED RESULT 1: Should set the host URL";
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: Set ping test run value as true"
            print "EXPECTED RESULT 2: Should set the ping test run value as true";
            ...
            if expectedresult in actualresult and details == "Error_CannotResolveHostName":
               #Set the result status of execution
               tdkTestObj.setResultStatus("SUCCESS");
               print "TEST STEP 3: Get ping test diagnostic state"
               print "EXPECTED RESULT 3: Should get the ping test diagnostic state as Error_CannotResolveHostName";
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print "TEST STEP 2: Set ping test run value as true"
            print "EXPECTED RESULT 2: Should set ping test run value as true";
            print "ACTUAL RESULT 2:  %s" %details;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : FAILURE";
        ...
        tdkTestObj.setResultStatus("FAILURE");
        print "TEST STEP 1: Set ping host URL"
        print "EXPECTED RESULT 1: Should set the ping host URL"
        print "ACTUAL RESULT 1:  %s" %details;
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : FAILURE";
