...
<xml>
  ...
  <name>TS_TAD_API_Traceroute_GetState</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>TADstub_GetState</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the functionality of diag_getstate() function in trace route</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TAD_40</test_case_id>
    <test_objective>To verify the functionality of diag_getstate() api in traceroute</test_objective>
    <test_type>Positive</test_type>
    <test_setup>xb3, emulator</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components.
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>diag_getstate()</api_or_interface_used>
...
3. Invoke TADstub_GetState() to check if the state is reset to DIAG_ST_NONE
4. check whether set , getstate returned success
...
6.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from TAD stub.</automation_approch>
    <except_output>Check if TADstub_GetState() returns the state as DIAG_ST_NONE</except_output>
    <priority>High</priority>
    <test_stub_interface>TAD</test_stub_interface>
    <test_script>TS_TAD_API_Traceroute_GetState</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_API_Traceroute_GetState');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if host == "NULL":
        ...
        if expectedresult in actualresult:
            ...
            #Check for getState output
            tdkTestObj = obj.createTestStep('TADstub_GetState');
            #setting mode 1 for traceroute
            tdkTestObj.addParameter("mode",1);
            ...
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
            if expectedresult in actualresult and "state is 0" in details:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 1: Get the state of traceroute object"
                print "EXPECTED RESULT 1: Should get the state of traceroute object"
                print "ACTUAL RESULT 1:  %s" %details;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";
            else:
