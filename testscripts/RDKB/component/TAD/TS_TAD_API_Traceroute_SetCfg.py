...
<xml>
  ...
  <name>TS_TAD_API_Traceroute_SetCfg</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To verify the functionality of diag_setcfg() api for trace route</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
6.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from TAD stub.</automation_approch>
    ...
    <test_script>TS_TAD_API_Traceroute_SetCfg</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_API_Traceroute_SetCfg');
