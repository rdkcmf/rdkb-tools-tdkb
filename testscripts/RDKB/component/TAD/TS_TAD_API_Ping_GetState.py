...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To test the functionality of diag_getstate() function in ping</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
6.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from TAD stub.</automation_approch>
    ...
    <test_script>TS_TAD_API_Ping_GetState</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_API_Ping_GetState');
    ...
    if host == "NULL":
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult and "state is 0" in details:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 1: Get the state of ping object"
                print "EXPECTED RESULT 1: Should get the state of ping object"
                print "ACTUAL RESULT 1:  %s" %details;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";
            else:
