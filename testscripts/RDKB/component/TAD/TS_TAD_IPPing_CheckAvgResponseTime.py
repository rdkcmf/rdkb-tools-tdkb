...
<xml>
  <id/>
  <version>2</version>
  <name>TS_TAD_IPPing_CheckAvgResponseTime</name>
  <primitive_test_id/>
  <primitive_test_name>TADstub_Get</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the average response time is greater than zero after a successful ping test</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TAD_21</test_case_id>
    <test_objective>To check if the average response time is greater than zero after a successful ping test</test_objective>
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_TAD_IPPing_CheckAvgResponseTime</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_IPPing_CheckAvgResponseTime');
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult:
                ...
                #Set the result status of execution
                    ...
                    time.sleep(30);
                    tdkTestObj = obj.createTestStep('TADstub_Get');
         	    tdkTestObj.addParameter("paramName","Device.IP.Diagnostics.IPPing.AverageResponseTime");
                    expectedresult="SUCCESS";
                    tdkTestObj.executeTestCase(expectedresult);
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "TEST STEP 3: Set diagnostic state of IPPing as requested ";
                    print "EXPECTED RESULT 3: Should set diagnostic state of IPPing as requested ";
                    print "ACTUAL RESULT 3: %s" %details;
            	    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                ...
                tdkTestObj.setResultStatus("FAILURE");
           	print "TEST STEP 2: Set the host of IPPing";
           	print "EXPECTED RESULT 2: Should set the host of IPPing as requested";
                print "ACTUAL RESULT 2: %s" %details;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            #Set the result status of execution
            tdkTestObj.setResultStatus("FAILURE");
            print "TEST STEP 1: Set the interface IPPing";
            print "EXPECTED RESULT 1: Should set the interface of IPPing as requested";
            print "ACTUAL RESULT 1: %s" %details;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : FAILURE";
