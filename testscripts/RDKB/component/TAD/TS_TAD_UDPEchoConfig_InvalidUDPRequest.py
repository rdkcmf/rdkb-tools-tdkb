...
<xml>
  <id/>
  <version>1</version>
  <name>TS_TAD_UDPEchoConfig_InvalidUDPRequest</name>
  <primitive_test_id/>
  <primitive_test_name>TADstub_Set</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that UDPEcho is failing with an invalid UDP Request. That is a request from an IP address other than the source IP address.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TAD_45</test_case_id>
    <test_objective>To verify that UDPEcho is failing with an invalid UDP Request. That is a request from an IP address other than the source IP address.</test_objective>
...
4. Send an invalid UDP echo request with source ip address
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_TAD_UDPEchoConfig_InvalidUDPRequest</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_UDPEchoConfig_InvalidUDPRequest');
sysObj.configureTestCase(ip,port,'TS_TAD_UDPEchoConfig_InvalidUDPRequest');
...
if "SUCCESS" in loadmodulestatus.upper() and sysloadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
		    ...
		    invalidip = obj.url.split('/')[2].split(':')[0];
                    command = "echo UDPEchoTest > /dev/udp/"+invalidip+"/7";
                    print command;
                    ...
                    if expectedresult in actualresult:
                        #Set the result status of execution
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 5: Send an invalid UDP request";
                        ...
                        if expectedresult in actualresult and int(pkts_rcvd)==0:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "TEST STEP 6: Check if no packets are received with invalid UDP request";
                            print "EXPECTED RESULT 6: Should not receive any packets with invalid UDP request";
