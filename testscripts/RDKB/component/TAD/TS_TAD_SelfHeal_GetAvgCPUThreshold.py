...
<xml>
  <id/>
  <version>1</version>
  <name>TS_TAD_SelfHeal_GetAvgCPUThreshold</name>
  <primitive_test_id/>
  <primitive_test_name>TADstub_Get</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>Change the value of Device.SelfHeal.ResourceMonitor.X_RDKCENTRAL-COM_AvgCPUThreshold to 30. Do factory reset and check if the value changed to default value. ie, 100</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TAD_70</test_case_id>
    <test_objective>Change the value of Device.SelfHeal.ResourceMonitor.X_RDKCENTRAL-COM_AvgCPUThreshold to 30. Do factory reset and check if the value changed to default value. ie, 100</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband,Emulator,RPI</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components.
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>TADstub_Get</api_or_interface_used>
    <input_parameters>Device.SelfHeal.ResourceMonitor.X_RDKCENTRAL-COM_AvgCPUThreshold
Device.X_CISCO_COM_DeviceControl.FactoryReset
</input_parameters>
    <automation_approch>1. Load TAD modules
2. Get and save the value of average CPU threshold
3. Set a random value to average CPU threshold
4. Initiate factory reset to obtain the default values of all parameters
5. Check if the average CPU threshold value is changed to default value
6. Revert the value to initial value
7.Unload module</automation_approch>
    <except_output>The average CPU threshold value should change to default value after factory reset</except_output>
    <priority>High</priority>
    <test_stub_interface>TAD</test_stub_interface>
    <test_script>TS_TAD_SelfHeal_GetAvgCPUThreshold</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_SelfHeal_GetAvgCPUThreshold');
pamobj.configureTestCase(ip,port,'TS_TAD_SelfHeal_GetAvgCPUThreshold');
