...
<xml>
  ...
  <name>TS_TAD_IPPingTest_CheckSuccessAndResponse</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check the success count and response time of ping test done using Device.IP.Diagnostics.X_RDKCENTRAL-COM_PingTest.Run</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TAD_53</test_case_id>
    <test_objective>Check the success count and response time of ping test done using Device.IP.Diagnostics.X_RDKCENTRAL-COM_PingTest.Run</test_objective>
...
5. Set Device.IP.Diagnostics.X_RDKCENTRAL-COM_PingTest.Run as true to start the ping test
6. After ping test, check of the success count, average response time, minimum response time, maximum response time is greater than zero
7. Unload the TAD module</automation_approch>
    <except_output>Success count, average response time, minimum response time and maximum response time should be greater than zero</except_output>
    <priority>High</priority>
    <test_stub_interface>tad</test_stub_interface>
    <test_script>TS_TAD_IPPingTest_CheckSuccessAndResponse</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_IPPingTest_CheckSuccessAndResponse');
    ...
    if expectedresult in actualresult:
        if host == "NULL":
		 ...
		 paramList=["Device.IP.Diagnostics.IPPing.SuccessCount", "Device.IP.Diagnostics.IPPing.AverageResponseTime", "Device.IP.Diagnostics.IPPing.MinimumResponseTime", "Device.IP.Diagnostics.IPPing.MaximumResponseTime"]
                 ...
                 print "EXPECTED RESULT 4: successcount, AverageResponseTime, MinimumResponseTime, MaximumResponseTime of ping test should be greater than 0"
