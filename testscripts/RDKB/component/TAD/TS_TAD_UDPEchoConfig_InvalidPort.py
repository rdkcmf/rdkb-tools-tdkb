...
<xml>
  <id/>
  <version>2</version>
  <name>TS_TAD_UDPEchoConfig_InvalidPort</name>
  <primitive_test_id/>
  <primitive_test_name>TADstub_Set</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if UDPEcho is failing with an invalid port number.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TAD_42</test_case_id>
    <test_objective>To check if UDPEcho is failing with an invalid port number.</test_objective>
...
2. From script invoke TADstub_Set to set an invalid port number 
3. Enable the UDPEchoConfig
4. Send UDP echo request with source ip address
5. verify that no packets are received
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_TAD_UDPEchoConfig_InvalidPort</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_UDPEchoConfig_InvalidPort');
sysObj.configureTestCase(ip,port,'TS_TAD_UDPEchoConfig_InvalidPort');
...
if "SUCCESS" in loadmodulestatus.upper()and sysloadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
	    ...
	    tdkTestObj = obj.createTestStep('TADstub_Set');
            ...
            tdkTestObj.addParameter("ParamValue","2333");
		    ...
		    tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in actualresult and int(pkts_rcvd)==0:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "TEST STEP 6: Check if no bytes are received with invalid port";
                            print "EXPECTED RESULT 6: Should not receive any bytes";
