...
<xml>
  ...
  <name>TS_TAD_IPPing_SetInvalidInterface</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set the invalid interface name and check whether the CPE uses the interface as directed by its bridging or routing policy to determine the appropriate interface</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TAD_24</test_case_id>
    <test_objective>To set the invalid interface name and check whether the CPE uses the interface as directed by its bridging or routing policy to determine the appropriate interface</test_objective>
...
Device.IP.Diagnostics.Ipping.SuccessCount</input_parameters>
    <automation_approch>1. Load TAD modules
2. From script invoke TADstub_Set to set the host name as empty string
3. Check if the success count is greater than zero
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_TAD_IPPing_SetInvalidinterface</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TAD_IPPing_SetInvalidInterface');
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult and int(details)>0:
                        #Set the result status of execution
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 4: Get the success count";
                        print "EXPECTED RESULT 4: The success count must be greater than zero";
