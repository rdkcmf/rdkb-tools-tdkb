...
<xml>
  ...
  <name>TS_SANITY_CheckErrorLogs_UnaryOperator</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if any  syntax error messages are seen in SelfHeal.txt.0 like 'unary operator expected' Log Message</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_SYSUTIL_33</test_case_id>
    <test_objective>This test case is to check if any  syntax error messages are seen in SelfHeal.txt.0 like 'unary operator expected' Log Message</test_objective>
...
2. Check if SelfHeal.txt.0  file is present in /rdklogs/logs/
3. If file is present loop for 5 minutes and check if any  synatx error message  like -eq: unary operator  are seen.
4. If found print the Failure message else print the Success message.
5. Unload the Module</automation_approch>
    <expected_output>synatx error message  like -eq: unary operator  should not be present in SelfHeal.txt.0 </expected_output>
    <priority>High</priority>
    <test_stub_interface>SYSUTIL</test_stub_interface>
    <test_script>TS_SANITY_CheckErrorLogs_UnaryOperator</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SANITY_CheckErrorLogs_UnaryOperator');
