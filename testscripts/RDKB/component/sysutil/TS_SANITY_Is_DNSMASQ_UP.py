...
<xml>
  ...
  <name>TS_SANITY_Is_DNSMASQ_UP</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if dnsmasq.conf is present and dns process is running</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_SYSUTIL_29</test_case_id>
    <test_objective>To check if dnsmasq.conf is present and dns proces is running</test_objective>
...
2.Retrieve the dnsmasq conf file path from properties file
3.Check if dnsmasq conf file is present
4.Check dnsmasq process is running
5.Unload sysutil module</automation_approch>
    <expected_output>dnsmasq conf file should be present and dnsmasq process should be running</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_SANITY_Is_DNSMASQ_UP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SANITY_Is_DNSMASQ_UP');
