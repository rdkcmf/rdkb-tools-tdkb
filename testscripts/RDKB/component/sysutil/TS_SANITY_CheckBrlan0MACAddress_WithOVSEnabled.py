...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>TS_SANITY_CheckBrlan0MACAddress_WithOVSEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>ExecuteCmd</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Mac Address of Brlan0 interface remains unchanged when the OVS Enable is toggled using Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_SANITY_71</test_case_id>
    <test_objective>To check if the Mac Address of Brlan0 interface remains unchanged when the OVS Enable is toggled using Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable.</test_objective>
...
2. Get the initial OVS enable status using Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable
3. Get the brlan0 interface mac address in the initial OVS enable state
4. Toggle the OVS enable using Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable
5. Reboot the DUT for the OVS enable RFC to take effect
6. Once the DUT is up, check if the OVS enable is toggled successfully
7. Get the current brlan0 interface mac address
8. Compare the brlan0 mac address when the OVS is enabled and disabled. It should be the same
9. Revert the enable state using Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable and reboot the DUT for the RFC to take effect.
10. Unload the modules</automation_approch>
    <expected_output>The Mac Address of Brlan0 interface should remain unchanged when the OVS Enable is toggled using Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_SANITY_CheckBrlan0MACAddress_WithOVSEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_SANITY_CheckBrlan0MACAddress_WithOVSEnabled');
tr181obj.configureTestCase(ip,port,'TS_SANITY_CheckBrlan0MACAddress_WithOVSEnabled');
...
if  "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in actualresult and initial_enable != "":
        ...
        print "\nTEST STEP %d : Get the Brlan0 Mac Address when the OVS enable state is %s" %(step, initial_enable);
        print "EXPECTED RESULT %d : Should get the Brlan0 Mac Address successfully when OVS enable state is %s" %(step, initial_enable);
            ...
            if expectedresult in actualresult and details != "":
                ...
                if expectedresult in actualresult and curr_enable != "":
                    ...
                    if curr_enable == setValue:
                        ...
                        print "\nTEST STEP %d : Get the Brlan0 Mac Address when the OVS enable state is set to %s" %(step, initial_enable);
                        print "EXPECTED RESULT %d : Should get the Brlan0 Mac Address successfully when OVS enable state is set to %s" %(step, initial_enable);
                        ...
                        if expectedresult in actualresult and details != "":
                            ...
                            step = step + 1;
                            print "\nTEST STEP %d : Check if the Brlan0 Mac Address remain unchanged irrespective of the OVS Enable state" %(step);
                            print "EXPECTED RESULT %d : The Brlan0 Mac Address should remain unchanged irrespective of the OVS Enable state" %(step);
                            print "Brlan0 Mac Address when OVS is %s : %s" %(initial_enable, brlan0_mac_initial);
                            print "Brlan0 Mac Address when OVS is %s : %s" %(curr_enable, brlan0_mac_final);
                        ...
                        print "\nTEST STEP %d : Revert OVS Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable to %s" %(step, initial_enable);
                        print "EXPECTED RESULT %d : Should revert the OVS Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable to %s" %(step, initial_enable);
