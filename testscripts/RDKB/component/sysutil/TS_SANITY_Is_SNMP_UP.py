...
<xml>
  ...
  <name>TS_SANITY_Is_SNMP_UP</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test case will check whether SNMP process is running or not.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_SYSUTIL_14</test_case_id>
    <test_objective>This test case will check whether SNMP process is running or not.</test_objective>
    ...
    <input_parameters>sh /usr/ccsp/tdk/tdk_utility.sh parseConfigFile SNMP_PROCESS" %TDK_PATH</input_parameters>
    <automation_approch>1. Invoke tdk_utility.sh and parse the tdk_platform.properties file to get the list of SNMP process to be verified.
2. Check whether the SNMP process are running and get the PID of the process.
3. If process is running then set the script to success else failure</automation_approch>
    <except_output>SNMP process should be running and should return the PID</except_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_SANITY_Is_SNMP_UP</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SANITY_Is_SNMP_UP');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    snmpProcessList = tdkTestObj.getResultDetails().strip();
