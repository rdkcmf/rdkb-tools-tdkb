...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_SANITY_CheckLogLocation_PreserveLogsOFF</name>
  <primitive_test_id/>
  <primitive_test_name>ExecuteCmd</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the tar ball of Bootup Logs is moved to Nvram2 Logs folder when         Device.SelfHeal.X_RDKCENTRAL-COM_LogBackupThreshold is set to the value 0 and
Device.SelfHeal.X_RDKCENTRAL-COM_NoWaitLogSync is set to False by checking if the string "Keeping the tar in /nvram2/logs/ for non-xb3" is present in Consolelog.txt.0.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_SANITY_62</test_case_id>
    <test_objective>To check if the tar ball of Bootup Logs is moved to Nvram2 Logs folder when         Device.SelfHeal.X_RDKCENTRAL-COM_LogBackupThreshold is set to the value 0 and
Device.SelfHeal.X_RDKCENTRAL-COM_NoWaitLogSync is set to False by checking if the string "Keeping the tar in /nvram2/logs/ for non-xb3" is present in Consolelog.txt.0.</test_objective>
...
3. Set the values to 0 and false respectively and validate the Set operation with a Get.
4. Reboot the device
5. Once the device comes up, as the Preserve Logs RFC parameters are disabled, the tar ball of the bootup logs should come under logs folder in nvram2. This can be validated by checking if the string "Keeping the tar in /nvram2/logs/ for non-xb3" under Consolelog.txt.0.
6. Unload the modules.</automation_approch>
    <expected_output>The  log message "Keeping the tar in /nvram2/logs/ for non-xb3" should be present under Consolelog.txt.0 on Bootup of the device when Device.SelfHeal.X_RDKCENTRAL-COM_LogBackupThreshold is set to the value 0 and
Device.SelfHeal.X_RDKCENTRAL-COM_NoWaitLogSync is set to False and rebooted.</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_SANITY_CheckLogLocation_PreserveLogsOFF</test_script>
    <skipped>No</skipped>
...
def checkLogs(obj2, step):
    #Check if the string "Keeping the tar in /nvram2/logs/ for non-xb3" is present in Consolelog.txt.0
    status = 1;
    step = step + 1;
    string_to_check = "Keeping the tar in /nvram2/logs/ for non-xb3";
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SANITY_CheckLogLocation_PreserveLogsOFF');
obj1.configureTestCase(ip,port,'TS_SANITY_CheckLogLocation_PreserveLogsOFF');
obj2.configureTestCase(ip,port,'TS_SANITY_CheckLogLocation_PreserveLogsOFF');
