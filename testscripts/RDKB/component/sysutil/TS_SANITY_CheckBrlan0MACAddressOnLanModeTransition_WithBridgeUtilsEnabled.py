...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_SANITY_CheckBrlan0MACAddressOnLanModeTransition_WithBridgeUtilsEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>ExecuteCmd</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Brlan0 interface Mac Address remains unchanged when the Lan Mode is transitioned from router to bridge-static or vice-versa when the Bridge Utils Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable is enabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_SANITY_74</test_case_id>
    <test_objective>To check if the Brlan0 interface Mac Address remains unchanged when the Lan Mode is transitioned from router to bridge-static or vice-versa when the Bridge Utils Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable is enabled.</test_objective>
...
2. Get the initial enable state of BridgeUtils enable Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable.
3. If its not enabled initially, set Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable to true.
4. Reboot the DUT for the BridgeUtils Enable RFC to take effect
5. Once the DUT is up, check if the BridgeUtils RFC is successfully enabled
...
12. Revert the BridgeUtils enable state and reboot the DUT if required.
13. Unload the modules</automation_approch>
    <expected_output>The Brlan0 interface Mac Address should remain unchanged when the Lan Mode is transitioned from router to bridge-static or vice-versa when the BridgeUtils Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable is enabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_SANITY_CheckBrlan0MACAddressOnLanModeTransition_WithBridgeUtilsEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_SANITY_CheckBrlan0MACAddressOnLanModeTransition_WithBridgeUtilsEnabled');
tr181obj.configureTestCase(ip,port,'TS_SANITY_CheckBrlan0MACAddressOnLanModeTransition_WithBridgeUtilsEnabled');
...
if  "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in actualresult and initial_enable != "":
        ...
        #If Bridge Utils is not enabled initially, enable it
        proceed_flag = 0;
        revert_flag = 0;
        setValue = "true";
        ...
        if initial_enable != "true":
            ...
            print "Bridge Utils is already in enabled state, SET operation not required";
            proceed_flag = 1;
        ...
        if proceed_flag == 1:
                        ...
                        if expectedresult in actualresult and curr_mode != "":
                            ...
                            if curr_mode == setmode :
                                ...
                                if expectedresult in actualresult and details != "":
                                    ...
                                    print "Brlan0 Mac Address when Lan Mode is %s with Bridge Utils Enabled : %s" %(initial_mode, brlan0_mac_initial);
                                    print "Brlan0 Mac Address when Lan Mode is %s with Bridge Utils Enabled : %s" %(curr_mode, brlan0_mac_final);
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print "Unable to enable Bridge Utils, cannot proceed further...";
        ...
        if revert_flag == 1:
            ...
            print "\nTEST STEP %d : Set Bridge Utils Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable to %s" %(step, initial_enable);
            print "EXPECTED RESULT %d : Should set the Bridge Utils Enable RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.BridgeUtilsEnable to %s" %(step, initial_enable);
            ...
            print "\nBridge Utils Enable revert operation not required";
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print "ACTUAL RESULT %d: Initial Enable state is : %s" %(step, initial_enable);
        #Get the result of execution
