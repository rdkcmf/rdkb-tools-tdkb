...
<xml>
  ...
  <name>TS_SANITY_GetDeviceNamespaceAttributes</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test case will retrieve the full list of TR-181 parameter attributes under Device.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_SYSUTIL_13</test_case_id>
    <test_objective>This test case will retrieve the full list of TR-181 parameter attributes under Device.</test_objective>
    ...
    <api_or_interface_used>WIFIAgent_GetAttr</api_or_interface_used>
...
4.From python script, invoke WIFIAgent_GetAttr with param as Device.
5.Responses from the wifiagent stub function will be logged in Agent Console log.
6.Check whether all the TR-181 parameter attributes under Device. are listed
7.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from wifiagent stub.</automation_approch>
    <except_output>All the TR-181 parameters attributes under Device. should be listed successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_SANITY_GetDeviceNamespaceAttributes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SANITY_GetDeviceNamespaceAttributes');
obj1.configureTestCase(ip,port,'TS_SANITY_GetDeviceNamespaceAttributes');
...
if "SUCCESS" in loadmodulestatus.upper() and loadmodulestatus1.upper():
    ...
    NamespacesList = tdkTestObj.getResultDetails().strip();
    NamespacesList = NamespacesList.replace("\\n", "");
    ...
    if expectedresult in actualresult:
        ...
        for i in NamespacesList:
             ...
             if expectedresult in actualresult and details not in error_list:
                 ...
                 tdkTestObj.setResultStatus("SUCCESS");
                 print "TEST STEP : Query the full list of namespace attributes under Device.%s." %i;
                 print "EXPECTED RESULT : Should list the namespace attributes under Device.%s." %i;
                 print "ACTUAL RESULT : Listed the namespace attributes under Device.%s. successfully" %i;
                 print "[TEST EXECUTION RESULT] : SUCCESS";
             else :
                 #Set the result status of execution
                 ...
                 print "TEST STEP : Query the full list of namespace attributes under Device.%s." %i;
                 print "EXPECTED RESULT : Should list the namespace attributes under Device.%s." %i;
                 print "ACTUAL RESULT : Failed to list the namespace attributes under Device.%s." %i;
