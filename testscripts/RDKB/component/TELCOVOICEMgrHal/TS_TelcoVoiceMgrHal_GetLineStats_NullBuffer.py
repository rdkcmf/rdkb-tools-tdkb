...
<xml>
  ...
  <name>TS_TelcoVoiceMgrHal_GetLineStats_NullBuffer</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the statistics of Telco Voice Line is retrieved using the parameter Device.Services.VoiceService.1.VoiceProfile.1.Line.1.Stats and Null Buffer as structure input.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELCOVOICEMGRHAL_07</test_case_id>
    <test_objective>Check if the statistics of Telco Voice Line is retrieved using the parameter Device.Services.VoiceService.1.VoiceProfile.1.Line.1.Stats and Null Buffer as structure input.</test_objective>
...
json_hal_get_param</api_or_interface_used>
    <input_parameters>paramName : Device.Services.VoiceService.1.VoiceProfile.1.Line.1.Stats.
...
9. Return success if the values of the structure parameters are not retrieved.
10. Unload the Telco Voice HAL Module</automation_approch>
    <expected_output>Should not retrieve the values of structure parameters for Telco Voice Line stats if Null Buffer is passed as input.</expected_output>
    <priority>High</priority>
    <test_stub_interface>telcovoicemgrhal</test_stub_interface>
    <test_script>TS_TelcoVoiceMgrHal_GetLineStats_NullBuffer</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_TelcoVoiceMgrHal_GetLineStats_NullBuffer');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            print "EXPECTED RESULT 2: GetLineStats operation should be failed when Null Buffer is passed as input";
            print "ACTUAL RESULT 2: GetLineStats operation has failed, Details : ", line_stats;
            #Get the result of execution
            ...
            print "TEST STEP 2: Get the Line Stats";
            print "EXPECTED RESULT 2: GetLineStats operation should be failed when Null Buffer is passed as input";
            print "ACTUAL RESULT 2: GetLineStats operation is success, Details : ", line_stats;
            #Get the result of execution
