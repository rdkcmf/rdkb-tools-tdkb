...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_LMLite_SetDscpCountEnableAllAndInterval_CheckDSCPTotal</name>
  <primitive_test_id/>
  <primitive_test_name>LMLiteStub_Set_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the DSCP WAN value as "-1" to enable all DSCP WAN values using Device.X_RDK_WAN.Interface.1.Stats.DscpCountEnable and to check if Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal is populated with the total traffic count values for the count interval set using Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LMLite_26</test_case_id>
    <test_objective>To set the DSCP WAN value as "-1" to enable all DSCP WAN values using Device.X_RDK_WAN.Interface.1.Stats.DscpCountEnable and to check if Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal is populated with the total traffic count values for the count interval set using Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval.</test_objective>
...
5. Set Device.X_RDK_WAN.Interface.1.Stats.DscpCountEnable to "-1" to enable all DSCP WAN values and validate with get operation.
...
10.Check if the DSCP WAN value of each record is a valid value and is in the acceptable range of 0-63.
...
16. Unload the module</automation_approch>
    <expected_output>Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal should be updated with valid DSCP WAN traffic counts when Device.X_RDK_WAN.Interface.1.Stats.DscpCountEnable is set to -1 and Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval is set to a non-zero value.</expected_output>
    <priority>High</priority>
    <test_stub_interface>lmlite</test_stub_interface>
    <test_script>TS_LMLite_SetDscpCountEnableAllAndInterval_CheckDSCPTotal</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LMLite_SetDscpCountEnableAllAndInterval_CheckDSCPTotal');
...
if "SUCCESS" in loadmodulestatus1.upper():
    ...
    if pre_req_set == 1:
        ...
        if get_flag == 0:
            ...
            #Set DscpCountEnable to -1 to enable traffic counts for all DSCP WAN values
            step = step + 1;
            ...
            if expectedresult in actualresult :
                ...
                if expectedresult in actualresult :
                    ...
                    if expectedresult in actualresult and details != "":
                        ...
                        #Check if the data held in the DSCP records are valid
                        step = step + 1;
                        record_flag = 0;
                        print "\nTEST STEP %d : Check if the value of Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal retrieved is valid" %step;
                        print "EXPECTED RESULT %d : Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal data retrieved should be valid" %step;
                        ...
                        for record in range(0, num_dscp_records):
                            ...
                            #Check if DSCP WAN value is in the applicable DSCP WAN range
                            if dscp_wan.isdigit() and int(dscp_wan) >= 0 and int(dscp_wan) < 63 :
                                #Set the result status of execution
                                tdkTestObj.setResultStatus("SUCCESS");
                        ...
                        if record_flag == 0:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT %d : DSCP count total retrieved is valid" %(step);
                            print "TEST EXECUTION RESULT : SUCCESS";
                        else:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("FAILURE");
                            print "ACTUAL RESULT %d : DSCP count total retrieved is NOT valid" %(step);
