...
<xml>
  ...
  <test_cases>
    ...
    <test_objective>Device.Hosts.Host.1.IPv4AddressNumberOfEntries should be always 1. Should not be zero.</test_objective>
    ...
    if expectedresult in (actualresult1 and actualresult2):
	    ...
	    tdkTestObj = obj.createTestStep('LMLiteStub_Get');
		    ...
		    for i in range(0,len(List)):
		        n = int(List[i]);
		        tdkTestObj.addParameter("paramName","Device.Hosts.Host.%d.IPv4AddressNumberOfEntries" %n);
                        #Execute the test case in DUT
                        tdkTestObj.executeTestCase(expectedresult);
                        actualresult = tdkTestObj.getResult();
                        Details = tdkTestObj.getResultDetails();
		        ...
		        #Set the result status of execution
        	        tdkTestObj.setResultStatus("SUCCESS");
        	        print "TEST STEP 4: Get the IPv4AddressNumberOfEntries";
        	        print "EXPECTED RESULT 4: Should get the IPv4AddressNumberOfEntries as one";
        	        print "ACTUAL RESULT 4: IPv4AddressNumberOfEntries :%s" %Details;
        	        #Get the result of execution
        	        print "[TEST EXECUTION RESULT] : SUCCESS";
	            else:
	                #Set the result status of execution
