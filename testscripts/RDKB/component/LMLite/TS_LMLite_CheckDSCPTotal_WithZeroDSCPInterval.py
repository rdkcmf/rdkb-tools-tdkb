...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_LMLite_CheckDSCPTotal_WithZeroDSCPInterval</name>
  <primitive_test_id/>
  <primitive_test_name>LMLiteStub_Set_Get</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the DSCP Total traffic counts value Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal updates to empty when the DSCP Interval Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval is set to 0 and DSCP enabled values are set.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_LMLite_28</test_case_id>
    <test_objective>To check if the DSCP Total traffic counts value Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal updates to empty when the DSCP Interval Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval is set to 0 and DSCP enabled values are set.</test_objective>
...
8. After that query Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal and check if the traffic counts does not give an empty value.
9. Set Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval to 0 and validate with get.
10.Sleep for a duration of 100s interval set previously to wait for traffic counts to be updated.
11. Check if the traffic counts retrieved using Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal is empty.
...
15. Unload the module</automation_approch>
    <expected_output>DSCP Total traffic counts value Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal should update to empty when the DSCP Interval Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval is set to 0 and DSCP enabled values are set.</expected_output>
    <priority>High</priority>
    <test_stub_interface>lmlite</test_stub_interface>
    <test_script>TS_LMLite_CheckDSCPTotal_WithZeroDSCPInterval</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_LMLite_CheckDSCPTotal_WithZeroDSCPInterval');
            ...
            if expectedresult in actualresult :
                ...
                if expectedresult in actualresult :
                    ...
                    if expectedresult in actualresult and details != "":
                        ...
                        #Set the Count Interval as 0
                        step = step + 1;
                        dscpCountInterval_new = "0";
                        print "\nTEST STEP %d : Set the DSCP Count Interval using Device.X_RDK_WAN.Interface.1.Stats.DscpCountInterval to %s and validate the operation" %(step, dscpCountInterval_new);
                        print "EXPECTED RESULT %d : DSCP count interval set validation should be success" %step;

                        tdkTestObj = obj.createTestStep("LMLiteStub_Set_Get");
                        tdkTestObj.addParameter("ParamName",paramList[1]);
                        tdkTestObj.addParameter("ParamValue",dscpCountInterval_new);
                        tdkTestObj.addParameter("ParamType","unsignedint");
                        tdkTestObj.executeTestCase(expectedresult);
                        actualresult = tdkTestObj.getResult();
                        details = tdkTestObj.getResultDetails();
                        ...
                        if expectedresult in actualresult :
                            ...
                            #Sleep for the DSCP Count Interval set in the previous step for DSCP Total to be updated
                            sleep_time = int(dscpCountInterval);
                            print "Sleeping the DSCP Count Interval set in the preceding step: %ss for WAN Traffic Counts to be updated" %sleep_time;
                            sleep(sleep_time);

                            #Check if the DSCP Total traffic count is updated to an empty value
                            step = step + 1;
                            print "\nTEST STEP %d : Get the DSCP Count Total using Device.X_RDK_WAN.Interface.1.Stats.DscpCountTotal" %(step);
                ...
                if initial_values[1] != dscpCountInterval_new:
                    step = step + 1;
