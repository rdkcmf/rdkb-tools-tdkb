...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_LMLite_CheckLANClientIPAddress</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_LMLite_08</test_case_id>
    <test_objective>To check if the IP address of active LAN client is same as the ip obtained from box.</test_objective>
...
Device.Hosts.Host.%d.IPv4Address.1.IPAddress</input_parameters>
    <automation_approch>1. Load Lmlite modules
2. From script invoke LMLiteStub_Get to get the number of connected devices.
3. if it is greater than zero get the LAN client ip from box and also using namespace
4.Check whether both ips are same or not.
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_LMLite_CheckLANClientIPAddress</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
wifiobj.configureTestCase(ip,port,'TS_LMLite_CheckLANClientIPAddress');
...
if "SUCCESS" in (loadmodulestatus.upper() and wifiloadmodulestatus.upper()):
    ...
    if expectedresult in (actualresult1 and actualresult2):
	    ...
	    if expectedresult in actualresult and IP_details:
	        #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 2: Get the LAN client ip using arp command"
                print "EXPECTED RESULT 2: Should get the LAN client ip using arp command";
		    ...
		    for i in range(1,int(NoOfHosts)+1):
		        ...
		        if expectedresult in actualresult and ret ==0:
		            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "TEST STEP 5: Compare the LAN client IPs obtained";
                            print "EXPECTED RESULT 5: Both IPs should match";
                            print "ACTUAL RESULT 5: The LAN client IPs matched successfully";
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
		        else:
		            #Set the result status of execution
                            ...
                            print "TEST STEP 5: Compare the LAN client IPs obtained";
                            print "EXPECTED RESULT 5: Both IPs should match";
                            print "ACTUAL RESULT 5: The LAN client IPs are not matching";
                            #Get the result of execution
	        ...
	        #Set the result status of execution
                ...
                print "TEST STEP 2: Get the LAN client ip using arp command"
                print "EXPECTED RESULT 2: Should get the LAN client ip using arp command";
                print "ACTUAL RESULT 2:Failed to get the LAN client ip";
        else:
            #Set the result status of execution
            tdkTestObj.setResultStatus("FAILURE");
            print "TEST STEP 1: Get the number of active LAN clients connected";
