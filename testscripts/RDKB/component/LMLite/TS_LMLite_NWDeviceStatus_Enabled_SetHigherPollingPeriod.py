...
<xml>
  ...
  <name>TS_LMLite_NWDeviceStatus_Enabled_SetHigherPollingPeriod</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>The new polling period cannot be greater than current polling period if the Device.X_RDKCENTRAL-COM_Report.NetworkDevicesStatus.Enabled is true</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_LMLite_03</test_case_id>
    <test_objective>The new polling period cannot be greater than current polling period if the Device.X_RDKCENTRAL-COM_Report.NetworkDevicesStatus.Enabled is true</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>Device.X_RDKCENTRAL-COM_Report.NetworkDevicesStatus.PollingPeriod
Device.X_RDKCENTRAL-COM_Report.NetworkDevicesStatus.Enabled</input_parameters>
    <automation_approch>1. Load Lmlite modules
2. From script invoke LMLiteStub_Get to get the polling period and network device status
3. If the status is true, set the new polling period greater than the current polling period
4. If the status is false,set the status as true and set the new polling period greater than the current polling period
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_LMLite_NWDeviceStatus_Enabled_SetHigherPollingPeriod</test_script>
    <skipped>No</skipped>
	...
	if expectedresult in actualresult:
	    ...
	    if "true" in default:
		    ...
		    if expectedresult in actualresult:
                        #Set the result status of execution
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 3: Set new PollingPeriod as greater than current PollingPeriod";
