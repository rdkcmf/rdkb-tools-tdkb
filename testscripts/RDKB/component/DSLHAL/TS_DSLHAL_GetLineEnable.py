...
<xml>
  ...
  <name>TS_DSLHAL_GetLineEnable</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the parameter value of DSL Line (Device.DSL.Line.1.Enable) using JSON HAL APIs</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_DSLHAL_2</test_case_id>
    <test_objective>To get the parameter value of DSL Line (Device.DSL.Line.1.Enable) using JSON HAL APIs</test_objective>
...
3. Invoke the json_hal_add_param with the parameter name "Device.DSL.Line.1.Enable"
...
6. Initiate the JSON_SCHEMA_VALIDATION process to check whether JSON HAL response is proper with respect to DSL HAL schema
7. Return the parameter value only if above API's are success else failure.
8. Unload the DSL HAL Module
</automation_approch>
    <expected_output>Should be able to get the value of Device.DSL.Line.1.Enable</expected_output>
    <priority>High</priority>
    <test_stub_interface>dslhal</test_stub_interface>
    <test_script>TS_DSLHAL_GetLineEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_DSLHAL_GetLineEnable');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
        enable_status = tdkTestObj.getResultDetails();
