...
<xml>
  ...
  <name>TS_COSAMTA_GetBatteryInstalled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if battery is installed or not</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_COSAMTA_29</test_case_id>
    <test_objective>Check if battery is installed or not</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>CosaDmlMtaBatteryGetInstalled</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. Load Cosamta module
2. Check if the battery is installed or not
3. Return the install status
4. Unload Cosamta module</automation_approch>
    <except_output>Install status should return either TRUE or FALSE</except_output>
    <priority>High</priority>
    <test_stub_interface>cosamta</test_stub_interface>
    <test_script>TS_COSAMTA_GetBatteryInstalled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_COSAMTA_GetBatteryInstalled');
