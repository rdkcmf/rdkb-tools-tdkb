...
<xml>
  ...
  <name>TS_COSAMTA_GetBatteryActualCapacity</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the actual capacity of the battery is greater than zero if battery is installed</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_COSAMTA_28</test_case_id>
    <test_objective>Check if the actual capacity of the battery is greater than zero if battery is installed</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>CosaDmlMtaBatteryGetActualCapacity</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. Load Cosamta module
2. Check if the battery is installed or not
3. Get the actual capacity of the battery
4. Check if actual capacity of the battery is 0 if battery is not installed and greater than 0 if installed
5. Unload Cosamta module</automation_approch>
    <except_output>Actual capacity of the battery is 0 if battery is not installed and greater than 0 if installed</except_output>
    <priority>High</priority>
    <test_stub_interface>cosamta</test_stub_interface>
    <test_script>TS_COSAMTA_GetBatteryActualCapacity</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_COSAMTA_GetBatteryActualCapacity');
