...
<xml>
  ...
  <name>TS_SNMP_IsWifiPassphraseExposed</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if wifi passphrase is exposed through snmp</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_SNMP_PA_33</test_case_id>
    <test_objective>To check if wifi passphrase is exposed through snmp</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>"snmpget", "-v 2c", ".1.3.6.1.4.1.17270.50.2.2.3.3.1.3.10001"</input_parameters>
...
4.With snmpget get oid for passphrase and check if its empty
5.Responses from the snmplib  will be logged in Script log.
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_SNMP_IsWifiPassphraseExposed</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SNMP_IsWifiPassphraseExposed');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    passphrase =snmplib.SnmpExecuteCmd("snmpget", communityString, "-v 2c", ".1.3.6.1.4.1.17270.50.2.2.3.3.1.3.10001", ipaddress);
    tdkTestObj = obj.createTestStep('ExecuteCmd');
    tdkTestObj.executeTestCase("SUCCESS");

    if passphrase:
        tdkTestObj.setResultStatus("SUCCESS");
        print "TEST STEP 1:Execute snmpget for wifi passphrase";
        print "EXPECTED RESULT 1: snmpget should get return the wifi passphrase";
        print "ACTUAL RESULT 1: SUCCESS, passphrase received"
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : %s" %passphrase
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print "TEST STEP 1:Execute snmpget for wifi passphrase";
        print "EXPECTED RESULT 1: snmpget should get return the wifi passphrase";
        print "ACTUAL RESULT 1: FAILURE, passphrase not exposed"
