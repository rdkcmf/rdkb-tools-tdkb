...
<xml>
  ...
  <name>TS_SNMP_ValidateSnmpSetWithGetAPIOnRipEnabledStatus</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To validate snmpset with Get API</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_SNMP_PA_09</test_case_id>
    <test_objective>To validate smpset by getting the set value through API</test_objective>
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_SNMP_ValidateSnmpSetWithGetAPI</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_SNMP_ValidateSnmpSetWithGetAPIOnRipEnabledStatus');
pamObj.configureTestCase(ip,port,'TS_SNMP_ValidateSnmpSetWithGetAPIOnRipEnabledStatus');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in pamloadmodulestatus.upper():
    ...
    actResponse =snmplib.SnmpExecuteCmd("snmpget", commGetStr, "-v 2c", "1.3.6.1.4.1.17270.50.2.5.1.1.0", ipaddress);
	...
	actResponse =snmplib.SnmpExecuteCmd("snmpset", commSetStr, "-v 2c", "1.3.6.1.4.1.17270.50.2.5.1.1.0 i 1", ipaddress);
	sleep(5);
	actResponse =snmplib.SnmpExecuteCmd("snmpget", commGetStr, "-v 2c", "1.3.6.1.4.1.17270.50.2.5.1.1.0", ipaddress);
        set_value=actResponse.rsplit(None, 1)[-1];
        if set_value :
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: snmpget request to get RipEnabled status";
            print "EXPECTED RESULT 2: Command should return  RipEnabled status";
	    ...
	    tdkTestObj.addParameter("ParamName","Device.Routing.RIP.Enable");
            ...
            if expectedresult in actualresult:
                print "TEST STEP 3: Get RipEnabled status";
                print "EXPECTED RESULT 3: Should get the RipEnabled status";
		...
		if details == "true":
		    ...
		    tdkTestObj.setResultStatus("SUCCESS");
		    print "TEST STEP 4:Validate snmpset";
		    print "EXPECTED RESULT 4: Value retrieved via snmp and API should be same"
		    print "ACTUAL RESULT 4: Value retrieved via snmp and API are same"
		    print "[TEST EXECUTION RESULT] : SUCCESS"
		else:
		    tdkTestObj.setResultStatus("FAILURE");
                    print "TEST STEP 4:Validate snmpset";
                    print "EXPECTED RESULT 4: Value retrieved via snmp and API should be same"
                    print "ACTUAL RESULT 4: Value retrieved via snmp and API are not same"
