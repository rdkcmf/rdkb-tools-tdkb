...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TDKB_TR181_DSL_GetAllDefaultParameterValues</name>
  <primitive_test_id/>
  <primitive_test_name>TDKB_TR181Stub_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if all TR181 parameters of DSL are having the default values after factory reset</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_TR181_121</test_case_id>
    <test_objective>Check if all TR181 parameters of DSL are having the default values after factory reset</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>TDK test agent should be running
DSL module's parameter xml should be available. The setup should have DSL connection.</pre_requisite>
...
5. Invoke getAllParams() by passing DSL as module name
...
8. Unload modules</automation_approch>
    <expected_output>Get operation for default values on all DSL parameters should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkbtr181</test_stub_interface>
    <test_script>TDKB_TR181_DSL_GetAllDefaultParameterValues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TDKB_TR181_DSL_GetAllDefaultParameterValues');
obj1.configureTestCase(ip,port,'TDKB_TR181_DSL_GetAllDefaultParameterValues');
obj2.configureTestCase(ip,port,'TDKB_TR181_DSL_GetAllDefaultParameterValues');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    print "The modules to test is: DSL ";
    setup_type = "TDK"
    factoryReset = "true"
    ...
    if expectedresult in actualresult:
        ...
        moduleStatus,failedParams = tdkbSetAllParams.getAllParams("DSL", setup_type, factoryReset, obj, obj1);
        print "Status of DSL validation is ", moduleStatus, "\n";
