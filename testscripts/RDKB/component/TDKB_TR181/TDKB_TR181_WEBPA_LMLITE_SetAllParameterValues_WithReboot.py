...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TDKB_TR181_WEBPA_LMLITE_SetAllParameterValues_WithReboot</name>
  <primitive_test_id/>
  <primitive_test_name>TDKB_TR181Stub_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set all tr181 parameters in LMLITE  module using WEBPA and check if the values that are set persist after a reboot.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TDKB_TR181_100</test_case_id>
    <test_objective>To set all tr181 parameters in LMLITE  module using WEBPA and check if the values that are set persist after a reboot.</test_objective>
...
3. Invoke setAllParams() by passing LMLITE as module name and rebootTest flag as true
4. Do the reboot
...
7. Unload modules</automation_approch>
    ...
    <test_script>TDKB_TR181_WEBPA_LMLITE_SetAllParameterValues_WithReboot</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TDKB_TR181_WEBPA_LMLITE_SetAllParameterValues_WithReboot');
obj1.configureTestCase(ip,port,'TDKB_TR181_WEBPA_LMLITE_SetAllParameterValues_WithReboot');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if "SUCCESS" in preRequisiteStatus:
        ...
        moduleStatus,failedParams= tdkbSetAllParams.setAllParams("LMLITE", setup_type, obj, obj1,"true");

        print "Status of LMLITE module is : ",moduleStatus, "\n";
