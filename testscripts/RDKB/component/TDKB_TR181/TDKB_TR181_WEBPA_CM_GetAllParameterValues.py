...
<xml>
  ...
  <name>TDKB_TR181_WEBPA_CM_GetAllParameterValues</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Get all tr181 parameters of CM module using WEBPA</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_TR181_54</test_case_id>
    <test_objective>Get all tr181 parameters of CM module using WEBPA</test_objective>
...
3. Invoke getAllParams() by passing CM as module name
4. Display the final status of get and the list of failed parameters, if any
5. Unload modules</automation_approch>
    <expected_output>Get operation on all CM parameters via WEBPA should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdktr181</test_stub_interface>
    <test_script>TDKB_TR181_WEBPA_CM_GetAllParameterValues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TDKB_TR181_WEBPA_CM_GetAllParameterValues');
obj1.configureTestCase(ip,port,'TDKB_TR181_WEBPA_CM_GetAllParameterValues');
