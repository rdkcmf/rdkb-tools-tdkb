...
<xml>
  ...
  <name>TDKB_TR181_WEBPA_WIFI_SetAllParameterValues</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Set all tr181 parameters in WIFI module using WEBPA</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  <execution_time>105</execution_time>
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_TR181_14</test_case_id>
    <test_objective>Set all tr181 parameters in WIFI module using WEBPA</test_objective>
...
3. Invoke setAllParams() by passing WIFI as module name
4. Display the final status of set and the list of failed parameters, if any
5. Unload modules</automation_approch>
    <expected_output>Set operation on all WIFI parameters via WEBPA should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>tdkbtr181</test_stub_interface>
    <test_script>TDKB_TR181_WEBPA_WIFI_SetAllParameterValues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TDKB_TR181_WEBPA_WIFI_SetAllParameterValues');
obj1.configureTestCase(ip,port,'TDKB_TR181_WEBPA_WIFI_SetAllParameterValues');
