...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_XCONF_ExceedMaxImageNameSize</name>
  <primitive_test_id/>
  <primitive_test_name>XCONF_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to configure xconf server with a firmware name which exceeds the maximum size limit of 200 characters</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XCONF_14</test_case_id>
    <test_objective>Test to configure xconf server with a firmware name which exceeds the maximum size limit of 200 characters</test_objective>
...
"rm " + cdnLog
 cdnFile + " &amp;"
"grep -inr \"Filename not set correctly\" " + cdnLog + " ;echo $?"</input_parameters>
    <automation_approch>1. Load sysutil module
3. In the curl command for xconf configuration, make firmwareVersion and firmwareName values as strings with more than 200 characters
4. Get CDN_LOG and CDN_FILE values from the device
5. Remove previous logs, CDN_LOG
6. Execute CDN_FILE
7. In the new log, check if error is thrown about filename
9. Unload sysutil module</automation_approch>
    <except_output>log, should throw error about filename</except_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_XCONF_ExceedMaxImageNameSize</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XCONF_ExceedMaxImageNameSize');
...
if "SUCCESS" in result.upper() :
    ...
    if "Successfully added configuration" not in details:
        ...
        tdkTestObj = obj.createTestStep('ExecuteCmd');
        tdkTestObj.addParameter("command","grep -inr \"Filename not set correctly\" " + cdnLog + " ;echo $?")
        tdkTestObj.executeTestCase("SUCCESS");
