...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_XCONF_InvalidFirmwareLocation</name>
  <primitive_test_id/>
  <primitive_test_name>XCONF_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to attempt a firmware upgrade after setting an invalid firmware location in xconf server config</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XCONF_7</test_case_id>
    <test_objective>Test to attempt a firmware upgrade after setting an invalid firmware location in xconf server config</test_objective>
...
3. In the curl command for xconf configuration, make firmwareLocation value as an invalid url
...
9. Unload sysutil module</automation_approch>
    <except_output>Server response should have HTTP download NOT Successful error</except_output>
    <priority>High</priority>
    <test_stub_interface>sysutil</test_stub_interface>
    <test_script>TS_XCONF_InvalidFirmwareLocation</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XCONF_InvalidFirmwareLocation');
...
if "SUCCESS" in result.upper() :
    ...
    ####Define an invalid Fw location
    InvalidFirmwareLocation = "http://Invalid.Fw.Location"
