...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_XCONF_InvalidFirmwareName</name>
  <primitive_test_id/>
  <primitive_test_name>XCONF_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to set invalid firmware name in xconf server config</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XCONF_8</test_case_id>
    <test_objective>Test to set invalid firmware name in xconf server config</test_objective>
...
3. In the curl command for xconf configuration, make firmwareVersion and firmwareName values as invalid values
4. Get CDN_LOG and CDN_FILE values from the device
5. Remove previous logs, CDN_LOG
6. Execute CDN_FILE
7. In the new log, check for 'HTTP download NOT Successful' error
9. Unload sysutil module</automation_approch>
    ...
    <test_script>TS_XCONF_InvalidFirmwareName</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XCONF_InvalidFirmwareName');
