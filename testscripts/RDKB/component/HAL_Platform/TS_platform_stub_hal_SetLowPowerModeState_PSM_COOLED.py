...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_platform_stub_hal_SetLowPowerModeState_PSM_COOLED</name>
  <primitive_test_id/>
  <primitive_test_name>platform_stub_hal_SetLowPowerModeState</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To invoke platform_hal_SetLowPowerModeState() HAL API successfully for PSM_COOLED state.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_HAL_Platform_83</test_case_id>
    <test_objective>To invoke platform_hal_SetLowPowerModeState() HAL API successfully for PSM_COOLED state.</test_objective>
...
state : 4</input_parameters>
    <automation_approch>1. Load  platform module.
2. From script invoke platform_hal_SetLowPowerModeState() by passing the state value as 4
...
5. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from HAL_Platform stub.</automation_approch>
    <expected_output>platform_hal_SetLowPowerModeState() HAL API returns success for PowerModeState PSM_COOLED</expected_output>
    <priority>High</priority>
    <test_stub_interface>HAL_Platform</test_stub_interface>
    <test_script>TS_platform_stub_hal_SetLowPowerModeState_PSM_COOLED</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_platform_stub_hal_SetLowPowerModeState_PSM_COOLED');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #For PSM_COOLED, state = 4
    tdkTestObj.addParameter("state",4);
