...
<xml>
  ...
  <name>TS_platform_stub_hal_SetLowPowerModeState_PSM_AC</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To invoke platform_hal_SetLowPowerModeState() HAL API successfully for PSM_AC state.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_HAL_Platform_80</test_case_id>
    <test_objective>To invoke platform_hal_SetLowPowerModeState() HAL API successfully for PSM_AC</test_objective>
...
state : 1</input_parameters>
    <automation_approch>1. Load  platform module.
2. From script invoke platform_hal_SetLowPowerModeState() by passing the state value as 1
...
5. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from HAL_Platform stub.</automation_approch>
    <expected_output>platform_hal_SetLowPowerModeState() HAL API returns success for PowerModeState PSM_AC</expected_output>
    <priority>High</priority>
    <test_stub_interface>HAL_Platform</test_stub_interface>
    <test_script>TS_platform_stub_hal_SetLowPowerModeState_PSM_AC</test_script>
    <skipped>No</skipped>
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #For PSM_AC, state = 1
    tdkTestObj.addParameter("state",1);
