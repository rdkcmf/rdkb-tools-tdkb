...
<xml>
  ...
  <name>TS_platform_stub_hal_GetWebAccessLevel_HomeUser</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Get the Web Access Level for Home User using platform_hal_GetWebAccessLevel API</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Get the Web Access Level for Home User using platform_hal_GetWebAccessLevel API</test_objective>
...
2. From script invoke platform_stub_hal_GetWebAccessLevel(). userIndex = 1 for Home user. The Interface Indices are 1,2,16,40 for Lan, CM, Mta and Wan respectively
3. Get the value
...
5. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from HAL_Platform stub.</automation_approch>
    <expected_output>Should successfully get the Web Access Levels for Home User using platform_hal_GetWebAccessLevel API</expected_output>
    <priority>High</priority>
    <test_stub_interface>HAL_Platform</test_stub_interface>
    <test_script>TS_platform_stub_hal_GetWebAccessLevel_HomeUser</test_script>
    <skipped>Yes</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_platform_stub_hal_GetWebAccessLevel_HomeUser');
...
if "SUCCESS" in result.upper():
   ...
   #For Home User, userIndex = 1
   tdkTestObj.addParameter("userIndex",1);
