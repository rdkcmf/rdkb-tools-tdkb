...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>TS_platform_stub_hal_GetHardwareVersion</name>
  <primitive_test_id/>
  <primitive_test_name>platform_stub_hal_GetHardwareVersion</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To Validate HAL API platform_hal_GetHardwareVersion()</synopsis>
  <groups_id />
  ...
  <test_cases>
    <test_case_id>TC_HAL_Platform_4</test_case_id>
    <test_objective>To validate Platform HAL API platform_hal_GetHardwareVersion()</test_objective>
...
2. From script invoke platform_hal_GetHardwareVersion.
3. Get the value 
...
5. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from HAL_Platform stub.</automation_approch>
    ...
    <test_script>TS_platform_stub_hal_GetHardwareVersion</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_platform_stub_hal_GetHardwareVersion');
...
if "SUCCESS" in loadmodulestatus.upper():
        ...
        if expectedresult in actualresult and details != " ":
            ...
            print "EXPECTED RESULT 1: Should retrieve the Platform_GetHardwareVersion successfully";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : %s" %actualresult;
            print "Hardware version: %s" %details;
        else:
