...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_platform_stub_hal_SetLowPowerModeState_PSM_HOT</name>
  <primitive_test_id/>
  <primitive_test_name>platform_stub_hal_SetLowPowerModeState</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To invoke platform_hal_SetLowPowerModeState() HAL API successfully for PSM_HOT</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_HAL_Platform_82</test_case_id>
    <test_objective>To invoke platform_hal_SetLowPowerModeState() HAL API successfully for PSM_HOT</test_objective>
...
state : 3</input_parameters>
    <automation_approch>1. Load  platform module.
2. From script invoke platform_hal_SetLowPowerModeState() by passing the state value as 3
...
5. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from HAL_Platform stub.</automation_approch>
    <expected_output>platform_hal_SetLowPowerModeState() HAL API returns success for PowerModeState PSM_HOT</expected_output>
    <priority>High</priority>
    <test_stub_interface>HAL_Platform</test_stub_interface>
    <test_script>TS_platform_stub_hal_SetLowPowerModeState_PSM_HOT</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_platform_stub_hal_SetLowPowerModeState_PSM_HOT');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #For PSM_HOT, state = 3
    tdkTestObj.addParameter("state",3);
