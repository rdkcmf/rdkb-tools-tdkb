...
<xml>
  ...
  <name>TS_MoCAHAL_GetMocaCPEs</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test the HAL api moca_GetMocaCPEs()</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MoCAHAL_08</test_case_id>
    <test_objective>Test the HAL api moca_GetMocaCPEs()</test_objective>
...
2. Invoke the HAL api moca_GetMocaCPEs()
3. Check if the return status was success or not
4. If Success display the MAC Address of all the Nodes Connected on MoCA Network
5. UnLoad mocahal module</automation_approch>
    <except_output>Call to moca_GetMocaCPEs() should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>mocahal</test_stub_interface>
    <test_script>TS_MoCAHAL_GetMocaCPEs</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MoCAHAL_GetMocaCPEs');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        print "TEST STEP 1: Get MAC Address of all the Nodes Connected on MoCA Network"
        print "EXPECTED RESULT 1: Should Get MAC Address of all the Nodes Connected on MoCA Network";
