...
<xml>
  ...
  <name>TS_MoCAHAL_GetIfAcaConfig</name>
  <!-- If you are adding a new script you can specify the script name. Script Name should be unique same as this file name with out .py extension -->
  <primitive_test_id> </primitive_test_id>
  <!-- Do not change primitive_test_id if you are editing an existing script. -->
  <primitive_test_name>MoCAHAL_GetIfAcaConfig</primitive_test_name>
  <!--  -->
  <primitive_test_version>1</primitive_test_version>
  <!--  -->
  <status>FREE</status>
  <!--  -->
  <synopsis>Get the MoCA Interface ACA configuration details using the hal api  moca_getIfAcaConfig()</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MoCAHAL_15</test_case_id>
    <test_objective>Get the MoCA Interface ACA configuration details using the hal api  moca_getIfAcaConfig()</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>moca_GetIfAcaConfig()</api_or_interface_used>
    <input_parameters>ifIndex</input_parameters>
    <automation_approch>1. Load mocahal module
2. Invoke the HAL api moca_GetIfAcaConfig()
3. Check if the return status was success or not
4. If Success display the MoCA ACA Configuration Parameters
5. Unload mocahal module</automation_approch>
    <expected_output>Call to moca_GetIfAcaConfig() should be success</expected_output>
    <priority>High</priority>
    <test_stub_interface>mocahal</test_stub_interface>
    <test_script>TS_MoCAHAL_GetIfAcaConfig</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MoCAHAL_GetIfAcaConfig');
