...
<xml>
  ...
  <name>TS_MoCAHAL_IfGetDynamicInfo</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Test the HAL api moca_IfGetDynamicInfo()</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MoCAHAL_02</test_case_id>
    <test_objective>Test the HAL api moca_IfGetDynamicInfo()</test_objective>
...
2. Invoke the HAL api moca_IfGetDynamicInfo()
3. Check if the return status was success or not
4. If Success display the Dynamic Status information
5. UnLoad mocahal module</automation_approch>
    <except_output>Call to moca_IfGetDynamicInfo() should be success</except_output>
    <priority>High</priority>
    <test_stub_interface>mocahal</test_stub_interface>
    <test_script>TS_MoCAHAL_IfGetDynamicInfo</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MoCAHAL_IfGetDynamicInfo');
