...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>TS_MoCAHAL_GetAssociatedDevicesNullBuffer</name>
  <primitive_test_id/>
  <primitive_test_name>MoCAHAL_GetAssociatedDevices</primitive_test_name>
  <primitive_test_version>4</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test the HAL api moca_GetAssociatedDevices() by passing a null buffer</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_MoCAHAL_27</test_case_id>
    <test_objective>Test the HAL api moca_GetAssociatedDevices() by passing a null buffer</test_objective>
...
2. Invoke the HAL api moca_GetAssociatedDevices() by pasing a null buffer
...
4. UnLoad mocahal module</automation_approch>
    <expected_output>Call to moca_GetAssociatedDevices() should fail</expected_output>
    <priority>High</priority>
    <test_stub_interface>mocahal</test_stub_interface>
    <test_script>TS_MoCAHAL_GetAssociatedDevicesNullBuffer</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MoCAHAL_GetAssociatedDevicesNullBuffer');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        print "TEST STEP 1: Get Information on all the associated Devices on the network by passing null buffer"
        print "EXPECTED RESULT 1: Should not Get Information on all the associated Devices on the network  by passing null buffer"
