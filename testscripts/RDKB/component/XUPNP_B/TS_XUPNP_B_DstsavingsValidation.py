...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_XUPNP_B_DstsavingsValidation</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNPStub_CheckXDiscOutputFile</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get all the dstsavings parameters in the output.json file from DUT and validate them by comparing with dstsavings parameters in the output.json file from the clients connected.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XUPNP_15</test_case_id>
    <test_objective>To get all the dstsavings parameters in the output.json file from DUT and validate them by comparing with dstsavings parameters in the output.json file from the clients connected.</test_objective>
...
2. Get the output.json file output from DUT and parse the output with bcastMacAddress:dstsavingsmapping and store it.
3. Get the output.json file output from clients and parse the output with bcastMacAddress:dstsavings mapping and store it.
4. Compare the both the outputs.
5. If equal return SUCCESS, else FAILURE.
6.Unload xupnp module.</automation_approch>
    <expected_output>All dstsavings parameters retrieved from DUT and clients should be equal.</expected_output>
    <priority>High</priority>
    <test_stub_interface>xupnp</test_stub_interface>
    <test_script>TS_XUPNP_B_DstsavingsValidation</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XUPNP_B_DstsavingsValidation');
...
if "SUCCESS" in loadmodulestatus.upper() :
            ...
            if expectedresult in actualresult :
                ...
                info = open(logpath,'r');
                output = info.read();
                dictionary = LogParser(output,"dstsavings");
                info.close()
                ...
                if len(dictionary) == NO_OF_CLIENTS + 1 :
                    ...
                    for key in clientip_logfile_dic :
                        ...
                        message = data.read()
                        cli_dictionary = LogParser(message,"dstsavings");
                        print "\n********XUPNP CLIENT WITH IP : %s LOG TRANSFER - END********\n\n"%key;
                        print "\n**************XUPNP OUTPUT - BEGIN*************\n\n"
                        for key in dictionary :
                            if dictionary.get(key) == cli_dictionary.get(key):
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "TEST STEP : Compare whether the dstsavings parameters from DUT and connected client are equal";
                                print "EXPECTED RESULT : The dstsavings parameters from DUT and connected client should be equal";
                                print "ACTUAL RESULT : The dstsavings is %s for corresponding %s bcastMacAddress"%(dictionary.get(key),key);
