...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_XUPNP_B_BaseStreamingUrlValidation</name>
  <primitive_test_id/>
  <primitive_test_name>XUPNPStub_CheckXDiscOutputFile</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get all the baseStreamingUrl parameters in the output.json file from DUT and validate them by comparing with baseStreamingUrl parameters in the output.json file from the clients connected.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XUPNP_17</test_case_id>
    <test_objective>To get all the baseStreamingUrl parameters in the output.json file from DUT and validate them by comparing with baseStreamingUrl parameters in the output.json file from the clients connected.</test_objective>
...
2. Get the output.json file output from DUT and parse the output with bcastMacAddress:baseStreamingUrl mapping and store it.
3. Get the output.json file output from clients and parse the output with bcastMacAddress:baseStreamingUrl mapping and store it.
4. Compare the both the outputs.
5. If equal return SUCCESS, else FAILURE.
6.Unload xupnp module.</automation_approch>
    <expected_output>All baseStreamingUrl parameters retrieved from DUT and clients should be equal.</expected_output>
    <priority>High</priority>
    <test_stub_interface>xupnp</test_stub_interface>
    <test_script>TS_XUPNP_B_BaseStreamingUrlValidation</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XUPNP_B_BaseStreamingUrlValidation');
...
if "SUCCESS" in loadmodulestatus.upper() :
            ...
            if expectedresult in actualresult :
                ...
                info = open(logpath,'r');
                output = info.read();
                dictionary = LogParser(output,"baseStreamingUrl");
                info.close()
                ...
                if len(dictionary) == NO_OF_CLIENTS + 1 :
                    ...
                    for key in clientip_logfile_dic :
                        ...
                        message = data.read()
                        cli_dictionary = LogParser(message,"baseStreamingUrl");
                        print "\n********XUPNP CLIENT WITH IP : %s LOG TRANSFER - END********\n\n"%key;
                        print "\n**************XUPNP OUTPUT - BEGIN*************\n\n"
                        for key in dictionary :
                            if dictionary.get(key) == cli_dictionary.get(key):
                                tdkTestObj.setResultStatus("SUCCESS");
                                print "TEST STEP : Compare whether the baseStreamingUrl parameters from DUT and connected client are equal";
                                print "EXPECTED RESULT : The baseStreamingUrl parameters from DUT and connected client should be equal";
                                print "ACTUAL RESULT : The baseStreamingUrl is %s for corresponding %s bcastMacAddress"%(dictionary.get(key),key);
