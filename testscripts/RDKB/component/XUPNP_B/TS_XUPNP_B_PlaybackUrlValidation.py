...
<xml>
  ...
  <name>TS_XUPNP_B_PlaybackUrlValidation</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get all the playbackUrl parameters in the output.json file from DUT and validate them by comparing with playbackUrl parameters in the output.json file from the clients connected.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_XUPNP_20</test_case_id>
    <test_objective>To get all the playbackUrl parameters in the output.json file from DUT and validate them by comparing with playbackUrl parameters in the output.json file from the clients connected.</test_objective>
...
2. Get the output.json file output from DUT and parse the output with bcastMacAddress:playbackUrl mapping and store it.
3. Get the output.json file output from clients and parse the output with bcastMacAddress:playbackUrl mapping and store it.
4. Compare the both the outputs.
5. If equal return SUCCESS, else FAILURE.
6.Unload xupnp module.</automation_approch>
    <expected_output>All playbackUrl parameters retrieved from DUT and clients should be equal.</expected_output>
    <priority>High</priority>
    <test_stub_interface>xupnp</test_stub_interface>
    <test_script>TS_XUPNP_B_PlaybackUrlValidation</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XUPNP_B_PlaybackUrlValidation');
