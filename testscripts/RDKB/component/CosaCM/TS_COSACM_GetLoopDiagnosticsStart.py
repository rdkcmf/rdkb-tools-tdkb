...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_COSACM_GetLoopDiagnosticsStart</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_COSACM_21</test_case_id>
    <test_objective>To Validate Cable Modem 
"CosaDmlCMGetLoopDiagnosticsStart" API</test_objective>
...
2.Create a Python Script in Test Manager with default primitive test case through add new rdkb script option (TS_COSACM_GetLoopDiagnosticsStart.py)
3.Customize the generated script template to handle load/unload and pass/fail scenarios
3.Execute the generated Script(TS_COSACM_GetLoopDiagnosticsStart.py) using execution page of  Test Manager GUI
...
7.cosacmstub function CosaCM_GetLoopDiagnosticsStart will validate the available result (return value from ssp_CosaCMGetLoopDiagnosticsStart as success(0)) with expected result (success(0)) and the output argument value is updated in agent console log and json output variable along with return value
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_COSACM_GetLoopDiagnosticsStart</test_script>
    <skipped>No</skipped>
    <release_version/>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'CosaCM_GetLoopDiagnosticsStart');
...
if "SUCCESS" in loadmodulestatus.upper():
        ...
        tdkTestObj.addParameter("boolValue",0);
        ...
        if expectedresult in actualresult:
            ...
            print "TEST STEP 1: Should get the loop diagonostics Start details";
            print "EXPECTED RESULT 1: Should get the loop diagnostics Start details successfully";
