...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_COSACM_GetUpstreamChannelWithInvalidBuffer</name>
  <primitive_test_id/>
  <primitive_test_name>COSACM_GetUpstreamChannel</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify CosaDmlCmGetUpstreamChannel API with invalid buffer</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_COSACM_56</test_case_id>
...
2.Create a Python Script  (TS_COSACM_GetUpstreamChannelWithInvalidBuffer.py) in Test Manager through Add new rdkb script option
...
Cosa CM Get Upstream Channel success log from DUT should be available in Agent Console Log
CheckPoint 2:
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_COSACM_GetUpstreamChannelWithInvalidBuffer</test_script>
    <skipped>No</skipped>
    <release_version/>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_COSACM_GetUpstreamChannelWithInvalidBuffer');
...
if "SUCCESS" in loadmodulestatus.upper():
        ...
        if expectedresult in actualresult:
            ...
            print "TEST STEP 1: Should not get the upstream channel info";
            print "EXPECTED RESULT 1: Should not get the upstream channel  info successfully";
