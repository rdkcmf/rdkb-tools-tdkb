...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_COSACM_GetDOCSISInfoWithInvalidBuffer</name>
  <primitive_test_id/>
  <primitive_test_name>COSACM_GetDOCSISInfo</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify CosaDmlCMGetDOCSISInfo API with invalid parameter</synopsis>
  ...
  <test_cases>
    <test_case_id>TC_COSACM_52</test_case_id>
    <test_objective>To Validate Cable Modem 
"CosaDmlCMGetDOCSISInfo" API under Negative scenario</test_objective>
...
(COSACM_GetDOCSISInfo - func name - ( This is considered as default Primitive test case)
 cosacm - module name
 Necessary I/P args if needed are mentioned in Input)
2.Create a Python Script  (TS_COSACM_GetDOCSISInfoWithInvalidBuffer.py) in Test Manager through Add new rdkb script option
...
Cosa CM Get DOCSIS Info failure log from DUT should be available in Agent Console Log
CheckPoint 2:
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_COSACM_GetDOCSISInfoWithInvalidBuffer</test_script>
    <skipped>No</skipped>
    <release_version/>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_COSACM_GetDOCSISInfoWithInvalidBuffer');
...
if "SUCCESS" in loadmodulestatus.upper():
        ...
        if expectedresult in actualresult:
            ...
            print "TEST STEP 1: Should not get the DOCSIS info";
            print "EXPECTED RESULT 1: Should not get the DOCSIS info successfully";
