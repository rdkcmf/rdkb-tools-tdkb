...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_XDNS_DuplicateMACAddress</name>
  <primitive_test_id/>
  <primitive_test_name>XDNS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify that two rows with same client device MAC address is not possible</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XDNS_08</test_case_id>
    <test_objective>To verify that two rows with same client device MAC address is not possible</test_objective>
...
4. Add another row to the table
5. Set the same values to the row as the previous
6. Check if it is success o not
7. Delete the added rules</automation_approch>
    <except_output>The DNS table should not contain more than one rule with same device mac address</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkbtr181</test_stub_interface>
    <test_script>TS_XDNS_DuplicateMACAddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XDNS_DuplicateMACAddress');
