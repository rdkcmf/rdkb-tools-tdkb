...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_XDNS_DeleteRule_CheckDnsConfFileValues</name>
  <primitive_test_id/>
  <primitive_test_name>XDNS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To verify if the contents of dnsmasq_servers.conf is removed when the table row is deleted</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XDNS_07</test_case_id>
    <test_objective>To verify if the contents of dnsmasq_servers.conf is removed when the table row is deleted</test_objective>
...
5. Delete the added rule
6. Check if the values are removed form conf file</automation_approch>
    <except_output>The dns config file should be updated with the values in the new rule and should be removed when the rule is deleted</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkbtr181</test_stub_interface>
    <test_script>TS_XDNS_DeleteRule_CheckDnsConfFileValues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XDNS_DeleteRule_CheckDnsConfFileValues');
sysobj.configureTestCase(ip,port,'TS_XDNS_DeleteRule_CheckDnsConfFileValues');
        ...
        if (instance > 0):
			...
			tdkTestObj = obj.createTestStep("TDKB_TR181Stub_DelObject");
            		...
            		if expectedresult in actualresult:
            		    #Set the result status of execution
            		    tdkTestObj.setResultStatus("SUCCESS");
            		    print "TEST STEP 5: Deleting the added rule";
            		    print "EXPECTED RESULT 5: Should delete the added rule";
            		    print "ACTUAL RESULT 5: %s" %details;
            		    print "TEST EXECUTION RESULT : %s" %actualresult;
            		    print "Added table is deleted successfully\n"

			    #check if the contents in dnsmasq conf file is removed or not
			    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
			    cmd = "cat %s/dnsmasq_servers.conf" %conf_path
                    	    print cmd;
                    	    tdkTestObj.addParameter("command", cmd);
                    	    ...
                    	    if "a:b:c:d x.y.z.a p:q:r:s Default" not in dns_conf_value:
                    	        tdkTestObj.setResultStatus("SUCCESS");
                    	        print "TEST STEP 6: Get dnsmasq conf and validate"
                    	        print "EXPECTED RESULT 6: dnsmasq conf file should not contain the values"
