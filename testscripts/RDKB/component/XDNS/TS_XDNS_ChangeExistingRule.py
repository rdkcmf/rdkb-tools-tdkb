...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_XDNS_ChangeExistingRule</name>
  <primitive_test_id/>
  <primitive_test_name>XDNS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>If the values are modified for the same mac address, the new values (new policy) should take effect for that client.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_XDNS_09</test_case_id>
    <test_objective>If the values are modified for the same mac address, the new values (new policy) should take effect for that client.</test_objective>
...
4. Check if the values are updated in dnsmasq conf file
5. Make some changes in the added rule
6. Check if the change is reflected in the conf file
7. Delete the added rule</automation_approch>
    <except_output>Should be able to edit the values in the added rule and the changes should reflect in the conf file</except_output>
    <priority>High</priority>
    <test_stub_interface>tdkbtr181</test_stub_interface>
    <test_script>TS_XDNS_ChangeExistingRule</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_XDNS_ChangeExistingRule');
sysobj.configureTestCase(ip,port,'TS_XDNS_ChangeExistingRule');
