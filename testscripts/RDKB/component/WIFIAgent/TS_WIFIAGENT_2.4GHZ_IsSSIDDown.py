...
<xml>
  ...
  <name>TS_WIFIAGENT_2.4GHZ_IsSSIDDown</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check when 2.4GHZ ssid state is disabled, its status is "down"</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Check when 2.4GHZ ssid state is disabled, its status is "down"</test_objective>
...
Device.WiFi.SSID.1.Status</input_parameters>
    <automation_approch>
1. Load wifiagent module
2. Using WIFIAgent_Get, get and save Device.WiFi.SSID.1.Enable value
3. Using WIFIAgent_Set disable Device.WiFi.SSID.1.Enable
4. Using WIFIAgent_Get, get Device.WiFi.SSID.1.Status and check if its down
5. Using WIFIAgent_Set set Device.WiFi.SSID.1.Enable to its previous value
...
7. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from pam stub.</automation_approch>
    ...
    <test_script>TS_WIFIAGENT_2.4GHZ_IsSSIDDown</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHZ_IsSSIDDown');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            tdkTestObj = obj.createTestStep('WIFIAgent_Get');
            tdkTestObj.addParameter("paramName","Device.WiFi.SSID.1.Status")
            tdkTestObj.executeTestCase("expectedresult");
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
