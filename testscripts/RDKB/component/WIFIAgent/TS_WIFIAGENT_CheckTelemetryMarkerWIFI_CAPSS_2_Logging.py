...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WIFIAGENT_CheckTelemetryMarkerWIFI_CAPSS_2_Logging</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Client capability spatial streams parameter Device.WiFi.AccessPoint.2.AssociatedDevice.1.X_RDK_CapSpaStr returns a valid value in the range (0, 8] and the telemetry marker "WIFI_CAPSS_2" is populated in wifihealth.txt within the logging interval with valid marker value when a 5G client is connected to the DUT.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_212</test_case_id>
    <test_objective>To check if the Client capability spatial streams parameter Device.WiFi.AccessPoint.2.AssociatedDevice.1.X_RDK_CapSpaStr returns a valid value in the range (0, 8] and the telemetry marker "WIFI_CAPSS_2" is populated in wifihealth.txt within the logging interval with valid marker value when a 5G client is connected to the DUT.</test_objective>
...
3. Connect a wifi client to 5G radio private access point</pre_requisite>
    ...
    <input_parameters>paramName : Device.WiFi.AccessPoint.2.AssociatedDevice.1.X_RDK_CapSpaStr
...
3. Retrieve the client capability spatial streams using Device.WiFi.AccessPoint.2.AssociatedDevice.1.X_RDK_CapSpaStr.
...
7. Get the initial count of "WIFI_CAPSS_2" markers in wifihealth.txt.
...
9. After the sleep time, get the final count of the marker "WIFI_CAPSS_2"
...
13. Then check if the client spatial stream component of the marker is equal to TR181 value retrieved using Device.WiFi.AccessPoint.2.AssociatedDevice.1.X_RDK_CapSpaStr.
...
16. Unload the modules.</automation_approch>
    <expected_output>Client capability spatial streams parameter Device.WiFi.AccessPoint.2.AssociatedDevice.1.X_RDK_CapSpaStr should return a valid value in the range (0, 8] and the telemetry marker "WIFI_CAPSS_2" should be populated in wifihealth.txt within the logging interval with valid marker value when a 5G client is connected to the DUT.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerWIFI_CAPSS_2_Logging</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWIFI_CAPSS_2_Logging');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWIFI_CAPSS_2_Logging');
