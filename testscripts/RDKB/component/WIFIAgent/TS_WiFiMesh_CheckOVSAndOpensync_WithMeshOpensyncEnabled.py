...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WiFiMesh_CheckOVSAndOpensync_WithMeshOpensyncEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if OVS enable, Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable, and Opensync enable, Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync is disabled after Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync alone is enabled from disabled state and rebooted.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_226</test_case_id>
    <test_objective>To check if OVS enable, Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable, and Opensync enable, Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync is disabled after Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync alone is enabled from disabled state and rebooted.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3. Ensure the OVS enable and Mesh Opensync enable are disabled as pre-requisite and device rebooted after that.</pre_requisite>
...
2. Get the initial values of Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable and Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync and check if they are disabled initially as a pre-requisite.
3. Enable the Mesh Opensync parameter Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync alone.
4. Reboot the DUT.
...
7. Unload the wifiagent module.</automation_approch>
    <expected_output>Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OVS.Enable, and Opensync enable, Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync should be disabled after Device.DeviceInfo.X_RDKCENTRAL-COM_xOpsDeviceMgmt.Mesh.Opensync alone is disabled from enabled state and rebooted.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WiFiMesh_CheckOVSAndOpensync_WithMeshOpensyncEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
wifiobj.configureTestCase(ip,port,'TS_WiFiMesh_CheckOVSAndOpensync_WithMeshOpensyncEnabled');
