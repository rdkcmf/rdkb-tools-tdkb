...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_WIFIAGENT_CheckPasswordConfigurationLog_WithDifferentKeyPassPhrase</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_169</test_case_id>
...
8. Unload the modules
    ...
    <test_script>TS_WIFIAGENT_CheckPasswordConfigurationLog_WithDifferentKeyPassPhrase</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckPasswordConfigurationLog_WithDifferentKeyPassPhrase');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_CheckPasswordConfigurationLog_WithDifferentKeyPassPhrase');
sysobj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckPasswordConfigurationLog_WithDifferentKeyPassPhrase');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() and "SUCCESS" in loadmodulestatus3.upper():
    ...
    if expectedresult in status and orgValue[0] != "" and orgValue[1] != "" :
        ...
        print "\nTEST STEP 3 : Set Device.WiFi.AccessPoint.1.Security.KeyPassphrase and Device.WiFi.AccessPoint.2.Security.KeyPassphrase to different values";
        print "EXPECTED RESULT 3 : Should successfully set Device.WiFi.AccessPoint.1.Security.KeyPassphrase to %s and Device.WiFi.AccessPoint.2.Security.KeyPassphraseto %s" %(PASSWORD_2G, PASSWORD_5G);
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in status and setValue[0] == PASSWORD_2G and setValue[1] == PASSWORD_5G :
                ...
                print "EXPECTED RESULT 7 : Should successfully set Device.WiFi.AccessPoint.1.Security.KeyPassphrase to %s and Device.WiFi.AccessPoint.1.Security.KeyPassphrase to %s" %(orgValue[0], orgValue[1]);
