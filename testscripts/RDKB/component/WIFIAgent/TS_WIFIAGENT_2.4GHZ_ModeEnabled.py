...
<xml>
  ...
  <name>TS_WIFIAGENT_2.4GHZ_ModeEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the security mode enabled for 2.4GHZ Radio is from the list of supported modes </synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    ...
    <test_objective>Check if the security mode enabled for 2.4GHZ Radio is from the list of supported modes</test_objective>
...
2. Using WIFIAgent_Get, get and save Device.WiFi.AccessPoint.1.Security.ModesSupported value
4. Using WIFIAgent_Get, get Device.WiFi.AccessPoint.1.Security.ModeEnabled
...
7.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from pam stub.</automation_approch>
    ...
    <test_script>TS_WIFIAGENT_2.4GHZ_ModeEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHZ_ModeEnabled');
