...
<xml>
  ...
  <name>TS_WIFIAGENT_5GHZ_IsSSIDUp</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check when 5GHZ ssid is enabled, its status is "up".</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_21</test_case_id>
    <test_objective>Check when 5GHZ ssid state is enabled, its status is "down"</test_objective>
...
7.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from wifiagent stub.</automation_approch>
    <except_output>When ssid state is enabled, its status should be "up"</except_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgnet</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHZ_IsSSIDUp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHZ_IsSSIDUp');
...
if "SUCCESS" in loadmodulestatus.upper():
	...
	tdkTestObj.addParameter("paramType","boolean")
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 1: Enable SSID2";
            print "EXPECTED RESULT 1: Should enable SSID2"
	    ...
	    if expectedresult in actualresult and "Up" in status:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 1: Check if SSID2 staus is up";
                print "EXPECTED RESULT 1: SSID2 staus should be up";
