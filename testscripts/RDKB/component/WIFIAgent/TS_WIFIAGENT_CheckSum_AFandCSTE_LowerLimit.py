...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIAGENT_CheckSum_AFandCSTE_LowerLimit</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the sum of Activity Factor, Device.WiFi.Radio.{i}.Stats.X_RDKCENTRAL-COM_AF and Carrier Sense Threshold Exceeded, Device.WiFi.Radio.{i}.Stats.X_RDKCENTRAL-COM_CSTE is greater than or equal to 0.95 times the Channel Utilization, Device.WiFi.Radio.{i}.Stats.X_COMCAST-COM_ChannelUtilization.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_145</test_case_id>
    <test_objective>To check if the sum of Activity Factor, Device.WiFi.Radio.{i}.Stats.X_RDKCENTRAL-COM_AF and Carrier Sense Threshold Exceeded, Device.WiFi.Radio.{i}.Stats.X_RDKCENTRAL-COM_CSTE is greater than or equal to 0.95 times the Channel Utilization, Device.WiFi.Radio.{i}.Stats.X_COMCAST-COM_ChannelUtilization.</test_objective>
...
3. The computed sum should be greater than or equal to 0.95 times the Channel Utilization, Device.WiFi.Radio.1.Stats.X_COMCAST-COM_ChannelUtilization.
...
5. Unload the modules</automation_approch>
    <expected_output>The sum of Activity Factor, Device.WiFi.Radio.{i}.Stats.X_RDKCENTRAL-COM_AF and Carrier Sense Threshold Exceeded, Device.WiFi.Radio.{i}.Stats.X_RDKCENTRAL-COM_CSTE should be greater than or equal to 0.95 times the Channel Utilization, Device.WiFi.Radio.{i}.Stats.X_COMCAST-COM_ChannelUtilization.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckSum_AFandCSTE_LowerLimit</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckSum_AFandCSTE_LowerLimit');
        ...
        if expectedresult in actualresult and details != "":
            ...
            if expectedresult in actualresult and details != "":
                ...
                if expectedresult in actualresult and details != "":
                    ...
                    step = step + 1;
                    print "\nCheck if AF + CSTE >= 0.95 *CU";
                    calculated_sum = ActivityFactor + CarrierSenseThresholdExceeded;
                    calculated_prod = 0.95 * ChannelUtilized;
                    print "\nTEST STEP %d: Check if the sum of AF and CSTE is greater than or equal to 0.95 times the channel utilization" %(step);
                    print "EXPECTED RESULT %d: The sum of AF and CSTE should be greater than or equal to 0.95 times the channel utilization" %(step);
                    print "Sum of AF and CSTE : %d" %calculated_sum;
                    print "0.95 times Channel Utilization : %2f" %calculated_prod;
