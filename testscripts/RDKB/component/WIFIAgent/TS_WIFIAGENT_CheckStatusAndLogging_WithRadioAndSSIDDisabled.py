...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_WIFIAGENT_CheckStatusAndLogging_WithRadioAndSSIDDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if disabling the Radios Device.WiFi.Radio.1.Enable, Device.WiFi.Radio.2.Enable and SSIDs Device.WiFi.SSID.1.Enable, Device.WiFi.SSID.2.Enable updates the status parameters of Radios and SSIDs as "Down" and the expected logs "Both 5G Radio(Radio 2) and 5G Private SSID are in DISABLED state" and "Both 2G Radio(Radio 1) and 2G Private SSID are in DISABLED state" are populated in SelfHeal.txt.0 within 15mins.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_215</test_case_id>
    <test_objective>To check if disabling the Radios Device.WiFi.Radio.1.Enable, Device.WiFi.Radio.2.Enable and SSIDs Device.WiFi.SSID.1.Enable, Device.WiFi.SSID.2.Enable updates the status parameters of Radios and SSIDs as "Down" and the expected logs "Both 5G Radio(Radio 2) and 5G Private SSID are in DISABLED state" and "Both 2G Radio(Radio 1) and 2G Private SSID are in DISABLED state" are populated in SelfHeal.txt.0 within 15mins.</test_objective>
...
4. Disable the SSIDs and Radios and validate with GET.
5. Sleep for 20s
...
9. Within a 15min duration comprising of 60s iterations, check if the logs "Both 5G Radio(Radio 2) and 5G Private SSID are in DISABLED state" and "Both 2G Radio(Radio 1) and 2G Private SSID are in DISABLED state" are populated in SelfHeal.txt.0.
...
12. Unload the modules.</automation_approch>
    <expected_output>After disabling Device.WiFi.Radio.1.Enable, Device.WiFi.Radio.2.Enable and SSIDs Device.WiFi.SSID.1.Enable, Device.WiFi.SSID.2.Enable the status parameters of Radios and SSIDs should be updated to "Down" and the expected logs "Both 5G Radio(Radio 2) and 5G Private SSID are in DISABLED state" and "Both 2G Radio(Radio 1) and 2G Private SSID are in DISABLED state" should be  populated in SelfHeal.txt.0 within 15mins.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckStatusAndLogging_WithRadioAndSSIDDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckStatusAndLogging_WithRadioAndSSIDDisabled');
sysobj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckStatusAndLogging_WithRadioAndSSIDDisabled');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if status == 0:
        ...
        if preReq == 0:
            ...
            #Disable the SSIDs and radios
            paramList_ssid_radio = ["Device.WiFi.SSID.1.Enable", "Device.WiFi.SSID.2.Enable", "Device.WiFi.Radio.1.Enable", "Device.WiFi.Radio.2.Enable"];
            setvalues = ["false", "false", "false", "false"];
            print "\nTEST STEP 3: Set the SSID and Radio enable parameters to disabled values - ", paramList_ssid_radio;
            print "EXPECTED RESULT 3 : The SSID and Radio enable parameters should be set to disabled successfully";

            for index in range(0, 4):
                status = setParameter(obj, paramList_ssid_radio[index], "false", "boolean");
                if status == 0:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "%s is set successfully to %s" %(paramList_ssid_radio[index], setvalues[index]);
                else:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "%s is NOT set successfully to %s" %(paramList_ssid_radio[index], setvalues[index]);
                    break;

            if status == 0:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 3: SSIDs and Radios are disabled successfully keeping the radios enabled" ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";
                ...
                if status == 0:
                    ...
                    if values[0] == "Down" and values[1] == "Down" and values[2] == "Down" and values[3] == "Down":
                        ...
                        if details == "File exist" :
                            ...
                            #Check if the "Both 5G Radio(Radio 2) and 5G Private SSID are in DISABLED state" and "Both 2G Radio(Radio 1) and 2G Private SSID are in DISABLED state" logs are found in SelfHeal.txt.0
                            step = 6;
                            search_strings = ["Both 5G Radio(Radio 2) and 5G Private SSID are in DISABLED state", "Both 2G Radio(Radio 1) and 2G Private SSID are in DISABLED state"];
                            tdkTestObj = sysobj.createTestStep('ExecuteCmd');
