...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>12</version>
  <name>TS_WIFIAGENT_CheckResetCountIncrement_AfterResetRadios</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the radio reset count for all applicable radios are incremented by 1 when radio reset operation is done using Device.WiFi.X_CISCO_COM_ResetRadios.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_192</test_case_id>
    <test_objective>To check if the radio reset count for all applicable radios are incremented by 1 when radio reset operation is done using Device.WiFi.X_CISCO_COM_ResetRadios.</test_objective>
...
4. Perform wifi module reset using Device.WiFi.X_CISCO_COM_ResetRadios
5. Sleep for 90s for the wifi reset to take effect
...
8. Unload the modules</automation_approch>
    <expected_output>The radio reset count for all applicable radios should be incremented by 1 when radio reset operation is done using Device.WiFi.X_CISCO_COM_ResetRadios.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckResetCountIncrement_AfterResetRadios</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckResetCountIncrement_AfterResetRadios');
sysobj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckResetCountIncrement_AfterResetRadios');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if status == 0 :
        ...
        print "\nACTUAL RESULT 1 : Initial Radio Reset Counts are retrieved successfully";
        ...
        print "\nTEST STEP 2 : Initiate WiFi reset using Device.WiFi.X_CISCO_COM_ResetRadios";
        print "EXPECTED RESULT 2 : The WiFi reset operation should be successful";

        if expectedresult in actualresult and details != "" :
            tdkTestObj.setResultStatus("SUCCESS");
            print "ACTUAL RESULT 2 : WiFi reset operation was success; Details : %s" %details;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : SUCCESS";
            ...
            print "\nTEST STEP 3 : Get the Radio Reset Count for all applicable radios after WiFi reset";
            print "EXPECTED RESULT 3 : Should get the Radio Reset Count for all applicable radios after WiFi reset";
            ...
            if status == 0:
                ...
                if result == 0:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "\nACTUAL RESULT 4 : The Radio Reset Counts are incremented by 1";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print "ACTUAL RESULT 2 : WiFi reset operation failed; Details : %s" %details;
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : FAILURE";
    else :
        tdkTestObj.setResultStatus("FAILURE");
