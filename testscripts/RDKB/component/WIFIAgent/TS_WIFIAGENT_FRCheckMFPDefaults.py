...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_FRCheckMFPDefaults</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the Factory Default value for MFP Config</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_138</test_case_id>
    <test_objective>This test case is to check the Factory Default value for MFP Config</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>WIFIAgent_Set,WIFIAgent_Get</api_or_interface_used>
    <input_parameters>Device.WiFi.FeatureMFPConfig
Device.WiFi.AccessPoint.{i}.Security.MFPConfig</input_parameters>
    <automation_approch>1.Load the module
2.Perform Factory reset on the DUT
3.Check if Feature MFP Config is disabled by default
4.Check the Security Access points for MFP Config have Optional and Disabled as the values alternatively
5.Unload the module</automation_approch>
    <expected_output>With FR  MFP Config feature is expected to be disabled and Security Access points for MFP Config should have Optional and Disabled as the values alternatively</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIAGENT</test_stub_interface>
    <test_script>TS_WIFIAGENT_FRCheckMFPDefaults</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_FRCheckMFPDefaults');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj.addParameter("paramName","Device.WiFi.FeatureMFPConfig");
        expectedresult="SUCCESS";
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
        details = tdkTestObj.getResultDetails();
