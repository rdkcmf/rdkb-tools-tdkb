...
<xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_5GHZ_PublicWiFi_GetLastChange</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Toggle the SSID status of 5GHZ public wifi and check whether the value of LastChange is changed or not</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_61</test_case_id>
    <test_objective>Toggle the SSID status of 5GHZ public wifi and check whether the value of LastChange is changed or not</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, Emulator, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>"Device.WiFi.SSID.6.LastChange"
"Device.WiFi.SSID.6.Enable"</input_parameters>
    <automation_approch>1. Load wifiagent module
2. Using WIFIAgent_Get, get and save Device.WiFi.SSID.6.LastChange and Device.WiFi.SSID.6.Enable
3. Toggle the value of Device.WiFi.SSID.6.Enable
4. Using WIFIAgent_Get, get Device.WiFi.SSID.6.LastChange and check if it is changed
5. Restore value of Device.WiFi.SSID.6.Enable</automation_approch>
    <except_output>The Last Change must be changed to a lower value when enable status is changed</except_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHZ_PublicWiFi_GetLastChange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHZ_PublicWiFi_GetLastChange');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            tdkTestObj.addParameter("paramName","Device.WiFi.SSID.6.LastChange")
            ...
            if expectedresult in actualresult:
                ...
                #Get the current status of SSID6
                tdkTestObj = obj.createTestStep('WIFIAgent_Get');
                tdkTestObj.addParameter("paramName","Device.WiFi.SSID.6.Enable")
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
