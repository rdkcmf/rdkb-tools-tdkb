...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>8</version>
  <name>TS_WIFIAGENT_CheckOpenHotspotSSIDConfiguration_AfterWiFiRestore</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Open Hotspot SSID Configuration (AP 5, AP 6) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;5,6".</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_198</test_case_id>
    <test_objective>To check if the Open Hotspot SSID Configuration (AP 5, AP 6) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;5,6".</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.SSID.5.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.6.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.5.SSID
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.SSID.6.SSID
paramValue : dynamically generated
paramType : string
...
2. Get the initial values of Device.WiFi.SSID.5.Enable and Device.WiFi.SSID.6.Enable and set to enabled if not already in enabled state.
3. Get the initial values of Device.WiFi.SSID.5.SSID and Device.WiFi.SSID.6.SSID
4. Set all the 2 above mentioned parameters to new dynamically generated values and verify with GET.
5. Perform WiFi restore of APs 5,6 with Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp
6. Sleep for 100s for the WiFi restore operation to take effect.
7. Check the values of Device.WiFi.SSID.5.SSID and Device.WiFi.SSID.6.SSID after WiFi restore. It should be different from the values before WiFi restore operation.
8. Revert to initial Open Hotspot SSID configuration and SSID Enable status if required.
9. Unload the modules
</automation_approch>
    <expected_output>The Open Hotspot SSID Configuration (AP 5, AP 6) should get restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;5,6".</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckOpenHotspotSSIDConfiguration_AfterWiFiRestore</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckOpenHotspotSSIDConfiguration_AfterWiFiRestore');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_CheckOpenHotspotSIDConfiguration_AfterWiFiRestore');
    ...
    if expectedresult in status and enableValue[0] != "" and enableValue[1] != "":
        ...
        if ssid_enable_flag == 0:
            ...
            if expectedresult in status and orgSSIDConfig[0] != "" and orgSSIDConfig[1] != "":
                ...
                step = 4;
                ssid1 = "OpenHotspot-SSID-" + str(randint(0, 99));
                ...
                if ssid_config_flag == 0:
                    ...
                    tdkTestObj.addParameter("paramValue","1,2;5,6");
