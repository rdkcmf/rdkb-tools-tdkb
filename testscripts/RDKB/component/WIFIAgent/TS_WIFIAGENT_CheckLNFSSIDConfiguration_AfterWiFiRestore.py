...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIAGENT_CheckLNFSSIDConfiguration_AfterWiFiRestore</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the LNF SSID Configuration (AP 7, AP 8) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;7,8".</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_199</test_case_id>
    <test_objective>To check if the LNF SSID Configuration (AP 7, AP 8) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;7,8".</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.SSID.7.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.8.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.7.SSID
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.SSID.8.SSID
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.AccessPoint.7.Security.KeyPassphrase
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.AccessPoint.8.Security.KeyPassphrase
paramValue : dynamically generated
paramType : string
...
2. Get the initial values of Device.WiFi.SSID.7.Enable and Device.WiFi.SSID.8.Enable and set to enabled if not already in enabled state.
3. Get the initial values of Device.WiFi.SSID.7.SSID, Device.WiFi.SSID.8.SSID, Device.WiFi.AccessPoint.7.Security.KeyPassphrase and Device.WiFi.AccessPoint.8.Security.KeyPassphrase.
4. Set all the 4 above mentioned parameters to new dynamically generated values and verify with GET.
5. Perform WiFi restore of APs 7,8 with Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp
6. Sleep for 100s for the WiFi restore operation to take effect.
7. Check the values of Device.WiFi.SSID.7.SSID, Device.WiFi.SSID.8.SSID, Device.WiFi.AccessPoint.7.Security.KeyPassphrase and Device.WiFi.AccessPoint.8.Security.KeyPassphrase after WiFi restore. It should be different from the values before WiFi restore operation.
8. Revert to initial LNF SSID configuration and SSID Enable status if required.
9. Unload the modules
</automation_approch>
    <expected_output>The LNF SSID Configuration (AP 7, AP 8) should get restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;7,8".</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckLNFSSIDConfiguration_AfterWiFiRestore</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckLNFSSIDConfiguration_AfterWiFiRestore');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_CheckLNFSIDConfiguration_AfterWiFiRestore');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    paramList_enable=["Device.WiFi.SSID.7.Enable", "Device.WiFi.SSID.8.Enable"];
    tdkTestObj,status,enableValue = getMultipleParameterValues(obj,paramList_enable)

    print "\nTEST STEP 1: Get the values of LNF SSID enable status - ", paramList_enable;
    ...
    if expectedresult in status and enableValue[0] != "" and enableValue[1] != "":
        ...
        print "ACTUAL RESULT 1: LNF SSID enable statuses are respectively : %s, %s" %(enableValue[0],enableValue[1]) ;
        ...
        if ssid_enable_flag == 0:
            ...
            #Get the initial LNF SSID and KeyPassphrase
            step = 3;
            paramList_ssid_config = ["Device.WiFi.SSID.7.SSID", "Device.WiFi.AccessPoint.7.Security.KeyPassphrase", "Device.WiFi.SSID.8.SSID", "Device.WiFi.AccessPoint.8.Security.KeyPassphrase"];
            tdkTestObj,status,orgSSIDConfig = getMultipleParameterValues(obj,paramList_ssid_config)

            print "\nTEST STEP 3: Get the LNF SSID configuration parameter values - ", paramList_ssid_config;
            ...
            if expectedresult in status and orgSSIDConfig[0] != "" and orgSSIDConfig[1] != "" and orgSSIDConfig[2] != "" and orgSSIDConfig[3] != "":
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 3: LNF SSID Configuration are respectively : %s, %s, %s, %s" %(orgSSIDConfig[0],orgSSIDConfig[1],orgSSIDConfig[2],orgSSIDConfig[3]) ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";

                #Set to new configuration
                step = 4;
                ssid1 = "LNF-SSID-" + str(randint(0, 99));
                key1 = "LNF-Key-" + str(randint(0, 99));
                ssid2 = "LNF-SSID-" + str(randint(100, 199));
                key2 = "LNF-Key-" + str(randint(100, 199));
                setvalues = [ssid1, key1, ssid2, key2];

                print "\nTEST STEP 4 : Set new LNF SSID Configuration using : ", paramList_ssid_config;
                print "EXPECTED RESULT 4 : New LNF SSID Configuration should be set successfully";
                ssid_config_flag = 0;
                ...
                if ssid_config_flag == 0:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "ACTUAL RESULT 4: New LNF SSID Configuration is set successfully" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    ...
                    tdkTestObj.addParameter("paramValue","1,2;7,8");
                    ...
                    if expectedresult in actualresult and details != "" :
                        ...
                        #Check if the LNF SSID configuration is changed to default values
                        step = 6;
                        tdkTestObj,status,finalSSIDConfig = getMultipleParameterValues(obj,paramList_ssid_config)

                        print "\nTEST STEP 6: Get the LNF SSID configuration parameter values - ", paramList_ssid_config;
                        ...
                        if expectedresult in status and finalSSIDConfig[0] != "" and finalSSIDConfig[1] != "" and finalSSIDConfig[2] != "" and finalSSIDConfig[3] != "":
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT 6: LNF SSID Configuration are respectively : %s, %s, %s, %s" %(finalSSIDConfig[0],finalSSIDConfig[1],finalSSIDConfig[2],finalSSIDConfig[3]) ;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                            ...
                            if finalSSIDConfig[0] == orgSSIDConfig[0] and finalSSIDConfig[1] == orgSSIDConfig[1] and finalSSIDConfig[2] == orgSSIDConfig[2] and finalSSIDConfig[3] == orgSSIDConfig[3]:
                                ...
                                step = 8;
                                print "\nTEST STEP 8 : Revert to initial LNF SSID Configuration";
                                print "EXPECTED RESULT 8 : Revert to initial LNF SSID Configuration should be successful";
                            ...
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("FAILURE");
                            print "ACTUAL RESULT 6: LNF SSID Configuration is NOT retrieved";
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : FAILURE";
                    else :
                        tdkTestObj.setResultStatus("FAILURE");
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 4: New LNF SSID Configuration is NOT set successfully" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                #Set the result status of execution
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT 3: LNF SSID Configuration is NOT retrieved";
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            #Set the result status of execution
        ...
        #Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE");
        print "ACTUAL RESULT 1: LNF SSID enable statuses are NOT retrieved";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : FAILURE";
