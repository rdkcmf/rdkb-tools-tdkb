...
<xml>
  ...
  <name>TS_WIFIAGENT_CheckMFPConfig_WithWPA2-PersonalSecurityMode</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the MFP Configuration retrieved using Device.WiFi.AccessPoint.{i}.Security.MFPConfig for the applicable radio's private AP is [2.4G - Disabled, 5G - Disabled] when  the security mode enabled, Device.WiFi.AccessPoint.{i}.Security.ModeEnabled is "WPA2-Personal".</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_205</test_case_id>
    <test_objective>To check if the MFP Configuration retrieved using Device.WiFi.AccessPoint.{i}.Security.MFPConfig for the applicable radio's private AP is [2.4G - Disabled, 5G - Disabled] when  the security mode enabled, Device.WiFi.AccessPoint.{i}.Security.ModeEnabled is "WPA2-Personal".</test_objective>
...
3. Set the security mode enabled to "WPA2-Personal" if not already in that mode. Validate the set operation.
4. Check the MFP Config using Device.WiFi.AccessPoint.{i}.Security.MFPConfig
5. Validate if the MFP Config are : for 2.4G - Disabled, 5G - Disabled.
6. Revert the security modes if required.
7. Unload the modules.</automation_approch>
    <expected_output>The MFP Configuration retrieved using Device.WiFi.AccessPoint.{i}.Security.MFPConfig for the applicable radio's private AP should be [2.4G - Disabled, 5G - Disabled] when the security mode enabled, Device.WiFi.AccessPoint.{i}.Security.ModeEnabled is "WPA2-Personal".</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckMFPConfig_WithWPA2-PersonalSecurityMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckMFPConfig_WithWPA2-PersonalSecurityMode');
