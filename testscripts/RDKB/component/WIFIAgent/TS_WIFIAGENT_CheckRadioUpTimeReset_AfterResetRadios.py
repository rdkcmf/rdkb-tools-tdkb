...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIAGENT_CheckRadioUpTimeReset_AfterResetRadios</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if radio uptimes are reset after initiating a reset radios using Device.WiFi.X_CISCO_COM_ResetRadios.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_195</test_case_id>
    <test_objective>To check if the radio uptimes of all applicable radios are getting reset after wifi reboot using Device.WiFi.X_CISCO_COM_ResetRadios.</test_objective>
...
paramName : Device.WiFi.X_CISCO_COM_ResetRadios
paramValue : true
Type : boolean
</input_parameters>
...
6. Perform reset radios operation using Device.WiFi.X_CISCO_COM_ResetRadios
7. Sleep for 90s for the reset radio operation to take effect
8. Retrieve the uptimes of all radios using Device.WiFi.Radio.{i}.X_COMCAST_COM_RadioUpTime
9. Check if the final uptime is less than the uptime before the reset operation
10. Unload the modules</automation_approch>
    <expected_output>The radio uptimes of all applicable radios should get reset after reset radios operation using Device.WiFi.X_CISCO_COM_ResetRadios.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckRadioUpTimeReset_AfterResetRadios</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckRadioUpTimeReset_AfterResetRadios');
sysobj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckRadioUpTimeReset_AfterResetRadios');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if status == 0 :
        ...
        if proceed_flag == 0:
            ...
            tdkTestObj.addParameter("paramName","Device.WiFi.X_CISCO_COM_ResetRadios");
            ...
            print "\nTEST STEP %d : Initiate WiFi radio reset using Device.WiFi.X_CISCO_COM_ResetRadios" %step;
            print "EXPECTED RESULT %d : The WiFi radio reset operation should be successful" %step;

            if expectedresult in actualresult and details != "" :
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT %d : WiFi radio reset operation was success; Details : %s" %(step, details);
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";

                #Sleep for 90s for the WiFi reset operation to take effect
                print "Sleeping 90s for the WiFi reset operation...";
                sleep(90);

                #Get the final radio uptimes
                step = step + 1;
                print "\nTEST STEP %d : Get the Radio Uptime for all applicable radios after WiFi radio reset" %step;
                print "EXPECTED RESULT %d : Should get the Radio Uptime for all applicable radios after WiFi radio reset" %step;
                ...
                if status == 0:
                    ...
                    for index in range(1, num_of_radios+1):
                        ...
                        #After wifi reset the radio uptime should be reset, the final uptime value should be less than the uptime before wifi reset
                    ...
                    if result == 0:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "ACTUAL RESULT %d : The Radio Uptimes are reset" %step;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "ACTUAL RESULT %d : The Radio Uptimes are NOT reset" %step;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : FAILURE";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                ...
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT %d : WiFi reset operation failed; Details : %s" %(step, details);
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            tdkTestObj.setResultStatus("FAILURE");
