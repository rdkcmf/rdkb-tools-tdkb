...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIAGENT_CheckTelemetryMarkerWith5GhzBytesSentAndReceived</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_105</test_case_id>
...
3.Connect a WiFi client to 5Ghz private SSID of DUT</pre_requisite>
    ...
    <input_parameters>ParamName -Device.WiFi.AccessPoint.2.AssociatedDevice.1.Stats.BytesSent and  Device.WiFi.AccessPoint.2.AssociatedDevice.1.Stats.BytesReceived Device.DeviceInfo.X_RDKCENTRAL-COM_WIFI_TELEMETRY.LogInterval
...
4.Check for telemetry marker WIFI_BYTESSENTCLIENTS_2 and get the bytes sent using Device.WiFi.AccessPoint.2.AssociatedDevice.1.Stats.BytesSent  .
5.Check if the values received are greater than zero
6.Check for telemetry marker WIFI_BYTESRECEIVEDCLIENTS_2 and get the bytes sent using Device.WiFi.AccessPoint.2.AssociatedDevice.1.Stats.BytesReceived.
...
9.Unload the Module
    ...
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerWith5GhzBytesSentAndReceived</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith5GhzBytesSentAndReceived');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith5GhzBytesSentAndReceived');
                   ...
                   for i in range(1,6):
                       if markerfound == 1:
                          break;
                           ...
                           query="cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_BYTESSENTCLIENTS_2:\""
                           print "query:%s" %query
                           tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                           ...
                           if (len(details) == 0) or details.endswith(":") or "WIFI_BYTESSENTCLIENTS_2" not in details:
                              markerfound = 0;
                              sleep(60);
                           else:
                               tel_bytessent = details.split("WIFI_BYTESSENTCLIENTS_2:")[1].split(',')[0].strip().replace("\\n","");
                               markerfound = 1;

                   if expectedresult in actualresult and markerfound == 1 and tel_bytessent != "":
                      tdkTestObj.setResultStatus("SUCCESS");
                      print "TEST STEP 5: WIFI_BYTESSENTCLIENTS_2 Marker should be present";
                      print "EXPECTED RESULT 5: WIFI_BYTESSENTCLIENTS_2 Marker should be present";
                      print "ACTUAL RESULT 5:WIFI_BYTESSENTCLIENTS_2 Marker is %s" %tel_bytessent
                      #Get the result of execution
                      print "[TEST EXECUTION RESULT] : SUCCESS";


                      tdkTestObj = obj.createTestStep('pam_GetParameterValues');
                      tdkTestObj.addParameter("ParamName","Device.WiFi.AccessPoint.2.AssociatedDevice.1.Stats.BytesSent");
                      expectedresult="SUCCESS";
                      #Execute the test case in DUT
                      tdkTestObj.executeTestCase(expectedresult);
                      actualresult = tdkTestObj.getResult();
                      ...
                      if expectedresult in actualresult:
                            ...
                            for i in range(1,6):
                                if markerfound == 1:
                                   break;
                                    ...
                                    #Query for the Telemetry Marker
                                    query="cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_BYTESRECEIVEDCLIENTS_2:\""
                                    print "query:%s" %query
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                                    tdkTestObj.addParameter("command", query)
                                    expectedresult="SUCCESS";
                                    ...
                                    details = tdkTestObj.getResultDetails().strip().replace("\\n","");
                                    print "Marker Detail Found fromLog file is: %s "%details;
                                    if (len(details) == 0) or details.endswith(":") or "WIFI_BYTESRECEIVEDCLIENTS_2" not in details:
                                       markerfound = 0;
                                       sleep(60);
                                    else:
                                        tel_bytesreceived = details.split("WIFI_BYTESRECEIVEDCLIENTS_2:")[1].split(',')[0].strip().replace("\\n","");
                                        markerfound = 1;

                            if expectedresult in actualresult and markerfound == 1 and tel_bytesreceived != "":
                               tdkTestObj.setResultStatus("SUCCESS");
                               print "TEST STEP 8: WIFI_BYTESRECEIVEDCLIENTS_2  Marker should be present";
                               print "EXPECTED RESULT 8: WIFI_BYTESRECEIVEDCLIENTS_2  Marker should be present";
                               print "ACTUAL RESULT 8:WIFI_BYTESRECEIVEDCLIENTS_2 is %s" %tel_bytesreceived
                               #Get the result of execution
                               print "[TEST EXECUTION RESULT] : SUCCESS";

                               tdkTestObj = obj.createTestStep('pam_GetParameterValues');
                               tdkTestObj.addParameter("ParamName","Device.WiFi.AccessPoint.2.AssociatedDevice.1.Stats.BytesReceived");
                               expectedresult="SUCCESS";
                               #Execute the test case in DUT
                               tdkTestObj.executeTestCase(expectedresult);
                               actualresult = tdkTestObj.getResult();
                               ...
                               if expectedresult in actualresult:
                                ...
                                print "TEST STEP 8: WIFI_BYTESRECEIVEDCLIENTS_2  Marker should be present";
                                print "EXPECTED RESULT 8: WIFI_BYTESRECEIVEDCLIENTS_2  Marker should be present";
                                print "ACTUAL RESULT 8:WIFI_BYTESRECEIVEDCLIENTS_2 is not present"
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : FAILURE";
                         else:
                             tdkTestObj.setResultStatus("FAILURE");
                       ...
                       print "TEST STEP 5: WIFI_BYTESSENTCLIENTS_2 Marker should be present";
                       print "EXPECTED RESULT 5: WIFI_BYTESSENTCLIENTS_2 Marker should be present";
                       print "ACTUAL RESULT 5:WIFI_BYTESSENTCLIENTS_2 Marker not present"
                       #Get the result of execution
                       print "[TEST EXECUTION RESULT] : FAILURE";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
