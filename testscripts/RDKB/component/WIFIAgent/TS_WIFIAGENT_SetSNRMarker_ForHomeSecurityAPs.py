...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIAGENT_SetSNRMarker_ForHomeSecurityAPs</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to enable SNR telemetry markers for Home Security access points.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_83</test_case_id>
    <test_objective>Test to enable SNR telemetry markers for Home Security access points.</test_objective>
...
3.Set the SNR list for Home Security access points
...
7. Unload wifiagent module</automation_approch>
    <expected_output>Should successfully enable SNR telemetry markers for Home Security access points.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIAGENT</test_stub_interface>
    <test_script>TS_WIFIAGENT_SetSNRMarker_ForHomeSecurityAPs</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_SetSNRMarker_ForHomeSecurityAPs');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        #Change SNR List to enable SNR marker for Home security APs
        setSNR = "3,4"
	...
	tdkTestObj.addParameter("paramType","string")
        ...
        if expectedresult in actualresult:
            #Set the result status of execution
            tdkTestObj.setResultStatus("SUCCESS");
            print "TEST STEP 2: Enable SNR telemetry markers for Home Security access points";
            print "EXPECTED RESULT 2: Should enable SNR telemetry markers for Home Security access points";
	    ...
	    #check if SNR List is set
            ...
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
	    newSNR = details.split("VALUE:")[1].split(' ')[0];
