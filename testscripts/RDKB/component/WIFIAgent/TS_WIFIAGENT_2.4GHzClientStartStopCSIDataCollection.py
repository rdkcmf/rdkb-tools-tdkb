...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_2.4GHzClientStartStopCSIDataCollection</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_AddObject</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Enable a CSI session with a 2.4GHz connected client using wifi_events_consumer process after enabling the CSI enable and setting ClientMAC value for new CSI table instance created. Kill the wifi_events_consumer process after 10s and check if the logs "WiFi_CSI_SubscriptionStarted" and "WiFi_CSI_SubscriptionCancelled" is found under /rdklogs/logs/WiFilog.txt.0 on starting and cancelling the CSI data collection respectively.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_174</test_case_id>
    <test_objective>Enable a CSI session with a 2.4GHz connected client using wifi_events_consumer process after enabling the CSI enable and setting ClientMAC value for new CSI table instance created. Kill the wifi_events_consumer process after 10s and check if the logs "WiFi_CSI_SubscriptionStarted" and "WiFi_CSI_SubscriptionCancelled" is found under /rdklogs/logs/WiFilog.txt.0 on starting and cancelling the CSI data collection respectively.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3. Device should be in RBUS mode
4. A client should be connected to 2.4G</pre_requisite>
...
14. Unload the modules.</automation_approch>
    ...
    <test_script>TS_WIFIAGENT_2.4GHzClientStartStopCSIDataCollection</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHzClientStartStopCSIDataCollection');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHzClientStartStopCSIDataCollection');
sysobj.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHzClientStartStopCSIDataCollection');
