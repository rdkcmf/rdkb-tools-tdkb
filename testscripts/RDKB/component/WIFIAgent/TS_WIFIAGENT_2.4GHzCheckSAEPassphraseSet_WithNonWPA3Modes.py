...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_WIFIAGENT_2.4GHzCheckSAEPassphraseSet_WithNonWPA3Modes</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the SET operation of Device.WiFi.AccessPoint.1.Security.SAEPassphrase returns failure when the Device.WiFi.AccessPoint.1.Security.ModeEnabled is any of the non-WPA3 modes after enabling Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_182</test_case_id>
    <test_objective>Check if the SET operation of Device.WiFi.AccessPoint.1.Security.SAEPassphrase returns failure when the Device.WiFi.AccessPoint.1.Security.ModeEnabled is any of the non-WPA3 modes after enabling Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable.</test_objective>
...
4. Get the supported security modes list using Device.WiFi.AccessPoint.1.Security.ModesSupported
5. Get the initial value of SAE Passphrase using Device.WiFi.AccessPoint.1.Security.SAEPassphrase and store it
6. Set Device.WiFi.AccessPoint.1.Security.ModeEnabled to security modes except WPA3 modes
7. For each of the non-WPA3 security modes set, check if the SAE Passphrase Device.WiFi.AccessPoint.1.Security.SAEPassphrase set operation returns failure. Cross verify with GET.
...
11. Unload the module.</automation_approch>
    <expected_output>Setting Device.WiFi.AccessPoint.1.Security.SAEPassphrase should return failure when the security mode configured is not WPA3-Personal or WPA3-Personal-Transition.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_2.4GHzCheckSAEPassphraseSet_WithNonWPA3Modes</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHzCheckSAEPassphraseSet_WithNonWPA3Modes');
        ...
        if pre_req_set == 1:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    ...
                    #Set SAEPassphrase for each of the supported modes for the 2.4G private access point except the WPA3 modes and Enterprise modes
                    final_sae = "";
                            ...
                            if expectedresult in actualresult :
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    if final_mode == mode:
                                        ...
                                        print "\nTEST STEP %d : Set Device.WiFi.AccessPoint.1.Security.SAEPassphrase to %s in %s mode" %(step, saePassphrase, mode);
                                        print "EXPECTED RESULT %d : Should not set Device.WiFi.AccessPoint.1.Security.SAEPassphrase to %s in %s mode" %(step, saePassphrase, mode);
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            print "EXPECTED RESULT %d : Should successfully get Device.WiFi.AccessPoint.1.Security.SAEPassphrase and it should remain unchanged from initial value" %step;
