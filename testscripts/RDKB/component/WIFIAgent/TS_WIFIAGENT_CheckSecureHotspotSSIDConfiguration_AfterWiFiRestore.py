...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>7</version>
  <name>TS_WIFIAGENT_CheckSecureHotspotSSIDConfiguration_AfterWiFiRestore</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the Secure Hotspot SSID Configuration (AP 9, AP 10) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;9,10".</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_200</test_case_id>
    <test_objective>To check if the Secure Hotspot SSID Configuration (AP 9, AP 10) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;9,10".</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.SSID.9.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.10.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.9.SSID
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.SSID.10.SSID
paramValue : dynamically generated
paramType : string
...
2. Get the initial values of Device.WiFi.SSID.9.Enable and Device.WiFi.SSID.10.Enable and set to enabled if not already in enabled state.
3. Get the initial values of Device.WiFi.SSID.9.SSID and Device.WiFi.SSID.10.SSID
4. Set all the 2 above mentioned parameters to new dynamically generated values and verify with GET.
5. Perform WiFi restore of APs 9,10 with Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp
6. Sleep for 100s for the WiFi restore operation to take effect.
7. Check the values of Device.WiFi.SSID.9.SSID and Device.WiFi.SSID.10.SSID after WiFi restore. It should be different from the values before WiFi restore operation.
8. Revert to initial Secure Hotspot SSID configuration and SSID Enable status if required.
9. Unload the modules
</automation_approch>
    <expected_output>The Secure Hotspot SSID Configuration (AP 9, AP 10) should get restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;5,6".</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckSecureHotspotSSIDConfiguration_AfterWiFiRestore</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckSecureHotspotSSIDConfiguration_AfterWiFiRestore');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_CheckSecureHotspotSIDConfiguration_AfterWiFiRestore');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    paramList_enable=["Device.WiFi.SSID.9.Enable", "Device.WiFi.SSID.10.Enable"];
    tdkTestObj,status,enableValue = getMultipleParameterValues(obj,paramList_enable);

    print "\nTEST STEP 1: Get the values of SecureHotspot SSID enable status - ", paramList_enable;
    ...
    if expectedresult in status and enableValue[0] != "" and enableValue[1] != "":
        ...
        print "ACTUAL RESULT 1: SecureHotspot SSID enable statuses are respectively : %s, %s" %(enableValue[0],enableValue[1]) ;
        ...
        if ssid_enable_flag == 0:
            ...
            #Get the initial SecureHotspot SSID
            step = 3;
            paramList_ssid_config = ["Device.WiFi.SSID.9.SSID", "Device.WiFi.SSID.10.SSID"];
            tdkTestObj,status,orgSSIDConfig = getMultipleParameterValues(obj,paramList_ssid_config)

            print "\nTEST STEP 3: Get the SecureHotspot SSID configuration parameter values - ", paramList_ssid_config;
            ...
            if expectedresult in status and orgSSIDConfig[0] != "" and orgSSIDConfig[1] != "":
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 3: SecureHotspot SSID Configuration are respectively : %s, %s" %(orgSSIDConfig[0],orgSSIDConfig[1]) ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";

                #Set to new configuration
                step = 4;
                ssid1 = "SecureHotspot-SSID-" + str(randint(0, 99));
                ssid2 = "SecureHotspot-SSID-" + str(randint(100, 199));
                setvalues = [ssid1, ssid2];

                print "\nTEST STEP 4 : Set new SecureHotspot SSID Configuration using : ", paramList_ssid_config;
                print "EXPECTED RESULT 4 : New SecureHotspot SSID Configuration should be set successfully";
                ssid_config_flag = 0;
                ...
                if ssid_config_flag == 0:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "ACTUAL RESULT 4: New SecureHotspot SSID Configuration is set successfully" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    ...
                    tdkTestObj.addParameter("paramValue","1,2;9,10");
                    ...
                    if expectedresult in actualresult and details != "" :
                        ...
                        #Check if the SecureHotspot SSID configuration is changed to default values
                        step = 6;
                        tdkTestObj,status,finalSSIDConfig = getMultipleParameterValues(obj,paramList_ssid_config)

                        print "\nTEST STEP 6: Get the SecureHotspot SSID configuration parameter values - ", paramList_ssid_config;
                        ...
                        if expectedresult in status and finalSSIDConfig[0] != "" and finalSSIDConfig[1] != "":
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT 6: SecureHotspot SSID Configuration are respectively : %s, %s" %(finalSSIDConfig[0],finalSSIDConfig[1]) ;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                            ...
                            if finalSSIDConfig[0] == orgSSIDConfig[0] and finalSSIDConfig[1] == orgSSIDConfig[1]:
                                ...
                                step = 8;
                                print "\nTEST STEP 8 : Revert to initial SecureHotspot SSID Configuration";
                                print "EXPECTED RESULT 8 : Revert to initial SecureHotspot SSID Configuration should be successful";
                                ...
                                if finalSSIDConfig[1] != orgSSIDConfig[1]:
                                    #Enable the SSID
                                    status3 = setParameter(obj1, paramList_enable[1], "true", "boolean");
                            ...
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("FAILURE");
                            print "ACTUAL RESULT 6: SecureHotspot SSID Configuration is NOT retrieved";
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : FAILURE";
                    else :
                        tdkTestObj.setResultStatus("FAILURE");
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 4: New SecureHotspot SSID Configuration is NOT set successfully" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                #Set the result status of execution
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT 3: SecureHotspot SSID Configuration is NOT retrieved";
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            #Set the result status of execution
        ...
        #Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE");
        print "ACTUAL RESULT 1: SecureHotspot SSID enable statuses are NOT retrieved";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : FAILURE";
