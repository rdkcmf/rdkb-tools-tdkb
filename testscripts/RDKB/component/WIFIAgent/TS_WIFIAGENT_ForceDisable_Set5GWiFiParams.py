...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_ForceDisable_Set5GWiFiParams</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if radioenable,KeyPassphrase,AccessPoint parameters for 5G are not  writable when WiFi Force Disable is enabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_130</test_case_id>
    <test_objective>This test case is to check if radioenable,KeyPassphrase,AccessPoint parameters for 5G are not  writable when WiFi Force Disable is enabled</test_objective>
...
"Device.WiFi.Radio.2.Enable" ,"Device.WiFi.AccessPoint.2.Enable","Device.WiFi.AccessPoint.2.Security.X_COMCAST-COM_KeyPassphrase"</input_parameters>
...
4.Do a write operation on the following parameters "Device.WiFi.Radio.2.Enable" ,"Device.WiFi.AccessPoint.2.Enable","Device.WiFi.AccessPoint.2.Security.X_COMCAST-COM_KeyPassphrase" ,and this write operation is expected to fail
...
6.Unload the module</automation_approch>
    <expected_output> Write operation on the parameters should fail and
"WIFI_ATTEMPT_TO_CHANGE_CONFIG_WHEN_FORCE_DISABLED " message should be present in WiFilog.txt.0  each time a set operation is done</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIAGENT</test_stub_interface>
    <test_script>TS_WIFIAGENT_ForceDisable_Set5GWiFiParams</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_ForceDisable_Set5GWiFiParams ');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_ForceDisable_Set5GWiFiParams ');
...
def SetOperation(tdkTestObj,parameter):
    ...
    if parameter == "Device.WiFi.AccessPoint.2.Security.X_COMCAST-COM_KeyPassphrase":
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
           ...
           params = ["Device.WiFi.Radio.2.Enable" ,"Device.WiFi.AccessPoint.2.Enable","Device.WiFi.AccessPoint.2.Security.X_COMCAST-COM_KeyPassphrase"]
