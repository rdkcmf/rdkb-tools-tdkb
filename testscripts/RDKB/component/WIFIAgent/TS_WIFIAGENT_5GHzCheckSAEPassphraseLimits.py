...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIAGENT_5GHzCheckSAEPassphraseLimits</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the SAEPassphrase Device.WiFi.AccessPoint.2.Security.SAEPassphrase SET operation fails when the length of the Passphrase is less than lower limit of 8 characters and more than upper limit of 63 characters.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_189</test_case_id>
    <test_objective>Check if the SAEPassphrase Device.WiFi.AccessPoint.2.Security.SAEPassphrase SET operation fails when the length of the Passphrase is less than lower limit of 8 characters and more than upper limit of 63 characters.</test_objective>
...
4. Get the initial value of Device.WiFi.AccessPoint.2.Security.SAEPassphrase and store it.
5. Set Device.WiFi.AccessPoint.2.Security.ModeEnabled to "WPA3-Personal-Transition" and validate the SET with GET.
6. Check if the SET operation of Device.WiFi.AccessPoint.2.Security.SAEPassphrase fails for a passphrase length less than the lower limit of 8 characters. Verify the SET with GET.
7. Check if the SET operation of Device.WiFi.AccessPoint.2.Security.SAEPassphrase fails for a passphrase length more than the upper limit of 63 characters. Verify the SET with GET.
8. Revert Device.WiFi.AccessPoint.2.Security.SAEPassphrase if required.
...
11. Unload the module</automation_approch>
    <expected_output>Setting Device.WiFi.AccessPoint.2.Security.SAEPassphrase to SAE Passphrases whose length is lower than the lower limit(8) and more than the upper limit(63) should return failure.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHzCheckSAEPassphraseLimits</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHzCheckSAEPassphraseLimits');
        ...
        if pre_req_set == 1:
            ...
            if expectedresult in actualresult:
                ...
                if mode_flag == 1:
                    ...
                    print "\nTEST STEP %d : Check if setting Device.WiFi.AccessPoint.2.Security.SAEPassphrase to a value less than the lower limit of charcters(<8) - %s fails" %(step, saePassphrase);
                    print "EXPECTED RESULT %d : Setting Device.WiFi.AccessPoint.2.Security.SAEPassphrase to a value less than the lower limit of charcters(<8) - %s should fail" %(step,saePassphrase);
                    ...
                    print "\nTEST STEP %d : Check if setting Device.WiFi.AccessPoint.2.Security.SAEPassphrase to a value more than the upper limit of charcters(>63) - %s fails" %(step, saePassphrase);
                    print "EXPECTED RESULT %d : Setting Device.WiFi.AccessPoint.2.Security.SAEPassphrase to a value more than the upper limit of charcters(>63) - %s should fail" %(step,saePassphrase);
