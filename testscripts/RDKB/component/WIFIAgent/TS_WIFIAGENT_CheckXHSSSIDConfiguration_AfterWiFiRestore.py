...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>TS_WIFIAGENT_CheckXHSSSIDConfiguration_AfterWiFiRestore</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the XHS SSID Configuration (AP 3, AP 4) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;3,4".</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_197</test_case_id>
    <test_objective>To check if the XHS SSID Configuration (AP 3, AP 4) is getting restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;3,4".</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.SSID.3.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.4.Enable
paramValue : true/false
paramType : boolean
paramName : Device.WiFi.SSID.3.SSID
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.SSID.4.SSID
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.AccessPoint.3.Security.KeyPassphrase
paramValue : dynamically generated
paramType : string
paramName : Device.WiFi.AccessPoint.4.Security.KeyPassphrase
paramValue : dynamically generated
paramType : string
...
2. Get the initial values of Device.WiFi.SSID.3.Enable and Device.WiFi.SSID.4.Enable and set to enabled if not already in enabled state.
3. Get the initial values of Device.WiFi.SSID.3.SSID, Device.WiFi.SSID.4.SSID, Device.WiFi.AccessPoint.3.Security.KeyPassphrase and Device.WiFi.AccessPoint.4.Security.KeyPassphrase.
4. Set all the 4 above mentioned parameters to new dynamically generated values and verify with GET.
5. Perform WiFi restore of APs 3,4 with Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp
6. Sleep for 100s for the WiFi restore operation to take effect.
7. Check the values of Device.WiFi.SSID.3.SSID, Device.WiFi.SSID.4.SSID, Device.WiFi.AccessPoint.3.Security.KeyPassphrase and Device.WiFi.AccessPoint.4.Security.KeyPassphrase after WiFi restore. It should be different from the values before WiFi restore operation.
8. Revert to initial XHS SSID configuration and SSID Enable status if required.
9. Unload the modules
</automation_approch>
    <expected_output>The XHS SSID Configuration (AP 3, AP 4) should get restored when Device.WiFi.X_CISCO_COM_FactoryResetRadioAndAp is set to "1,2;3,4".</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckXHSSSIDConfiguration_AfterWiFiRestore</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckXHSSSIDConfiguration_AfterWiFiRestore');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_CheckXHSSIDConfiguration_AfterWiFiRestore');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    paramList_enable=["Device.WiFi.SSID.3.Enable", "Device.WiFi.SSID.4.Enable"];
    tdkTestObj,status,enableValue = getMultipleParameterValues(obj,paramList_enable)

    print "\nTEST STEP 1: Get the values of XHS SSID enable status - ", paramList_enable;
    ...
    if expectedresult in status and enableValue[0] != "" and enableValue[1] != "":
        ...
        print "ACTUAL RESULT 1: XHS SSID enable statuses are respectively : %s, %s" %(enableValue[0],enableValue[1]) ;
        ...
        if ssid_enable_flag == 0:
            ...
            #Get the initial XHS SSID and KeyPassphrase
            step = 3;
            paramList_ssid_config = ["Device.WiFi.SSID.3.SSID", "Device.WiFi.AccessPoint.3.Security.KeyPassphrase", "Device.WiFi.SSID.4.SSID", "Device.WiFi.AccessPoint.4.Security.KeyPassphrase"];
            tdkTestObj,status,orgSSIDConfig = getMultipleParameterValues(obj,paramList_ssid_config)

            print "\nTEST STEP 3: Get the XHS SSID configuration parameter values - ", paramList_ssid_config;
            ...
            if expectedresult in status and orgSSIDConfig[0] != "" and orgSSIDConfig[1] != "" and orgSSIDConfig[2] != "" and orgSSIDConfig[3] != "":
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 3: XHS SSID Configuration are respectively : %s, %s, %s, %s" %(orgSSIDConfig[0],orgSSIDConfig[1],orgSSIDConfig[2],orgSSIDConfig[3]) ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";

                #Set to new configuration
                step = 4;
                ssid1 = "XHS-SSID-" + str(randint(0, 99));
                key1 = "XHS-Key-" + str(randint(0, 99));
                ssid2 = "XHS-SSID-" + str(randint(100, 199));
                key2 = "XHS-Key-" + str(randint(100, 199));
                setvalues = [ssid1, key1, ssid2, key2];

                print "\nTEST STEP 4 : Set new XHS SSID Configuration using : ", paramList_ssid_config;
                print "EXPECTED RESULT 4 : New XHS SSID Configuration should be set successfully";
                ssid_config_flag = 0;
                ...
                if ssid_config_flag == 0:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "ACTUAL RESULT 4: New XHS SSID Configuration is set successfully" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    ...
                    tdkTestObj.addParameter("paramValue","1,2;3,4");
                    ...
                    if expectedresult in actualresult and details != "" :
                        ...
                        #Check if the XHS SSID configuration is changed to default values
                        step = 6;
                        tdkTestObj,status,finalSSIDConfig = getMultipleParameterValues(obj,paramList_ssid_config)

                        print "\nTEST STEP 6: Get the XHS SSID configuration parameter values - ", paramList_ssid_config;
                        ...
                        if expectedresult in status and finalSSIDConfig[0] != "" and finalSSIDConfig[1] != "" and finalSSIDConfig[2] != "" and finalSSIDConfig[3] != "":
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT 6: XHS SSID Configuration are respectively : %s, %s, %s, %s" %(finalSSIDConfig[0],finalSSIDConfig[1],finalSSIDConfig[2],finalSSIDConfig[3]) ;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                            ...
                            if finalSSIDConfig[0] == orgSSIDConfig[0] and finalSSIDConfig[1] == orgSSIDConfig[1] and finalSSIDConfig[2] == orgSSIDConfig[2] and finalSSIDConfig[3] == orgSSIDConfig[3]:
                                ...
                                step = 8;
                                print "\nTEST STEP 8 : Revert to initial XHS SSID Configuration";
                                print "EXPECTED RESULT 8 : Revert to initial XHS SSID Configuration should be successful";
                            ...
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("FAILURE");
                            print "ACTUAL RESULT 6: XHS SSID Configuration is NOT retrieved";
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : FAILURE";
                    else :
                        tdkTestObj.setResultStatus("FAILURE");
                    ...
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 4: New XHS SSID Configuration is NOT set successfully" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                #Set the result status of execution
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT 3: XHS SSID Configuration is NOT retrieved";
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            #Set the result status of execution
        ...
        #Set the result status of execution
        tdkTestObj.setResultStatus("FAILURE");
        print "ACTUAL RESULT 1: XHS SSID enable statuses are NOT retrieved";
        #Get the result of execution
        print "[TEST EXECUTION RESULT] : FAILURE";
