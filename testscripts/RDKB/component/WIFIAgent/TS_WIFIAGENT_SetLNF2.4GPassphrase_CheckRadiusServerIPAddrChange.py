...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_WIFIAGENT_SetLNF2.4GPassphrase_CheckRadiusServerIPAddrChange</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the Radius Server IP Address configuration for LNF SECURE 2.4G, Device.WiFi.AccessPoint.11.Security.RadiusServerIPAddr remains unchanged when the KeyPassphrase, Device.WiFi.AccessPoint.11.Security.KeyPassphrase is updated.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_164</test_case_id>
    <test_objective>Check if the Radius Server IP Address configuration for LNF SECURE 2.4G, Device.WiFi.AccessPoint.11.Security.RadiusServerIPAddr remains unchanged when the KeyPassphrase, Device.WiFi.AccessPoint.11.Security.KeyPassphrase is updated.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.AccessPoint.11.Security.RadiusServerIPAddr
paramName : Device.WiFi.AccessPoint.11.Security.KeyPassphrase
paramValue : randomly chosen key passphrase
paramType : string</input_parameters>
    <automation_approch>1. Load the modules
2. Get the initial Radius IP Address for LNF SECURE 2.4G using Device.WiFi.AccessPoint.11.Security.RadiusServerIPAddr and store it.
3. Get the initial KeyPassphrase Device.WiFi.AccessPoint.11.Security.KeyPassphrase and check whether it is exposed via TR181
4. Set Device.WiFi.AccessPoint.11.Security.KeyPassphrase to a new value. The SET operation should be success, but the GET operation should not expose the keypassphrase.
5. Check if the Radius Server IP configuration remains unchanged.
6. Unload the modules</automation_approch>
    <expected_output>The Radius Server IP Address configuration for LNF SECURE 2.4G, Device.WiFi.AccessPoint.11.Security.RadiusServerIPAddr should remain unchanged when the KeyPassphrase, Device.WiFi.AccessPoint.11.Security.KeyPassphrase is updated.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_SetLNF2.4GPassphrase_CheckRadiusServerIPAddrChange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_SetLNF2.4GPassphrase_CheckRadiusServerIPAddrChange');
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            if initial_key == "":
                ...
                step = step + 1;
                key_new = "lnf-2.4G-wifi_" + str(randint(0,100));
