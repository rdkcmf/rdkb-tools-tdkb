...
<xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_5GHZ_IsPublicWiFiSSIDUp</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set Device.WiFi.SSID.6.Enable to true and check whether the status is Up</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_54</test_case_id>
    <test_objective>Set Device.WiFi.SSID.6.Enable to true and check whether the status is Up</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, Emulator, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Device.WiFi.SSID.6.Enable
Device.WiFi.SSID.6.Status</input_parameters>
    <automation_approch>1. Load wifiagent module
2. Using WIFIAgent_Get, get and save Device.WiFi.SSID.6.Enable
3.Set DDevice.WiFi.SSID.6.Enable as true
3. Using WIFIAgent_Get, get Device.WiFi.SSID.6.Status
6. Restore value of Device.WiFi.SSID.6.Enable</automation_approch>
    <except_output>Device.WiFi.SSID.6.Status should be Up</except_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHZ_IsPublicWiFiSSIDUp</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHZ_IsPublicWiFiSSIDUp');
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            if expectedresult in actualresult and "Up" in status:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3: Check if SSID staus is Up";
                print "EXPECTED RESULT 3: SSID staus should be Up";
                print "ACTUAL RESULT 3: Status of SSID6: %s" %status;
                ...
                print "TEST STEP 3: Check if SSID staus is Up";
                print "EXPECTED RESULT 3: SSID staus should be Up";
                print "ACTUAL RESULT 3: Status of SSID6: %s" %details;
                #Get the result of execution
