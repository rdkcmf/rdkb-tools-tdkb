...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_2_LoggingWithErrorsReceivedDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the telemetry for WiFi Packet Retries in the uplink direction for 5G "UP_RetransCount_2" is not getting logged in wifihealth.txt within the specified Log Interval after the RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable is enabled and Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_204</test_case_id>
    <test_objective>Check if the telemetry for WiFi Packet Retries in the uplink direction for 5G "UP_RetransCount_2" is not getting logged in wifihealth.txt within the specified Log Interval after the RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable is enabled and Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable is disabled.</test_objective>
...
6. If the enable parameter for 5G private AP is  not disabled, set to false and validate the set.
...
12. The difference between the final and initial number of "UP_RetransCount_2" markers in wifihealth.txt should be equal to 0 after disabling Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable.
13. Revert the telemetry log interval to initial value
14. Revert Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable if required.
15. Revert Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable if required.
16. Unload the modules</automation_approch>
    <expected_output>The telemetry for WiFi Packet Retries in the uplink direction for 5G "UP_RetransCount_2" should not be logged in wifihealth.txt within the specified Log Interval after the RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable is enabled and Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable is disabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_2_LoggingWithErrorsReceivedDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_2_LoggingWithErrorsReceivedDisabled');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_2_LoggingWithErrorsReceivedDisabled');
        ...
        if expectedresult in actualresult and details != "":
            ...
            if proceed_flag == 1:
                ...
                if expectedresult in actualresult and details != "":
                    ...
                    if error_stats_initial != "false":
                        step = step + 1;
                        actualresult, details = setParameter(obj, paramName, "false", "boolean");

                        print "\nTEST STEP %d: Set Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable to false" %step;
                        print "EXPECTED RESULT %d: Should set the enable state of Device.WiFi.AccessPoint.2.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable to false" %step;
                        ...
                        if expectedresult in actualresult and details != "":
                            ...
                            if DeflogInt.isdigit():
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    print "\nThe UP_RetransCount_2 markers can be = 0, after accounting for the polling interval and the new log interval set";

                                    print "TEST STEP %d: Should get the number of UP_RetransCount_2 markers count equal to 0 after disabling the Errors Received Stats" %step;
                                    print "EXPECTED RESULT %d: The number of UP_RetransCount_2 markers count should be equal to 0 after disabling the Errors Received Stats" %step;

                                    if difference == 0:
                                        tdkTestObj.setResultStatus("SUCCESS");
                                        print "ACTUAL RESULT %d: Number of new UP_RetransCount_2 markers is : %d" %(step, difference);
                                        #Get the result of execution
                                        print "[TEST EXECUTION RESULT] : SUCCESS";
                                    else:
                                        tdkTestObj.setResultStatus("FAILURE");
