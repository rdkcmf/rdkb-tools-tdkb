...
<xml>
  ...
  <name>TS_WIFIAGENT_5GHZ_ModeEnabled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the security mode enabled is from the list of supported modes</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_22</test_case_id>
    <test_objective>Check if the security mode enabled is from the list of supported modes</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>Device.WiFi.AccessPoint.2.Security.ModesSupported
Device.WiFi.AccessPoint.2.Security.ModeEnabled</input_parameters>
    <automation_approch>1. Load wifiagent module
2. Using WIFIAgent_Get, get and save Device.WiFi.AccessPoint.2.Security.ModesSupported value
4. Using WIFIAgent_Get, get Device.WiFi.AccessPoint.2.Security.ModeEnabled
5.Check if enabled mode is from the supported modes list
...
7.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from pam stub.</automation_approch>
    <except_output>The enabled security mode should be from the supported modes list</except_output>
    <priority>High</priority>
    <test_stub_interface>WifiAgent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHZ_ModeEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHZ_ModeEnabled');
