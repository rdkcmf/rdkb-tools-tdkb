...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WIFIAGENT_5GHzOffChannelNchannelValidation_WithDFSDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the number of channels being scanned, Device.WiFi.Radio.2.X_RDK_OffChannelNchannel,  returns the value 8 when DFS is disabled using Device.WiFi.Radio.2.X_COMCAST_COM_DFSEnable and Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OffChannelScan.Enable is enabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_221</test_case_id>
    <test_objective>To check if the number of channels being scanned, Device.WiFi.Radio.2.X_RDK_OffChannelNchannel,  returns the value 8 when DFS is disabled using Device.WiFi.Radio.2.X_COMCAST_COM_DFSEnable and Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OffChannelScan.Enable is enabled.</test_objective>
...
4. If the DFS is not already disabled, disable it and validate with get.
5. Get the value of Device.WiFi.Radio.2.X_RDK_OffChannelNchannel.
6. When DFS is disabled the expected N channel value is "8". Check if the value returned is 8.
...
9. Unload the wifiagent module.</automation_approch>
    <expected_output>Device.WiFi.Radio.2.X_RDK_OffChannelNchanne should return the value 8 when DFS is disabled using Device.WiFi.Radio.2.X_COMCAST_COM_DFSEnable and Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.OffChannelScan.Enable is enabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHzOffChannelNchannelValidation_WithDFSDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHzOffChannelNchannelValidation_WithDFSDisabled');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if pre_req_set == 0:
        ...
        if expectedresult in actualresult and details != "":
            ...
            #Check if the DFS enabled is "false", else set to "false"
            proceed_flag = 0;
            if dfs_initial != "false":
                step = step + 1;
                set_val = "false"
                print "\nTEST STEP %d: Set Device.WiFi.Radio.2.X_COMCAST_COM_DFSEnable to %s" %(step, set_val);
                print "EXPECTED RESULT %d: Device.WiFi.Radio.2.X_COMCAST_COM_DFSEnable should be disabled successfully" %step;
            ...
            #Check the N channel value if DFS is disabled
            if proceed_flag == 0:
                step = step + 1;
                ...
                if expectedresult in actualresult and details != "":
                    ...
                    #Check if the value is 8 when DFS is in disabled state
                    step = step + 1;
                    print "\nTEST STEP %d: Check if the number of channels that are scanned using Device.WiFi.Radio.2.X_RDK_OffChannelNchannel returns 8 with DFS disabled" %step;
                    print "EXPECTED RESULT %d: Should get the value of Device.WiFi.Radio.2.X_RDK_OffChannelNchannel as 8 with DFS in disabled state" %step;

                    if n_channel.isdigit() and int(n_channel) == 8:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "ACTUAL RESULT %d: The N channel value is 8 as expected" %(step);
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "ACTUAL RESULT %d: The N channel value is NOT 8 as expected" %(step);
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : FAILURE";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                ...
                tdkTestObj.setResultStatus("FAILURE");
                print "Unable to disable Device.WiFi.Radio.2.X_COMCAST_COM_DFSEnable, cannot proceed..."
        else:
            tdkTestObj.setResultStatus("FAILURE");
