...
<xml>
  ...
  <name>TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzConnectedClientMacaddress</name>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_93</test_case_id>
    <test_objective>Check the WiFi Telemetry Marker "WIFI_MAC_1" for 2.4 GHZ connected client and verify the value of host table connected client mac address</test_objective>
...
2.Grep "WIFI_MAC_1" in wifihealth.txt file and get the MAC address value
...
5.Unload module</automation_approch>
    ...
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzConnectedClientMacaddress</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzConnectedClientMacaddress');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzConnectedClientMacaddress');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in sysutilloadmodulestatus.upper():
    ...
    if details == "File exist":
        ...
        for i in range(1,15):
	        ...
	        query="cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_MAC_1:\""
	        print "query:%s" %query
		tdkTestObj = sysObj.createTestStep('ExecuteCmd');
		...
		details = tdkTestObj.getResultDetails().strip().replace("\\n","");
        	print "Marker Detail Found fromLog file is: %s "%details;
 	        ...
 	        if (len(details) == 0) or details.endswith(":") or "WIFI_MAC_1" not in details:
		    markerfound = 0;
                    sleep(60);
		else:
            	    telemetryMacaddress = details.split("WIFI_MAC_1:")[1].split(',')[0];
	            markerfound = 1;

	if expectedresult in actualresult and markerfound == 1:
            tdkTestObj.setResultStatus("SUCCESS");
	    print "TEST STEP 2: WIFI_MAC_1 Marker should be present";
	    print "EXPECTED RESULT 2: WIFI_MAC_1 Marker should be present";
	    print "ACTUAL RESULT 2:WIFI_MAC_1 Marker is %s" %telemetryMacaddress
	    #Get the result of execution
            print "[TEST EXECUTION RESULT] : SUCCESS";
	    ...
	    print "TEST STEP 2: WIFI_MAC_1 Marker should be present";
	    print "EXPECTED RESULT 2: WIFI_MAC_1 Marker should be present";
	    print "ACTUAL RESULT 2:WIFI_MAC_1 Marker is  Not Present";
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : FAILURE";
    else:
        tdkTestObj.setResultStatus("FAILURE");
