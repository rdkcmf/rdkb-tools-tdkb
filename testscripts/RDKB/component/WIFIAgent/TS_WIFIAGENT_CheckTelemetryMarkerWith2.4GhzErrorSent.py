...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzErrorSent</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_108</test_case_id>
...
3.Connect a WiFi client to 2.4Ghz private SSID of DUT</pre_requisite>
    ...
    <input_parameters>Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.ErrorsSent
...
4.Check for telemetry marker WIFI_ERRORSSENT_1 and get the error sent using Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.ErrorsSent .
...
7.Unload the Module
    ...
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzErrorSent</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzErrorSent');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzErrorSent');
                   ...
                   for i in range(1,6):
                       if markerfound == 1:
                          break;
                           ...
                           query="cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_ERRORSSENT_1:\""
                           print "query:%s" %query
                           tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                           ...
                           if (len(details) == 0) or details.endswith(":") or "WIFI_ERRORSSENT_1" not in details:
                              markerfound = 0;
                              sleep(60);
                           else:
                               tel_errorsent = details.split("WIFI_ERRORSSENT_1:")[1].split(',')[0].strip().replace("\\n","");
                               markerfound = 1;

                   if expectedresult in actualresult and markerfound == 1 and tel_errorsent!= "":
                      tdkTestObj.setResultStatus("SUCCESS");
                      print "TEST STEP 5:WIFI_ERRORSSENT_1 Marker should be present";
                      print "EXPECTED RESULT 5: WIFI_ERRORSSENT_1 Marker should be present";
                      print "ACTUAL RESULT 5:WIFI_ERRORSSENT_1 Marker is %s" %tel_errorsent;
                      #Get the result of execution
                      print "[TEST EXECUTION RESULT] : SUCCESS";


                      tdkTestObj = obj.createTestStep('pam_GetParameterValues');
                      tdkTestObj.addParameter("ParamName","Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.ErrorsSent");
                      expectedresult="SUCCESS";
                      #Execute the test case in DUT
                      tdkTestObj.executeTestCase(expectedresult);
                      actualresult = tdkTestObj.getResult();
                      ...
                      if expectedresult in actualresult:
                         ...
                         if int(tel_errorsent) >= 0 and  int(tr181_errorsent) >= 0 and int(tel_errorsent) ==  int(tr181_errorsent):
                            ...
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                             ...
                             tdkTestObj.setResultStatus("FAILURE");
                             print "TEST STEP 7: Check if ERRORSENT via TR-181 parameter and telemetry marker are equal and greater than or equal to zero";
                             print "EXPECTED RESULT 7: Should get the ERRORSENT via TR-181 parameter and telemetry marker equal and greater than or equal to zero";
                       ...
                       tdkTestObj.setResultStatus("FAILURE");
                       print "TEST STEP 5: WIFI_ERRORSENT_1 Marker should be present";
                       print "EXPECTED RESULT 5: WIFI_ERRORSENT_1 Marker should be present";
                       print "ACTUAL RESULT 5:WIFI_ERRORSENT_1 Marker not present"
                       #Get the result of execution
                       print "[TEST EXECUTION RESULT] : FAILURE";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
