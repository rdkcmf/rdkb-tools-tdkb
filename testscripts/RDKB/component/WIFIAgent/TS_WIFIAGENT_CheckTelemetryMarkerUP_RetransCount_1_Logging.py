...
<xml>
  ...
  <name>TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_1_Logging</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if the telemetry for WiFi Packet Retries in the uplink direction for 2.4G "UP_RetransCount_1" is getting logged properly in wifihealth.txt within the specified Log Interval after the RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable and Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable are enabled.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_201</test_case_id>
    <test_objective>Check if the telemetry for WiFi Packet Retries in the uplink direction for 2.4G "UP_RetransCount_1" is getting logged properly in wifihealth.txt within the specified Log Interval after the RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable and Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable are enabled.</test_objective>
...
5. Get the initial value of Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable
6. If the enable parameter for 2.4G private AP is  not enabled, set to true and validate the set.
...
9. Get the initial marker count of "UP_RetransCount_1" in wifihealth.txt for the connected client.
10. Sleep for a duration set by the log interval and 60s to include the telemetry log interval polling time.
11. Get the final marker count of "UP_RetransCount_1" in wifihealth.txt for the connected client.
12. The difference between the final and initial number of "UP_RetransCount_1" markers in wifihealth.txt should be greater than or equal to 1.
13. Check if the marker value is a valid integer greater than 0.
14. Revert the telemetry log interval to initial value
15. Revert Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable if required.
...
17. Unload the modules</automation_approch>
    <expected_output>The telemetry for WiFi Packet Retries in the uplink direction for 2.4G "UP_RetransCount_1" should get logged properly in wifihealth.txt within the specified Log Interval after the RFC Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.ErrorsReceived.Enable and Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable are enabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_1_Logging</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_1_Logging');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerUP_RetransCount_1_Logging');
            ...
            if proceed_flag == 1:
                ...
                if expectedresult in actualresult and details != "":
                    ...
                    if error_stats_initial != "true":
                        ...
                        print "\nTEST STEP %d: Set Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable to true" %step;
                        print "EXPECTED RESULT %d: Should set the enable state of Device.WiFi.AccessPoint.1.X_COMCAST-COM_AssociatedDevicesErrorsReceivedStatsEnable to true" %step;
                        ...
                        if expectedresult in actualresult and details != "":
                            ...
                            if DeflogInt.isdigit():
                                ...
                                if expectedresult in actualresult:
                                    ...
                                    print "\nThe UP_RetransCount_1 markers can be >= 1, after accounting for the polling interval and the new log interval set";

                                    print "TEST STEP %d: Should get UP_RetransCount_1 markers count greater than or equal to 1" %step;
                                    print "EXPECTED RESULT %d: The UP_RetransCount_1 markers count should be greater than or equal to 1" %step;
                                    ...
                                    if difference >= 1:
                                        ...
                                        #Check if the marker value of UP_RetransCount_1 >= 0
                                        step = step + 1;
                                        cmd= "cat /rdklogs/logs/wifihealth.txt | grep \"UP_RetransCount_1:\"";
                                        print "\nCommand : ", cmd;
                                        tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                                        actualresult, details = doSysutilExecuteCommand(tdkTestObj,cmd);

                                        print "\nTEST STEP %d : Check if the marker value of UP_RetransCount_1 is greater than or equal to 0" %(step);
                                        print "EXPECTED RESULT %d : The marker value of UP_RetransCount_1 should be greater than or equal to 0" %(step);

                                        if expectedresult in actualresult and details != "":
                                            marker_val = details.strip().split("UP_RetransCount_1:")[1].split(",")[0];
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            ...
                                            if marker_val.isdigit() and int(marker_val) >= 0:
                                                ...
                                                tdkTestObj.setResultStatus("FAILURE");
                                                print "Marker value is NOT valid or NOT greater than 0 or equal to 0";
                                        else:
                                            tdkTestObj.setResultStatus("FAILURE");
