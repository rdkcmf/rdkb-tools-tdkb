...
<xml>
  ...
  <name>TS_WIFIAGENT_GetDefaultHomeSecurityPassPharseValues</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Home Security WiFi Key passpharses of 2.4GHz and 5GHz should be same after a factory reset</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_100</test_case_id>
    <test_objective>Home Security WiFi Key passpharses of 2.4GHz and 5GHz should be same after a factory reset</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>N/A</api_or_interface_used>
    <input_parameters>Device.WiFi.AccessPoint.3.Security.KeyPassphrase, Device.WiFi.AccessPoint.4.Security.KeyPassphrase</input_parameters>
    <automation_approch>1. Load the Module
2. Factory reset the DUT
3. Get the key passpharse values of Homesecurity 2.4Ghz and 5Ghz WiFi
4. Compare the passpharse of 2.4Ghz and 5Ghz and it should be same
5. Unload the Module</automation_approch>
    <expected_output>Home Security Key passpharses of 2.4GHz and 5GHz should be same after a factory reset</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIAGENT</test_stub_interface>
    <test_script>TS_WIFIAGENT_GetDefaultHomeSecurityPassPharseValues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_GetDefaultHomeSecurityPassPharseValues');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj.addParameter("paramName","Device.WiFi.AccessPoint.3.Security.KeyPassphrase");
        expectedresult="SUCCESS";
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
