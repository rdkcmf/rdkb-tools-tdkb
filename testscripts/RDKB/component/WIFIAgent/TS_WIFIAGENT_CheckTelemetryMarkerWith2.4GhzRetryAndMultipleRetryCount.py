...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzRetryAndMultipleRetryCount</name>
  <primitive_test_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_112</test_case_id>
...
3.Connect a WiFi client to 2.4Ghz private SSID of DUT</pre_requisite>
    ...
    <input_parameters>Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.RetryCount
Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.MultipleRetryCount
...
4.Check for telemetry marker WIFI_RETRYCOUNT_1 and get the bytes sent using Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.RetryCount
5.Check if the values received are greater than or equal to zero
6.Check for telemetry marker WIFI_MULTIPLERETRYCOUNT_1 and get the bytes sent using Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.MultipleRetryCount
...
9.Unload the Module
    ...
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzRetryAndMultipleRetryCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzRetryAndMultipleRetryCount');
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerWith2.4GhzRetryAndMultipleRetryCount');
                   ...
                   for i in range(1,6):
                       if markerfound == 1:
                          break;
                           ...
                           query="cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_RETRYCOUNT_1:\""
                           print "query:%s" %query
                           tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                           ...
                           if (len(details) == 0) or details.endswith(":") or "WIFI_RETRYCOUNT_1" not in details:
                              markerfound = 0;
                              sleep(60);
                           else:
                               tel_retrycount  = details.split("WIFI_RETRYCOUNT_1:")[1].split(',')[0].strip().replace("\\n","");
                               markerfound = 1;

                   if expectedresult in actualresult and markerfound == 1 and tel_retrycount != "":
                      tdkTestObj.setResultStatus("SUCCESS");
                      print "TEST STEP 5:WIFI_RETRYCOUNT_1 Marker should be present";
                      print "EXPECTED RESULT 5: WIFI_RETRYCOUNT_1 Marker should be present";
                      print "ACTUAL RESULT 5:WIFI_RETRYCOUNT_1 Marker is %s" %tel_retrycount;
                      #Get the result of execution
                      print "[TEST EXECUTION RESULT] : SUCCESS";


                      tdkTestObj = obj.createTestStep('pam_GetParameterValues');
                      tdkTestObj.addParameter("ParamName","Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.RetryCount");
                      expectedresult="SUCCESS";
                      #Execute the test case in DUT
                      tdkTestObj.executeTestCase(expectedresult);
                      actualresult = tdkTestObj.getResult();
                      ...
                      if expectedresult in actualresult:
                            ...
                            for i in range(1,6):
                                if markerfound == 1:
                                   break;
                                    ...
                                    #Query for the Telemetry Marker
                                    query="cat /rdklogs/logs/wifihealth.txt | grep -i \"WIFI_MULTIPLERETRYCOUNT_1:\""
                                    print "query:%s" %query
                                    tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                                    tdkTestObj.addParameter("command", query)
                                    expectedresult="SUCCESS";
                                    ...
                                    details = tdkTestObj.getResultDetails().strip().replace("\\n","");
                                    print "Marker Detail Found fromLog file is: %s "%details;
                                    if (len(details) == 0) or details.endswith(":") or "WIFI_MULTIPLERETRYCOUNT_1" not in details:
                                       markerfound = 0;
                                       sleep(60);
                                    else:
                                        tel_mretrycount = details.split("WIFI_MULTIPLERETRYCOUNT_1:")[1].split(',')[0].strip().replace("\\n","");
                                        markerfound = 1;

                            if expectedresult in actualresult and markerfound == 1 and tel_mretrycount != "":
                               tdkTestObj.setResultStatus("SUCCESS");
                               print "TEST STEP 8: WIFI_MULTIPLERETRYCOUNT_1  Marker should be present";
                               print "EXPECTED RESULT 8: WIFI_MULTIPLERETRYCOUNT_1  Marker should be present";
                               print "ACTUAL RESULT 8:WIFI_MULTIPLERETRYCOUNT_1 is %s" %tel_mretrycount;
                               #Get the result of execution
                               print "[TEST EXECUTION RESULT] : SUCCESS";

                               tdkTestObj = obj.createTestStep('pam_GetParameterValues');
                               tdkTestObj.addParameter("ParamName","Device.WiFi.AccessPoint.1.AssociatedDevice.1.Stats.MultipleRetryCount");
                               expectedresult="SUCCESS";
                               #Execute the test case in DUT
                               tdkTestObj.executeTestCase(expectedresult);
                               actualresult = tdkTestObj.getResult();
                               ...
                               if expectedresult in actualresult:
                                ...
                                print "TEST STEP 8: WIFI_MULTIPLERETRYCOUNT_1  Marker should be present";
                                print "EXPECTED RESULT 8: WIFI_MULTIPLERETRYCOUNT_1  Marker should be present";
                                print "ACTUAL RESULT 8:WIFI_MULTIPLERETRYCOUNT_1 is not present"
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : FAILURE";
                         else:
                             tdkTestObj.setResultStatus("FAILURE");
                       ...
                       print "TEST STEP 5: WIFI_RETRYCOUNT_1 Marker should be present";
                       print "EXPECTED RESULT 5: WIFI_RETRYCOUNT_1 Marker should be present";
                       print "ACTUAL RESULT 5:WIFI_RETRYCOUNT_1 Marker not present"
                       #Get the result of execution
                       print "[TEST EXECUTION RESULT] : FAILURE";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
