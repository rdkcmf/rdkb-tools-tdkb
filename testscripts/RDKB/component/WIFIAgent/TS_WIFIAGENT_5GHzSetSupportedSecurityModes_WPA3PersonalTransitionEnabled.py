...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_WIFIAGENT_5GHzSetSupportedSecurityModes_WPA3PersonalTransitionEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>When the WPA3 Personal Transition RFC, Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable is enabled, check if it possible to set Device.WiFi.AccessPoint.2.Security.ModeEnabled to all the supported security modes that are applicable.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_177</test_case_id>
    <test_objective>When the WPA3 Personal Transition RFC, Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable is enabled, check if it possible to set Device.WiFi.AccessPoint.2.Security.ModeEnabled to all the supported security modes that are applicable.</test_objective>
...
paramType : string
paramName : Device.WiFi.AccessPoint.2.Security.ModesSupported</input_parameters>
...
4. Get the supported modes using Device.WiFi.AccessPoint.2.Security.ModesSupported
5. Check if the SET operation of Device.WiFi.AccessPoint.2.Security.ModeEnabled is success for all supported modes that can be set for AP 2.
...
8. Unload the module.</automation_approch>
    <expected_output>Should be able to set all the supported security modes that are applicable to Device.WiFi.AccessPoint.2.Security.ModeEnabled when the WPA3 RFC  Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.WPA3_Personal_Transition.Enable is enabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHzSetSupportedSecurityModes_WPA3PersonalTransitionEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_5GHzSetSupportedSecurityModes_WPA3PersonalTransitionEnabled');
    ...
    if expectedresult in actualresult:
        ...
        if pre_req_set == 1:
            ...
            if expectedresult in actualresult:
                ...
                #Set all supported modes to the 5G private access point except the Enterprise modes when the WPA3 RFC is in enabled state
