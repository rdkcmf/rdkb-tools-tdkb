...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WIFIAGENT_CheckOperatingStandards_With2G80211axEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the operating standards retrieved using Device.WiFi.Radio.1.OperatingStandards is "g,n,ax" when the parameter Device.WiFi.2G80211axEnable is in enabled state.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_191</test_case_id>
    <test_objective>Check if the operating standards retrieved using Device.WiFi.Radio.1.OperatingStandards is "g,n,ax" when the parameter Device.WiFi.2G80211axEnable is in enabled state.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
...
3. If it not initially true, set to true and cross check with GET
4. Get the operating standards using Device.WiFi.Radio.1.OperatingStandards. When the 2G80211axEnable is in enabled state, the operating standards should be "g,n,ax"
5. Revert Device.WiFi.2G80211axEnable to initial value if required.
6. Unload the module.</automation_approch>
    <expected_output>The operating standards retrieved using Device.WiFi.Radio.1.OperatingStandards should be "g,n,ax" when the parameter Device.WiFi.2G80211axEnable is in enabled state.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckOperatingStandards_With2G80211axEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckOperatingStandards_With2G80211axEnabled');
