...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>TS_WIFIAGENT_CheckTelemetryMarkerCHUTIL_2_Values</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the CHUTIL_2 marker comes up with the three values that are comma separated :   CU due to self BSS from transmitted traffic , CU due to self BSS from received traffic and CU due to overlapping BSS.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_150</test_case_id>
    <test_objective>To check if the CHUTIL_2 marker comes up with the three values that are comma separated :   CU due to self BSS from transmitted traffic , CU due to self BSS from received traffic and CU due to overlapping BSS.</test_objective>
...
3. Check if the channel utilization marker CHUTIL_2 is present in wifihealth.txt. If not found, check every 60s for 15 minutes to see if the log is getting populated.
...
5. Unload the modules.</automation_approch>
    <expected_output>The CHUTIL_2 marker should come up with the three values that are comma separated : CU due to self BSS from transmitted traffic , CU due to self BSS from received traffic and CU due to overlapping BSS.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckTelemetryMarkerCHUTIL_2_Values</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckTelemetryMarkerCHUTIL_2_Values');
...
if "SUCCESS" in sysutilloadmodulestatus.upper():
    ...
    if details == "File exist":
        ...
        #Check for the maraker CHUTIL_2
        step = step + 1;
        tdkTestObj = sysObj.createTestStep('ExecuteCmd');
        cmd = "grep -ire \"CHUTIL_2\" /rdklogs/logs/wifihealth.txt";
        tdkTestObj.addParameter("command",cmd);
        expectedresult="SUCCESS";

        print "\nTEST STEP %d: Check for the presence of the marker CHUTIL_2" %step;
        print "EXPECTED RESULT %d: CHUTIL_2 marker should be present" %step;
        markerfound = 0;
        ...
        for iteration in range(1,16):
            ...
            if expectedresult in actualresult and "CHUTIL_2" in details:
                markerfound = 1;
                break;
            else:
                sleep(60);
        ...
        if markerfound == 1:
            tdkTestObj.setResultStatus("SUCCESS");
            print "ACTUAL RESULT %d: CHUTIL_2 marker is found; Details : %s" %(step,details);
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : SUCCESS";
            ...
            tdkTestObj.setResultStatus("FAILURE");
            print "ACTUAL RESULT %d: CHUTIL_2 marker is not found; Details : %s" %(step,details);
            #Get the result of execution
