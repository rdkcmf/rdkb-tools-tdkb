...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WIFIAGENT_2.4GHzSetManagementFramePowerControl_OutOfRange</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the set operation of Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl is success for values out of the acceptable range -20db to 0db and if the value converts to 0db when values greater than 0db is set and to -20db when values less than -20db is set.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_213</test_case_id>
    <test_objective>To check if the set operation of Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl is success for values out of the acceptable range -20db to 0db and if the value converts to 0db when values greater than 0db is set and to -20db when values less than -20db is set.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl
paramValue : integer values beyond the applicable range of -20db to 0db
paramType : int</input_parameters>
    <automation_approch>1. Load the module
2. Get the initial value of Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl.
3. Check if the initial MFPC is in the range of -20db to 0db.
4. Set Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl to a list of values greater than 0db : ["1", "10", "23", "50", "100", "257", "1000"]. The SET operations are expected to return success.
5. After each set get the value of Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl and check if it is converted to 0db.
6. Set Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl to a list of values lesser than -20db : ["-21", "-30", "-53", "-80", "-100", "-257", "-1000"]. The SET operations are expected to return success.
7. After each set get the value of Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl and check if it is converted to -20db.
8. Revert Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl to the initial value if required.
9. Unload the module</automation_approch>
    <expected_output>Setting Device.WiFi.AccessPoint.1.X_RDKCENTRAL-COM_ManagementFramePowerControl to values beyond the MFPC range of -20db to 0db should return success and the get operation should return 0db and -20db when the set values are greater than 0db and lesser than -20db respectively.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_2.4GHzSetManagementFramePowerControl_OutOfRange</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_2.4GHzSetManagementFramePowerControl_OutOfRange');
