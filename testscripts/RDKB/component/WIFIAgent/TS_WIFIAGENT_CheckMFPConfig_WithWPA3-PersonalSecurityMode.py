...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>19</version>
  <name>TS_WIFIAGENT_CheckMFPConfig_WithWPA3-PersonalSecurityMode</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set_Get</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the MFP Configuration retrieved using Device.WiFi.AccessPoint.{i}.Security.MFPConfig are "Required" for all private VAPs of applicable radios when the security mode enabled, Device.WiFi.AccessPoint.{i}.Security.ModeEnabled is "WPA3-Personal".</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_207</test_case_id>
    <test_objective>To check if the MFP Configuration retrieved using Device.WiFi.AccessPoint.{i}.Security.MFPConfig are "Required" for all private VAPs of applicable radios when the security mode enabled, Device.WiFi.AccessPoint.{i}.Security.ModeEnabled is "WPA3-Personal".</test_objective>
...
4. Set the security mode enabled to "WPA3-Personal" if not already in that mode. Validate the set operation.
5. Check the MFP Config using Device.WiFi.AccessPoint.{i}.Security.MFPConfig
6. Validate if the MFP Config are : for 2.4G - Required, 5G - Required
7. Revert the WPA3 RFC if required.
8. Revert the security modes if required.
9. Unload the modules.</automation_approch>
    <expected_output>The MFP Configuration retrieved using Device.WiFi.AccessPoint.{i}.Security.MFPConfig for the applicable radio's private AP should be [2.4G - Required, 5G - Required] when the security mode enabled, Device.WiFi.AccessPoint.{i}.Security.ModeEnabled is "WPA3-Personal".</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckMFPConfig_WithWPA3-PersonalSecurityMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckMFPConfig_WithWPA3-PersonalSecurityMode');
...
if "SUCCESS" in loadmodulestatus1.upper():
    ...
    if status == 0:
        ...
        if pre_req_set == 1:
            ...
            #Set the security mode to WPA3-Personal if the initial security mode is different
            step = step + 1;
            mode = "WPA3-Personal";
            status = 0;
            revert = [];
            ...
            print "\nTEST STEP %d : Set the security mode enabled using Device.WiFi.AccessPoint.{i}.Security.ModeEnabled to WPA3-Personal" %step;
            print "EXPECTED RESULT %d : The security modes should be set to WPA3-Personal mode successfully" %step;

            if status == 0:
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT %d: Security Modes are set to WPA3-Personal successfully" %step;
                #Get the result of execution
                print "TEST EXECUTION RESULT :SUCCESS";

                #Get the MFP Configuration values when the security mode enabled is WPA3-Personal mode
                step = step + 1;
                ...
                if status == 0:
                    ...
                    #For VAPs 1, 2, 17 MFP Config = Required when WPA3-Personal is the security mode enabled
                    step = step + 1;
                    expected_mfp = ["Required", "Required"];
                    status = 0;

                    print "\nTEST STEP %d : Check if the MFP Configurations are as expected for the applicable VAPs when the security mode enabled in WPA3-Personal" %step;
                    print "EXPECTED RESULT %d : The MFP Configurations should be as expected for the applicable VAPs when the security mode enabled in WPA3-Personal" %step;
                    ...
                    if status == 0:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "\nACTUAL RESULT %d: MFP Configurations are as expected when the security mode enabled is WPA3-Personal" %step;
                        #Get the result of execution
                        print "TEST EXECUTION RESULT :SUCCESS";
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "ACTUAL RESULT %d: MFP Configurations are NOT as expected when the security mode enabled is WPA3-Personal" %step;
                        #Get the result of execution
                ...
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT %d: Security Modes are NOT set to WPA3-Personal successfully" %step;
                #Get the result of execution
                print "TEST EXECUTION RESULT :FAILURE";
