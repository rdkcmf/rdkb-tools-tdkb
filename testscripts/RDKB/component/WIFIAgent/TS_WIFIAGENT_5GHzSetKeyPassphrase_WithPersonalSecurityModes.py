...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WIFIAGENT_5GHzSetKeyPassphrase_WithPersonalSecurityModes</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if the KeyPassphrase is successfully set using Device.WiFi.AccessPoint.2.Security.KeyPassphrase when different personal security modes is set using Device.WiFi.AccessPoint.2.Security.ModeEnabled from the supported list of modes retrieved using Device.WiFi.AccessPoint.2.Security.ModesSupported.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_163</test_case_id>
    <test_objective>Check if the KeyPassphrase is successfully set using Device.WiFi.AccessPoint.2.Security.KeyPassphrase when different personal security modes is set using Device.WiFi.AccessPoint.2.Security.ModeEnabled from the supported list of modes retrieved using Device.WiFi.AccessPoint.2.Security.ModesSupported.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband, RPI</test_setup>
    <pre_requisite>1.Ccsp Components in DUT should be in a running state that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>paramName : Device.WiFi.AccessPoint.2.Security.KeyPassphrase
...
2. Get the supported security modes using Device.WiFi.AccessPoint.2.Security.ModesSupported.
4. Get the initial security mode using Device.WiFi.AccessPoint.2.Security.ModeEnabled
3. Get the initial security KeyPassphrase using Device.WiFi.AccessPoint.2.Security.KeyPassphrase
...
6. Unload the modules.</automation_approch>
    <expected_output>The KeyPassphrase should be successfully set using Device.WiFi.AccessPoint.2.Security.KeyPassphrase when different personal security modes is set using Device.WiFi.AccessPoint.2.Security.ModeEnabled from the supported list of modes retrieved using Device.WiFi.AccessPoint.2.Security.ModesSupported.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_5GHzSetKeyPassphrase_WithPersonalSecurityModes</test_script>
    <skipped>No</skipped>
...
def setKeyPassphrase(obj1, keyPassphrase, step):
    ...
    print "\nTEST STEP %d : Set Device.WiFi.AccessPoint.2.Security.KeyPassphrase to %s" %(step, keyPassphrase);
    print "EXPECTED RESULT %d : Should successfully set Device.WiFi.AccessPoint.2.Security.KeyPassphrase to %s" %(step,keyPassphrase);
...
def getKeyPassphrase(obj1, step):
    ...
    print "\nTEST STEP %d : Get the Security KeyPassphrase using Device.WiFi.AccessPoint.2.Security.KeyPassphrase" %step;
    print "EXPECTED RESULT %d : Should successfully get Device.WiFi.AccessPoint.2.Security.KeyPassphrase" %step;
...
ip = <ipaddress>
port = <port>
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_5GHzSetKeyPassphrase_WithPersonalSecurityModes');
