...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>14</version>
  <name>TS_WIFIAGENT_CheckStatusAndLogging_WithRadioEnabledAndSSIDDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>WIFIAgent_Set</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if enabling the Radios Device.WiFi.Radio.1.Enable, Device.WiFi.Radio.2.Enable and disabling the SSIDs Device.WiFi.SSID.1.Enable, Device.WiFi.SSID.2.Enable updates the status parameters of Radios to "Up" and SSIDs to "Down" and the expected logs "5G Radio(Radio 2) is Enabled, only 5G Private SSID is DISABLED" and "2G Radio(Radio 1) is Enabled, only 2G Private SSID is DISABLED" are populated in SelfHeal.txt.0 within 15mins.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_217</test_case_id>
    <test_objective>To check if enabling the Radios Device.WiFi.Radio.1.Enable, Device.WiFi.Radio.2.Enable and disabling the SSIDs Device.WiFi.SSID.1.Enable, Device.WiFi.SSID.2.Enable updates the status parameters of Radios to "Up" and SSIDs to "Down" and the expected logs "5G Radio(Radio 2) is Enabled, only 5G Private SSID is DISABLED" and "2G Radio(Radio 1) is Enabled, only 2G Private SSID is DISABLED" are populated in SelfHeal.txt.0 within 15mins.</test_objective>
...
4. Disable the SSIDs and validate with GET.
5. Sleep for 20s
...
7. The status of Radios should be "Up" and the status of SSIDs should be "Down".
8. Check if SelfHeal.txt.0 is present under /rdklogs/logs/.
9. Within a 15min duration comprising of 60s iterations, check if the logs "5G Radio(Radio 2) is Enabled, only 5G Private SSID is DISABLED" and "2G Radio(Radio 1) is Enabled, only 2G Private SSID is DISABLED"  are populated in SelfHeal.txt.0.
10. Revert the SSIDs if required and validate with GET.
11. Finally revert the radios if required.
12. Unload the modules.</automation_approch>
    <expected_output>After enabling Device.WiFi.Radio.1.Enable, Device.WiFi.Radio.2.Enable and disabling the SSIDs Device.WiFi.SSID.1.Enable, Device.WiFi.SSID.2.Enable the status parameters of Radios should be "Up" and SSIDs should be updated to "Down" and the expected logs "5G Radio(Radio 2) is Enabled, only 5G Private SSID is DISABLED" and "2G Radio(Radio 1) is Enabled, only 2G Private SSID is DISABLED" should be  populated in SelfHeal.txt.0 within 15mins.</expected_output>
    <priority>High</priority>
    <test_stub_interface>wifiagent</test_stub_interface>
    <test_script>TS_WIFIAGENT_CheckStatusAndLogging_WithRadioEnabledAndSSIDDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckStatusAndLogging_WithRadioEnabledAndSSIDDisabled');
sysobj.configureTestCase(ip,port,'TS_WIFIAGENT_CheckStatusAndLogging_WithRadioEnabledAndSSIDDisabled');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper():
    ...
    if status == 0:
        ...
        if preReq == 0:
            ...
            #Disable the SSIDs keeping the radio as enabled
            paramList_ssid = ["Device.WiFi.SSID.1.Enable", "Device.WiFi.SSID.2.Enable"];
            setvalues = ["false", "false"];
            print "\nTEST STEP 3: Set the SSID enable parameters to disabled values - ", paramList_ssid;
            print "EXPECTED RESULT 3 : The SSID enable parameters should be set to disabled successfully";

            for index in range(0, 2):
                status = setParameter(obj, paramList_ssid[index], "false", "boolean");
                if status == 0:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "%s is set successfully to %s" %(paramList_ssid[index], setvalues[index]);
                else:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "%s is NOT set successfully to %s" %(paramList_ssid[index], setvalues[index]);
                    break;

            if status == 0:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 3: SSIDs are disabled successfully keeping the radios enabled" ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";
                ...
                if status == 0:
                    ...
                    #Check if the Radio status is Up and SSID status is Down
                    print "\nTEST STEP 5: Check if the Radio statuses are Up and SSID statuses are Down";
                    print "EXPECTED RESULT 5 : The Radio statuses should be Up and SSID statuses should be Down";

                    if values[0] == "Up" and values[1] == "Up" and values[2] == "Down" and values[3] == "Down":
                        tdkTestObj.setResultStatus("SUCCESS");
                        ...
                        if details == "File exist" :
                            ...
                            #Check if the "5G Radio(Radio 2) is Enabled, only 5G Private SSID is DISABLED" and "2G Radio(Radio 1) is Enabled, only 2G Private SSID is DISABLED" logs are found in SelfHeal.txt.0
                            step = 6;
                            search_strings = ["5G Radio(Radio 2) is Enabled, only 5G Private SSID is DISABLED", "2G Radio(Radio 1) is Enabled, only 2G Private SSID is DISABLED"];
                            tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                ...
                status = 0;
                print "\nTEST STEP 9 : Revert the SSID Enable if required";
                print "EXPECTED RESULT 9 : Revert operation should be success wherever required";
                ...
                if status == 0:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "ACTUAL RESULT 9: SSID enable revert operations completed" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                else:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("FAILURE");
                    print "ACTUAL RESULT 9: SSID enable revert operations NOT completed" ;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                #Set the result status of execution
            ...
            status = 0;
            print "\nTEST STEP 10 : Revert the Radio Enable if required";
            ...
            if status == 0:
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "ACTUAL RESULT 9: Radio enable revert operations completed" ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";
            else:
                #Set the result status of execution
                tdkTestObj.setResultStatus("FAILURE");
                print "ACTUAL RESULT 9: Radio enable revert operations NOT completed" ;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";
        else:
            #Set the result status of execution
