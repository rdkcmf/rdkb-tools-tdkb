...
<xml>
  ...
  <name>TS_WIFIAGENT_Disable2.4GHZ_CheckBandSteering</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if bandsteering process is running after disabling 2.4GHZ</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WIFIAGENT_65</test_case_id>
    <test_objective>To check if bandsteering process is running after disabling 2.4GHZ</test_objective>
...
5.Disable 2.4GHZ SSID
6.Check if lbd process is running
7.Enable 2.4GHZ SSID
8.Check if lbd process is running
9. Unload wifiagent module</automation_approch>
    <expected_output>Band steering process should not run after disabling 2.4GHZ</expected_output>
    <priority>High</priority>
    <test_stub_interface>WIFIAGENT</test_stub_interface>
    <test_script>TS_WIFIAGENT_Disable2.4GHZ_CheckBandSteering</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WIFIAGENT_Disable2.4GHZ_CheckBandSteering');
obj1.configureTestCase(ip,port,'TS_WIFIAGENT_Disable2.4GHZ_CheckBandSteering');
obj2.configureTestCase(ip,port,'TS_WIFIAGENT_Disable2.4GHZ_CheckBandSteering');
pamobj.configureTestCase(ip,port,'TS_WIFIAGENT_Disable2.4GHZ_CheckBandSteering');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() and "SUCCESS" in loadmodulestatus3.upper() and "SUCCESS" in loadmodulestatus4.upper():
    ...
    if expectedresult in actualresult:
        ...
        paramList=["Device.WiFi.X_RDKCENTRAL-COM_BandSteering.Enable", "Device.WiFi.SSID.1.SSID", "Device.WiFi.SSID.2.SSID", "Device.WiFi.SSID.1.Enable"]
        ...
        if expectedresult in status and orgValue[0] != "" and orgValue[1] != "" and orgValue[2] != "" and orgValue[3] != "":
            ...
            print "ACTUAL RESULT 2: Bandstering enable status,SSID names and 2.4GHZ SSID Enable status: %s %s %s %s" %(orgValue[0],orgValue[1],orgValue[2],orgValue[3]);
            #Get the result of execution
            print "[TEST EXECUTION RESULT] : SUCCESS";

            tdkTestObj = obj1.createTestStep("TDKB_TR181Stub_SetMultiple");
            tdkTestObj.addParameter("paramList","Device.WiFi.X_RDKCENTRAL-COM_BandSteering.Enable|true|bool|Device.WiFi.SSID.1.SSID|%s|string|Device.WiFi.SSID.2.SSID|%s|string|Device.WiFi.SSID.1.Enable|false|bool|Device.WiFi.Radio.1.X_CISCO_COM_ApplySetting|true|bool|Device.WiFi.Radio.2.X_CISCO_COM_ApplySetting|true|bool" %(SSID1,SSID1));
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
            if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3:Set Bandsteering enable status,wifi 2.4GHZ SSID enable status as false and same wifi SSID names for 2.4GHZ and 5 GHZ"
                ...
                if expectedresult in actualresult and pid == "":
                    ...
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    tdkTestObj = obj1.createTestStep("TDKB_TR181Stub_SetMultiple");
                    tdkTestObj.addParameter("paramList","Device.WiFi.SSID.1.Enable|true|bool|Device.WiFi.Radio.1.X_CISCO_COM_ApplySetting|true|bool")
                    ...
                    if expectedresult in  actualresult:
                        #Set the result status of execution
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 5: Set the enable status of 2.4GHZ WIFI SSID as true";
                    ...
                    print "[TEST EXECUTION RESULT] : FAILURE";
                tdkTestObj = obj1.createTestStep("TDKB_TR181Stub_SetMultiple");
                tdkTestObj.addParameter("paramList","Device.WiFi.X_RDKCENTRAL-COM_BandSteering.Enable|%s|bool|Device.WiFi.SSID.1.SSID|%s|string|Device.WiFi.SSID.2.SSID|%s|string|Device.WiFi.SSID.1.Enable|%s|bool|Device.WiFi.Radio.1.X_CISCO_COM_ApplySetting|true|bool|Device.WiFi.Radio.2.X_CISCO_COM_ApplySetting|true|bool" %(orgValue[0],orgValue[1],orgValue[2],orgValue[3]));
                tdkTestObj.executeTestCase(expectedresult);
                actualresult = tdkTestObj.getResult();
                details = tdkTestObj.getResultDetails();
                if expectedresult in actualresult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "TEST STEP :Revert 2.4GHZ WIFI SSID enable status and wifi SSID names for 2.4GHZ and 5 GHZ"
                    print "ACTUAL RESULT : %s" %details;
                    print "TEST EXECUTION RESULT :SUCCESS";
                else:
                    tdkTestObj.setResultStatus("FAILURE");
                    print "TEST STEP :Revert 2.4GHZ WIFI SSID enable status and different wifi SSID names for 2.4GHZ and 5 GHZ"
                    print "ACTUAL RESULT : %s" %details;
                    print "TEST EXECUTION RESULT :FAILURE";
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print "TEST STEP 3:Set 2.4GHZ WIFI SSID enable status as false and different wifi SSID names for 2.4GHZ and 5 GHZ"
                print "ACTUAL RESULT 3: %s" %details;
                print "TEST EXECUTION RESULT :FAILURE";
        else:
            #Set the result status of execution
