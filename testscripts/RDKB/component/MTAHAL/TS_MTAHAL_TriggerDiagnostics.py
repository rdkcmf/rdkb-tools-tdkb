...
<xml>
  ...
  <name>TS_MTAHAL_TriggerDiagnostics</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Trigger GR909 Diagnostics</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MTAHAL_5</test_case_id>
    <test_objective>Trigger GR909 Diagnostics</test_objective>
...
MTAMGMT_MTA_LINETABLE_INFO</input_parameters>
    <automation_approch>1. Load mtahal module
2. Get the number of entries for the MTA
3. Using TS_MTAHAL_TriggerDiagnostics invoke mta_hal_TriggerDiagnostics() to see if the diagnostics can be triggered or not. If available return SUCCESS and exit else return FAILURE and exit.
4. Unload mtahal module</automation_approch>
    <except_output>mta_hal_TriggerDiagnostics() returns RETURN_OK (0)</except_output>
    <priority>High</priority>
    <test_stub_interface>MTAHAL</test_stub_interface>
    <test_script>TS_MTAHAL_TriggerDiagnostics</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MTAHAL_TriggerDiagnostics')
