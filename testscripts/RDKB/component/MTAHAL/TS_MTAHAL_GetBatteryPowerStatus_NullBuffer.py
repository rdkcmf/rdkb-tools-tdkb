...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_MTAHAL_58</test_case_id>
    <test_objective>To validate mta_hal_GetBatteryPowerStatus() by passing a Null Buffer</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>mta_hal_GetBatteryPowerStatus()</api_or_interface_used>
    <input_parameters>NullBuffer</input_parameters>
    <automation_approch>1.Load mtahal module
2. Invoke the HAL api mta_hal_GetBatteryPowerStatus() by passing a null buffer
...
4. UnLoad mtahal module</automation_approch>
    <expected_output>Call to mta_hal_GetBatteryPowerStatus() api should fail</expected_output>
    <priority>High</priority>
    <test_stub_interface>matahal</test_stub_interface>
    <test_script>TS_MTAHAL_GetBatteryPowerStatus_NullBuffer</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MTAHAL_GetBatteryPowerStatus_NullBuffer');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        print "TEST STEP 1: Get the MtaBatteryPowerStatus with NULL Buffer";
        print "EXPECTED RESULT 1: Should not get the MtaBatteryPowerStatus with NULL Buffer";
