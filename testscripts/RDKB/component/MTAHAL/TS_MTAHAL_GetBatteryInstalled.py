...
<xml>
  ...
  <name>TS_MTAHAL_GetBatteryInstalled</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check to see if the battery is installed</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MTAHAL_26</test_case_id>
    <test_objective>Check to see if the battery is installed</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>mta_hal_BatteryGetInstalled()</api_or_interface_used>
    <input_parameters>N/A</input_parameters>
    <automation_approch>1. Load mtahal module
2. Using TS_MTAHAL_GetBatteryInstalled invoke mta_hal_BatteryGetInstalled() to check to see if the battery is installed or not. If available return SUCCESS and exit else return FAILURE and exit.
3. Unload mtahal module</automation_approch>
    <except_output>TRUE/FALSE</except_output>
    <priority>High</priority>
    <test_stub_interface>MTAHAL</test_stub_interface>
    <test_script>TS_MTAHAL_GetBatteryInstalled</test_script>
    <skipped>No</skipped>
