...
<xml>
  ...
  <name>TS_MTAHAL_GetLineResetCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Get the reset count of MTA Lines</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MTAHAL_21</test_case_id>
    <test_objective>Get the reset count of MTA Lines</test_objective>
...
2. Using TS_MTAHAL_GetLineResetCount invoke mta_hal_Get_LineResetCount() to see if the reset count of MTA Lines can be retrieved or not. If available return SUCCESS and exit else return FAILURE and exit.
3. Unload mtahal module</automation_approch>
    <except_output>The reset count of MTA Lines</except_output>
    <priority>High</priority>
    <test_stub_interface>MTAHAL</test_stub_interface>
    <test_script>TS_MTAHAL_GetLineResetCount</test_script>
    <skipped>No</skipped>
