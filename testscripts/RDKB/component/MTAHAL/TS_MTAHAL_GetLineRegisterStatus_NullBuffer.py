...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_MTAHAL_GetLineRegisterStatus_NullBuffer</name>
  <primitive_test_id/>
  <primitive_test_name>MTAHAL_GetLineRegisterStatus</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate mta_hal_GetLineRegisterStatus() by passing a Null Buffer</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_MTAHAL_66</test_case_id>
    <test_objective>To validate mta_hal_GetLineRegisterStatus() by passing a Null Buffer</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>mta_hal_GetLineRegisterStatus() </api_or_interface_used>
    <input_parameters>NullBuffer</input_parameters>
    <automation_approch>1.Load mtahal module
2. Invoke the HAL api mta_hal_GetLineRegisterStatus()  by passing a null buffer
...
4. UnLoad mtahal module</automation_approch>
    <expected_output>Call to mta_hal_GetLineRegisterStatus()  api should fail</expected_output>
    <priority>High</priority>
    <test_stub_interface>MTAHAL</test_stub_interface>
    <test_script>TS_MTAHAL_GetLineRegisterStatus_NullBuffer</test_script>
    <skipped>No</skipped>
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj = obj.createTestStep("MTAHAL_GetLineRegisterStatus")
