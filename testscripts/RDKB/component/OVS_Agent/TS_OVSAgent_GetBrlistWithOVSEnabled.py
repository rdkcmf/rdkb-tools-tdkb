...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_OVSAgent_GetBrlistWithOVSEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the br list when OVS is enabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_10</test_case_id>
    <test_objective>To get the br list when OVS is enabled.</test_objective>
...
4.Get the expected br entries to be listed
5.Verify all the expected br entries are present with ovs enabled
6.Revert the set values
7.Unload the module</automation_approch>
    <expected_output>With ovs enabled all the expected entries of br list should be populated</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_Agent</test_stub_interface>
    <test_script>TS_OVSAgent_GetBrlistWithOVSEnabled</test_script>
    <skipped>No</skipped>
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in result:
        ...
        if ovs_set == 1:
            ...
            cmd = "sh %s/tdk_utility.sh parseConfigFile OVS_BrList" %TDK_PATH;
            print cmd;
            expectedresult="SUCCESS";
            ...
            if expectedresult in actualresult  and details!= "":
                ...
                for item in details:
                    query="ovs-vsctl list-br | grep \"%s\""%item;
                    print "query:%s" %query
