...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_OVSAgent_GetBrlanPortsIn_RouterMode</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  all expected brlan ports are present after enabling ovs in router mode</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_11</test_case_id>
    <test_objective>To check if  all expected brlan ports are present after enabling ovs in router mode</test_objective>
...
5.Check if the device is in router mode , else bring it to router mode
6.Get brlan interface using (ovs-vsctl list-ports brlan0 ) command and verify all the  expected entries are present
7.Revert the set values
8.Unload the module</automation_approch>
    <expected_output>with ovs enabled and device in router mode all the expected brlan entries needs to be present</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_Agent</test_stub_interface>
    <test_script>TS_OVSAgent_GetBrlanPortsIn_RouterMode</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_OVSAgent_GetBrlanPortsIn_RouterMode');
tr181obj.configureTestCase(ip,port,'TS_OVSAgent_GetBrlanPortsIn_RouterMode');
revert_flag=0;
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in result:
        ...
        if ovs_set == 1:
            ...
            cmd = "sh %s/tdk_utility.sh parseConfigFile OVS_Router_Port" %TDK_PATH;
            print cmd;
            expectedresult="SUCCESS";
