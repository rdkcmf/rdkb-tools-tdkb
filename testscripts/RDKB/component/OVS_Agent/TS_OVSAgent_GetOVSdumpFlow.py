...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_OVSAgent_GetOVSdumpFlow</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if ovs dump flow packets and bytes are non-zero</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_13</test_case_id>
    <test_objective>To check if ovs dump flow packets and bytes are non-zero</test_objective>
...
4.Get the ovs-ofctl dump-flows brlan0
5. Verify that packets and bytes are greater than zero
6.Revert the set values
7.Unload the module</automation_approch>
    <expected_output>With OVS enabled get the dumpflow and check packets and bytes are greater then zero</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_AGENT</test_stub_interface>
    <test_script>TS_OVSAgent_GetOVSdumpFlow</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_OVSAgent_GetOVSdumpFlow');
tr181obj.configureTestCase(ip,port,'TS_OVSAgent_GetOVSdumpFlow');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in result:
        ...
        if ovs_set == 1:
            ...
            query="ovs-ofctl dump-flows brlan0 | grep -v \"n_packets=0\"| grep -i \"n_packets\"";
            print "query:%s" %query
            ...
            if expectedresult in actualresult and details!="":
               packets=details.split(",")[4].split("=")[1];
               byte=details.split(",")[3].split("=")[1];
               if packets.isdigit() and byte.isdigit():
                  packets = int(packets);
                  byte = int(byte);
                  if packets >0 and byte>0:
                     tdkTestObj.setResultStatus("SUCCESS");
                     print "TEST STEP 3: Get the OVS dump flow detail ie no. of packets and bytes ";
                     print "EXPECTED RESULT 3: Should get OVs dump flow detail ie no. of packets and bytes greater than zero";
                     print "ACTUAL RESULT 3:packets = %d ,bytes= %d are received "%(packets,byte);
                     print "[TEST EXECUTION RESULT] : SUCCESS";
                  else:
                    ...
                    tdkTestObj.setResultStatus("FAILURE");
                    print "A non-digit value is received for packets and bytes";
            else:
                tdkTestObj.setResultStatus("FAILURE");
                print "TEST STEP 3: Get the OVS dump flow detail ie no. of packets and bytes ";
                print "EXPECTED RESULT 3: Should get OVS dump flow detail ie no. of packets and bytes greater than zero";
