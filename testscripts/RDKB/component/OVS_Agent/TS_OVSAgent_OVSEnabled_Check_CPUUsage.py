...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_OVSAgent_OVSEnabled_Check_CPUUsage</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check CPU usage when OVS is enabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_02</test_case_id>
    <test_objective>To check the CPU usage with Ovs Enabled for time lapse of 5min and verify that average CPU usage is less that 10 %</test_objective>
...
4.with wait time of 60 sec until 5 min capture the CPU usage
5.Caluclate the average of the CPU Usage
6.Avergae value is expected to be less than 10 %
7.Revert the set values
8.Unload the module</automation_approch>
    <expected_output>With Ovs enabled CPU usage captured for 5 min should be less that 10 %.</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_AGENT</test_stub_interface>
    <test_script>TS_OVSAgent_OVSEnabled_Check_CPUUsage</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_OVSAgent_OVSEnabled_Check_CPUUsage');
tr181obj.configureTestCase(ip,port,'TS_OVSAgent_OVSEnabled_Check_CPUUsage');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in result:
        ...
        if ovs_set == 1:
            ...
            print "Check CPU Usage very 60 sec "
            retryCount = 0;
            MAX_RETRY = 6;
            values =[];
            flag =0;
            while retryCount < MAX_RETRY:
                  query="top -n 1 |grep CPU:";
                  print "query:%s" %query
                  ...
                  if expectedresult in actualresult and ovs != "":
                     ovs=ovs.split(" ")[3];
                     print "The value received is :",ovs;
                     values.append(ovs);
                     sleep(60);
                     retryCount = retryCount + 1;
                  else:
                       flag =1;
                       print "Fetching CPU usage failed";
                       break;

            if flag == 0:
                print "The received value is  : ",values
                avg = 0;
                total =0;
                for nums in range(0, len(values)):
                    values[nums] = values[nums].replace("%","");

                for nums in range(0, len(values)):
                    total = total + int(values[nums]);
                    avg = total/5;
                    print ("The caluclated average of the above value is : %d"%avg);

                if avg < 10 :
                   print "TEST STEP 3: Calculate the average CPU usage Captured for 5 min ";
                   print "EXPECTED RESULT 3 : The average value is expected to be less than 10%";
                   print "ACTUAL RESULT 3: The avergae value is :%d" %avg;
                   print "[TEST EXECUTION RESULT] : SUCCESS";
