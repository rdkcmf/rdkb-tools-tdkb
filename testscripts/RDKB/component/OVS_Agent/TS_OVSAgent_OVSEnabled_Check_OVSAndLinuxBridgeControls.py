...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_OVSAgent_OVSEnabled_Check_OVSAndLinuxBridgeControls</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  brlan entries are present in ovs-vsctl show and not in brctl show with ovs enabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_04</test_case_id>
    <test_objective>To check if  brlan entries are present in ovs-vsctl show and not in brctl show with ovs enabled</test_objective>
...
5.brlan entries should be present in ovs-vsctl show and not in brctl show
6.Revert the set values
7.Unload the module</automation_approch>
    <expected_output>with ovs enabled brlan entries should be present in ovs-vsctl show and not in brctl show</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_AGENT</test_stub_interface>
    <test_script>TS_OVSAgent_OVSEnabled_Check_OVSAndLinuxBridgeControls</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_OVSAgent_OVSEnabled_Check_OVSAndLinuxBridgeControls');
tr181obj.configureTestCase(ip,port,'TS_OVSAgent_OVSEnabled_Check_OVSAndLinuxBridgeControls');
        ...
        if ovs_set == 1:
            ...
            if expectedresult in (actualresult1 and actualresult2):
                ...
                if ovs !="" and brctl == "" :
                    ...
                    print "EXPECTED RESULT 4: Should get  brlan entries in OVS  and Linux bridge controls as non-empty and empty respectively";
