...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>5</version>
  <name>TS_OVSAgent_OVSEnabled_CheckTCP_UDPThreshold</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check TCP and UDP Threshold when OVS is enabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_03</test_case_id>
    <test_objective>To check the TCP and UDP thresholds with ovs enabled being greater than or equal to 10</test_objective>
...
4.Get the tcp  and udp threshold values under flowmgr
5.The threshold values should be greater than or equal to 10
6.Revert the set values
7.Unload the module</automation_approch>
    <expected_output>with ovs enabled tcp and udp threshold values should be less than or equal or to 10</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_AGENT</test_stub_interface>
    <test_script>TS_OVSAgent_OVSEnabled_CheckTCP_UDPThreshold</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_OVSAgent_OVSEnabled_CheckTCP_UDPThreshold');
tr181obj.configureTestCase(ip,port,'TS_OVSAgent_OVSEnabled_CheckTCP_UDPThreshold');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in result:
        ...
        if ovs_set == 1:
            ...
            query="cat /proc/sys/net/flowmgr/tcp_pkt_threshold";
            print "query:%s" %query
            ...
            tcp = tdkTestObj.getResultDetails().strip().replace("\\n","");

            query="cat /proc/sys/net/flowmgr/udp_pkt_threshold";
            print "query:%s" %query
            tdkTestObj = tdkTestObj_Sys_ExeCmd;
            tdkTestObj.addParameter("command", query)
            expectedresult="SUCCESS";
            ...
            udp = tdkTestObj.getResultDetails().strip().replace("\\n","");
            if expectedresult in (actualresult1 and actualresult2 ) and udp !="" and tcp !="":
               print "TEST STEP 3: Get the UDP and TCP threshold values";
               print "EXPECTED RESULT 3 : should get the UDP and TCP threshold values ";
               print "ACTUAL RESULT 3: udp : %s and tcp  : %s" %(udp,tcp);
               print "[TEST EXECUTION RESULT] : SUCCESS";
               tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");

               if int(udp) >= 10  and int(tcp) >=10:
                  print "TEST STEP 4: Get the UDP and TCP threshold values ";
                  print "EXPECTED RESULT 4: should get the UDP and TCP threshold values greater than 10";
                  print "ACTUAL RESULT 4: udp : %s and tcp  : %s" %(udp,tcp);
                  print "[TEST EXECUTION RESULT] : SUCCESS";
                  tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
               else:
                  print "TEST STEP 4: Get the UDP and TCP threshold values ";
