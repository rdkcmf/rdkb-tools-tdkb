...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_OVSAgent_OVSDisable_CheckLoggingInMeshAgentLog</name>
  <primitive_test_id/>
  <primitive_test_name>OVS_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if marker OVS_RFC_changed_to_disabled  is logged in MeshAgentLog.txt.0 when ovs is disabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_OVSAGENT_07</test_case_id>
    <test_objective>To check if marker OVS_RFC_changed_to_disabled  is logged in MeshAgentLog.txt.0 when ovs is disabled</test_objective>
...
4. Check if OVS_RFC_changed_to_disabled  is logged in MeshAgentLog.txt.0
5.Revert the set values
6.Unload the module</automation_approch>
    <expected_output>with ovs disabled OVS_RFC_changed_to_disabled  should be logged in MeshAgentLog.txt.0</expected_output>
    <priority>High</priority>
    <test_stub_interface>OVS_AGENT</test_stub_interface>
    <test_script>TS_OVSAgent_OVSDisable_CheckLoggingInMeshAgentLog</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_OVSAgent_OVSDisable_CheckLoggingInMeshAgentLog');
tr181obj.configureTestCase(ip,port,'TS_OVSAgent_OVSDisable_CheckLoggingInMeshAgentLog');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if expectedresult in result:
        ...
        print "\n\nTEST STEP 2: Get the OVS status"
        print "EXPECTED RESULT 2: OVS Status should disabled else enable it "
        ...
        if ovs_set == 1:
            ...
            query="grep -rin \"OVS_RFC_changed_to_disabled\" /rdklogs/logs/MeshAgentLog.txt.0";
            print "query:%s" %query
            ...
            if expectedresult in actualresult1 and "OVS_RFC_changed_to_disabled" in ovs:
    ...
    #Revert Flag will set to 1 only when initial value was true, so enable the OVS using doEnableDisableOVS function
    if revert_flag == 1:
        ovs_set,revert_flag = doEnableDisableOVS("false",sysobj,tdkTestObj_Tr181_Get,tdkTestObj_Tr181_Set);
        if ovs_set == 1:
