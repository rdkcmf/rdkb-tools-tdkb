...
<xml>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate SYS_ERROR_CPU100 telemetry2_0 marker by loading 100% CPU</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_34</test_case_id>
    <test_objective>To simulate SYS_ERROR_CPU100 telemetry2_0 marker by loading 100% CPU</test_objective>
...
8. Check if marker SYS_ERROR_CPU100 is present in telemetry log file (only between the initial line count and line count after simulation)
...
12. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_ERROR_CPU100</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_CPU100');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_CPU100');
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                            ...
                            query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : SYS_ERROR_CPU100\"" %(initialLinesCount,lineCountAfterSimu)
                            print "query:%s" %query
                            ...
                            if expectedresult in actualresult and details!="" and (len(details) > 0) and "SYS_ERROR_CPU100" in details:
                                tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                                markervalue = details.split("SYS_ERROR_CPU100 value : ")[1]
                                print "TEST STEP 7: SYS_ERROR_CPU100  Marker should be present";1
                                print "EXPECTED RESULT 7: SYS_ERROR_CPU100 Marker should be present";
                                print "ACTUAL RESULT 7: SYS_ERROR_CPU100  Marker Value is %s" %markervalue;
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : SUCCESS";
                            else:
                                tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                                print "TEST STEP 7: SYS_ERROR_CPU100  Marker should be present";
                                print "EXPECTED RESULT 7: SYS_ERROR_CPU100 Marker should be present";
                                print "ACTUAL RESULT 7: SYS_ERROR_CPU100  Marker is %s" %details;
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : FAILURE";
                        else:
                            tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
