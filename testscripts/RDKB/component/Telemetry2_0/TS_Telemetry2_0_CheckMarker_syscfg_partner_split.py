...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_Telemetry2_0_CheckMarker_syscfg_partner_split</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate syscfg_partner_split  marker by running log_factoryPartnerId.sh  shell script</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_37</test_case_id>
    <test_objective>This test case is to simulate syscfg_partner_split  marker by running log_factoryPartnerId.sh  shell script</test_objective>
...
6. Check if marker syscfg_partner_split  is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_syscfg_partner_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_syscfg_partner_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_syscfg_partner_split');
        ...
        if expectedresult in lineCountResult:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : syscfg_partner_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "syscfg_partner_split" in details:
                        ...
                        print "TEST STEP 5: syscfg_partner_split  Marker should be present";
                        print "EXPECTED RESULT 5: syscfg_partner_split Marker should be present";
                        print "ACTUAL RESULT 5: syscfg_partner_split  Marker is %s" %details;
                        #Get the result of execution
