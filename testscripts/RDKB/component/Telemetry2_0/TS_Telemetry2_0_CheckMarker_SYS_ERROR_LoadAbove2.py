...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove2</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate SYS_ERROR_LoadAbove2 telemetry2_0 marker by loading CPU Load value more than 2</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_28</test_case_id>
    <test_objective>To simulate SYS_ERROR_LoadAbove2 telemetry2_0 marker by loading CPU Load value more than 2
</test_objective>
...
5. If the CPULoad value hits 2, then execute the script log_mem_cpu_info.sh from TAD module
...
8. Check if marker SYS_ERROR_LoadAbove2 is present in telemetry log file (only between the initial line count and line count after simulation)
...
12. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove2</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove2');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove2');
        ...
        if expectedresult in lineCountResult:
            ...
            for loop in range (1,24):
                ...
                if int(cpuload) == 2:
                    cpuloadabove2 = 1;
                    tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                    print "EXPECTED RESULT 3: CPULOAD Increased to value 2";
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    break;
                elif int(cpuload) > 2:
                    print "EXPECTED RESULT 3: CPULOAD value was more than 2";
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                            ...
                            if expectedresult in actualresult and details!="" and (len(details) > 0) and "SYS_ERROR_LoadAbove2" in details:
                                tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                                markervalue = details.split("SYS_ERROR_LoadAbove2<#=#>")[1]
