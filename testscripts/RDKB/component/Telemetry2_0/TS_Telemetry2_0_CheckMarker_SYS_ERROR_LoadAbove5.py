...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove5</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate SYS_ERROR_LoadAbove5 telemetry2_0 marker by loading CPU Load value more than 5</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_31</test_case_id>
    <test_objective>To simulate SYS_ERROR_LoadAbove5 telemetry2_0 marker by loading CPU Load value more than 5
</test_objective>
...
5. If the CPULoad value hits 5, then execute the script log_mem_cpu_info.sh from TAD module
...
8. Check if marker SYS_ERROR_LoadAbove5 is present in telemetry log file (only between the initial line count and line count after simulation)
...
12. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove5</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove5');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_LoadAbove5');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            print "EXPECTED RESULT 3: CPULOAD average should be increased to value 5";
            ...
            for loop in range (1,24):
                ...
                if int(cpuload) == 5:
                    cpuloadabove5 = 1;
                    tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                    print "EXPECTED RESULT 3: CPULOAD average Increased to value ";
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    break;
                elif int(cpuload) > 5:
                    print "EXPECTED RESULT 3: CPULOAD average value was more than 5";
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in actualresult:
                        ...
                        if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                            ...
                            query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : SYS_ERROR_LoadAbove5\"" %(initialLinesCount,lineCountAfterSimu)
                            print "query:%s" %query
                            ...
                            if expectedresult in actualresult and details!="" and (len(details) > 0) and "SYS_ERROR_LoadAbove5" in details:
                                tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                                markervalue = details.split("SYS_ERROR_LoadAbove5 value : ")[1]
                                print "TEST STEP 7: SYS_ERROR_LoadAbove5  Marker should be present";1
                                print "EXPECTED RESULT 7: SYS_ERROR_LoadAbove5 Marker should be present";
                                print "ACTUAL RESULT 7: SYS_ERROR_LoadAbove5  Marker Value is %s" %markervalue;
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : SUCCESS";
                            else:
                                tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                                print "TEST STEP 7: SYS_ERROR_LoadAbove5  Marker should be present";
                                print "EXPECTED RESULT 7: SYS_ERROR_LoadAbove5 Marker should be present";
                                print "ACTUAL RESULT 7: SYS_ERROR_LoadAbove5  Marker is %s" %details;
                                #Get the result of execution
                                print "[TEST EXECUTION RESULT] : FAILURE";
                        else:
                            tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
