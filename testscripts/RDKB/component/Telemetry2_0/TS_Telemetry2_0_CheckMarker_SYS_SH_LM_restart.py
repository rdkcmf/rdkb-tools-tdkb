...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_Telemetry2_0_CheckMarker_SYS_SH_LM_restart</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate SYS_SH_LM_restart  by restarting CcspLMLite process</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_44</test_case_id>
    <test_objective>This test case is  to simulate SYS_SH_LM_restart  by restarting CcspLMLite process</test_objective>
...
4. Restart CcspLMLite  process
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker SYS_SH_LM_restart   is present in  telemetry log file (only between the initial line count and line count after simulation)
...
10. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_SH_LM_restart</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_SH_LM_restart');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_SH_LM_restart');
            ...
            if expectedresult in actualresult and details != "":
               ...
               if expectedresult in actualresult :
                  ...
                  if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                     ...
                     query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : SYS_SH_LM_restart\"" %(initialLinesCount,lineCountAfterSimu)
                     print "query:%s" %query
                     ...
                     if expectedresult in actualresult and details!="" and (len(details) > 0) and "SYS_SH_LM_restart" in details:
                        tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                        markervalue = details.split("SYS_SH_LM_restart<#=#>")[1]
                        print "TEST STEP 6:SYS_SH_LM_restart  Marker should be present";1
                        print "EXPECTED RESULT 6: SYS_SH_LM_restart Marker should be present";
                        print "ACTUAL RESULT 6: SYS_SH_LM_restart  Marker Value is %s" %markervalue;
                        ...
                        if expectedresult in actualresult and pid != "" and pid != Initialpid:
                         ...
                         tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                         print "TEST STEP 6:SYS_SH_LM_restart  Marker should be present";
                         print "EXPECTED RESULT 6: SYS_SH_LM_restart Marker should be present";
                         print "ACTUAL RESULT 6: SYS_SH_LM_restart  Marker is %s" %details;
                         #Get the result of execution
                         print "[TEST EXECUTION RESULT] : FAILURE";
                  else:
                      tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
