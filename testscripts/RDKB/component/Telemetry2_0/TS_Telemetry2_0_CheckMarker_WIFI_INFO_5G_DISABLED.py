...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>6</version>
  <name>TS_Telemetry2_0_CheckMarker_WIFI_INFO_5G_DISABLED</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate WIFI_INFO_5G_DISABLED telemetry2_0 marker by executing task_health_monitor.sh from TAD module</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_27</test_case_id>
    <test_objective>To simulate WIFI_INFO_5G_DISABLED telemetry2_0 marker by executing task_health_monitor.sh from TAD module</test_objective>
...
4. Get the Enable status of SSID2, if its not disabled, Disable it
5. Execute task_health_monitor.sh script from TAD module
6. Get the number of lines from the telemetry log file after simulation
7. Check if marker WIFI_INFO_5G_DISABLED is present in telemetry log file (only between the initial line count and line count after simulation)
8. Make the script as success if the Marker is present else make it failure
9. Revert the Enable status, if its changed in step 4
...
11. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_WIFI_INFO_5G_DISABLED</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_WIFI_INFO_5G_DISABLED');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_WIFI_INFO_5G_DISABLED');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()) :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            disabled = 0;
            SSID2_res, SSID2_enable, tdkTestObj = getSSIDEnable(tdkTestObj_Tr181_Get,"Device.WiFi.SSID.2.Enable");
            if expectedresult in SSID2_res:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3: Get the SSID Enable status of SSID2";
                print "EXPECTED RESULT 3: Should get the SSID Enable status";
                print "ACTUAL RESULT 3: SSID Enable status Retrieved Successfully ";
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : SUCCESS";

                if str(SSID2_enable) == "false":
                    disabled = 1;
                else:
                    set_result1,set_details1, tdkTestObj = setSSIDEnable(tdkTestObj_Tr181_Set,"Device.WiFi.SSID.2.Enable","false");
                    if expectedresult in set_result1:
                        disabled = 1;
                        ssid_RevertFlag = 1;
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 4: Set the SSID Enable Status to False";
                        print "EXPECTED RESULT 4: Should Set the SSID Enable to False";
                        print "ACTUAL RESULT 4: SSID Enable status Set was Successful ";
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                    else:
                        disabled = 0;
                        ...
                        print "TEST STEP 4: Set the SSID Enable Status to False";
                        print "EXPECTED RESULT 4: Should Set the SSID Enable to False";
                        print "ACTUAL RESULT 4: SSID Enable status Set was Failed ";
                        #Get the result of execution
                ...
                print "TEST STEP 3: Get the SSID Enable status of SSID2";
                print "EXPECTED RESULT 3: Should get the SSID Enable status";
                print "ACTUAL RESULT 3: Failed to get SSID Enable Status ";
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE";

            if disabled == 1:
                query="sh /usr/ccsp/tad/task_health_monitor.sh &"
                print "query:%s" %query
                ...
                if expectedresult in actualresult:
                    ...
                    print "EXPECTED RESULT 5: Script task_health_monitor.sh should run";
                    print "ACTUAL RESULT 5: Script task_health_monitor.sh was running";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                    ...
                    if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                        ...
                        query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : WIFI_INFO_5G_DISABLED\"" %(initialLinesCount,lineCountAfterSimu)
                        print "query:%s" %query
                        tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                        tdkTestObj.addParameter("command", query);
                        tdkTestObj.executeTestCase(expectedresult);
                        ...
                        if expectedresult in actualresult and details!="" and (len(details) > 0) and "WIFI_INFO_5G_DISABLED" in details:
                            ...
                            print "ACTUAL RESULT 7: WIFI_INFO_5G_DISABLED Marker value is %s" %markervalue;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                        else:
                            tdkTestObj.setResultStatus("FAILURE");
                            print "TEST STEP 7: WIFI_INFO_5G_DISABLED Marker should be present";
                            print "EXPECTED RESULT 7: WIFI_INFO_5G_DISABLED Marker should be present";
                            print "ACTUAL RESULT 7: WIFI_INFO_5G_DISABLED Marker is %s" %details;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : FAILURE";
                    else:
                    ...
                    print "TEST STEP 5: Run the task_health_monitor.sh Script";
                    print "EXPECTED RESULT 5: Script task_health_monitor.sh should run";
                    print "ACTUAL RESULT 5: Script task_health_monitor.sh was not running";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : FAILURE";

                if ssid_RevertFlag == 1:
                    revert_Set1,set_details1, tdkTestObj = setWiFiSSIDvalue(wifiobj,"Device.WiFi.SSID.2.Enable",SSID2_enable);
                    if expectedresult in revert_Set1:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 8: Revert the SSID Enable";
                        print "EXPECTED RESULT 8: Should Revert the SSID Enable to original value";
                ...
                print "SSID Enable Status was Not Disabled"
                tdkTestObj.setResultStatus("FAILURE");
        else:
            tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
            print "TEST STEP 2: Get the initial Line count of Telemetry Log file";
