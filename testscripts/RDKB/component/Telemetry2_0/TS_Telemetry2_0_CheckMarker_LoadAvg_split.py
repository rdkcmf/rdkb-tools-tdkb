...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_Telemetry2_0_CheckMarker_LoadAvg_split</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate LoadAvg_split marker by running log_mem_cpu_info.sh script</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_48</test_case_id>
    <test_objective>This test case is to simulate LoadAvg_split marker by running log_mem_cpu_info.sh script</test_objective>
...
4. Run log_mem_cpu_info.sh script in the background
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker LoadAvg_split is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_LoadAvg_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_LoadAvg_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_LoadAvg_split');
        ...
        if expectedresult in lineCountResult:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : LoadAvg_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "LoadAvg_split" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        markervalue = details.split("LoadAvg_split<#=#>")[1]
                        print "TEST STEP 5: LoadAvg_split  Marker should be present";1
                        print "EXPECTED RESULT 5: LoadAvg_split Marker should be present";
                        print "ACTUAL RESULT 5: LoadAvg_split  Marker Value is %s" %markervalue;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] 5: SUCCESS";
                        print "***************************************************************"
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "TEST STEP 5: LoadAvg_split  Marker should be present";
                        print "EXPECTED RESULT 5: LoadAvg_split Marker should be present";
                        print "ACTUAL RESULT 5: LoadAvg_split  Marker is %s" %details;
                        #Get the result of execution
