...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_Telemetry2_0_CheckMarker_Total_devices_connected_split</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate Total_devices_connected_split marker by restarting the ccsp-lmlite process</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_3</test_case_id>
    <test_objective>This test case is to simulate Total_devices_connected_split marker by restarting the ccsp-lmlite process</test_objective>
...
4. Restart the CcspLMLite  process
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker  Total_devices_connected_split  is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    <expected_output>On simulation by restarting the CcspLMLite process Total_devices_connected_split marker should be present.</expected_output>
    <priority>High</priority>
    <test_stub_interface>Telemetry2_0</test_stub_interface>
    <test_script>TS_Telemetry2_0_CheckMarker_Total_devices_connected_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Total_devices_connected_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Total_devices_connected_split');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            details,actualresult = getPID(tdkTestObj_Sys_ExeCmd,"CcspLMLite");
            if expectedresult in actualresult and details != "":
               Initialpid = int(details);
               tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
               print "TEST STEP 3: Check if CcspLMLite process is running";
               print "EXPECTED RESULT 3:CcspLMLite  process should be running";
               print "ACTUAL RESULT 3: pid of CcspLMLite:",details;
               print "[TEST EXECUTION RESULT] : SUCCESS";

               actualresult =killProcess(tdkTestObj_Sys_ExeCmd,Initialpid,"");
               if expectedresult in actualresult :
                  tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                  print "TEST STEP 4: Kill CcspLMLite process and run task_health_monitor.sh script";
                  print "EXPECTED RESULT 4:Should Kill the CcspLMLite  process and run task_health_monitor.sh script";
                  print "ACTUAL RESULT 4: CcspLMLite process killed and run task_health_monitor.sh script successfully";
                  print "[TEST EXECUTION RESULT] : SUCCESS";
                  ...
                  if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                     ...
                     query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : Total_devices_connected_split\"" %(initialLinesCount,lineCountAfterSimu)
                     print "query:%s" %query
                     ...
                     if expectedresult in actualresult and details!="" and (len(details) > 0) and "Total_devices_connected_split" in details:
                        tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                        markervalue = details.split("Total_devices_connected_split<#=#>")[1]
                        print "TEST STEP 6:Total_devices_connected_split  Marker should be present";1
                        print "EXPECTED RESULT 6: Total_devices_connected_split Marker should be present";
                        print "ACTUAL RESULT 6: Total_devices_connected_split  Marker Value is %s" %markervalue;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";

                        actualresult,pid = checkProcessRestarted(tdkTestObj_Sys_ExeCmd,"CcspLMLite");
                        if expectedresult in actualresult and pid != "" and pid != Initialpid:
                           tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                           print "TEST STEP 7: Check if the CcspLMLite proccess restarted";
                           print "EXPECTED RESULT 7:CcspLMLite proccess should restart ";
                           print "ACTUAL RESULT 7: CcspLMLite restarted successfully";
                           print "[TEST EXECUTION RESULT] : SUCCESS";
                        else:
                            tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                            print "TEST STEP 7: Check if the CcspLMLite proccess restarted";
                            print "EXPECTED RESULT 7:CcspLMLite proccess should restart ";
                            print "ACTUAL RESULT 7: CcspLMLite restarted failed";
                            print "[TEST EXECUTION RESULT] : FAILURE";
                     else:
                         tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                         print "TEST STEP 6:Total_devices_connected_split  Marker should be present";
                         print "EXPECTED RESULT 6: Total_devices_connected_split Marker should be present";
                         print "ACTUAL RESULT 6: Total_devices_connected_split  Marker is %s" %details;
                         #Get the result of execution
                         print "[TEST EXECUTION RESULT] : FAILURE";
                  else:
                      tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                   ...
                   print "TEST STEP 4: Kill CcspLMLite process and run task_health_monitor.sh script";
                   print "EXPECTED RESULT 4:Should Kill the CcspLMLite  process and run task_health_monitor.sh script";
                   print "ACTUAL RESULT 4: Failed to kill CcspLMLite process and run task_health_monitor.sh script";
                   print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                print "TEST STEP 3: Check if CcspLMLite process is running";
