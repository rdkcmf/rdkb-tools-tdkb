...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_Wifi_2G_utilization_split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate Wifi_2G_utilization_split telemetry2_0 marker by running the radiohealth.sh script from WIFI Module</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_19</test_case_id>
    <test_objective>To simulate Wifi_2G_utilization_split telemetry2_0 marker by running the radiohealth.sh script from WIFI Module</test_objective>
...
6. Check if marker Wifi_2G_utilization_split is present in telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_Wifi_2G_utilization_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Wifi_2G_utilization_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Wifi_2G_utilization_split');
        ...
        if expectedresult in lineCountResult:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : Wifi_2G_utilization_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "Wifi_2G_utilization_split" in details:
                        ...
                        print "TEST STEP 5: Wifi_2G_utilization_split  Marker should be present";
                        print "EXPECTED RESULT 5: Wifi_2G_utilization_split Marker should be present";
                        print "ACTUAL RESULT 5: Wifi_2G_utilization_split  Marker is %s" %details;
                        #Get the result of execution
