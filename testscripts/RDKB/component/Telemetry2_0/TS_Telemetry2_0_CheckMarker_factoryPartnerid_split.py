...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_Telemetry2_0_CheckMarker_factoryPartnerid_split</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate factoryPartnerid_split marker by running log_factoryPartnerId.sh shell script</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_36</test_case_id>
    <test_objective>This test case is to simulate factoryPartnerid_split marker by running log_factoryPartnerId.sh shell script</test_objective>
...
4. Execute the script  log_factoryPartnerId.sh
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker factoryPartnerid_split  is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_factoryPartnerid_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_factoryPartnerid_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_factoryPartnerid_split');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            cmd = "sh /rdklogger/log_factoryPartnerId.sh  &";
            ...
            if expectedresult in actualresult:
                ...
                print "TEST STEP 3: Execute the log_factoryPartnerId.sh script in background";
                print "EXPECTED RESULT 3: log_factoryPartnerId.sh script should be running";
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : factoryPartnerid_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "factoryPartnerid_split" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        markervalue = details.split("factoryPartnerid_split value : ")[1]
                        print "TEST STEP 5: factoryPartnerid_split  Marker should be present";1
                        print "EXPECTED RESULT 5: factoryPartnerid_split Marker should be present";
                        print "ACTUAL RESULT 5: factoryPartnerid_split  Marker Value is %s" %markervalue;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] 5: SUCCESS";
                        print "***************************************************************"
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "TEST STEP 5: factoryPartnerid_split  Marker should be present";
                        print "EXPECTED RESULT 5: factoryPartnerid_split Marker should be present";
                        print "ACTUAL RESULT 5: factoryPartnerid_split  Marker is %s" %details;
                        #Get the result of execution
