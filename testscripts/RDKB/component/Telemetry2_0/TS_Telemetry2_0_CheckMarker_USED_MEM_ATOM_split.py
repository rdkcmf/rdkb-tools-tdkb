...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_USED_MEM_ATOM_split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate USED_MEM_ATOM_split  telemetry2_0 marker by running the log_mem_cpu_info.sh script from TAD</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_12</test_case_id>
    <test_objective>To simulate USED_MEM_ATOM_split  telemetry2_0 merker by running the log_mem_cpu_info.sh script from TAD</test_objective>
...
6. Check if marker USED_MEM_ATOM_split is present in telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    <expected_output>The Maker should be present in the telemetry log file </expected_output>
    <priority>High</priority>
    <test_stub_interface>telemetry2_0</test_stub_interface>
    <test_script>TS_Telemetry2_0_CheckMarker_USED_MEM_ATOM_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_USED_MEM_ATOM_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_USED_MEM_ATOM_split');
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "USED_MEM_ATOM_split" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        markervalue = details.split("USED_MEM_ATOM_split<#=#>")[1]
