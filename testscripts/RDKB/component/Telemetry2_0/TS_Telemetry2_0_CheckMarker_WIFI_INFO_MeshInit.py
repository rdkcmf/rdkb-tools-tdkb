...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_Telemetry2_0_CheckMarker_WIFI_INFO_MeshInit</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate WIFI_INFO_MeshInit marker by restarting meshAgent process</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_39</test_case_id>
    <test_objective>This test case is to simulate WIFI_INFO_MeshInit marker by restarting meshAgent process</test_objective>
...
4. Restart the meshAgent process
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker WIFI_INFO_MeshInit    is present in  telemetry log file (only between the initial line count and line count after simulation)
...
10. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_WIFI_INFO_MeshInit</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_WIFI_INFO_MeshInit');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_WIFI_INFO_MeshInit');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            details,actualresult = getPID(tdkTestObj_Sys_ExeCmd,"meshAgent");
            if expectedresult in actualresult and details != "":
               Initialpid = int(details);
               tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
               print "TEST STEP 3: Check if meshAgent process is running";
               print "EXPECTED RESULT 3:meshAgent  process should be running";
               print "ACTUAL RESULT 3: pid of meshAgent:",details;
               print "[TEST EXECUTION RESULT] : SUCCESS";

               actualresult =killProcess(tdkTestObj_Sys_ExeCmd,Initialpid,"");
               if expectedresult in actualresult :
                  tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                  print "TEST STEP 4: Kill meshAgent process";
                  print "EXPECTED RESULT 4:Should Kill the meshAgent process";
                  print "ACTUAL RESULT 4: meshAgent process killed";
                  print "[TEST EXECUTION RESULT] : SUCCESS";
                  ...
                  if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                     ...
                     query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : WIFI_INFO_MeshInit\"" %(initialLinesCount,lineCountAfterSimu)
                     print "query:%s" %query
                     ...
                     if expectedresult in actualresult and details!="" and (len(details) > 0) and "WIFI_INFO_MeshInit" in details:
                        tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                        markervalue = details.split("WIFI_INFO_MeshInit<#=#>")[1]
                        print "TEST STEP 6:WIFI_INFO_MeshInit  Marker should be present";1
                        print "EXPECTED RESULT 6: WIFI_INFO_MeshInit Marker should be present";
                        print "ACTUAL RESULT 6: WIFI_INFO_MeshInit  Marker Value is %s" %markervalue;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";

                        actualresult,pid = checkProcessRestarted(tdkTestObj_Sys_ExeCmd,"meshAgent");
                        if expectedresult in actualresult and pid != "" and pid != Initialpid:
                           tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                           print "TEST STEP 7: Check if the meshAgent proccess restarted";
                           print "EXPECTED RESULT 7:meshAgent proccess should restart ";
                           print "ACTUAL RESULT 7: meshAgent restarted successfully";
                           print "[TEST EXECUTION RESULT] : SUCCESS";
                        else:
                            tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                            print "TEST STEP 7: Check if the meshAgent proccess restarted";
                            print "EXPECTED RESULT 7:meshAgent proccess should restart ";
                            print "ACTUAL RESULT 7: meshAgent restart failed";
                            print "[TEST EXECUTION RESULT] : FAILURE";
                     else:
                         tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                         print "TEST STEP 6:WIFI_INFO_MeshInit  Marker should be present";
                         print "EXPECTED RESULT 6: WIFI_INFO_MeshInit Marker should be present";
                         print "ACTUAL RESULT 6: WIFI_INFO_MeshInit  Marker is %s" %details;
                         #Get the result of execution
                         print "[TEST EXECUTION RESULT] : FAILURE";
                  else:
                      tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                   ...
                   print "TEST STEP 4: Kill meshAgent process";
                   print "EXPECTED RESULT 4:Should Kill the meshAgent  process";
                   print "ACTUAL RESULT 4: Failed to kill meshAgent process";
                   print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                print "TEST STEP 3: Check if meshAgent process is running";
