...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_Wifi_5G_utilization_split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate Wifi_5G_utilization_split telemetry2_0 marker by running the radiohealth.sh script from WIFI Module</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_20</test_case_id>
    <test_objective>To simulate Wifi_5G_utilization_split telemetry2_0 marker by running the radiohealth.sh script from WIFI Module</test_objective>
...
4. Execute the script radiohealth.sh from TAD module
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker Wifi_5G_utilization_split is present in telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_Wifi_5G_utilization_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Wifi_5G_utilization_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Wifi_5G_utilization_split');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            cmd = "sh /usr/ccsp/wifi/radiohealth.sh &";
            ...
            if expectedresult in actualresult:
                ...
                print "TEST STEP 3: Execute the radiohealth script in background";
                print "EXPECTED RESULT 3: radiohealth script should be running";
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : Wifi_5G_utilization_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "Wifi_5G_utilization_split" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        markervalue = details.split("Wifi_5G_utilization_split value : ")[1]
                        print "TEST STEP 5: Wifi_5G_utilization_split  Marker should be present";1
                        print "EXPECTED RESULT 5: Wifi_5G_utilization_split Marker should be present";
                        print "ACTUAL RESULT 5: Wifi_5G_utilization_split  Marker Value is %s" %markervalue;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] 5: SUCCESS";
                        print "***************************************************************"
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "TEST STEP 5: Wifi_5G_utilization_split  Marker should be present";
                        print "EXPECTED RESULT 5: Wifi_5G_utilization_split Marker should be present";
                        print "ACTUAL RESULT 5: Wifi_5G_utilization_split  Marker is %s" %details;
                        #Get the result of execution
