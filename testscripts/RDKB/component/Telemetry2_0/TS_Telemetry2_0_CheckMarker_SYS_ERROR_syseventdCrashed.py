...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_SYS_ERROR_syseventdCrashed</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate SYS_SH_syseventd_restart marker by restarting the syseventd process</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_22</test_case_id>
    <test_objective>This test case is to simulate SYS_SH_syseventd_restart marker by restarting the syseventd  process</test_objective>
...
4. Restart the syseventd  process
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker SYS_SH_syseventd_restart is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the module</automation_approch>
    <expected_output>On simulation by restarting the syseventd process SYS_ERROR_syseventdCrashed  marker should be present.</expected_output>
    <priority>High</priority>
    <test_stub_interface>Telemetry2_0</test_stub_interface>
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_ERROR_syseventdCrashed</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_syseventdCrashed');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_ERROR_syseventdCrashed');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() :
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            details,actualresult = getPID(tdkTestObj_Sys_ExeCmd,"syseventd");
            if expectedresult in actualresult and details != "":
               Initialpid = int(details);
               tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
               print "TEST STEP 3: Check if syseventd process is running";
               print "EXPECTED RESULT 3:syseventd  process should be running";
               print "ACTUAL RESULT 3: pid of syseventd:",details;
               print "[TEST EXECUTION RESULT] : SUCCESS";

               actualresult =killProcess(tdkTestObj_Sys_ExeCmd,Initialpid,"/usr/ccsp/tad/task_health_monitor.sh");
               if expectedresult in actualresult :
                  tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                  print "TEST STEP 4: Kill syseventd process and run task_health_monitor.sh script";
                  ...
                  if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                     ...
                     query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : SYS_ERROR_syseventdCrashed\"" %(initialLinesCount,lineCountAfterSimu)
                     print "query:%s" %query
                     ...
                     if expectedresult in actualresult and details!="" and (len(details) > 0) and "SYS_ERROR_syseventdCrashed" in details:
                        ...
                        if expectedresult in actualresult and pid != "" and pid != Initialpid:
                           ...
                           print "ACTUAL RESULT 7: syseventd restarted successfully";
                           print "[TEST EXECUTION RESULT] : SUCCESS";
                        else:
                         ...
                         tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                         print "TEST STEP 6:SYS_ERROR_syseventdCrashed  Marker should be present";
                         print "EXPECTED RESULT 6: SYS_ERROR_syseventdCrashed Marker should be present";
                         print "ACTUAL RESULT 6: SYS_ERROR_syseventdCrashed  Marker is %s" %details;
                         #Get the result of execution
                         print "[TEST EXECUTION RESULT] : FAILURE";
                  else:
                      tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                   ...
                   print "TEST STEP 4: Kill syseventd process and run task_health_monitor.sh script";
                   print "EXPECTED RESULT 4:Should Kill the syseventd  process and run task_health_monitor.sh script";
                   print "ACTUAL RESULT 4: Failed to kill syseventd process and run task_health_monitor.sh script";
                   print "[TEST EXECUTION RESULT] : FAILURE";
            else:
                tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                print "TEST STEP 3: Check if syseventd process is running";
