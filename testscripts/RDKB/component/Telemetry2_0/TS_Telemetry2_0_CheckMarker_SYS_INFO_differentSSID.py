...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_SYS_INFO_differentSSID</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate SYS_INFO_differentSSID telemetry2_0 marker by setting the different SSID names for 2.4GHz and 5GHz SSID followed by running the getSSIDNames.sh script from TAD</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_18</test_case_id>
    <test_objective>To simulate SYS_INFO_differentSSID telemetry2_0 marker by setting the different SSID names for 2.4GHz and 5GHz SSID followed by running the getSSIDNames.sh script from TAD</test_objective>
...
5. If the SSID names are same then set the SSID's with the different name
...
8. Check if marker SYS_INFO_differentSSID is present in telemetry log file (only between the initial line count and line count after simulation)
...
12. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_INFO_differentSSID</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_INFO_differentSSID');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_INFO_differentSSID');
wifiobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_INFO_differentSSID');
        ...
        if expectedresult in lineCountResult:
            ...
            if expectedresult in (ssid1_res and ssid1_res):
                ...
                if str(ssid1_name) != str(ssid2_name):
                    ...
                    print "SSID Names are same, Initiating SET operation"
                    set_result1,set_details1, tdkTestObj = setWiFiSSIDvalue(wifiobj,"Device.WiFi.SSID.1.SSID","Tel_24Test_WiFi");
                    set_result2,set_details2, tdkTestObj1 = setWiFiSSIDvalue(wifiobj,"Device.WiFi.SSID.2.SSID","Tel_5Test_WiFi");
            ...
            if diff_name == 1:
                ...
                if expectedresult in actualresult:
                    ...
                    if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                        ...
                        query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : SYS_INFO_differentSSID\"" %(initialLinesCount,lineCountAfterSimu)
                        print "query:%s" %query
                        tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                        tdkTestObj.addParameter("command", query);
                        tdkTestObj.executeTestCase(expectedresult);
                        ...
                        if expectedresult in actualresult and details!="" and (len(details) > 0) and "SYS_INFO_differentSSID" in details:
                            ...
                            tdkTestObj.setResultStatus("FAILURE");
                            print "TEST STEP 7: SYS_INFO_differentSSID  Marker should be present";
                            print "EXPECTED RESULT 7: SYS_INFO_differentSSID Marker should be present";
                            print "ACTUAL RESULT 7: SYS_INFO_differentSSID  Marker is %s" %details;
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : FAILURE";
                            print "***************************************************************"
                    else:
