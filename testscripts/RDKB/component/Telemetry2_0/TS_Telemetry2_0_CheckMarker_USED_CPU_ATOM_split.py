...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_USED_CPU_ATOM_split</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate USED_CPU_ATOM_split telemetry2_0 marker by running the log_mem_cpu_info.sh script from TAD</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_14</test_case_id>
    <test_objective>To simulate USED_CPU_ATOM_split telemetry2_0 marker by running the log_mem_cpu_info.sh script from TAD</test_objective>
...
6. Check if marker USED_CPU_ATOM_split is present in telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    ...
    <test_script>TS_Telemetry2_0_CheckMarker_USED_CPU_ATOM_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_USED_CPU_ATOM_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_USED_CPU_ATOM_split');
        ...
        if expectedresult in lineCountResult:
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : USED_CPU_ATOM_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "USED_CPU_ATOM_split" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        markervalue = details.split("USED_CPU_ATOM_split value : ")[1]
                        print "TEST STEP 5: USED_CPU_ATOM_split  Marker should be present";1
                        print "EXPECTED RESULT 5: USED_CPU_ATOM_split Marker should be present";
                        print "ACTUAL RESULT 5: USED_CPU_ATOM_split  Marker Value is %s" %markervalue;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] 5: SUCCESS";
                        print "***************************************************************"
                    else:
                        tdkTestObj.setResultStatus("FAILURE");
                        print "TEST STEP 5: USED_CPU_ATOM_split  Marker should be present";
                        print "EXPECTED RESULT 5: USED_CPU_ATOM_split Marker should be present";
                        print "ACTUAL RESULT 5: USED_CPU_ATOM_split  Marker is %s" %details;
                        #Get the result of execution
