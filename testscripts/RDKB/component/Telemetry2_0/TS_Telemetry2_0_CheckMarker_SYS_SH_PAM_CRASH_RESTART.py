...
<xml>
  ...
  <name>TS_Telemetry2_0_CheckMarker_SYS_SH_PAM_CRASH_RESTART</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To simulate SYS_SH_PAM_CRASH_RESTART marker by restarting the CcspPandMSsp process</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_24</test_case_id>
    <test_objective>This test case is to simulate SYS_SH_PAM_CRASH_RESTART marker by restarting the CcspPandMSsp  process</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>N/A</api_or_interface_used>
    <input_parameters>Telemetry status
Telemetry version
Telemetry config URL</input_parameters>
...
4. Restart the CcspPandMSsp  process
5. Get the number of lines from the telemetry log file after simulation
6. Check if marker SYS_SH_PAM_CRASH_RESTART  is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the module</automation_approch>
    <expected_output>On simulation by restarting the  CcspPandMSsp process SYS_SH_PAM_CRASH_RESTART  marker should be present.</expected_output>
    <priority>High</priority>
    <test_stub_interface>Telemetry2_0</test_stub_interface>
    <test_script>TS_Telemetry2_0_CheckMarker_SYS_SH_PAM_CRASH_RESTART</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_SH_PAM_CRASH_RESTART');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_SYS_SH_PAM_CRASH_RESTART');
