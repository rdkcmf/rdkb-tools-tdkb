...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_Telemetry2_0_CheckMarker_Total_wifi_clients_split</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate Total_wifi_clients_split marker by restarting the ccsp-lmlite process</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_6</test_case_id>
    <test_objective>This test case is to simulate Total_wifi_clients_split marker by restarting the ccsp-lmlite process</test_objective>
...
6. Check if marker Total_wifi_clients_split   is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules
    ...
    <expected_output>On simulation by restarting the CcspLMLite process Total_wifi_clients_split  marker should be present.</expected_output>
    <priority>High</priority>
    <test_stub_interface>Telemetry2_0</test_stub_interface>
    <test_script>TS_Telemetry2_0_CheckMarker_Total_wifi_clients_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Total_wifi_clients_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_Total_wifi_clients_split');
            ...
            if expectedresult in actualresult and details != "":
               ...
               if expectedresult in actualresult :
                  ...
                  if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                     ...
                     query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -i \"Received eventInfo : Total_wifi_clients_split\"" %(initialLinesCount,lineCountAfterSimu)
                     print "query:%s" %query
                     ...
                     if expectedresult in actualresult and details!="" and (len(details) > 0) and "Total_wifi_clients_split" in details:
                        tdkTestObj_Sys_ExeCmd.setResultStatus("SUCCESS");
                        markervalue = details.split("Total_wifi_clients_split<#=#>")[1]
                        print "TEST STEP 6:Total_wifi_clients_split  Marker should be present";1
                        print "EXPECTED RESULT 6: Total_wifi_clients_split Marker should be present";
                        print "ACTUAL RESULT 6: Total_wifi_clients_split  Marker Value is %s" %markervalue;
                        ...
                        if expectedresult in actualresult and pid != "" and pid != Initialpid:
                         ...
                         tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
                         print "TEST STEP 6:Total_wifi_clients_split  Marker should be present";
                         print "EXPECTED RESULT 6: Total_wifi_clients_split Marker should be present";
                         print "ACTUAL RESULT 6: Total_wifi_clients_split  Marker is %s" %details;
                         #Get the result of execution
                         print "[TEST EXECUTION RESULT] : FAILURE";
                  else:
                      tdkTestObj_Sys_ExeCmd.setResultStatus("FAILURE");
