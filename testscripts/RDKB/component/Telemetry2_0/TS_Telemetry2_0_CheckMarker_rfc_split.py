...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_Telemetry2_0_CheckMarker_rfc_split</name>
  <primitive_test_id/>
  <primitive_test_name>Telemetry_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To simulate rfc_split marker by running RFCbase.sh shell script</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_TELEMETRY2_0_8</test_case_id>
    <test_objective>This test case is to simulate rfc_split marker by running RFCbase.sh shell script</test_objective>
...
4. Execute the script RFCbase.sh.
5. Get the numer of lines from the telemetry log file after simulation
6. Check if marker rfc_split is present in  telemetry log file (only between the initial line count and line count after simulation)
...
9. Unload the modules</automation_approch>
    <expected_output>On simulation by restarting the RFCbase.sh shell script rfc_split  marker should be present.</expected_output>
    <priority>High</priority>
    <test_stub_interface>Telemetry2_0</test_stub_interface>
    <test_script>TS_Telemetry2_0_CheckMarker_rfc_split</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysobj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_rfc_split');
tr181obj.configureTestCase(ip,port,'TS_Telemetry2_0_CheckMarker_rfc_split');
    ...
    if preReq_Status == 1:
        ...
        if expectedresult in lineCountResult:
            ...
            if expectedresult in actualresult:
                ...
                print "TEST STEP 3: Execute the RFCbase.sh script in background";
                print "EXPECTED RESULT 3: RFCbase.sh script should be running";
                ...
                if expectedresult in lineCountResult and  int(lineCountAfterSimu) > int(initialLinesCount):
                    ...
                    query = "sed -n -e %s,%sp /rdklogs/logs/telemetry2_0.txt.0 | grep -o \"Received eventInfo : rfc_split\"" %(initialLinesCount,lineCountAfterSimu)
                    print "query:%s" %query
                    tdkTestObj = sysobj.createTestStep('ExecuteCmd');
                    ...
                    if expectedresult in actualresult and details!="" and (len(details) > 0) and "rfc_split" in details:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 5: rfc_split  Marker should be present";1
                        print "EXPECTED RESULT 5: rfc_split Marker should be present";
                        ...
                        print "TEST STEP 5: rfc_split  Marker should be present";
                        print "EXPECTED RESULT 5: rfc_split Marker should be present";
                        print "ACTUAL RESULT 5: rfc_split  Marker is %s" %details;
                        #Get the result of execution
