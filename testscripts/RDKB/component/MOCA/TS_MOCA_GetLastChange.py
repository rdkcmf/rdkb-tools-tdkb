...
<xml>
  ...
  <name>TS_MOCA_GetLastChange</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the LastChange after disabling and enabling Moca Interface. The Lastchange should be zero in case of standalone setup and should be greater than zero otherwise.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MOCA_16</test_case_id>
    <test_objective>To get the LastChange after disabling and enabling Moca Interface. The Lastchange should be zero in case of standalone setup and should be greater than zero otherwise.</test_objective>
    <test_type>Positive</test_type>
    <test_setup>XB3</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components.
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>Mocastub_Get,Mocastub_Set</api_or_interface_used>
    <input_parameters>Device.MoCA.Interface.1.X_CISCO_COM_NumberOfConnectedClients
Device.MoCA.Interface.1.LastChange</input_parameters>
    <automation_approch>1. Load MOCA modules
2. From script invoke Mocastub_Get to get the number of interface entries
3. Enable Moca Interface and get the LastChange
4. Check whether the last change is zero if it is standalone setup and greater than zero otherwise.
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_MOCA_GetLastChange</test_script>
    <skipped>No</skipped>
