...
<xml>
  ...
  <name>TS_MOCA_SetCurrentOperFreq</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the current operating frequency is getting changed when  the FreqCurrentMaskSetting is changed.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_MOCA_10</test_case_id>
    <test_objective>To check if the current operating frequency is getting changed when  the FreqCurrentMaskSetting is changed.</test_objective>
...
3. If the initial Scan Mode is not false, then set the value to false
...
6. Check the Operating Frequency value, the value should get changed to new value , since Scan Mode was false (Manual Mode)
7. Revert the Frequency Mask value to its initial value
8. Revert the Scan Mode value if it was changed in step 3
9. Unload MOCA modules</automation_approch>
    <expected_output>In Manual Mode current operating frequency value should change when the FreqCurrentMaskSetting is changed</expected_output>
    <priority>High</priority>
    <test_stub_interface>None</test_stub_interface>
    <test_script>TS_MOCA_SetCurrentOperFreq</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_MOCA_SetCurrentOperFreq');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult_get:
        ...
        if cur_scan_mode == "false":
            print "Channel ScanMode value value is false (Manual mode), No need to Set the value again"
            scanMode_false = 1;
        else:
            print "\n********** Set the Channel ScanMode to False ********** "
            actualresult_set,scan_mode = Mocastub_Set_Function(tdkTestObj_set,"Device.MoCA.Interface.1.X_CISCO_COM_ChannelScanning","false","bool");
            if actualresult_set in expectedresult:
                scanMode_false = 1;
                revert_flag = 1;
                #Set the result status of execution
                tdkTestObj_set.setResultStatus("SUCCESS");
                print "TEST STEP 2: Set the channel ScanMode value to false";
                print "EXPECTED RESULT 2: Should set channel ScanMode value to false";
        ...
        if scanMode_false == 1:
            ...
            if expectedresult in (actualresult_freq and actualresult_fremask):
                ...
                actualresult_freq_set,scan_mode = Mocastub_Set_Function(tdkTestObj_set,"Device.MoCA.Interface.1.FreqCurrentMaskSetting",Freq_Mask1,"string");
                if actualresult_freq_set in expectedresult:
                    ...
                    print "\n********** Check whether the CurrentOperFreq is changed **********"
                    ...
                    if expectedresult in actualresult_freq and int(new_freq) != int(cur_freq):
                        ...
                        print "EXPECTED RESULT 5: Current operating frequency should change";
                        ...
                        print "TEST STEP 5: Get the current operating frequency and check with previous frequency";
                        print "EXPECTED RESULT 5: Current operating frequency should change";
                        print "ACTUAL RESULT 5: The current operating frequency and new operating frequency were same "
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : FAILURE";
