...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_CCSPMBUS_22</test_case_id>
    <test_objective>To Validate Ccsp Base Interface request session id Function</test_objective>
...
API Name
CCSPMBUS_ReqSessionId
Input
N/A</input_parameters>
    <automation_approch>1.Configure the Function info in Test Manager GUI  which needs to be tested  
(CCSPMBUS_ReqSessionId - func name - "If not exists already"
 ccspcommon_mbus - module name
 Necessary I/P args as Mentioned in Input)
2.Python Script will be generated/overrided automically by Test Manager with provided arguments in configure page (TS_CCSPCOMMON_MBUS_ReqSessionId.py)
3.Execute the generated Script(TS_CCSPCOMMON_MBUS_ReqSessionId.py) using execution page of  Test Manager GUI
4.mbusstub which is a part of TDK Agent process, will be in listening mode to execute TDK Component function named MBus_ReqSessionId through registered TDK mbusstub function along with necessary Entry Values as arguments
5.CCSPMBUS_ReqSessionId function will call ssp_mbus_init and other neccessary init functions,then it calls ssp_mbus_req_sessionid function that inturn will call CCSP Base Interface Function named CcspBaseIf_requestSessionID along with necessary input arguments to get the current session of system through Message Bus
...
7.mbusstub will validate the available result (from ssp_mbus_inform_end_session as CCSP_Message_Bus_OK [100], Non negative Session ID value ) with expected result (CCSP_Message_Bus_OK [100],Non Negative session id value) and the result is updated in agent console log and json output variable
8.ssp_mbus_exit function is invoked by CCSPMBUS_ReqSessionId to close the bus handle created by ssp_mbus_init and returns the updated results to Test Manager 
9.TestManager will publish the result in GUI as PASS/FAILURE based on the response from CCSPMBUS_ReqSessionId function</automation_approch>
    <except_output>CheckPoint 1:
Session ID Value from DUT should be available in Agent Console Log
CheckPoint 2:
...
TestManager GUI will publish the result as PASS in Execution page</except_output>
    ...
    <test_script>TS_CCSPCOMMON_MBUS_ReqSessionId</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
