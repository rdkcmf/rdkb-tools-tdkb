...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_MAC</name>
  <primitive_test_id/>
  <primitive_test_name>ETHAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if marker ETH_MAC_{i} is present when EthLog Enabled with a connected LAN Client</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_ETHAGENT_7</test_case_id>
    <test_objective>This test case is to check if marker ETH_MAC{i} is present when EthLog Enabled with a connected LAN Client</test_objective>
...
8. Now grep on /rdklogs/logs/eth_telemetry.txt and check for ETH_MAC_{i} marker with polling logic of 15 min  and retrieve the mac.
9  Check if the mac equals to the value present in Device.Ethernet.{i}.X_RDKCENTRAL-COM_AssociatedDevice.1.MACAddress
10.Revert the parameters set to their default
11.Unload the module</automation_approch>
    <expected_output>ETH_MAC_{i} marker should be present when Ethlog is enabled </expected_output>
    <priority>High</priority>
    <test_stub_interface>ETHAGENT</test_stub_interface>
    <test_script>TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_MAC</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_MAC');
sysObj.configureTestCase(ip,port,'TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_MAC');
                          ...
                          if flag == 1:
                             ...
                             if expectedresult in actualresult:
                                ...
                                if expectedresult in actualresult:
                                      ...
                                      for i in range(1,5):
                                          if markerfound == 1:
                                             break;
                                              ...
                                              query="cat /rdklogs/logs/eth_telemetry.txt | grep -i \"ETH_MAC_%d:\""%j;
                                              print "query:%s" %query
                                              tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                                          ...
                                          if  details.endswith(":") or details == "" or  "MAC" not in details:
                                              markerfound = 0;
                                              sleep(300);
                                          else:
                                               markerValue = details.split("ETH_MAC")[1].strip().replace("\\n","");
                                               markerValue = markerValue.split(":",1)[1].strip().replace("\\n","")
                                               markerfound = 1;

                                      if expectedresult in actualresult and markerfound == 1 and markerValue!= "":
                                         tdkTestObj.setResultStatus("SUCCESS");
                                         print "TEST STEP 10: ETH_MAC_%d Marker should be present"%j;
                                         print "EXPECTED RESULT 10: ETH_MAC_%d Marker should be present"%j;
                                         print "ACTUAL RESULT 10:ETH_MAC_%d Marker is %s" %(j,markerValue);
                                         #Get the result of execution
                                         print "[TEST EXECUTION RESULT] : SUCCESS";

                                         if EthernetMAC == markerValue:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print "TEST STEP 11: Associated Device Mac via TR-181 parmeter and via marker should be equal";
                                            print "EXPECTED RESULT 11: Should get the  Associated Device Mac  and via marker equal";
                                            print "ACTUAL RESULT 11:Associated Device Mac via TR-181 parmeter:",EthernetMAC,"value from marker:",markerValue;
                                            #Get the result of execution
                                            print "[TEST EXECUTION RESULT] : SUCCESS";
                                         else:
                                             tdkTestObj.setResultStatus("FAILURE");
                                             print "TEST STEP 11: Associated Device Number Of Entries via TR-181 parmeter and via marker should be equal";
                                             print "EXPECTED RESULT 11: Should get the  Associated Device Number Of Entries and via marker equal";
                                             print "ACTUAL RESULT 11:Associated Device Number Of Entries via TR-181 parmeter:",EthernetMAC,"value from marker:",markerValue;
                                             #Get the result of execution
                                             print "[TEST EXECUTION RESULT] : FAILURE";
                                      else:
                                          tdkTestObj.setResultStatus("FAILURE");
                                          print "TEST STEP 10: ETH_MAC_%d Marker should be present"%j;
                                          print "EXPECTED RESULT 10: ETH_MAC_%d Marker should be present"%j;
                                          print "ACTUAL RESULT 10:ETH_MAC_%d Marker not found or doesnt have non-zero value"%j;
                                          #Get the result of execution
                                          print "[TEST EXECUTION RESULT] : FAILURE";
                                   else:
                                       tdkTestObj.setResultStatus("FAILURE");
                                   ...
                                   if expectedresult in actualresult:
                                      tdkTestObj.setResultStatus("SUCCESS");
                                      print "TEST STEP 12: Revert the Ethernet LogPeriod to previous";
                                      print "EXPECTED RESULT 12: Should revert the Ethernet LogPeriod to previous";
                             ...
                             if revertflag == 1:
                                ...
                                if expectedresult in actualresult:
                                   tdkTestObj.setResultStatus("SUCCESS");
                                   print "TEST STEP 13: Revert the Ethernet LogEnable state to previous";
                                   print "EXPECTED RESULT 13: Should revert the Ethernet LogEnable state to previous";
                                   print "ACTUAL RESULT 13: Revert success";
                                   #Get the result of execution
                                   print "[TEST EXECUTION RESULT] :SUCCESS";
                                else:
                                   tdkTestObj.setResultStatus("FAILURE");
                                   print "TEST STEP 13: Revert the Ethernet LogEnable state to previous";
                                   print "EXPECTED RESULT 13: Should revert the Ethernet LogEnable state to previous";
                                   print "ACTUAL RESULT 13: Revert failed";
                                   #Get the result of execution
                                   print "[TEST EXECUTION RESULT] :FAILURE";
                          else:
                              tdkTestObj.setResultStatus("FAILURE");
