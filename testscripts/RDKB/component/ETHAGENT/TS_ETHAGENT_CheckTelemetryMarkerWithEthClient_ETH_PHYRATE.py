...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_PHYRATE</name>
  <primitive_test_id/>
  <primitive_test_name>ETHAgent_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if marker ETH_PHYRATE_{i} is present  when EthLog is Enabled with a connected LAN Client</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_ETHAGENT_9</test_case_id>
    <test_objective>This test case is to check if marker ETH_PHYRATE{i} is present  when EthLog is Enabled with a connected LAN Client</test_objective>
...
Device.Ethernet.Interface.{i}.CurrentBitRate</input_parameters>
...
8. Now grep on /rdklogs/logs/eth_telemetry.txt and check for ETH_PHYRATE{i} marker with polling logic of 15 min  and retrieve the mac.
9  Check if the value equals to the value present in Device.Ethernet.Interface.{i}.CurrentBitRate
10.Revert the parameters set to their default
11.Unload the module</automation_approch>
    <expected_output>ETH_PHYRATE_{i} marker should be present when Ethlog is enabled </expected_output>
    <priority>High</priority>
    <test_stub_interface>ETHAGENT</test_stub_interface>
    <test_script>TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_PHYRATE</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_PHYRATE');
sysObj.configureTestCase(ip,port,'TS_ETHAGENT_CheckTelemetryMarkerWithEthClient_ETH_PHYRATE');
                                      ...
                                      for i in range(1,5):
                                          if markerfound == 1:
                                             break;
                                              ...
                                              query="cat /rdklogs/logs/eth_telemetry.txt | grep -i \"ETH_PHYRATE_%d:\""%j;
                                              print "query:%s" %query
                                              tdkTestObj = sysObj.createTestStep('ExecuteCmd');
                                          ...
                                          if  details.endswith(":") or details == "" or  "PHYRATE" not in details:
                                              markerfound = 0;
                                              sleep(300);
                                          else:
                                               markerValue = details.split("ETH_PHYRATE")[1].strip().replace("\\n","");
                                               markerValue = markerValue.split(":",1)[1].strip().replace("\\n","")
                                               markerfound = 1;

                                      if expectedresult in actualresult and markerfound == 1 and markerValue!= "":
                                         tdkTestObj.setResultStatus("SUCCESS");
                                         print "TEST STEP 10: ETH_PHYRATE_%d Marker should be present"%j;
                                         print "EXPECTED RESULT 10: ETH_PHYRATE_%d Marker should be present"%j;
                                         print "ACTUAL RESULT 10:ETH_PHYRATE_%d Marker is %s" %(j,markerValue);
                                         #Get the result of execution
                                         print "[TEST EXECUTION RESULT] : SUCCESS";

                                         tdkTestObj = obj.createTestStep('pam_GetParameterValues');
                                         tdkTestObj.addParameter("ParamName","Device.Ethernet.Interface.%d.CurrentBitRate" %j);
                                         expectedresult="SUCCESS";
                                         #Execute the test case in DUT
                                         tdkTestObj.executeTestCase(expectedresult);
                                         actualresult = tdkTestObj.getResult();
                                         BitRate = tdkTestObj.getResultDetails();
                                         if expectedresult in actualresult:
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print "TEST STEP 11: Get the Associated Device BitRate via TR-181 parameter";
                                            print "EXPECTED RESULT 11: Should get the  Associated Device  Of Entries";
                                            print "ACTUAL RESULT 11:Associated Device BitRate via TR-181 parameter:",BitRate;
                                            #Get the result of execution
                                            print "[TEST EXECUTION RESULT] : SUCCESS";

                                            if int(BitRate) == int(markerValue):
                                               tdkTestObj.setResultStatus("SUCCESS");
                                               print "TEST STEP 12: Associated Device BitRate via TR-181 parmeter and via marker should be equal";
                                               print "EXPECTED RESULT 12: Should get the  Associated Device BitRate  and via marker equal";
                                               print "ACTUAL RESULT 12:Associated Device BitRate via TR-181 parmeter:",BitRate,"value from marker:",markerValue;
                                               #Get the result of execution
                                               print "[TEST EXECUTION RESULT] : SUCCESS";
                                            else:
                                                tdkTestObj.setResultStatus("FAILURE");
                                                ...
                                                print "ACTUAL RESULT 12:Associated Device Number Of Entries via TR-181 parmeter:",BitRate,"value from marker:",markerValue;
                                                #Get the result of execution
                                                print "[TEST EXECUTION RESULT] : FAILURE";
                                         else:
                                             tdkTestObj.setResultStatus("FAILURE");
                                             print "TEST STEP 11: Get the Associated BitRate via TR-181 parameter";
                                             print "EXPECTED RESULT 11: Should get the  Associated Device BitRate";
                                             print "ACTUAL RESULT 11:Associated Device MAc via TR-181 parameter:",BitRate;
                                             #Get the result of execution
                                             print "[TEST EXECUTION RESULT] : FAILURE";
                                      else:
                                          tdkTestObj.setResultStatus("FAILURE");
                                          print "TEST STEP 10: ETH_PHYRATE_%d Marker should be present"%j;
                                          print "EXPECTED RESULT 10: ETH_PHYRATE_%d Marker should be present"%j;
                                          print "ACTUAL RESULT 10:ETH_PHYRATE_%d Marker not found or doesnt have non-zero value"%j;
                                          #Get the result of execution
                                          print "[TEST EXECUTION RESULT] : FAILURE";
                                   else:
                                       tdkTestObj.setResultStatus("FAILURE");
