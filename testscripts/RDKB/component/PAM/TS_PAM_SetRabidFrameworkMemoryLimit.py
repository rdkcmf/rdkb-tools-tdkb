...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_PAM_SetRabidFrameworkMemoryLimit</name>
  <primitive_test_id/>
  <primitive_test_name>pam_SetParameterValues</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the Rabid Framework memory limit with different values and verify with syscfg show | grep Advsecurity_RabidMemoryLimit</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>This test case is to check the  set operation on  Rabid Framework memory limit with different values and verify with syscfg show | grep Advsecurity_RabidMemoryLimit</test_objective>
...
2.Get the Rabid Framework memory limit and store the value
3.Set the Rabid Framework memory limit with the possible values and cross verify the set value with
syscfg show | grep Advsecurity_RabidMemoryLimit
4.Revert the value back to is original
5.Unload the module</automation_approch>
    <expected_output>We should be able to set the Rabid Framework memory limit with the possible values successfully</expected_output>
    <priority>High</priority>
    <test_stub_interface>PAM</test_stub_interface>
    <test_script>TS_PAM_SetRabidFrameworkMemoryLimit</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'TS_PAM_SetRabidFrameworkMemoryLimit');
pamObj.configureTestCase(ip,port,'TS_PAM_SetRabidFrameworkMemoryLimit');
