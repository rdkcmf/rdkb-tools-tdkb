...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>17</version>
  <name>TS_PAM_CheckProcessStatus</name>
  <primitive_test_id/>
  <primitive_test_name>pam_GetParameterValues</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Test to get the status of tdk process from ProcessStatus table</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_101</test_case_id>
    <test_objective>To  check the status of tdk process</test_objective>
...
Input:
ParamName -
dmcli %s getvalues Device.DeviceInfo.ProcessStatus.Process. | grep -A 10 $(pidof tdk_agent) | grep -A 1 State | cut -f3 -d ':' | tr '\r\n' ' '</input_parameters>
    <automation_approch>1.Function which needs to be tested will be configured in Test Manager GUI.
2.Python Script will be generated by Test Manager with provided arguments in configure page.
3.TM will load thesysutil library via Test agent
4.From python script, invoke ExecuteCmd() stub function to get the status of tdk process
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_PAM_CheckProcessStatus</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_CheckProcessStatus');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj.addParameter("command", "dmcli %s getvalues Device.DeviceInfo.ProcessStatus.Process. | grep -A 10 $(pidof tdk_agent) | grep -A 1 State | cut -f3 -d ':' | tr '\r\n' ' '"%prefix);
