...
<xml>
  ...
  <name>TS_PAM_SetFirewallSecurityIPv4BlockFragIPPkts</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Validate whether iptable rules(IPv4) associated with  BlockFragIPPkts is added/removed into iptables for Enable/Disable on BlockFragIPPkt TR181 parameter</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_PAM_192</test_case_id>
    <test_objective>To Validate whether iptable rules(Ipv4) associated with  BlockFragIPPkt is added/removed into iptables for Enable/Disable the BlockFragIPPkt TR181 parameter
</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>Device.Firewall.X_RDKCENTRAL-COM_Security.V4.BlockFragIPPkts
</input_parameters>
    <automation_approch>1. Load the PAM Module
2. Get the current value of BlockFragIPPkt parameter and store it
3. If the current value is True then Verify whether all iptables rules associated with BlockFragIPPkt was added into iptables, if the verification was success, set the parameter value to False, then verify all iptables rules associated with BlockFragIPPkt was removed from iptables
4. If the current value is false then verify whether all iptables rules associated with BlockFragIPPkt was not added into iptables, if the verification was success, set the parameter value to true, then verify all iptables rules associated with BlockFragIPPkt was added into iptables
...
7. Unload the Module</automation_approch>
    <expected_output>Iptable rules(Ipv4) associated with  BlockFragIPPkt should be added/removed into iptables for Enable/Disable of BlockFragIPPkt TR181 parameter
</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_SetFirewallSecurityIPv4BlockFragIPPkts</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_SetFirewallSecurityIPv4BlockFragIPPkts');
sysobj.configureTestCase(ip,port,'TS_PAM_SetFirewallSecurityIPv4BlockFragIPPkts');
