...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_PAM_EnableRFCWebConfig</name>
  <primitive_test_id/>
  <primitive_test_name>pam_GetParameterValues</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if URL and Supported Docs are populated when Web-Config is enabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_204</test_case_id>
    <test_objective>This test case is to check if URL and Supported Docs are populated when Web-Config is enabled</test_objective>
...
4.Check if the expected URL and supported Docs are populated on enabling the parameter
5.Revert the set value
6.Unload the module</automation_approch>
    <expected_output>The expected URL and supported Docs must populate on enabling the Web config.</expected_output>
    <priority>High</priority>
    <test_stub_interface>PAM</test_stub_interface>
    <test_script>TS_PAM_EnableRFCWebConfig</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
pamobj.configureTestCase(ip,port,'TS_PAM_EnableRFCWebConfig');
sysobj.configureTestCase(ip,port,'TS_PAM_EnableRFCWebConfig');
