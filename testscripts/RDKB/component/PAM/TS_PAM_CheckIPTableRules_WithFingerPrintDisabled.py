...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_PAM_CheckIPTableRules_WithFingerPrintDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>pam_GetParameterValues</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the iptable rules for finger print are not present when Finger print is disabled</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_202</test_case_id>
    <test_objective>This test case is to check if the iptable rules for finger print are not present when Finger print is disabled</test_objective>
...
3.Disable the Device.DeviceInfo.X_RDKCENTRAL-COM_DeviceFingerPrint.Enable if not disabled
4.Check no iptable rules related to finger print are present
5.Revert the value to previous
6.Unload the module</automation_approch>
    <expected_output>When the Device.DeviceInfo.X_RDKCENTRAL-COM_DeviceFingerPrint.Enable is disabled no iptable rules related to finger print should be present</expected_output>
    <priority>High</priority>
    <test_stub_interface>PAM</test_stub_interface>
    <test_script>TS_PAM_CheckIPTableRules_WithFingerPrintDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_WithFingerPrintDisabled');
sysobj.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_WithFingerPrintDisabled');
...
if "SUCCESS" in (loadmodulestatus.upper() and sysutilloadmodulestatus.upper()):
    ...
    if expectedresult in actualresult:
        ...
        if initial_value == "false":
           ...
           if result == 1:
              tdkTestObj.setResultStatus("SUCCESS");
              print "TEST STEP 2: Verify iptables rules for  Device FingerPrint Enable when disabled"
              print "EXPECTED RESULT 2: The iptables rules specific to Device FingerPrint when Disabled should not be present"
              print "ACTUAL TEST 2: Verification on the iptables rules specific to Device FingerPrint Disable is success"
              #Get the result of execution
              print "[TEST EXECUTION RESULT] : SUCCESS"
           else:
               ...
               print "TEST STEP 2: Verify iptables rules for  Device FingerPrint Enable when disabled"
               print "EXPECTED RESULT 2: The iptables rules specific to Device FingerPrint when Disabled should not be present"
               print "ACTUAL TEST 2: Verification on the iptables rules specific to Device FingerPrint Disable Failed";
               #Get the result of execution
            ...
            if expectedresult in actualresult:
               ...
               if expectedresult in actualresult:
                ...
                #Set the result status of execution
                tdkTestObj.setResultStatus("FAILURE");
                print "TEST STEP 2: Set Device FingerPrint Enable status to true"
                print "EXPECTED RESULT 2: Should set Device FingerPrint Enable status to true"
                print "ACTUAL RESULT 2: %s" %result;
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : FAILURE"
    else:
