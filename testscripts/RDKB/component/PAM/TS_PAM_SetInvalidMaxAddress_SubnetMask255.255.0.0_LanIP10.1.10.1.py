...
<xml>
  ...
  <name>TS_PAM_SetInvalidMaxAddress_SubnetMask255.255.0.0_LanIP10.1.10.1</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if setting of an invalid Max Address where the last octet of the IP is less than 2 fails when the Subnet Mask is 255.255.0.0 and Lan IP Address is 10.1.10.1.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_PAM_233</test_case_id>
    <test_objective>To check if setting of an invalid Max Address where the last octet of the IP is less than 2 fails when the Subnet Mask is 255.255.0.0 and Lan IP Address is 10.1.10.1.</test_objective>
...
min_addr = 10.1.10.2
max_addr = 10.1.255.1</input_parameters>
...
4. Set the Max Address to an invalid value 10.1.255.1(where the last octet of the IP is less than 2) with respect to the Subnet Mask and Lan IP and check if the SET operation returns failure.
5. Revert to initial values if required
6. Unload the modules</automation_approch>
    <expected_output>Setting of an invalid Max Address where the last octet of the IP is less than 2 should fail when the Subnet Mask is 255.255.0.0 and Lan IP Address is 10.1.10.1.</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_SetInvalidMaxAddress_SubnetMask255.255.0.0_LanIP10.1.10.1</test_script>
    <skipped>No</skipped>
...
def setMaxAddr(obj1, step, addr):
    ...
    print "\nSetting Max Address to an invalid value %s where the last octet of the IP is less than 2" %addr;
    print "TEST STEP %d : Set Device.DHCPv4.Server.Pool.1.MaxAddress to an invalid value : %s" %(step, addr);
    print "EXPECTED RESULT %d : SET operations should Fail as the Max Address is invalid" %step;

    if expectedresult in actualresult:
        tdkTestObj.setResultStatus("SUCCESS");
        print "ACTUAL RESULT %d: Set operation failed as expected as the last octet of the IP is less than 2; Details : %s" %(step, details);
        print "TEST EXECUTION RESULT :SUCCESS";
    else:
        tdkTestObj.setResultStatus("FAILURE");
        print "ACTUAL RESULT %d: Set operation is success even though the last octet of the IP is less than 2; Details : %s" %(step, details);
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_SetInvalidMaxAddress_SubnetMask255.255.0.0_LanIP10.1.10.1');
obj1.configureTestCase(ip,port,'TS_PAM_SetInvalidMaxAddress_SubnetMask255.255.0.0_LanIP10.1.10.1');
...
#10.1.255.1 is an invalid Max Address as the last octet of the IP is less than 2
max_addr = "10.1.255.1";
