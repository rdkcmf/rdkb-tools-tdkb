...
<xml>
  ...
  <name>TS_PAM_IpIfSetV6Addr</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test will set the IPV6 address alias</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_PAM_80</test_case_id>
    <test_objective>To set the IPV6 address</test_objective>
...
Input:
ParamName - Device.IP.Interface.1.IPv6Address.1.Alias</input_parameters>
    <automation_approch>1.Function which needs to be tested will be configured in Test Manager GUI.
2.Python Script will be generated by Test Manager with provided arguments in configure page.
3.TM will load the PAM library via Test agent
4.From python script, invoke pam_SetParameterValues() stub function to set the IPV6 address.
...
8.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from pam stub.</automation_approch>
 ...
 The IPV6 address after setting should be logged in the Agent console/Component log
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_PAM_IpIfSetV6Addr</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
pamObj.configureTestCase(ip,port,'TS_PAM_IpIfSetV6Addr');
...
if "SUCCESS" in loadmodulestatus.upper():
	...
	if expectedresult in actualresult:
	    ...
	    if expectedresult in actualresult:
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 1: Get IPv6address Alias value"
                print "EXPECTED RESULT 1: Should get IPv6address Alias"
                ...
                if expectedresult in actualresult:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "TEST STEP 1: Set IPv6address Alias value"
                    print "EXPECTED RESULT 1: Should set IPv6address Alias"
                    print "ACTUAL RESULT 1: IPv6address is %s" %details;
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS, %s" %details
                    tdkTestObj = pamObj.createTestStep('pam_SetParameterValues');
                    ...
                    if expectedresult in actualresult:
                        tdkTestObj.setResultStatus("SUCCESS");
                        print "TEST STEP 1: Set IPv6address Alias value to original"
                        print "EXPECTED RESULT 1: Should set IPv6address Alias to original"
                        print "ACTUAL RESULT 1: IPv6address is %s" %details;
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS, %s" %details
                    else:
