...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_PAM_CheckDnsmasqRestartLogs_DNSStrictOrderDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>pam_SetParameterValues</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the log "RFC DNSTRICT ORDER is not defined or Enabled" is populated in the Consolelog.txt.0 during dnsmasq process restart when the DNS Strict Order RFC parameter Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.DNSStrictOrder.Enable is disabled.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_221</test_case_id>
    <test_objective>To check if the log "RFC DNSTRICT ORDER is not defined or Enabled" is populated in the Consolelog.txt.0 during dnsmasq process restart when the DNS Strict Order RFC parameter Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.DNSStrictOrder.Enable is disabled.</test_objective>
...
2. Get the initial value of Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.DNSStrictOrder.Enable. If it is not false, set it to false.
...
6. Once the process comes up, check the final number of log lines "RFC DNSTRICT ORDER is not defined or Enabled" present in Consolelog.txt.0. It should be be incremented by 1 as DNSStrict is Disabled.
7. Revert to initial state if required.
8. Unload the modules.</automation_approch>
    <expected_output>The log "RFC DNSTRICT ORDER is not defined or Enabled" should be populated in the Consolelog.txt.0 during dnsmasq process restart when the DNS Strict Order RFC parameter Device.DeviceInfo.X_RDKCENTRAL-COM_RFC.Feature.DNSStrictOrder.Enable is disabled.</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_CheckDnsmasqRestartLogs_DNSStrictOrderDisabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_CheckDnsmasqRestartLogs_DNSStrictOrderDisabled');
pamobj.configureTestCase(ip,port,'TS_PAM_CheckDnsmasqRestartLogs_DNSStrictOrderDisabled');
