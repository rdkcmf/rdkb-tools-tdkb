...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_PAM_CheckIPTableRules_InBridgeMode_RemoteMgmtDisabled</name>
  <primitive_test_id/>
  <primitive_test_name>pam_SetParameterValues</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the required IPTABLE rules are present in the DUT in bridge-static mode with Remote Management(HTTP Enable and HTTPS Enable) in Disabled state.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_218</test_case_id>
    <test_objective>To check if the required IPTABLE rules are present in the DUT in bridge-static mode with Remote Management(HHTP Enable and HTTPS Enable) in Disabled state.</test_objective>
...
Device.UserInterface.X_CISCO_COM_RemoteAccess.HttpsEnable are bridge-static, false, false respectively. If not, perform a set operation to set them to the required values.
3. Cross verify SET operation with GET.
4. Retrieve the IPTABLE rules required to be present in the DUT when Lan Mode is bridge-static and the Remote Management is disabled from the platform properties file.
...
7. Unload the modules.</automation_approch>
    <expected_output>The required IPTABLE rules should be present in the DUT in bridge-static mode with Remote Management(HHTP Enable and HTTPS Enable) in Disabled state.</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_CheckIPTableRules_InBridgeMode_RemoteMgmtDisabled</test_script>
    <skipped>No</skipped>
...
def verify_iptable_rules(tdkTestObj, step):
    rulesFound = 0;
    cmd = "sh %s/tdk_utility.sh parseConfigFile IPTABLE_RULES_BRIDGE_REMOTEDISABLED  | tr \"\n\" \"  \"" %TDK_PATH;
    print cmd;
...
def checkIPTableRules(result, actualresult, expectedresult, step):
    print "\nTEST STEP %d: Verify IPTABLE rules for Bridge Mode with Remote Access Disabled" %step;
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_InBridgeMode_RemoteMgmtDisabled');
obj1.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_InBridgeMode_RemoteMgmtDisabled');
obj2.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_InBridgeMode_RemoteMgmtDisabled');
...
if "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() and "SUCCESS" in loadmodulestatus3.upper():
    ...
    if expectedresult in status and orgValue[0] != "" and orgValue[1] != "" and orgValue[2] != "":
        ...
        if orgValue[0] == "bridge-static" and orgValue[1] == "false" and orgValue[2] == "false":
