...
<xml>
  ...
  <name>TS_COSAPAM_DhcpsGetState</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>This test case returns the state of DHCP server</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_COSAPAM_07</test_case_id>
    <test_objective>To Validate PAM API CosaDmlDhcpsGetState</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Emulator,Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components.
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>CosaDmlDhcpsGetState</api_or_interface_used>
...
4.From python script, invoke COSAPAM_DhcpGet() stub function to get the state of DHCP server
5.COSAPAM stub function will call the ssp_CosaDmlDhcpGet TDK Component and COSAPAM stub function will be logged in Agent Console log.
...
Values associated with the parameter specified should be logged in the Agent console/Component log and Should get the state of DHCP server successfully
...
Stub function result should be success and should see corresponding log in the agent console log</except_output>
    ...
    <test_script>TS_COSAPAM_DhcpsGetState</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_COSAPAM_DhcpsGetState');
