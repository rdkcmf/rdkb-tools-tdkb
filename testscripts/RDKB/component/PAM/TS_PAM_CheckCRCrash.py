...
<xml>
  ...
  <test_cases>
    ...
    <test_objective>To kill the CR and check if the box reboots with new CR process.</test_objective>
...
3.TM will load the PAM library and sysutil via Test agent
4.From python script, invoke the utility function to check CR process's staus
5.If CR is up, kill it using tdk_platform_utility.sh
6.Before the device goes to reboot, save its state using saveCurrentState()
7.Once the device is up after reboot, restore its previously saved state using restorePreviousStateAfterReboot()
8.Check if the CR process is running, if not reboot the device
9. Unload pam and sysutil modules</automation_approch>
    <except_output>CheckPoint 1:
CR should be killed and the box should be restarted successfully with new CR process running, the status of it should be logged in the Agent console/Component log
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'TS_PAM_CheckCRCrash');
