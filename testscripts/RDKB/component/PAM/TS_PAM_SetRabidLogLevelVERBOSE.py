...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_PAM_SetRabidLogLevelVERBOSE</name>
  <primitive_test_id/>
  <primitive_test_name>pam_GetParameterNames</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To set the Rabid logging level to VERBOSE and verify.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_227</test_case_id>
    <test_objective>To set the Rabid logging level to VERBOSE and verify </test_objective>
...
5.Check the same logged in agent.txt with log message "to LogLevel-4"
...
8.Unload the module</automation_approch>
    ...
    <test_script>TS_PAM_SetRabidLogLevelVERBOSE</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
sysObj.configureTestCase(ip,port,'TS_PAM_SetRabidLogLevelVERBOSE');
pamObj.configureTestCase(ip,port,'TS_PAM_SetRabidLogLevelVERBOSE');
...
if "SUCCESS" in (loadmodulestatus1.upper() and loadmodulestatus2.upper()):
    ...
    if expectedresult in actualresult:
       ...
       print "TEST STEP 2: Setting Finger print log level to VERBOSE";
       ...
       if expectedresult in actualresult:
           ...
           expectedresult="SUCCESS";
           cmd= "cat /rdklogs/logs/agent.txt | grep -i \"to LogLevel-4\""
           tdkTestObj.addParameter("command",cmd);
           tdkTestObj.executeTestCase(expectedresult);
           actualresult = tdkTestObj.getResult();
           details=tdkTestObj.getResultDetails().strip().replace("\\n", "");
           if expectedresult in actualresult  and "LogLevel-4" in details:
               #Set the result status of execution
               tdkTestObj.setResultStatus("SUCCESS");
               ...
               if expectedresult in actualresult and CUJO_GET!="":
                   ...
                   actualresult = tdkTestObj.getResult();
                   details = tdkTestObj.getResultDetails().strip().replace("\\n", "");
                   if expectedresult in actualresult and details  == "4":
                       #Set the result status of execution
                       tdkTestObj.setResultStatus("SUCCESS");
                       print "TEST STEP 5 : Get the log level set with cujo api";
                       print "EXPECTED RESULT 5: should get log level as 4 for Error log level";
