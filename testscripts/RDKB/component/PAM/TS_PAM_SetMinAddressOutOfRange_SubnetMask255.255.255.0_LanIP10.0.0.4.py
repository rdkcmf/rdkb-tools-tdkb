...
<xml>
  ...
  <name>TS_PAM_SetMinAddressOutOfRange_SubnetMask255.255.255.0_LanIP10.0.0.4</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if setting an out of range Min Address fails when the Subnet Mask is 255.255.255.0 and Lan IP is 10.0.0.4.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_PAM_232</test_case_id>
    <test_objective>To check if setting an out of range Min Address fails when the Subnet Mask is 255.255.255.0 and Lan IP is 10.0.0.4.</test_objective>
...
lanip_addr = 10.0.0.4
min_addr = 10.0.0.3
max_addr = 10.0.0.253</input_parameters>
...
3. Check if the initial values of Subnet Mask is 255.255.255.0, Lan IP is 10.0.0.4 and Max Address is 10.0.0.253. If not set them to the required values. Cross check the GET with SET if required.
4. Set the Min Address to an out or range value 10.0.0.3 with respect to the Subnet Mask and Lan IP and check if the SET operation returns failure.
5. Revert to initial values if required
6. Unload the modules</automation_approch>
    <expected_output>Setting an out of range Min Address should fail when the Subnet Mask is 255.255.255.0 and Lan IP is 10.0.0.4.</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_SetMinAddressOutOfRange_SubnetMask255.255.255.0_LanIP10.0.0.4</test_script>
    <skipped>No</skipped>
...
def setMinAddr(obj1, step, addr):
    ...
    print"\nSetting Min Address to an out of range value %s with respect to the current Subnet Mask and the Lan IP" %addr;
    print "TEST STEP %d : Set Device.DHCPv4.Server.Pool.1.MinAddress to an out of range value : %s" %(step, addr);
    print "EXPECTED RESULT %d : SET operations should Fail as the Min Address is out of range" %step;
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_SetMinAddressOutOfRange_SubnetMask255.255.255.0_LanIP10.0.0.4');
obj1.configureTestCase(ip,port,'TS_PAM_SetMinAddressOutOfRange_SubnetMask255.255.255.0_LanIP10.0.0.4');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    if expectedresult in status and orgValue[0] != "" and orgValue[1] != "" and orgValue[2] != "" and orgValue[3] != "":
        ...
        if orgValue[0] == "255.255.255.0" and orgValue[1] == "10.0.0.4" and orgValue[3] == "10.0.0.253":
            print "Set operation not required as Lan Subnet Mask, Lan IP Address and Max Address have expected values initially";
            #Set the Min Address out of range
            step = 2;
            setMinAddr(obj1, step, min_addr);
        else :
            #values to be set
            ...
            tdkTestObj.addParameter("paramList", "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanSubnetMask|%s|string|Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress|%s|string|Device.DHCPv4.Server.Pool.1.MaxAddress|%s|string"%(subnet_mask,lanip_addr,max_addr));
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
            sleep(30);

            print "\nTEST STEP 2 : Set Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanSubnetMask to %s, Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress to %s and Device.DHCPv4.Server.Pool.1.MaxAddress to %s" %(subnet_mask,lanip_addr,max_addr);
            ...
            if expectedresult in actualresult:
                ...
                paramList=["Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanSubnetMask", "Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress", "Device.DHCPv4.Server.Pool.1.MaxAddress"];
                tdkTestObj,status,setValue = getMultipleParameterValues(obj,paramList)

                print "\nTEST STEP 3: Get the values of Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanSubnetMask, Device.X_CISCO_COM_DeviceControl.LanManagementEntry.1.LanIPAddress and Device.DHCPv4.Server.Pool.1.MaxAddress";
                print "EXPECTED RESULT 3: The values should be retrieved successfully and should be the same as set values";

                if expectedresult in status and setValue[0] == subnet_mask and setValue[1] == lanip_addr and setValue[2] == max_addr:
                    #Set the result status of execution
                    tdkTestObj.setResultStatus("SUCCESS");
