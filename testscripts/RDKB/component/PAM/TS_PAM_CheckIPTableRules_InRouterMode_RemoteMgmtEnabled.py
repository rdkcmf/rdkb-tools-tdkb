...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_PAM_CheckIPTableRules_InRouterMode_RemoteMgmtEnabled</name>
  <primitive_test_id/>
  <primitive_test_name>pam_SetParameterValues</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if the required IPTABLE rules are present when the DUT is in Router Mode with Remote Management(HTTP Enable and HTTPS Enable) in Enabled state.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_217</test_case_id>
    <test_objective>To check if the required IPTABLE rules are present when the DUT is in Router Mode with Remote Management(HTTP Enable and HTTPS Enable) in Enabled state.</test_objective>
...
Device.UserInterface.X_CISCO_COM_RemoteAccess.HttpsEnable are router, true, true respectively. If not, perform a set operation to set them to the required values.
3. Cross verify SET operation with GET.
4. Retrieve the IPTABLE rules required to be present in the DUT when Lan Mode is router and the Remote Management is Enabled from the platform properties file.
...
7. Unload the modules.</automation_approch>
    <expected_output>The required IPTABLE rules should be present when the DUT is in Router Mode with Remote Management(HTTP Enable and HTTPS Enable) in Enabled state.</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_CheckIPTableRules_InRouterMode_RemoteMgmtEnabled</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_InRouterMode_RemoteMgmtEnabled');
obj1.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_InRouterMode_RemoteMgmtEnabled');
obj2.configureTestCase(ip,port,'TS_PAM_CheckIPTableRules_InRouterMode_RemoteMgmtEnabled');
