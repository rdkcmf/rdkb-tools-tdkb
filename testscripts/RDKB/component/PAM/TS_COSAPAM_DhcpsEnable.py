...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_COSAPAM_DhcpsEnable</name>
  <primitive_test_id/>
  <primitive_test_name>COSAPAM_DhcpGet</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>This testcase is to enable the DHCP server</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_COSAPAM_05</test_case_id>
    <test_objective>To Validate PAM API CosaDmlDhcpsEnable</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Emulator,Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components.
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>CosaDmlDhcpsEnable</api_or_interface_used>
...
4.From python script, invoke COSAPAM_DhcpsEnable() stub function to set the DHCP server enable
5.COSAPAM stub function will call the ssp_CosaDmlDhcpsEnable function in TDK component which in turn will call cosa api CosaDmlDhcpsEnable() of the PAM Agent in RDKB stack.
...
Values associated with the parameter specified should be logged in the Agent console/Component log and Should set DHCP server enable successfully


CheckPoint 2:
Stub function result should be success and should see corresponding log in the agent console log</except_output>
    <priority>High</priority>
    <test_stub_interface>COSAPAM_DhcpsEnable</test_stub_interface>
    <test_script>TS_COSAPAM_DhcpsEnable</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_COSAPAM_DhcpsEnable');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj.addParameter("MethodName","DhcpsState");
