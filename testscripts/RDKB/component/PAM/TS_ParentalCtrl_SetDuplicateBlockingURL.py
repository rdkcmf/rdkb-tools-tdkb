...
<xml>
  ...
  <name>TS_ParentalCtrl_SetDuplicateBlockingURL</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if two site blocking rules can have the same url attribute</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_PAM_128</test_case_id>
    <test_objective>Check if two site blocking rules can have the same url attribute</test_objective>
...
Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s.BlockMethod|URL|string|Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s.Site|http://go ogle.com|string|Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s.AlwaysBlock|true|bool" %(instance, instance, instance)</input_parameters>
    <automation_approch>1.Function which needs to be tested will be configured in Test Manager GUI.
2.Python Script will be generated by Test Manager with provided arguments in configure page.
3.TM will load the pamstub lib via Test agent
4.with pamstub's getparamvalue() get Device.X_Comcast_com_ParentalControl.ManagedSites.Enablestore it and set it as true
5. With AdvancedConfig_AddObject add new blockedsite instance
6. With AdvancedConfig_SetMultiple, set mode of blocking and invalid url
7. Repeat steps 5 and 6
7. With AdvancedConfig_DelObject delete the added instances
...
TestManager GUI will publish the result as PASS in Execution/Console page of Test Manager</except_output>
    ...
    <test_script>TS_ParentalCtrl_SetDuplicateBlockingURL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ParentalCtrl_SetDuplicateBlockingURL');
pamObj.configureTestCase(ip,port,'TS_ParentalCtrl_SetDuplicateBlockingURL');
		...
		temp = details.split(':');
		    ...
		    expectedresult="SUCCESS";
        	    ...
        	    if expectedresult in actualresult:
            		...
            		if expectedresult in actualresult:
            		    ...
            		    instance2 = temp[1];
                	        ...
                	        #Set the same url as above to create duplicate
                                tdkTestObj = obj.createTestStep("AdvancedConfig_SetMultiple");
        	                tdkTestObj.addParameter("paramList","Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s.BlockMethod|URL|string|Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s.Site|google.com|string|Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s.AlwaysBlock|true|bool" %(instance2, instance2, instance2));
	                        ...
	                        if expectedresult in actualresult:
        	                    tdkTestObj.setResultStatus("SUCCESS");
                	            print "[TEST STEP 4]: Setting duplicate url as url to be blocked"
                            	    print "[EXPECTED RESULT 4]: Should fail in setting duplicate url"
	                            print "[ACTUAL RESULT 4]: SUCESS: Didn't set duplicate url %s" %details;
        	                    print "[TEST EXECUTION RESULT] : %s" %actualresult;
                	        else:
	                            tdkTestObj.setResultStatus("FAILURE");
        	                    print "[TEST STEP 4]: Setting duplicate url as url to be blocked"
                	            print "[EXPECTED RESULT 4]: Should fail in setting duplicate url"
                        	    print "[ACTUAL RESULT 4]: FAILURE: Could add duplicate url in rule";
	                            print "[TEST EXECUTION RESULT] : %s" %actualresult;

                    	   #Delete the created table entry
	                    tdkTestObj = obj.createTestStep("AdvancedConfig_DelObject");
        	            tdkTestObj.addParameter("paramName","Device.X_Comcast_com_ParentalControl.ManagedSites.BlockedSite.%s." %instance2);
                	    expectedresult = "SUCCESS";
 	                    tdkTestObj.executeTestCase(expectedresult);
        	            actualresult = tdkTestObj.getResult();
                	    print "[TEST EXECUTION RESULT] : %s" %actualresult ;
		        ...
		        tdkTestObj.setResultStatus("FAILURE");
                        print "[TEST STEP 4]: Setting a valid url as url to be blocked"
                        print "[EXPECTED RESULT 4]: Should set the url"
                        print "[ACTUAL RESULT 4]: FAILURE: Could not add url in rule";
                        print "[TEST EXECUTION RESULT] : %s" %actualresult;
