...
<xml>
  ...
  <name>TS_PAM_CheckSyscfgFilesAfterReboot_WithUpdateNvramTrue</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Check syscfg files in nvram location and secured location are restored after deleting in reboot scenario when UpdateNvram valule was true</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
8. Unload the pam module</automation_approch>
    <expected_output>The syscfg files should get restored in reboot scenario if files deleted before</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_CheckSyscfgFilesAfterReboot_WithUpdateNvramTrue</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_PAM_CheckSyscfgFilesAfterReboot_WithUpdateNvramTrue');
sysobj.configureTestCase(ip,port,'TS_PAM_CheckSyscfgFilesAfterReboot_WithUpdateNvramTrue');
    ...
    if expectedresult in actualresult and secLocation != "":
        ...
        if expectedresult in actualresult:
            ...
            if initialValue == "true":
                print "Update Nvram initial value is True";
                flag = 1;
            else:
                tdkTestObj = obj.createTestStep('pam_SetParameterValues');
                set_enable_res,set_enable_details = set_SysCfgUpdateNvram(tdkTestObj,"true");

                if expectedresult in actualresult:
                     flag = 1;
                     revertflag = 1;
                     tdkTestObj.setResultStatus("SUCCESS");
                     print "TEST STEP 3: Verify Update Nvram is set to True";
                     print "EXPECTED RESULT 3: Update Nvram should set to True";
            ...
            if flag == 1:
   	        ...
   	        if expectedresult in (tmp_result and secure_result) and tmp_details == tmpLocation and secure_details == secureLocation :
                    ...
                    print "EXPECTED RESULT 4: Both syscfg files should be present";
                    print "ACTUAL RESULT 4: Syscfg files are present ";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : SUCCESS";
                ...
                print "Flag was not set, UpdateNvram Value was not set to True"
                tdkTestObj.setResultStatus("FAILURE");
