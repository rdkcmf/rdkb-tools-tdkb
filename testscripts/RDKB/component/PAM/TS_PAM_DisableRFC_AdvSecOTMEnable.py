...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_PAM_DisableRFC_AdvSecOTMEnable</name>
  <primitive_test_id/>
  <primitive_test_name>pam_SetParameterValues</primitive_test_name>
  <primitive_test_version>2</primitive_test_version>
  <status>FREE</status>
  <synopsis>Set the Advanced Security OTM Enable RFC to disabled state and check if the set value is reflecting in the TR181 get and syscfg get. Also check if the required logging pattern for RFC change in getting logged in agent.txt file.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_PAM_242</test_case_id>
    <test_objective>Set the Advanced Security OTM Enable RFC to disabled state and check if the set value is reflecting in the TR181 get and syscfg get. Also check if the required logging pattern for RFC change in getting logged in agent.txt file.</test_objective>
...
3. If initially value is "false" set to "true" and check if the SET operation returns success.
4. Get the initial line count of the string "ADVANCE_SECURITY_OTM_DISABLED" in /rdklogs/logs/agent.txt.
5. Now toggle the RFC back to false and cross check if SET is reflected in GET.
6. Get the syscfg value of "Adv_AdvSecOTMRFCEnable" and cross check with the TR181 GET.
7. Get the final log line count of the string "ADVANCE_SECURITY_OTM_DISABLED" and check if its incremented by 1 on toggling the RFC to enabled state.
8. If the revert flag is set, perform revert operation of the RFC
9. Unload the modules.
</automation_approch>
    <expected_output>The set operation of Advanced Security OTM Enable RFC to disabled state should be success and the set value should reflect in the TR181 get and syscfg get. Also, the required logging pattern for RFC change should get logged in agent.txt file.</expected_output>
    <priority>High</priority>
    <test_stub_interface>pam</test_stub_interface>
    <test_script>TS_PAM_DisableRFC_AdvSecOTMEnable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
pamobj.configureTestCase(ip,port,'TS_PAM_DisableRFC_AdvSecOTMEnable');
sysobj.configureTestCase(ip,port,'TS_PAM_DisableRFC_AdvSecOTMEnable');
