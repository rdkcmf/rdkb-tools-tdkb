...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WEBCONFIG_DisableRFC_QueryWebConfig</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To Query the parameters under Device.X_RDK_WebConfig. with Webconfig RFC disabled and check the webConfig.log</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_04</test_case_id>
    <test_objective>This test case is to Query the parameters under Device.X_RDK_WebConfig. with Webconfig RFC disabled and check the webConfig.log</test_objective>
...
pam_SetParameterValues</api_or_interface_used>
    <input_parameters>Device.X_RDK_WebConfig.</input_parameters>
    <automation_approch>1.Load the module
2.Get the current status of Webconfig RFC and disable
3.Query the parameters under  Device.X_RDK_WebConfig. the call is expected to be success
4.At the same time check if "PeriodicSyncCheckInterval Get from DB failed" message is logged in webConfig.log file
5.Revert the RFC status to previous
6.Unload the module</automation_approch>
    <expected_output>With webconfig RFC disabled query the Device.X_RDK_WebConfig.
 the "PeriodicSyncCheckInterval Get from DB failed"  is expected to be logged in webConfig.log file
</expected_output>
    <priority>High</priority>
    <test_stub_interface>WEBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_DisableRFC_QueryWebConfig</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
pamobj.configureTestCase(ip,port,'TS_WEBCONFIG_DisableRFC_QueryWebConfig');
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_DisableRFC_QueryWebConfig');
