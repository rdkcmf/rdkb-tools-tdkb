...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WEBCONFIG_AddConfigFile</name>
  <primitive_test_id/>
  <primitive_test_name>Webconfig_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To add new Config file instance to webConfig</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WEBCONFIG_06</test_case_id>
    <test_objective>This test is case is to add new Config file instance to webConfig </test_objective>
...
3.Webconfig distro should be enabled else enable with custom image</pre_requisite>
    <api_or_interface_used>TDKB_TR181Stub_AddObject
TDKB_TR181Stub_DelObject
TDKB_TR181Stub_Get</api_or_interface_used>
    <input_parameters>Device.X_RDK_WebConfig.ConfigFile.</input_parameters>
    <automation_approch>1.Load the module
2.Get the next instance number that is expected on adding new instance to Confg file from syscfg.db
3.Add a new table or a instance entry for Device.X_RDK_WebConfig.ConfigFile.
4.Do a get operation for the newly added instance and is expected to be success
5.Revert the added table entry by deleting the added instance
6.Unload the module</automation_approch>
    <expected_output>The new table entry added for webconfig Config file should be added successfully</expected_output>
    <priority>High</priority>
    <test_stub_interface>WBCONFIG</test_stub_interface>
    <test_script>TS_WEBCONFIG_AddConfigFile</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WEBCONFIG_AddConfigFile');
sysobj.configureTestCase(ip,port,'TS_WEBCONFIG_AddConfigFile');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
    ...
    inst = tdkTestObj.getResultDetails().replace("\\n", "");
    if expectedresult in actualresult and inst.isdigit():
        ...
        if expectedresult in actualresult:
            ...
            tdkTestObj.addParameter("paramName","Device.X_RDK_WebConfig.ConfigFile.%s." %inst);
            expectedresult = "SUCCESS";
            tdkTestObj.executeTestCase(expectedresult);
            actualresult = tdkTestObj.getResult();
            details = tdkTestObj.getResultDetails();
