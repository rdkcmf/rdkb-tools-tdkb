...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>7</version>
  <name>TS_ethsw_stub_hal_SetPortCfg_GetPortCfg_AutoMode</name>
  <primitive_test_id/>
  <primitive_test_name>ethsw_stub_hal_SetPortCfg</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate Ethsw HAL API CcspHalEthSwSetPortCfg() and CcspHalEthSwGetPortCfg()  by setting link rate and duplex mode as Auto</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_HAL_Ethsw_28</test_case_id>
    <test_objective>To validate Ethsw HAL API CcspHalEthSwSetPortCfg() and CcspHalEthSwGetPortCfg()  by setting link rate and duplex mode as Auto</test_objective>
...
CcspHalEthSwGetPortCfg</api_or_interface_used>
    <input_parameters>None</input_parameters>
    <automation_approch>1. Load  halethsw module.
2. Get and save the current port cfg values using CcspHalEthSwGetPortCfg()
3. Using CcspHalEthSwSetPortCfg(), set link rate and duplex mode as Auto
4. Iinvoke CcspHalEthSwGetPortCfg() and check if link rate and duplex mode is changed to Auto
5. Revert back to the initial port cfg values
6. Unload halethsw module.</automation_approch>
    <expected_output>CcspHalEthSwSetPortCfg() should change link rate and duplex mode as Auto and this should reflect in subsequent CcspHalEthSwGetPortCfg() output</expected_output>
    <priority>High</priority>
    <test_stub_interface>halethsw</test_stub_interface>
    <test_script>TS_ethsw_stub_hal_SetPortCfg_GetPortCfg_AutoMode</test_script>
    <skipped>No</skipped>
...
testLinkRateAuto = 1;
obj.configureTestCase(ip,port,'TS_ethsw_stub_hal_SetPortCfg_GetPortCfg_AutoMode');
    ...
    if expectedresult in actualresult and details:
        ...
        if expectedresult in actualresult and details:
            ...
            if expectedresult in actualresult and details:
                ...
                #Set the result status of execution
                tdkTestObj.setResultStatus("SUCCESS");
                print "TEST STEP 3: Retrieve the Ethsw_Get_Port_Cfg" ;
                ...
                #Get the result of execution
                print "[TEST EXECUTION RESULT] : %s" %actualresult;
                if duplexModeAfterSet == testDuplexMode and int(linkRateAfterSet) == testLinkRateAuto:
                    print "TEST STEP 4: Cross verify value if the get and set values match"
