...
<?xml version="1.0" encoding="UTF-8"?><xml>
  ...
  <test_cases>
    <test_case_id>TC_HAL_Ethsw_1</test_case_id>
    <test_objective>To validate Ethsw HAL API CcspHalEthSwGetPortStatus()</test_objective>
...
2. From script invoke ethsw_stub_hal_Get_Port_Status().
3. Get the value of Port status and Link rate
...
5. Test Manager will publish the result in GUI as PASS/FAILURE based on the response from HAL_Ethsw stub.</automation_approch>
    <except_output>Values should be from below:
1. port status - {"CCSP_HAL_ETHSW_LINK_Up","CCSP_HAL_ETHSW_LINK_Down","CCSP_HAL_ETHSW_LINK_Disconnected"}
2. link rate - {"10","100","1000","10000"}</except_output>
    <priority>High</priority>
    <test_stub_interface>HAL_Ethsw</test_stub_interface>
    <test_script>TS_ethsw_stub_hal_Get_Port_Status</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
obj.configureTestCase(ip,port,'TS_ethsw_stub_hal_Get_Port_Status');

obj1 = tdklib.TDKScriptingLibrary("tdkbtr181","1");
obj1.configureTestCase(ip,port,'TS_ethsw_stub_hal_Get_Port_Status');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper():
	...
	#Number of interface entries minus 2 will be the number of ports
        ...
        LinkRate_List = ["0","-1","10","100","1000","10000"];
