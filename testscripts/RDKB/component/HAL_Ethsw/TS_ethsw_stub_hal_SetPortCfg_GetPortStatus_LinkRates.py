...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_ethsw_stub_hal_SetPortCfg_GetPortStatus_LinkRates</name>
  <primitive_test_id/>
  <primitive_test_name>ethsw_stub_hal_SetPortCfg</primitive_test_name>
  <primitive_test_version>3</primitive_test_version>
  <status>FREE</status>
  <synopsis>To validate Ethsw HAL API CcspHalEthSwSetPortCfg() and CcspHalEthSwGetPortStatus() by setting all possible link rates</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_HAL_Ethsw_39</test_case_id>
    <test_objective>To validate Ethsw HAL API CcspHalEthSwSetPortCfg() and CcspHalEthSwGetPortStatus() by setting all possible link rates</test_objective>
...
3. Using CcspHalEthSwSetPortCfg(), set all possible link rate values one by one
4. After each set operation, invoke CcspHalEthSwGetPortStatus() and check if  set value is reflecting
5. Revert back to the initial port cfg values
6. Unload halethsw module.</automation_approch>
    <expected_output>CcspHalEthSwSetPortCfg() should change link rate to all possible values and this should reflect in subsequent CcspHalEthSwGetPortStatus() output</expected_output>
    <priority>High</priority>
    <test_stub_interface>halethsw</test_stub_interface>
    <test_script>TS_ethsw_stub_hal_SetPortCfg_GetPortStatus_LinkRates</test_script>
    <skipped>No</skipped>
...
testLinkRateList = [10,100,1000];
obj.configureTestCase(ip,port,'TS_ethsw_stub_hal_SetPortCfg_GetPortStatus_LinkRates');
...
if "SUCCESS" in loadmodulestatus.upper():
            ...
            if expectedresult in actualresult and details:
                ...
                #Setting the values
                #Script to load the configuration file of the component
		for LinkRate in testLinkRateList:
                    tdkTestObj = obj.createTestStep("ethsw_stub_hal_SetPortCfg");
                    print "PortID : %s" %port_ID
                    tdkTestObj.addParameter("PortID",port_ID);
                    tdkTestObj.addParameter("linkrate", LinkRate);
                    tdkTestObj.addParameter("mode", testDuplexMode);
                    expectedresult = "SUCCESS";
                    print "Setting link rate = %d and Duplex mode = %s" %(LinkRate, testDuplexMode);
                    tdkTestObj.executeTestCase(expectedresult);
                    actualresult = tdkTestObj.getResult();
                    details = tdkTestObj.getResultDetails();
                    if expectedresult in actualresult and details:
		        ...
		        if expectedresult in actualresult and details:
		    	    ...
		    	    duplexModeAfterSet = details.split("/")[3];
                            ...
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : %s" %actualresult;
                            if int (linkRateAfterSet) == LinkRate:
                                print "TEST STEP 4: Cross verify value if the get and set values are equal"
			...
			tdkTestObj.setResultStatus("FAILURE");
                ...
                if expectedresult in actualresult and details:
                    tdkTestObj.setResultStatus("SUCCESS");
                    print "TEST STEP : Resetting the Ethsw_Set_Port_Cfg " ;
                    print "EXPECTED RESULT : Should reset the Ethsw_Set_Port_Cfg successfully";
                    #Get the result of execution
                    print "[TEST EXECUTION RESULT] : %s" %actualresult;
                else:
                    tdkTestObj.setResultStatus("FAILURE");
