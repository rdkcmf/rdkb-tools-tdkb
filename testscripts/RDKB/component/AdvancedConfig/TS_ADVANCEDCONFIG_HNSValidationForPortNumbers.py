...
<xml>
  ...
  <test_cases>
    <test_case_id>TC_ADVANCEDCONFIG_13</test_case_id>
    <test_objective>To verify, HNS must have validation check for port numbers</test_objective>
...
2.Python Script will be generated/overrided automically by Test Manager with provided arguments in configure page (TS_ADVANCEDCONFIG_HNSValidationForPortNumbers.py)
3.Execute the generated Script(TS_ADVANCEDCONFIG_HNSValidationForPortNumbers.py) using execution page of  Test Manager GUI
			...
			temp = details.split(':');
			    ...
			    tdkTestObj = obj.createTestStep("AdvancedConfig_SetMultiple");
					...
					print "[TEST STEP 5]: Setting external port with invalid port number ";
                                ...
                                print "[ACTUAL RESULT 3]: %s" %details;
                                print "[TEST EXECUTION RESULT] : %s" %actualresult;
                                print "Failed to set port trigger \n"
			else:
