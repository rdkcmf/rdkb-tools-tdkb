...
<xml>
  ...
  <name>TS_ADVANCEDCONFIG_TriggerNumberOfEntries</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the number of entries in PortTriggering</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_ADVANCEDCONFIG_27</test_case_id>
    <test_objective>To get the number of port triggeing rules</test_objective>
...
2.Python Script will be generated/overrided automically by Test Manager with provided arguments in configure page (TS_ADVANCEDCONFIG_TriggerNumberOfEntries.py)
3.Execute the generated Script(TS_ADVANCEDCONFIG_TriggerNumberOfEntries.py) using execution page of  Test Manager GUI
...
Check if Advanced -&gt; Port Triggering -&gt; get the number of port triggering rules
CheckPoint 2:
...
TestManager GUI will publish the result as SUCCESS in Execution page</except_output>
    ...
    <test_script>TS_ADVANCEDCONFIG_TriggerNumberOfEntries</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_ADVANCEDCONFIG_TriggerNumberOfEntries');
