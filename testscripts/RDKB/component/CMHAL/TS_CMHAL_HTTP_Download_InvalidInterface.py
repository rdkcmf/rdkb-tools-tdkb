...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_CMHAL_HTTP_Download_InvalidInterface</name>
  <primitive_test_id/>
  <primitive_test_name>CMHAL_GetParamUlongValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Check if cm_hal_Set_HTTP_Download_Interface() returns failure on setting an invalid interface</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_CMHAL_107</test_case_id>
    <test_objective>Check if cm_hal_Set_HTTP_Download_Interface() returns failure on setting an invalid interface</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>cm_hal_Set_HTTP_Download_Interface
...
3. Set an invalid interface value of 100 using cm_hal_Set_HTTP_Download_Interface() and check if the set api returns failure
4. If set didn't fail, revert back the initial interface value
5. Unload  cmhal module</automation_approch>
    <expected_output>cm_hal_Set_HTTP_Download_Interface() returns failure on setting an invalid interface</expected_output>
    <priority>High</priority>
    <test_stub_interface>cmhal</test_stub_interface>
    <test_script>TS_CMHAL_HTTP_Download_InvalidInterface</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CMHAL_HTTP_Download_InvalidInterface');
