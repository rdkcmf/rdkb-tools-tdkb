...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_CMHAL_GetChannelIDWithLockStatus</name>
  <primitive_test_id/>
  <primitive_test_name>CMHAL_GetParamCharValue</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To get the channelID and LockStatus. If the LockStatus is  "Not Locked" ChannelID should be zero. Otherwise it should not be zero</synopsis>
  <groups_id/>
...
5.Test Manager will publish the result in GUI as PASS/FAILURE based on the response from TAD stub.</automation_approch>
    <except_output>If the Lockstatus is "Not Locked" ChannelID should be zero. Otherwise it should not be zero</except_output>
    <priority>High</priority>
    <test_stub_interface>CosaCM</test_stub_interface>
    <test_script>TS_CMHAL_GetChannelIDWithLockStatus</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
    ...
    if Details !="" :
	    ...
	    #Data[0] is LockStatus and Data[1] is ChannelID
            ...
            if "Locked" in Data[0] and int(Data[1]) > 0:
                status = "Success";
            elif "NotLocked" in Data[0] and int(Data[1]) == 0:
                status = "Success";
            else:
                status = "Failure";
                break;
