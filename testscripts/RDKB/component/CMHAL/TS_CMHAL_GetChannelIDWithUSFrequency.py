...
<xml>
  ...
  <name>TS_CMHAL_GetChannelIDWithUSFrequency</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To retrieve Channel ID and corresponding frequencies of the upstream channels</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_CMHAL_08</test_case_id>
    <test_objective>To retrieve Channel ID and corresponding frequencies of the upstream channels</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>docsis_GetUSChannel</api_or_interface_used>
    <input_parameters>paramName: USChannelIDAndFrequency</input_parameters>
    <automation_approch>1. Load  cmhal module
2. Invoke docsis_GetUSChannel to get the channel ID and corresponding frequencies of upstream channels.
...
6. Unload cmhal module</automation_approch>
    <except_output>The Channel ID and frequencies for each upstream channel are retrieved.</except_output>
    <priority>High</priority>
    <test_stub_interface>CMHAL</test_stub_interface>
    <test_script>TS_CMHAL_GetChannelIDWithUSFrequency</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CMHAL_GetChannelIDWithUSFrequency');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #This method invokes the HAL API docsis_GetUSChannel and retrieves the channelID and frequency of upstream channels.
    tdkTestObj = obj.createTestStep("CMHAL_GetParamCharValue");
    tdkTestObj.addParameter("paramName","USChannelIDAndFrequency");
    ...
    if expectedresult in actualresult and "Success" in status:
        ...
        print "TEST STEP 1: Get and validate the UpStream Frequency and ChannelID";
