...
<xml>
  ...
  <name>TS_CMHAL_GetUSLockStatusWithModulation</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the LockStatus and modulation of upstream channel. If the LockStatus is  "Not Locked",modulation should be empty. Otherwise it should not be empty</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>paramName : "USLockStatusAndModulation"</input_parameters>
    <automation_approch>1. Load  cmhal module
2. Invoke docsis_GetUSChannel to get the modulation and lock status of upstream channel
...
5. Unload cmhal module</automation_approch>
    <except_output>If the Lockstatus is "Not Locked", upstream modulation should not be empty . Otherwise it should be empty</except_output>
    <priority>High</priority>
    <test_stub_interface>CM_HAL</test_stub_interface>
    <test_script>TS_CMHAL_GetUSLockStatusWithModulation</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CMHAL_GetUSLockStatusWithModulation');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #This method invokes the HAL API docsis_GetUSChannel and retrieves the modulation and lock status of upstream channel
    tdkTestObj = obj.createTestStep("CMHAL_GetParamCharValue");
    tdkTestObj.addParameter("paramName","USLockStatusAndModulation");
