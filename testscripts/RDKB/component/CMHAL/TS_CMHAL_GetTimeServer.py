...
<xml>
  ...
  <name>TS_CMHAL_GetTimeServer</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether the Time Server of DHCP is getting updated properly</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>paramName : "TimeServer"</input_parameters>
    <automation_approch>1. Load  cmhal module
2. From script invoke cm_hal_GetDHCPInfo to get the TimeServer of DHCP
...
5. Unload cmhal module</automation_approch>
    <except_output>The Time Server of DHCP should be retrieved successfully</except_output>
    <priority>High</priority>
    <test_stub_interface>CM_HAL</test_stub_interface>
    <test_script>TS_CM_HAL_GetTimeServer</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
if "SUCCESS" in loadmodulestatus.upper():
    obj.setLoadModuleStatus("SUCCESS");
     ...
     #This method invokes the HAL API cm_hal_GetDHCPInfo and retrieve the TimeServer and checks if it empty or not.
    tdkTestObj = obj.createTestStep("CMHAL_GetParamCharValue");
    tdkTestObj.addParameter("paramName","TimeServer");
    expectedresult="SUCCESS";
    tdkTestObj.executeTestCase(expectedresult);
    actualresult = tdkTestObj.getResult();
    TimeServer = tdkTestObj.getResultDetails();
