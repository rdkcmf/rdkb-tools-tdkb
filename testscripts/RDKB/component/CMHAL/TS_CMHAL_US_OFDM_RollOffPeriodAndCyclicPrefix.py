...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_CMHAL_US_OFDM_RollOffPeriodAndCyclicPrefix</name>
  <primitive_test_id/>
  <primitive_test_name>CMHAL_GetUsOfdmChanTable</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Get the value of CyclicPrefix and RollOffPeriod for upstream. The RollOffPeriod should not be greater than or equal to cyclic prefix</synopsis>
  <groups_id/>
  ...
  <test_cases>
    ...
    <test_objective>Get the value of CyclicPrefix and RollOffPeriod for upstream. The RollOffPeriod should not be greater than or equal to cyclic prefix</test_objective>
...
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    ...
    <input_parameters>US_OFDM_CyclicPrefix , US_OFDM_RollOffPeriod</input_parameters>
    <automation_approch>1. Load  cmhal module
2. Invoke docsis_GetUsOfdmaChanTable to get the number of cyclic prefix and roll of period
3. Validate the value
4. Unload cmhal module</automation_approch>
    <except_output>The RollOffPeriod should not be greater than or equal to cyclic prefix</except_output>
    <priority>High</priority>
    <test_stub_interface>CMHAL</test_stub_interface>
    <test_script>TS_CMHAL_US_OFDM_RollOffPeriodAndCyclicPrefix</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CMHAL_US_OFDM_RollOffPeriodAndCyclicPrefix');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj.addParameter("paramName","US_OFDM_CyclicPrefix");
