...
<xml>
  ...
  <name>TS_CMHAL_GetUSRanging</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check whether the upstream ranging is Complete if the CMStatus is OPERTAIONAL</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_CMHAL_46</test_case_id>
    <test_objective>To check whether the upstream ranging is Complete if the CMStatus is OPERTAIONAL</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>docsis_GetDOCSISInfo</api_or_interface_used>
    <input_parameters>paramName : "UpstreamRanging"</input_parameters>
    <automation_approch>1. Load  cmhal module
2. Invoke docsis_GetDOCSISInfo to get the upstream ranging
3. Check if the upstream ranging is "Complete" if the CMStatus is "OPERATIONAL"
4. The test should return FAILURE otherwise
5. Unload cmhal module</automation_approch>
    <except_output>The upstream ranging should be Complete if the CMStatus is OPERATIONAL</except_output>
    <priority>High</priority>
    <test_stub_interface>CMHAL</test_stub_interface>
    <test_script>TS_CMHAL_GetUSRanging</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CMHAL_GetUSRanging');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
        ...
        tdkTestObj.executeTestCase(expectedresult);
        actualresult = tdkTestObj.getResult();
        UpstreamRanging= tdkTestObj.getResultDetails();
