...
<xml>
  ...
  <name>TS_CMHAL_GetUSChannelIDWithPower</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To get the channel ID and power of upstream channel. If the channel ID is zero,power should be empty. Otherwise it should not be empty</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_CMHAL_41</test_case_id>
    <test_objective>To get the channel ID and power of upstream channel. If the channel ID is zero,power should be empty. Otherwise it should not be empty</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state of DUT that includes component under test Cable Modem
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>docsis_GetUSChannel</api_or_interface_used>
    <input_parameters>paramName : "USChannelIDAndPower"</input_parameters>
    <automation_approch>1. Load  cmhal module
2. Invoke docsis_GetUSChannel to get the channel ID and power of upstream channel
...
5. Unload cmhal module</automation_approch>
    <except_output>If the channel id is non zero, upstream channel power should not be empty</except_output>
    <priority>High</priority>
    <test_stub_interface>CMHAL</test_stub_interface>
    <test_script>TS_CMHAL_GetUSChannelIDWithPower</test_script>
    <skipped>No</skipped>
    <release_version/>
    <remarks/>
  </test_cases>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_CMHAL_GetUSChannelIDWithPower');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    #this method invokes the HAL API docsis_GetUSChannel to retrieve the channel id and power of the upstream channel
    tdkTestObj = obj.createTestStep("CMHAL_GetParamCharValue");
    tdkTestObj.addParameter("paramName","USChannelIDAndPower");
