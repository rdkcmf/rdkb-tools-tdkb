...
<xml>
  ...
  <name>TS_EPONHAL_GetLocalResetCount</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To check if the count is greater than or equal to zero for Local Reset count</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_EPONHAL_16</test_case_id>
    <test_objective>Get the  Local Reset count  using dpoe_hal_LocalResetCount and check  if  count is greater than or equal to 0,Reset count value will be zero by default and gets increments by 1 when corresponding reset happens</test_objective>
...
3. Unload eponhal module</automation_approch>
    ...
    <test_script>TS_EPONHAL_GetLocalResetCount</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_EPONHAL_GetLocalResetCount');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj.addParameter("paramName","GetLocalResetCount");
