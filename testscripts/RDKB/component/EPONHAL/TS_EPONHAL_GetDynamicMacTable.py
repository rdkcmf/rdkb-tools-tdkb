...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_EPONHAL_GetDynamicMacTable</name>
  <primitive_test_id/>
  <primitive_test_name>EPONHAL_GetDynamicMacTable</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Using dpoe_getDynamicMacTable get the dynamic mac and for each entries check if  the MAC address is valid .</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_EPONHAL_21</test_case_id>
    <test_objective>Get the no of Dynamic Mac Table  entry.For each entries check if  the MAC address is valid .</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script</pre_requisite>
    <api_or_interface_used>dpoe_getDynamicMacTable</api_or_interface_used>
    <input_parameters>numentries</input_parameters>
    <automation_approch>1. Load eponhal module
2. Get the no of Dynamic Mac Table  entry.using Device.DPoE.DPoE_DynamicMacTableNumberOfEntries
3. Check if the count is greater than  0.
4.Invoke dpoe_getDynamicMacTable using the received numEntries.
...
6. Unload eponhal module</automation_approch>
    ...
    <test_script>TS_EPONHAL_GetDynamicMacTable</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_EPONHAL_GetDynamicMacTable');
tr181obj.configureTestCase(ip,port,'TS_EPONHAL_GetDynamicMacTable');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj.addParameter("ParamName","Device.DPoE.DPoE_DynamicMacTableNumberOfEntries");
