...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_EPONHAL_GetMinOnuLinkStatistics</name>
  <primitive_test_id/>
  <primitive_test_name>EPONHAL_GetOnuLinkStatistics</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Using dpoe_getOnuStatistics get  list of all LLID port traffic stats and check if the values retrieved are greater than or equal to zero</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_EPONHAL_23</test_case_id>
    <test_objective>Giving  the minimum numentries and check if the values retrieved are greater than or equal to zero</test_objective>
...
2.Invoke dpoe_getOnuLinkStatistics using the min number of  numEntries.
3.Check if the values received are greater than or equal to zero.
4. Unload eponhal module</automation_approch>
    <expected_output>Received list of all LLID port traffic stats after the succesful call should be greater than or equal to zero.</expected_output>
    <priority>High</priority>
    <test_stub_interface>EPONHAL</test_stub_interface>
    <test_script>TS_EPONHAL_GetMinOnuLinkStatistics</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_EPONHAL_GetMinOnuLinkStatistics');
minonu = [];
flag = 1;
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    if expectedresult in actualresult:
       ...
       print "ACTUAL RESULT 1: Succesfully invoke dpoe_getOnuLinkStatistics";
       ...
       if flag == 1:
          ...
          print "ACTUAL RESULT 2: The values received are:",minonu;
          tdkTestObj.setResultStatus("SUCCESS");
          print "[TEST EXECUTION RESULT] : SUCCESS";
