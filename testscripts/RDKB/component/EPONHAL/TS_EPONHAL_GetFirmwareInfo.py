...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_EPONHAL_GetFirmwareInfo</name>
  <primitive_test_id/>
  <primitive_test_name>EPONHAL_GetFirmwareInfo</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check the functionality of dpoe_getFirmwareInfo</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_EPONHAL_10</test_case_id>
    <test_objective>Get the FirmwareInfog using dpoe_GetFirmwareInfo() and check if the reveived details is non empty.</test_objective>
...
3. Unload eponhal module</automation_approch>
    ...
    <test_script>TS_EPONHAL_GetFirmwareInfo</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_EPONHAL_GetFirmwareInfo');
...
if "SUCCESS" in loadmodulestatus.upper():
    ...
    tdkTestObj = obj.createTestStep('EPONHAL_GetFirmwareInfo');
    ...
    if expectedresult in actualresult and resultDetails != " " :
        info_bootVersion = resultDetails.split(':')[1].split(',')[0].strip()
        info_bootCrc32  =  resultDetails.split(':')[2].split(',')[0].strip()
