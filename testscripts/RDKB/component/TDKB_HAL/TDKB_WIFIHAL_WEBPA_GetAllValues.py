...
<xml>
  ...
  <name>TDKB_WIFIHAL_WEBPA_GetAllValues</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if all TR181 parameter values of WIFI (using WEBPA) matching with its corresponding HAL API Values</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
...
8. Unload modules</automation_approch>
    ...
    <test_script>TDKB_WIFIHAL_WEBPA_GetAllValues</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
objhal.configureTestCase(ip,port,'TDKB_WIFIHAL_WEBPA_GetAllValues');
obj.configureTestCase(ip,port,'TDKB_WIFIHAL_WEBPA_GetAllValues');
sysobj.configureTestCase(ip,port,'TDKB_WIFIHAL_WEBPA_GetAllValues');
