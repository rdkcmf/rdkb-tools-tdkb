...
<xml>
  ...
  <name>TDKB_CMHAL_SNMP_GetAllValues</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if all TR181 parameter values (using SNMP) of CM matching with its corresponding HAL API Values</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_TDKB_HAL_1</test_case_id>
    <test_objective>To Check if all TR181 parameter values of CM (Using SNMP) matching with its corresponding HAL API Values</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>TDK test agent should be running
CM HAL module's parameter xml should be available</pre_requisite>
    <api_or_interface_used>none</api_or_interface_used>
    <input_parameters>none</input_parameters>
    <automation_approch>1. Load tdkbtr181 module, CMHAL module and sysutil module
...
4. Invoke parseXML() by passing CMHAL as module name and get the Parameters and HAL API list
5. Invoke getTR181andHALAPIValue() by passing CMHAL as module name
...
8. Unload modules</automation_approch>
    ...
    <test_script>TDKB_CMHAL_SNMP_GetAllValues</test_script>
    <skipped>No</skipped>
...
objhal = tdklib.TDKScriptingLibrary("cmhal","1");
...
ip = <ipaddress>
port = <port>
objhal.configureTestCase(ip,port,'TDKB_CMHAL_SNMP_GetAllValues');
obj.configureTestCase(ip,port,'TDKB_CMHAL_SNMP_GetAllValues');
sysobj.configureTestCase(ip,port,'TDKB_CMHAL_SNMP_GetAllValues');
...
if "SUCCESS" in (loadmodulestatus1.upper() and loadmodulestatus2.upper() and loadmodulestatus3.upper()):
    ...
    if prerequisite_status == 1:
        ...
        if xmlName != "Error":
            paramList,HALAPIList,tr181_ExpRes,hal_ExpRes = tdkbHalTr181Utility.parseXML(xmlName,"CMHAL",setup_type);
