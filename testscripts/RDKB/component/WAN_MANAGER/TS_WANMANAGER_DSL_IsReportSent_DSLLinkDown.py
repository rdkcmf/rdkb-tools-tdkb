...
<xml>
  ...
  <name>TS_WANMANAGER_DSL_IsReportSent_DSLLinkDown</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify whether the DSL harvester reports are sent when DSL link is down</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_49</test_case_id>
    <test_objective>Verify whether the DSL harvester reports are sent when DSL link is down</test_objective>
    <test_type>Negative</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.Wan Manager should be enabled
4.DSL connection must be disabled</pre_requisite>
    <api_or_interface_used>None</api_or_interface_used>
    <input_parameters>Device.DSL.X_RDK_Report.DSL.Enabled : True
Device.DSL.X_RDK_Report.DSL.ReportingPeriod : 60
Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod : 5
Device.DSL.X_RDK_Report.DSL.Default.OverrideTTL : 60</input_parameters>
    <automation_approch>1. Ensure that the DSL link is down
2. Get the initial DSL Reporting parameters
3. Invoke the pre-requisite function for DSL reports.
4. Set DSL Enable to true, Reporting period to 5, Default Reporting period to 60 and override TTL to 60.
5. Sleep for 60s and check if the log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 0 in 60s as the DSL Link is down.
6. Revert back to initial values</automation_approch>
    <expected_output>The DSL reports should not be sent when DSL LInk is down.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportSent_DSLLinkDown</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_DSLLinkDown');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_DSLLinkDown');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_DSLLinkDown');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() :
    ...
    #Verify is DSL is disabled.
    step = 1;
    tdkTestObj, dsl_wan, enable = getDSLWANStatus(obj, step);

    step = step + 1;
    print "TEST STEP %d : Check if the DSL WAN Status is Disabled and Active Link is false" %step;
    print "EXPECTED RESULT %d : The DSL WAN Status is Disabled and Active Link is false" %step;

    #If DSL is disabled
    if enable == 1:
        #Set the result status of execution
        tdkTestObj.setResultStatus("SUCCESS");
        print "ACTUAL RESULT %d: DSL WAN Status is Disabled and Active Link is false" %step;
            ...
            if flag == 0:
                ...
                if set_status == 0:
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] == str(value_list[1])) and (set_value[2] == str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod is set to 60s and override TTL =0, sleeping for 120s
                        ...
                        #Reports should not be sent in the last 120s as DSL Link is down
                        step = step + 1;
