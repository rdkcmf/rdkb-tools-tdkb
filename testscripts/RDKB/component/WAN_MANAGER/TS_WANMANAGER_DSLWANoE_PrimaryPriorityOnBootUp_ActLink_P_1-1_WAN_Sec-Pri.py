...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-1_WAN_Sec-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if WANoE line is active with PRIMARY_PRIORITY_ON_BOOTUP policy ,WAN Type and priorities being (1,1) (Secondary,Primary) for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_79</test_case_id>
    <test_objective>This test case is to check if WANoE line is active with PRIMARY_PRIORITY_ON_BOOTUP  policy ,WAN Type and priorities being (1,1) (Secondary,Primary) for DSL and WANOE respectively </test_objective>
...
5.Set the  WAN Type and priorities as (1,1) (Secondary,Primary)  for DSL and WANOE respectively and reboot the device for changes to reflect
...
9.Unload the Module</automation_approch>
    <expected_output>With Primary Priority on  BootUp policy Wan Type and priorities being (1,1) (Secondary,Primary) for DSL and WANOE respectively -WANOE Line is expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-1_WAN_Sec-Pri</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-1_WAN_Sec-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-1_WAN_Sec-Pri');
