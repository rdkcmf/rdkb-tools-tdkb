...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if DSL line is active with PRIMARY_PRIORITY_ON_BOOTUP policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL interface should make the active link of DSL false</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_107</test_case_id>
    <test_objective>This test case is to check if DSL line is active with PRIMARY_PRIORITY_ON_BOOTUP policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL interface should make the active link of DSL false</test_objective>
...
6.Set the  Wan Type and priorities as (0,0) (Primary,Secondary) for DSL and WANOE respectively reboot for changes to take place
7.Get the active link status for DSL and WANOE
8.With the current configurations DSL Line is expected to be active
9.Disable the DSL interface and reboot for changes to take place
...
12.Unload the Module</automation_approch>
    <expected_output>with PRIMARY_PRIORITY_ON_BOOTUP policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL interface should make the active link of DSL false</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL');
                                ...
                                if expectedresult in (actualresult1 and actualresult2):
                                    ...
                                    if activeDSL == "true" and activeWANOE == "false":
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            print "\n\n***** Initiating a reboot for DSL Disable operation to take place since its PRIMARY_PRIORITY_ON_BOOTUP WAN Policy *******";
                                            tdkTestObj = obj1.createTestStep('ExecuteCmd');
                                            obj1.initiateReboot();
                                            sleep(300);
