...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestReportingPeriod</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether the report is send when the Reporting Period is changed to lowest supported values and the OverrideTTL is set</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_32</test_case_id>
    <test_objective>Verify whether the report is send when the Reporting Period is changed to lowest supported value and the OverrideTTL is set</test_objective>
...
Device.DSL.X_RDK_Report.DSL.Default.OverrideTTL : 10</input_parameters>
...
3. Set DSL Enable to true, Reporting period to 1, Default Reporting period to 300 and override TTL to 10.
4. Sleep for 10s and check if the expected log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 10 in 10s.
5. Revert the Reporting parameters to initial values.</automation_approch>
    <expected_output>The DSL report is sent when the Reporting Period is changed to lowest supported value and the OverrideTTL is set.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestReportingPeriod</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportName_TTLSet_LowestReportingPeriod');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportName_TTLSet_LowestReportingPeriod');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportName_TTLSet_LowestReportingPeriod');
    ...
    if enable == 0:
        ...
        if status == 0:
            ...
            if flag == 0:
                ...
                print "Set Reporting period to the lowest supported value";
                step = step + 1;
                value_list = [];
                expectedresult = "SUCCESS";
                ...
                if set_status == 0:
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] == str(value_list[1])) and (set_value[2] == str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the Reporting period is set to 10s and override TTL = 10s,  sleeping for 10s
                        ...
                        #10 reports should be sent in the last 10 seconds
                        step = step + 1;
                        print "TEST STEP %d : Check if the number of DSL reports sent after setting Reporting parameters is 10" %step;
