...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check with  PRIMARY_PRIORITY_ON_BOOTUP  policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_110</test_case_id>
    <test_objective>This test case is to check with  PRIMARY_PRIORITY_ON_BOOTUP  policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false</test_objective>
...
5.If DUT_Mode_Primary_Priority_OnBootup is set to 1 set the policy to PRIMARY_PRIORITY_ON_BOOTUP and run the script</pre_requisite>
...
2.If DUT_Mode_Primary_Priority_OnBootup is set to 0 script will handle the policy change else check if expected policy is set and proceed else mark script as FAILURE    
...
5.Get the current WAN policy , set the policy to PRIMARY_PRIORITY_ON_BOOTUP if not in the  same policy
6.Set the  Wan Type and priorities as (0,0) (Secondary,Primary) for DSL and WANOE Disable the WANoE and DSL interface respectively and reboot for set to reflect
...
10.Unload the Module</automation_approch>
    <expected_output>With  PRIMARY_PRIORITY_ON_BOOTUP  policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
    ...
    if  (DUT_Mode_Primary_Priority_OnBootup == 0 and status == 0) or (status == 0 and DUT_Mode_Primary_Priority_OnBootup == 1 and policy_initial=="PRIMARY_PRIORTIY_ON_BOOTUP"):
             ...
             if status == 0:
                 ...
                 if expectedresult in actualresult:
                     ...
                     if expectedresult in actualresult:
                         ...
                         new_policy = "PRIMARY_PRIORTIY_ON_BOOTUP"
                         expectedresult="SUCCESS";
                         policyStatus =1;
                         revert = 0
                         ...
                         if policyStatus == 1:
                             ...
                             if expectedresult in actualresult:
                                 ...
                                 if expectedresult in actualresult:
                                     ...
                                     print "\n\n***** Initiating a reboot for WAN Changes to take place since its PRIMARY_PRIORTIY_ON_BOOTUP WAN Policy *******";
