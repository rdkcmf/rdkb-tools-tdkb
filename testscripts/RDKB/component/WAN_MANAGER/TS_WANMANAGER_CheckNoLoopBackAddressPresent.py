...
<xml>
  ...
  <name>TS_WANMANAGER_CheckNoLoopBackAddressPresent</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Check if no loopback address is present in resolv.conf</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_19</test_case_id>
    <test_objective>To Check if no loopback address is present in resolv.conf</test_objective>
...
3.Check for resolv.conf file presence
4.No loopback address should be present in the file
5.Unload the module</automation_approch>
    <expected_output>In a WAN manager enabled image no loopback address should be present in resolv.conf</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_CheckNoLoopBackAddressPresent</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_CheckNoLoopBackAddressPresent');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_CheckNoLoopBackAddressPresent');
