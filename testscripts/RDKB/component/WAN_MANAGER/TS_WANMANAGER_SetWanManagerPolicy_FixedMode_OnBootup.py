...
<xml>
  ...
  <name>TS_WANMANAGER_SetWanManagerPolicy_FixedMode_OnBootup</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To set Device.X_RDK_WanManager.Policy to FIXED_MODE_ON_BOOTUP and check if the required logs are found in WANMANAGERlog.txt on bootup.</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_13</test_case_id>
    <test_objective>To set Device.X_RDK_WanManager.Policy to FIXED_MODE_ON_BOOTUP and check if the required logs are found in WANMANAGERlog.txt on bootup.</test_objective>
...
Sysutil_Stub</api_or_interface_used>
    <input_parameters>Device.X_RDK_WanManager.Policy : FIXED_MODE_ON_BOOTUP</input_parameters>
    <automation_approch>1. Load the module
2. Get the value of Device.X_RDK_WanManager.Policy
3. Set Device.X_RDK_WanManager.Policy to FIXED_MODE_ON_BOOTUP. The device is expected to go for a reboot after the set operation
...
6. Unload the module</automation_approch>
    <expected_output>Setting Device.X_RDK_WanManager.Policy to FIXED_MODE_ON_BOOTUP should be success and the required logs are found in WANMANAGERlog.txt on bootup.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_SetWanManagerPolicy_FixedMode_OnBootup</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_SetWanManagerPolicy_FixedMode_OnBootup');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_SetWanManagerPolicy_FixedMode_OnBootup');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
    ...
    if status == 0:
        ...
        if status == 0:
            #Set the Wan Manager Policy to FIXED_MODE_ON_BOOTUP
            ...
            if status == 0:
                if policy == new_policy:
                    ...
                    log = "Fixed Mode On Bootup Policy Thread Starting"
                    result = [];
