...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Pri-Sec</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check with  PRIMARY_PRIORITY_ON_BOOTUP  policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_108</test_case_id>
    <test_objective>This test case is to check with  PRIMARY_PRIORITY_ON_BOOTUP  policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false</test_objective>
...
6.Set the  Wan Type and priorities as (0,0) (Primary,Secondary) for DSL and WANOE Disable the WANoE and DSL interface respectively reboot for set to reflect
...
10.Unload the Module</automation_approch>
    <expected_output>With  PRIMARY_PRIORITY_ON_BOOTUP  policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Pri-Sec</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Pri-Sec');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_DisableWANOEDSL_P_0-0_WAN_Pri-Sec');
                         ...
                         if policyStatus == 1:
                             ...
                             if expectedresult in actualresult:
                                 ...
                                 if expectedresult in actualresult:
                                     ...
                                     print "\n\n***** Initiating a reboot for WAN Changes to take place since its PRIMARY_PRIORITY_ON_BOOTUP WAN Policy *******";
