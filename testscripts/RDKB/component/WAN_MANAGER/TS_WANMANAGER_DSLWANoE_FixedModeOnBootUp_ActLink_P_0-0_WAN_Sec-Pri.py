...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_0-0_WAN_Sec-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if WANOE line is active with FIXED_MODE_ON_BOOTUP policy,WAN Type and priorities being (0,0) (Secondary,Primary) for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_66</test_case_id>
    <test_objective>This test case is to check if WANOE line is active with FIXED_MODE_ON_BOOTUP policy ,WAN Type and priorities being (0,0) (Secondary,Primary) for DSL and WANOE respectively </test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.WAN Manager should be enabled
4.Both DSL WAN Connection and WANoE WAN connection should be available.</pre_requisite>
...
4.Get the current WAN policy , set the policy to FIXED_MODE_ON_BOOTUP if not in the  same policy
5.Set the  Wan Type and priorities as (0,0) (Secondary,Primary) for DSL and WANOE respectively and reboot the device for changes to reflect
6.Get the active link status for DSL and WANOE
7.With the current configurations WANOE Line is expected to be active
8.Revert the set values
9.Unload the Module</automation_approch>
    <expected_output>With Fixed Mode  on  BootUp policy WAN Type and priorities being  (0,0) (Secondary,Primary) for DSL and WANOE respectively -WANOE Line is expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_0-0_WAN_Sec-Pri</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_0-0_WAN_Sec-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_0-0_WAN_Sec-Pri');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
                ...
                if expectedresult in actualresult:
                    ...
                    if policyStatus == 1:
                       ...
                       if expectedresult in actualresult:
                           ...
                           print "***** Initiating a reboot for WAN Changes to take place since its FIXED_MODE_ON_BOOTUP WAN Policy *******";
    ...
    if revertwantype == 1:
        ...
        paramList = ["Device.X_RDK_WanManager.CPEInterface.2.Wan.Type","Device.X_RDK_WanManager.CPEInterface.2.Wan.Type"];
        tdkTestObj = obj.createTestStep('TDKB_TR181Stub_Set');
        index = 0;
        for item in paramList:
            tdkTestObj.addParameter("ParamName",item);
    ...
    if revertflag == 1:
       print "Revert operation sucessful for WAN Type and WAN priority";
    else:
