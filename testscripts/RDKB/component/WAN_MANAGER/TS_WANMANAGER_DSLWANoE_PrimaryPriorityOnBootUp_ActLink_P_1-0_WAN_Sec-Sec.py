...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-0_WAN_Sec-Sec</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if WANoE or DSL line is active with PRIMARY_PRIORITY_ON_BOOTUP policy Wan Type and priorities being (1,0) (Secondary,Secondary) for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_77</test_case_id>
    <test_objective>This test case is to check if WANoE or DSL line is active with PRIMARY_PRIORITY_ON_BOOTUP  policy Wan Type and priorities being (1,0) (Secondary,Secondary) for DSL and WANOE respectively </test_objective>
...
4.Get the current WAN policy , set the policy to PRIMARY_PRIORITY_ON_BOOTUP if not in the  same policy
5.Set the  WAN  Type and priorities as(1,0) (Secondary,Secondary) for DSL and WANOE respectively and reboot the device for changes to reflect
6.Get the active link status for DSL and WANOE
7.With the current configurations WANOE or DSL Line is expected to be active
8.Revert the set values
9.Unload the Module</automation_approch>
    <expected_output>With Primary Priority on  BootUp policy Wan Type and priorities being (1,0) (Secondary,Secondary) for DSL and WANOE respectively -WANOE  or DSL Line is expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-0_WAN_Sec-Sec</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-0_WAN_Sec-Sec');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriorityOnBootUp_ActLink_P_1-0_WAN_Sec-Sec');
            ...
            if expectedresult in actualresult:
                ...
                if expectedresult in actualresult:
                    ...
                    if policyStatus == 1:
                       ...
                       if expectedresult in actualresult:
                           tdkTestObj.setResultStatus("SUCCESS");
                           print "TEST STEP 3: Set the (WANtypee,Priority)for DSL(%s,%s) and WANOE(%s,%s)" %(wanDSL,priDSL,wanWANOE,priWANOE);
                           print "EXPECTED RESULT 3:Set operation is expected to be successful";
                           print "ACTUAL RESULT 3:set operations are successful";
                           #Get the result of execution
                           print "[TEST EXECUTION RESULT] : SUCCESS";

                           print "****Initiating reboot for WAN Change take place since its PRIMARY_PRIORITY_ON_BOOTUP policy***";
                           ...
                           if expectedresult in (actualresult1 and actualresult2):
                              ...
                              #Get the result of execution
                              print "[TEST EXECUTION RESULT] : SUCCESS";
                              if activeDSL == "true" or activeWANOE == "true":
                                  #Set the result status of execution
                                  tdkTestObj.setResultStatus("SUCCESS");
                                  print "TEST STEP 5: Get the Active link status of DSL and WANOE";
                                  print "EXPECTED RESULT 5: Active link status of DSL or WANOE  expected to be true";
                               ...
                               tdkTestObj.setResultStatus("FAILURE");
                               print "TEST STEP 4: Get the Active link status of DSL and WANOE";
                               print "EXPECTED RESULT 4:  Active link status of DSL or WANOE  expected to be true";
