...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_2-3_WAN_Pri-Sec</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if DSL line is active with FIXED_MODE_ON_BOOTUP policy Wan priorities and Type being (2,3) (Primary,Secondary) for DSL and WANOE respectively.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANGER_114</test_case_id>
    <test_objective>To check if DSL line is active with FIXED_MODE_ON_BOOTUP policy Wan priorities and Type being (2,3) (Primary,Secondary) for DSL and WANOE respectively</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>1.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.WAN Manager should be enabled
4.Both DSL WAN and WANOE WAN connections should be available
...
6.Set the  Wan Type and priorities as (2,3) (Primary,Secondary) for DSL and WANOE respectively and reboot the device for changes to reflect
...
10.Unload the Module</automation_approch>
    ...
    <test_script>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_2-3_WAN_Pri-Sec</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_2-3_WAN_Pri-Sec');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_2-3_WAN_Pri-Sec');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
    ...
    if  (DUT_Mode_Fixed_Mode_OnBootup == 0 and status == 0) or (status == 0 and DUT_Mode_Fixed_Mode_OnBootup == 1 and policy_initial=="FIXED_MODE_ON_BOOTUP"):
        ...
        tdkTestObj.setResultStatus("FAILURE");
        print "\n\n Since DUT_Mode_Fixed_Mode is selected as 1 ,set the policy to FIXED_MODE and run the script";
