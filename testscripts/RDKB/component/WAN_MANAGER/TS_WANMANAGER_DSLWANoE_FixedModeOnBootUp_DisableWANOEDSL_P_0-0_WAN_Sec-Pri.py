...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check with  FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_102</test_case_id>
    <test_objective>This test case is to check with  FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces should make the active link of WANoE  and DSL as false</test_objective>
...
5.If DUT_Mode_Fixed_Mode_OnBootup is set to 1 set the policy to FIXED_MODE_ON_BOOTUP and run the script
</pre_requisite>
...
2.If DUT_Mode_Fixed_Mode_OnBootup is set to 0 script will handle the policy change else check if expected policy is set and proceed else mark script as FAILURE
...
5.Get the current WAN policy , set the policy to FIXED_MODE_ON_BOOTUP if not in the  same policy
6.Set the  Wan Type and priorities as (0,0) (Secondary,Primary) for DSL and WANOE Disable the WANoE and DSL interface respectively and reboot the device for changes to reflect
7.Get the active link status for DSL and WANOE
8.With the current configurations WANoE Line and DSL Line is expected to be inactive
9.Revert the set values
10.Unload the Module</automation_approch>
    <expected_output>With  FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL and WANoE interfaces  -  should make the active link of WANoE  and DSL as false</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_DisableWANOEDSL_P_0-0_WAN_Sec-Pri');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
    ...
    if  (DUT_Mode_Fixed_Mode_OnBootup == 0 and status == 0) or (status == 0 and DUT_Mode_Fixed_Mode_OnBootup == 1 and policy_initial=="FIXED_MODE_ON_BOOTUP"):
             ...
             if status == 0:
                 ...
                 if expectedresult in actualresult:
                     ...
                     if expectedresult in actualresult:
                         ...
                         new_policy = "FIXED_MODE_ON_BOOTUP"
                         expectedresult="SUCCESS";
                         policyStatus =1;
                         revert = 0
                         ...
                         if policyStatus == 1:
                             ...
                             if expectedresult in actualresult:
                                 ...
                                 actualresult = ToggleDSLAndWANOEInterfaces(tdkTestObj_Set,"false");
                                 if expectedresult in actualresult:
                                     tdkTestObj.setResultStatus("SUCCESS");
                                     print "TEST STEP 4: Disable the DSL and WANOE Interfaces"
                                     print "EXPECTED RESULT 4:Set operation is expected to be successful";
                                     print "ACTUAL RESULT 4:set operations are successful";
                                     #Get the result of execution
                                     print "[TEST EXECUTION RESULT] : SUCCESS";

                                     print "\n\n***** Initiating a reboot for WAN Changes to take place since its FIXED_MODE_ON_BOOTUP WAN Policy *******";
                                     ...
                                     if expectedresult in (actualresult1 and actualresult2):
                                         ...
                                         print "EXPECTED RESULT 5: Active link status of DSL should be fetched successfully";
                                         print "ACTUAL RESULT 5: Get operation succeeded";
                                         #Get the result of execution
                                         print "[TEST EXECUTION RESULT] : SUCCESS";

                                         if activeDSL == "false" and activeWANOE == "false":
                                             #Set the result status of execution
                                             tdkTestObj.setResultStatus("SUCCESS");
                                             print "TEST STEP 6: Get the Active link status of DSL and WANOE";
                                             print "EXPECTED RESULT 6: Active link status of DSL is expected to be false and WANOE as false";
                                             print "ACTUAL RESULT 6: DSL status :%s, WANOE status : %s" %(activeDSL,activeWANOE);
                                             #Get the result of execution
                                             print "[TEST EXECUTION RESULT] : SUCCESS";
                                         else:
                                             #Set the result status of execution
                                         ...
                                         tdkTestObj.setResultStatus("FAILURE");
                                         print "TEST STEP 5: Get the Active link status of DSL and WANOE";
                                         print "EXPECTED RESULT 5: Active link status of DSL is expected to be false and WANOE as false";
                                         print "ACTUAL RESULT 5: Get operation failed ";
                                         #Get the result of execution
                                         print "[TEST EXECUTION RESULT] : FAILURE";

                                     actualresult = ToggleDSLAndWANOEInterfaces(tdkTestObj_Set,"true");
                                     if expectedresult in actualresult:
                                         tdkTestObj.setResultStatus("SUCCESS");
                                         print "TEST STEP 7: Enable the DSL and WANOE Interfaces"
                                         print "EXPECTED RESULT 7:Set operation is expected to be successful";
                                         print "ACTUAL RESULT 7:set operations are successful";
                                         #Get the result of execution
                                         print "[TEST EXECUTION RESULT] : SUCCESS";
                                     else:
                                         tdkTestObj.setResultStatus("FAILURE");
        ...
        tdkTestObj.setResultStatus("FAILURE");
        print "\n\n Since DUT_Mode_Fixed_Mode_OnBootup is selected as 1 ,set the policy to FIXED_MODE_ON_BOOTUP and run the script";
    #Revert operations
    revertflag =1;
    if revertwantype == 1:
