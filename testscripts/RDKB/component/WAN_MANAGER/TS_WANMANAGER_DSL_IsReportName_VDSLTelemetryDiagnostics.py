...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>4</version>
  <name>TS_WANMANAGER_DSL_IsReportName_VDSLTelemetryDiagnostics</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether the report is send with the name as "VDSLTelemetryDiagnostics"</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_30</test_case_id>
    <test_objective>Verify whether the report is send with the name as "VDSLTelemetryDiagnostics"</test_objective>
...
Device.DSL.X_RDK_Report.DSL.ReportingPeriod : 60
Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod : 60
Device.DSL.X_RDK_Report.DSL.Default.OverrideTTL : 60</input_parameters>
    <automation_approch>1. Get the DSL Diagnostic Reporting parameters.
2. Invoke DSL reports pre-requisite function to set reporting parameters to 0.
3. Set DSL Enable to true, Reporting period to 60, Default Reporting period to 60 and override TTL is 60.
4. Sleep for 60s and check if the expected log line "VDSLTelemetryDiagnostics" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 1 in 60s.
5. Revert the Reporting parameters to initial values.</automation_approch>
    <expected_output>The DSL report is send with the name as "VDSLTelemetryDiagnostics".</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportName_VDSLTelemetryDiagnostics</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportName_VDSLTelemetryDiagnostics');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportName_VDSLTelemetryDiagnostics');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportName_VDSLTelemetryDiagnostics');
    ...
    if enable == 0:
        ...
        if status == 0:
            ...
            if flag == 0:
                ...
                #Get the number of log lines "VDSLTelemetryDiagnostics" in /rdklogs/logs/XDSLMANAGERLog.txt.0
                step = step + 1;
                tdkTestObj1 = obj1.createTestStep('ExecuteCmd');
                log = "VDSLTelemetryDiagnostics";
                no_of_lines,step = getLogFileTotalLinesCount(tdkTestObj1, log, step);
                print "The initial number of log lines \"VDSLTelemetryDiagnostics\" in XDSLMANAGERLog.txt.0 is : %d" %no_of_lines;

                #Set Default Reporting Period & Reporting Period to supported values
                print "Set Reporting parameters and override TTL to 60s";
                step = step + 1;
                value_list = [];
                expectedresult = "SUCCESS";
                ...
                if set_status == 0:
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] == str(value_list[1])) and (set_value[2] == str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod is set to 60s and override TTL = 60s,  sleeping for 60s
                        ...
                        print "The final number of log lines \"VDSLTelemetryDiagnostics\" in XDSLMANAGERLog.txt.0 is : %d" %no_of_lines_after_rp;

                        #One more report should be sent in the last 1 minute
                        step = step + 1;
                        print "TEST STEP %d : Check if the DSL report sent after setting the Reporting period values has the name \"VDSLTelemetryDiagnostics\"" %step;
                        print "EXPECTED RESULT %d : The DSL report name should be \"VDSLTelemetryDiagnostics\"" %step;
                        difference = no_of_lines_after_rp - no_of_lines;

                        if difference == 1:
                            #Set the result status of execution
                            tdkTestObj.setResultStatus("SUCCESS");
                            print "ACTUAL RESULT %d: The DSL report name is \"VDSLTelemetryDiagnostics\". The number of DSL reports sent after the set operation is : %d" %(step, difference);
                            #Get the result of execution
                            print "[TEST EXECUTION RESULT] : SUCCESS";
                        else:
                            #Set the result status of execution
