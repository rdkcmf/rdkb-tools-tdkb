...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>12</version>
  <name>TS_WANMANAGER_DSL_IsReportSent_TTLZero_UnSupportedPeriods</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether the report is send when the  Default Reporting Period and Reporting Period  is changed to non- supported values and Override TTL is zero</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_29</test_case_id>
    <test_objective>Verify whether the report is send when the  Default Reporting Period and Reporting Period  is changed to non- supported values and Override TTL is zero</test_objective>
...
Device.DSL.X_RDK_Report.DSL.ReportingPeriod : 65
Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod : 100
...
2. Check if override TTL is 0. If not Zero, reset to 0 with a reboot.
3. Invoke the DSL reports pre-requisite function and set the parameters to 0.
4. Set DSL Enable to true, Reporting period to 65 and Default Reporting period to 100. These are un-supported periods. The set operation should fail.
4. Sleep for 100s and check if the expected log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 0.
5. Revert the Reporting parameters to initial values.</automation_approch>
    <expected_output>The DSL report should not be sent when the  Default Reporting Period and Reporting Period  is changed to un-supported values and Override TTL Is zero.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportSent_TTLZero_UnSupportedPeriods</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLZero_UnSupportedPeriods');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLZero_UnSupportedPeriods');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLZero_UnSupportedPeriods');
    ...
    if enable == 0:
        ...
        if TTL_flag == 0:
            ...
            if flag == 0:
                ...
                #Set Default Reporting Period & Reporting Period to un-supported values
                print "Set Reporting periods to unsupported values and override TTL to 0";
                step = step + 1;
                value_list = [];
                expectedresult = "FAILURE";
                reporting_enable = "true";
                def_reporting_period = "100";
                reporting_period = "65";
                override_TTL = "0";
                ...
                if set_status == 0:
                    ...
                    print "EXPECTED RESULT %d : The get values should not match the set values" %step;
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] != str(value_list[1])) and (set_value[2] != str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the values set are unsupported and override TTL =0,  sleeping for 100s, the unsupported default reporting period set
                        ...
                        #No report should be sent in the last 100s
                        step = step + 1;
                        print "TEST STEP %d : Check if the number of DSL reports sent after Default Reporting Period & Reporting Period are set to un-supported values is 0" %step;
