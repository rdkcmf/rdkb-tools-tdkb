...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Pri-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  WANOE line is active with PRIMARY_PRIORITY policy ,WAN priorities  and Type being (1,0) (Primary,Primary)  for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_68</test_case_id>
    <test_objective>This test case is to check if  WANOE line is active with PRIMARY_PRIORITY policy ,WAN priorities  and Type being (1,0) (Primary,Primary)  for DSL and WANOE respectively </test_objective>
...
5.Set the  WAN Type and priorities as (1,0) (Primary,Primary)  for DSL and WANOE respectively
...
9.Unload the module</automation_approch>
    <expected_output>With Primary Priority policy Wan Type and priorities being (1,0) (Primary,Primary)  for DSL and WANOE respectively - WANoE Line is expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Pri-Pri</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Pri-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Pri-Pri');
                       ...
                       if expectedresult in actualresult:
                           ...
                           if expectedresult in (actualresult1 and actualresult2):
                              ...
                              if activeDSL == "false" and activeWANOE == "true":
                                  ...
                                  tdkTestObj.setResultStatus("FAILURE");
                                  print "TEST STEP 5 Get the Active link status of DSL and WANOE";
                                  print "EXPECTED RESULT 5Active link status of DSL is expected to be false and WANOE as true";
                                  print "ACTUAL RESULT 5: DSL status :%s, WANOE status : %s" %(activeDSL,activeWANOE);
                                  #Get the result of execution
