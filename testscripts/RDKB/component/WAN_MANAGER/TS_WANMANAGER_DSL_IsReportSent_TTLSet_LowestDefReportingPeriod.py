...
<xml>
  ...
  <name>TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestDefReportingPeriod</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>Verify whether the report is send when the Default Reporting Period is changed to lowest supported values and the OverrideTTL is set</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_33</test_case_id>
    <test_objective>Verify whether the report is send when the Default Reporting Period is changed to lowest supported values and the OverrideTTL is set</test_objective>
...
3. Set DSL Enable to true, Reporting period to 300, Default Reporting period to 1 and override TTL to 10.
4. Sleep for 10s and check if the expected log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 0 in 10s.
5. Revert the Reporting parameters to initial values.</automation_approch>
    <expected_output>The report is not sent when the Default Reporting Period is changed to lowest supported values and the OverrideTTL is set in the given interval time.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportName_TTLSet_LowestDefReportingPeriod</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestDefReportingPeriod');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestDefReportingPeriod');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestDefReportingPeriod');
    ...
    if enable == 0:
        ...
        if status == 0:
            ...
            if flag == 0:
                ...
                print "Set Default reporting period to the lowest value";
                step = step + 1;
                value_list = [];
                expectedresult = "SUCCESS";
                ...
                if set_status == 0:
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] == str(value_list[1])) and (set_value[2] == str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the Def Reporting period is set to 1s and override TTL = 10s,  sleeping for 10s
                        ...
                        #Reports should not be sent in the last 10s
                        step = step + 1;
