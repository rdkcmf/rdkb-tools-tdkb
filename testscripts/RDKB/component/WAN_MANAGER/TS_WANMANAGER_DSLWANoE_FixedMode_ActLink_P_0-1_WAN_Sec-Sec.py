...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-1_WAN_Sec-Sec</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  DSL line is active with FIXED_MODE policy ,WAN Type and priorities being (0,1) (Secondary,Secondary)  for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_84</test_case_id>
    <test_objective>This test case is to check if  DSL line is active with FIXED_MODE policy ,WAN Type and priorities being (0,1) (Secondary,Secondary)  for DSL and WANOE respectively</test_objective>
    <test_type>Positive</test_type>
    <test_setup>Broadband</test_setup>
    <pre_requisite>.Ccsp Components  should be in a running state else invoke cosa_start.sh manually that includes all the ccsp components and TDK Component
2.TDK Agent should be in running state or invoke it through StartTdk.sh script
3.WAN Manager should be enabled
4.Both DSL WAN and WANOE WAN connections should be available
5.If DUT_Mode_Fixed_Mode is set to 1 set the policy to FIXED_MODE and run the script</pre_requisite>
...
2.If DUT_Mode_Fixed_Mode is set to 0 script will handle the policy change else check if expected policy is set and proceed else mark script as FAILURE
...
5.Get the current WAN policy , set the policy to FIXED_MODE if not in the  same policy
6.Set the  WAN Type and priorities as (0,1) (Secondary,Secondary)  for DSL and WANOE respectively
...
10.Unload the module</automation_approch>
    <expected_output>With Fixed Mode policy Wan Type and priorities being (0,1) (Secondary,Secondary)   for DSL and WANOE respectively - DSL Line is expected to be active</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-1_WAN_Sec-Sec</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-1_WAN_Sec-Sec');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-1_WAN_Sec-Sec');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
    ...
    if  (DUT_Mode_Fixed_Mode == 0 and status == 0) or (status == 0 and DUT_Mode_Fixed_Mode == 1 and policy_initial=="FIXED_MODE"):
