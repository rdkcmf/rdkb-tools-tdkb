...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-1_WAN_Sec-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  WANOE line is active with PRIMARY_PRIORITY policy ,WAN priorities and Type being (1,1) (Secondary,Primary)  for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_69</test_case_id>
    <test_objective>This test case is to check if  WANOE line is active with PRIMARY_PRIORITY policy ,WAN priorities and Type  being (1,1) (Secondary,Primary)  for DSL and WANOE respectively </test_objective>
...
5.Set the  Wan Type and priorities as (1,1) (Secondary,Primary)  for DSL and WANOE respectively
...
9.Unload the module</automation_approch>
    <expected_output>With Primary Priority policy Wan Type and priorities being (1,1) (Secondary,Primary) for DSL and WANOE respectively - WANoE Line is expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-1_WAN_Sec-Pri</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-1_WAN_Sec-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-1_WAN_Sec-Pri');
