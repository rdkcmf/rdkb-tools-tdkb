...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Sec-Pri_DisableWANOE</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if WANOE line is active with FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL interface should make the active link of WANoE  false</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_101</test_case_id>
    <test_objective>This test case is to check if WANOE line is active with FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL interface should make the active link of WANoE  false</test_objective>
...
6.Set the  Wan Type and priorities as (0,0) (Secondary,Primary) for DSL and WANOE respectively and reboot the device for changes to reflect
7.Get the active link status for DSL and WANOE
8.With the current configurations WANoE Line is expected to be active
...
12.Unload the Module</automation_approch>
    <expected_output>With FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Sec,Pri) for DSL and WANOE respectively then Disabling DSL interface should make the active link of WANoE  false</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Sec-Pri_DisableWANOE</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Sec-Pri_DisableWANOE');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Sec-Pri_DisableWANOE');
                                ...
                                if expectedresult in (actualresult1 and actualresult2):
                                    ...
                                    if activeDSL == "false" and activeWANOE == "true":
                                        ...
                                        if expectedresult in actualresult:
                                            ...
                                            print "\n\n***** Initiating a reboot for WANoE Disable operation to take place since its FIXED_MODE_ON_BOOTUP WAN Policy *******";
                                            tdkTestObj = obj1.createTestStep('ExecuteCmd');
                                            obj1.initiateReboot();
                                            sleep(300);
