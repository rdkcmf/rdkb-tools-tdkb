...
<xml>
  ...
  <name>TS_WANMANAGER_WANOE_CheckPPOEProcessRunning</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>To Check if pppoe process is running When WAN Manager is Enabled</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WANAMANAGER_15</test_case_id>
    <test_objective>This test case is to check if  pppoe process is running when WAN Manager is Enabled</test_objective>
...
7.Unload the module
    ...
    <test_script>TS_WANMANAGER_WANOE_CheckPPOEProcessRunning</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_WANOE_CheckPPOEProcessRunning');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_WANOE_CheckPPOEProcessRunning');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_WANOE_CheckPPOEProcessRunning');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper() and loadmodulestatus2.upper()):
    ...
    wan0e_wan,active = getWANoEWANStatus(obj2,1);
