...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-0_WAN_Sec-Pri</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  WANOE line is active with FIXED_MODE policy ,WAN Type and priorities being (0,0) (Secondary,Primary)  for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_60</test_case_id>
    <test_objective>This test case is to check if  WANOE line is active with FIXED_MODE policy ,WAN Type and priorities being (0,0) (Secondary,Primary)  for DSL and WANOE respectively </test_objective>
...
9.Unload the module</automation_approch>
    <expected_output>With Fixed Mode policy Wan Type and priorities being (0,0) (Secondary,Primary)   for DSL and WANOE respectively - WANOE Line is expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-0_WAN_Sec-Pri</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-0_WAN_Sec-Pri');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedMode_ActLink_P_0-0_WAN_Sec-Pri');
...
if "SUCCESS" in (loadmodulestatus.upper() and loadmodulestatus1.upper()):
    ...
    if revertpriority ==1:
        ...
        paramList = ["Device.X_RDK_WanManager.CPEInterface.2.Wan.Priority","Device.X_RDK_WanManager.CPEInterface.1.Wan.Priority"];
        tdkTestObj = obj.createTestStep('TDKB_TR181Stub_Set');
        index = 2;
        for item in paramList:
            tdkTestObj.addParameter("ParamName",item);
