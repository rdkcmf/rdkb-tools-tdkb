...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>66</version>
  <name>TS_WANMANAGER_DSL_IsReportSent_TTLZero_SupportedPeriods</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether the report is send when the  Default Reporting Period and Reporting Period  is changed to supported values and Override TTL Is zero</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_28</test_case_id>
    <test_objective>Verify whether the report is send when the  Default Reporting Period and Reporting Period  is changed to supported values and Override TTL Is zero</test_objective>
...
Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod : 300
...
3. Invoke the DSL reports pre-requisite function which will set the reporting parameters to default values.
4. Set DSL Enable to true, Reporting period to 60 and Default Reporting period to 60. These are supported periods.
5. As override TTL is 0, The Default period will only be active and the DSL Diagnostic reports will be sent at 300s interval.
6. Sleep for 300s and check if the expected log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 1.
7. Revert the Reporting parameters to initial values.</automation_approch>
    <expected_output>The DSL report is sent when the  Default Reporting Period and  Reporting Period  is changed to supported values and Override TTL Is zero.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportSent_TTLZero_SupportedPeriods</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLZero_SupportedPeriods');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLZero_SupportedPeriods');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLZero_SupportedPeriods');
