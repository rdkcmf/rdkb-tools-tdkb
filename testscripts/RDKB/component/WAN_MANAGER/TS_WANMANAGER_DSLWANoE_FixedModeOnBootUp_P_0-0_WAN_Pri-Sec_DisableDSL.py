...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if DSL line is active with FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL interface should make the active link of DSL false.</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_100</test_case_id>
    <test_objective>This test case is to check if DSL line is active with FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL interface should make the active link of DSL false</test_objective>
...
5.If DUT_Mode_Fixed_Mode_OnBootup is set to 1 set the policy to FIXED_MODE_ON_BOOTUP and run the script</pre_requisite>
...
6.Set the  Wan Type and priorities as (0,0) (Primary,Secondary) for DSL and WANOE respectively and reboot the device for changes to reflect
7.Get the active link status for DSL and WANOE
8.With the current configurations DSL Line is expected to be active
9.Disable the DSL interface and reboot the device for changes to reflect
10. Check the active link status of DSL and is expected to be false
11.Revert the set values
12.Unload the Module</automation_approch>
    <expected_output>With FIXED_MODE_ON_BOOTUP policy WAN Type and Priority being (0,0),(Pri,Sec) for DSL and WANOE respectively then Disabling DSL interface should make the active link of DSL false</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL</test_script>
    <skipped>No</skipped>
...
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_P_0-0_WAN_Pri-Sec_DisableDSL');
                            ...
                            if expectedresult in actualresult:
                                ...
                                if expectedresult in (actualresult1 and actualresult2):
                                    ...
                                    if activeDSL == "true" and activeWANOE == "false":
                                        ...
                                        tdkTestObj.addParameter("ParamName","Device.X_RDK_WanManager.CPEInterface.1.Wan.Enable");
                                        ...
                                        if expectedresult in actualresult:
                                            #Set the result status of execution
                                            tdkTestObj.setResultStatus("SUCCESS");
                                            print "TEST STEP 6: Disable the DSL Interface";
                                            print "EXPECTED RESULT 6 : Should Disable the DSL Interface";
                                            print "ACTUAL RESULT 6: Disable operation successful";
                                            #Get the result of execution
                                            print "[TEST EXECUTION RESULT] : SUCCESS";

                                            print "\n\n***** Initiating a reboot for DSL Disable operation to take place since its FIXED_MODE_ON_BOOTUP WAN Policy *******";
                                            tdkTestObj = obj1.createTestStep('ExecuteCmd');
                                            obj1.initiateReboot();
                                            sleep(300);
                                            ...
                                            if expectedresult in actualresult and activeDSL == "false":
                                                #Set the result status of execution
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print "TEST STEP 7: Get the active link status of DSL";
                                                print "EXPECTED RESULT 7 : The active link status of DSL is expected to be false ";
                                                print "ACTUAL RESULT 7: activeDSL: ",activeDSL;
                                                #Get the result of execution
                                                print "[TEST EXECUTION RESULT] : SUCCESS";
                                            else:
                                                #Set the result status of execution
                                            ...
                                            if expectedresult in actualresult:
                                                #Set the result status of execution
                                                tdkTestObj.setResultStatus("SUCCESS");
                                                print "TEST STEP 8: Enable the DSL interface";
                                                print "EXPECTED RESULT 8 : Should enable the DSL Interafce ";
