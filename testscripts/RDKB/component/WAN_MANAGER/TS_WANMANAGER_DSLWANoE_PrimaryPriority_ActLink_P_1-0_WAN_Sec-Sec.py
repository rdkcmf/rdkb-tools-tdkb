...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Sec-Sec</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if  WANOE or DSL line is active with PRIMARY_PRIORITY policy,Wan priorities and Type being (1,0) (Secondary,Secondary)  for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_97</test_case_id>
    <test_objective>To check if  WANOE or DSL line is active with PRIMARY_PRIORITY policy,Wan priorities and Type being (1,0) (Secondary,Secondary)  for DSL and WANOE respectively</test_objective>
...
6.Set the  Wan Type and priorities as (1,0) (Secondary,Secondary)   for DSL and WANOE respectively
7.Get the active link status for DSL and WANOE
8.With the current configurations  DSL or WANOE Line is expected to be active
9.Revert the set Values
10.Unload the module</automation_approch>
    <expected_output>With Primary Priority policy Wan Type and priorities being (1,0) (Secondary,Secondary)   for DSL and WANOE respectively -DSL or WANOE Line is expected to be active
</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Sec-Sec</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Sec-Sec');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_PrimaryPriority_ActLink_P_1-0_WAN_Sec-Sec');
                             ...
                             if expectedresult in actualresult:
                                 ...
                                 if expectedresult in (actualresult1 and actualresult2):
                                     ...
                                     if activeDSL == "true" or  activeWANOE == "true":
                                         ...
                                         print "EXPECTED RESULT 5: Active link status of DSL or WANOE is expected to be true";
                                     ...
                                     tdkTestObj.setResultStatus("FAILURE");
                                     print "TEST STEP 4: Get the Active link status of DSL and WANOE";
                                     print "EXPECTED RESULT 4: Active link status of DSL or WANOE is expected to be true ";
