...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestOverrideTTL</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether the report is send when the Override TTL is set to the lowest supported value</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_35</test_case_id>
    <test_objective>Verify whether the report is send when the Override TTL is set to the lowest supported value</test_objective>
...
Device.DSL.X_RDK_Report.DSL.Default.OverrideTTL : 1</input_parameters>
...
3. Set DSL Enable to true, Reporting period to 1, Default Reporting period to 300 and override TTL to 1.
4. Sleep for 1s and check if the expected log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 1 in 1s.
5. Revert the Reporting parameters to initial values.</automation_approch>
    <expected_output>The report must be sent in the lowest override TTL set when the Reporting Period is set as the lowest value(1s).</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestOverrideTTL</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestOverrideTTL');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestOverrideTTL');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsReportSent_TTLSet_LowestOverrideTTL');
    ...
    if enable == 0:
        ...
        if status == 0:
            ...
            if flag == 0:
                ...
                #Set Default Reporting Period & Reporting Period to supported values and override TTL to the lowest value
                print "Set Reporting periods to supported values and override TTL to the lowest value";
                step = step + 1;
                value_list = [];
                expectedresult = "SUCCESS";
                ...
                if set_status == 0:
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] == str(value_list[1])) and (set_value[2] == str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the Reporting period is set to 1s and override TTL = 1s,  sleeping for 5s to ensure log line is captured
                        ...
                        #1 report should be sent in the last 5 seconds
                        step = step + 1;
                        print "TEST STEP %d : Check if the number of DSL reports sent after setting Reporting parameters is 1" %step;
