...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>2</version>
  <name>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_1-0_WAN_Sec-Sec</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if none of lines are active with FIXED_MODE_ON_BOOTUP policy ,WAN Type and priorities being (1,0) (Secondary,Secondary) for DSL and WANOE respectively</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_90</test_case_id>
    <test_objective>This test case is to check if none of lines are active with FIXED_MODE_ON_BOOTUP policy ,WAN Type and priorities being (1,0) (Secondary,Secondary) for DSL and WANOE respectively</test_objective>
...
6.Set the  Wan Type and priorities as (1,0) (Secondary,Secondary) for DSL and WANOE respectively and reboot the device for changes to reflect
7.Get the active link status for DSL and WANOE
8.With the current configurations none of  Lines are  expected to be active
9.Revert the set values
10.Unload the Module</automation_approch>
    <expected_output>With Fixed Mode  on  BootUp policy WAN Type and priorities being ( 1,0) (Primary,Secondary) for DSL and WANOE respectively -none of  Lines are expected to be active </expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_1-0_WAN_Sec-Sec</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_1-0_WAN_Sec-Sec');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSLWANoE_FixedModeOnBootUp_ActLink_P_1-0_WAN_Sec-Sec');
                     ...
                     if expectedresult in actualresult:
                         ...
                         if policyStatus == 1:
                             ...
                             if expectedresult in actualresult:
                                 ...
                                 print "\n ****Initiating reboot for WAN Change take place since its FIXED_MODE_ON_BOOTUP  policy***";
                                 ...
                                 if expectedresult in (actualresult1 and actualresult2):
                                    ...
                                    if activeDSL == "false" and activeWANOE == "false":
                                     ...
                                     tdkTestObj.setResultStatus("FAILURE");
                                     print "TEST STEP 4: Get the Active link status of DSL and WANOE";
                                     print "EXPECTED RESULT 4: Active link status of DSL is expected to be false and WANOE as false";
