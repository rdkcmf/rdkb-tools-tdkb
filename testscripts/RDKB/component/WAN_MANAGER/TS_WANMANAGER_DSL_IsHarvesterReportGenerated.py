...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>1</version>
  <name>TS_WANMANAGER_DSL_IsHarvesterReportGenerated</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>Verify whether the Harvester Report generation is successful</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_51</test_case_id>
    <test_objective>Verify whether the Harvester Report generation is successful</test_objective>
...
Device.DSL.X_RDK_Report.DSL.ReportingPeriod : 0
...
2. Get the initial DSL Reporting parameters. If override TTL is not 0, reboot the device and reset to 0.
3. Invoke the pre-requisite function for DSL reports.
4. Set DSL Enable to true, Reporting period to 0, Default Reporting period to 60 and override TTL to 0.
5. Sleep for 60s
6. Check if the log line "Sent message successfully to parodus" in XDSLWANMANAGER.txt.0 is present. The number of DSL reports sent should be 1 in 60s
7. Revert back to initial values</automation_approch>
    <expected_output>Harvester Report generation should be successful in the configured default reporting period.</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_DSL_IsHarvesterReportGenerated</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsHarvesterReportGenerated');
obj1.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsHarvesterReportGenerated');
obj2.configureTestCase(ip,port,'TS_WANMANAGER_DSL_IsHarvesterReportGenerated');
...
if "SUCCESS" in loadmodulestatus.upper() and "SUCCESS" in loadmodulestatus1.upper() and "SUCCESS" in loadmodulestatus2.upper() :
    ...
    if enable == 0:
        ...
        if status == 0:
            ...
            print "The DSL Reporting parameters are not retrieved successfully";
        ...
        if TTL_flag == 0:
            ...
            if flag == 0:
                ...
                print "Set Default reporting period to supported value and reporting period to 0"
                step = step + 1;
                value_list = [];
                expectedresult = "SUCCESS";
                ...
                if set_status == 0:
                    ...
                    if (set_value[0] == str(value_list[0])) and (set_value[1] == str(value_list[1])) and (set_value[2] == str(value_list[2])) and (set_value[3] == str(value_list[3])):
                        ...
                        #Get the result of execution
                        print "[TEST EXECUTION RESULT] : SUCCESS";
                        #As the Device.DSL.X_RDK_Report.DSL.Default.ReportingPeriod is set to 60s and override TTL =0,  sleeping for 60s
                        ...
                        step = step + 1;
                        print "TEST STEP %d : Check if the number of DSL reports sent after Default Reporting Period set is 1" %step;
