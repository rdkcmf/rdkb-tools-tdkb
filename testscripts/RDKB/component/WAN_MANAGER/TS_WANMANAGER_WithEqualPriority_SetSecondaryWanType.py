...
<?xml version="1.0" encoding="UTF-8"?><xml>
  <id/>
  <version>3</version>
  <name>TS_WANMANAGER_WithEqualPriority_SetSecondaryWanType</name>
  <primitive_test_id/>
  <primitive_test_name>wanmanager_DoNothing</primitive_test_name>
  <primitive_test_version>1</primitive_test_version>
  <status>FREE</status>
  <synopsis>To check if Secondary Wan Type can be set to both the interfaces when Wan Priorities are equal</synopsis>
  <groups_id/>
  ...
  <test_cases>
    <test_case_id>TC_WANMANAGER_45</test_case_id>
    <test_objective>This test case is to check  if  Secondary Wan Type can be set to both the interfaces when Wan Priorities are equal</test_objective>
...
3.Try setting the Wan Type for DSL and WANOE as Secondary and is expected to fail
4.Revert the priorities if changed
5.Unload the module</automation_approch>
    <expected_output>When a equal priority is given for both interfaces then setting a similar Wan Type i,e secondary should fail</expected_output>
    <priority>High</priority>
    <test_stub_interface>WAN_MANAGER</test_stub_interface>
    <test_script>TS_WANMANAGER_WithEqualPriority_SetSecondaryWanType</test_script>
    <skipped>No</skipped>
...
ip = <ipaddress>
port = <port>
obj.configureTestCase(ip,port,'TS_WANMANAGER_WithEqualPriority_SetSecondaryWanType');
...
if "SUCCESS" in loadmodulestatus.upper() :
    ...
    if expectedresult in actualresult:
        ...
        if expectedresult in actualresult:
            ...
            print "TEST STEP 2: Setting Wan Type as Secondary for DSL and WANOE";
