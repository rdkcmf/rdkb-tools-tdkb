...
<xml>
  ...
  <test_cases>
    <test_case_id>TC_WECB_9</test_case_id>
    <test_objective>To verify setting value for the read only parameter and committing the changes.</test_objective>
...
Input:
1. paramName - Device.MoCA.X_CISCO_COM_WiFi_Extender.Radio.
2. paramValue - true
3. paramType - boolean
...
4.From python script, invoke WECB_SetParamValues() stub function to set the parameter values associated with the argument passed.
5.WECB stub function will call the ssp_setParameterValue() function in TDK component which in turn will call Ccsp Base Function CcspBaseIf_setParameterValues() of Ccsp Common Library in RDKB.
...
WECB should fail to set the parameter value since the specified parameter is a read only object and corresponding log should be seen in the Agent console/Component log
...
Stub function result should be success and should see corresponding log in the agent console log</except_output>
    ...
    <test_script>TS_WECB_SetCommit_WithNoChange</test_script>
    <skipped>No</skipped>
