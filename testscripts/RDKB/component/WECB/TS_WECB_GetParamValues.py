...
<xml>
  ...
  <name>TS_WECB_GetParamValues</name>
  ...
  <status>FREE</status>
  <!--  -->
  <synopsis>TC_WECB_2 - To verify whether values of the specified parameter is retrieved correctly</synopsis>
  <!--  -->
  <groups_id />
  <!--  -->
  ...
  <test_cases>
    <test_case_id>TC_WECB_2</test_case_id>
    <test_objective>To verify whether values of the specified parameter is retrieved correctly</test_objective>
...
4.From python script, invoke WECB_GetParamValues() stub function to get the parameter values associated with the argument passed.
5.WECB stub function will call the ssp_getParameterValue() function in TDK component which in turn will call Ccsp Base Function CcspBaseIf_getParameterValues() of Ccsp Common Library in RDKB.
...
Parameter values associated with the Object specified should be listed in the Agent console/Component log
...
Stub function result should be success and should see corresponding log in the agent console log</except_output>
    ...
    <test_script>TS_WECB_GetParamValues</test_script>
    <skipped>No</skipped>
